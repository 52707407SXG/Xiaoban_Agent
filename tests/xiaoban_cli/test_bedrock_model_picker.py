...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (XIAOBAN_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via XIAOBAN_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in XIAOBAN_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from xiaoban_cli.providers import XIAOBAN_OVERLAYS
        assert "bedrock" in XIAOBAN_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from xiaoban_cli.providers import XIAOBAN_OVERLAYS
        assert XIAOBAN_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from xiaoban_cli.providers import XIAOBAN_OVERLAYS
        assert XIAOBAN_OVERLAYS["bedrock"].auth_type == "aws_sdk"
