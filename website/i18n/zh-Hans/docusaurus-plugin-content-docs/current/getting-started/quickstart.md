...
| 目标 | 先做这步 | 再做这步 |
|---|---|---|
| 只想让 Hermes 在本机跑起来 | `xiaoban setup` | 运行一次真实对话并验证有响应 |
...
# ~/.hermes/xiaoban-agent，在 Windows 上为 %LOCALAPPDATA%\hermes\xiaoban-agent）：
...
（如果安装时未包含 `[all]`，请先运行 `cd ~/.hermes/xiaoban-agent && uv pip install -e ".[acp]"`。）
...
| 现象 | 可能原因 | 解决方法 |
|---|---|---|
| Hermes 启动但回复为空或异常 | Provider 认证或模型选择有误 | 重新运行 `xiaoban model`，确认 provider、模型和认证信息 |
