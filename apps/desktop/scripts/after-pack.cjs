 ...
 *
 * Stamps the Xiaoban icon + identity onto the packed Windows Xiaoban.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `xiaoban desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Xiaoban')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Xiaoban'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Xiaoban.exe keeps the stock Electron icon`)
  }
}
