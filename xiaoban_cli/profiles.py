"""
Profile management for multiple isolated Xiaoban instances.

Each profile is a fully independent XIAOBAN_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.xiaoban/profiles/<name>/`` by default.

The "default" profile is ``~/.xiaoban`` itself — backward compatible,
zero migration needed.

Usage::

    xiaoban profile create coder          # fresh profile + bundled skills
    xiaoban profile create coder --clone  # also copy config, .env, SOUL.md, skills
    xiaoban profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    xiaoban -p coder chat                 # or via flag
    xiaoban profile use coder             # set as sticky default
    xiaoban profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See xiaoban_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.xiaoban``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   xiaoban-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `xiaoban backup` archives
...
# Marker file written by `xiaoban profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `xiaoban
...
# `xiaoban skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Xiaoban infrastructure directories that only the default profile
         (``~/.xiaoban``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_xiaoban_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.xiaoban) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "xiaoban-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "xiaoban", "default", "test", "tmp", "root", "sudo",
})

# Xiaoban subcommands that cannot be used as profile names/aliases
_XIAOBAN_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the xiaoban root, NOT to the current XIAOBAN_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where XIAOBAN_HOME points outside
    ``~/.xiaoban``, profiles live under ``XIAOBAN_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_xiaoban_home() / "profiles"


def _get_default_xiaoban_home() -> Path:
    """Return the default (pre-profile) XIAOBAN_HOME path.

    In standard deployments this is ``~/.xiaoban``.
    In Docker/custom deployments where XIAOBAN_HOME is outside ``~/.xiaoban``
    (e.g. ``/opt/data``), returns XIAOBAN_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_xiaoban_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Xiaoban installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its XIAOBAN_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_xiaoban_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, xiaoban subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _XIAOBAN_SUBCOMMANDS:
        return f"'{canon}' conflicts with a xiaoban subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            xiaoban_exe = shutil.which("xiaoban") or "xiaoban"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(xiaoban_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Xiaoban. If we
    ...
    try:
        ...
        # not be migrated. The next `xiaoban doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``xiaoban -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``xiaoban profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``xiaoban profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Xiaoban config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_xiaoban_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, xiaoban_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own XIAOBAN_HOME.
    ...
    The returned ``xiaoban_home`` is the path to pass to
    ``set_xiaoban_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_xiaoban_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``xiaoban update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.xiaoban)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.xiaoban/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `xiaoban -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Xiaoban profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `xiaoban update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`xiaoban profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Xiaoban (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `xiaoban profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `xiaoban -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches XIAOBAN_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``xiaoban profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "XIAOBAN_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_xiaoban_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.xiaoban).\n"
            "To remove everything, use: xiaoban uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``XIAOBAN_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from xiaoban_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set XIAOBAN_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("XIAOBAN_HOME")
    try:
        os.environ["XIAOBAN_HOME"] = str(profile_dir)
        from xiaoban_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["XIAOBAN_HOME"] = old_home
        elif "XIAOBAN_HOME" in os.environ:
            del os.environ["XIAOBAN_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.xiaoban/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: xiaoban profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from XIAOBAN_HOME.

    Returns ``"default"`` if XIAOBAN_HOME is not set or points to ``~/.xiaoban``.
    Returns the profile name if XIAOBAN_HOME points into ``~/.xiaoban/profiles/<name>``.
    Returns ``"custom"`` if XIAOBAN_HOME is set to an unrecognized path.
    """
    from xiaoban_constants import get_xiaoban_home
    xiaoban_home = get_xiaoban_home()
    resolved = xiaoban_home.resolve()

    default_resolved = _get_default_xiaoban_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.xiaoban itself — its parent is ~/ and its
        # directory name is ".xiaoban", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: xiaoban profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.xiaoban itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.xiaoban). "
            "Specify a different name: xiaoban profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="xiaoban_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"xiaoban_{old_name}"
    legacy_old_host = f"xiaoban.{old_name}"
    new_host = f"xiaoban_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_xiaoban_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("xiaoban_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a XIAOBAN_HOME path string.

    Called early in the CLI entry point, before any xiaoban modules
    are imported, to set the XIAOBAN_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
