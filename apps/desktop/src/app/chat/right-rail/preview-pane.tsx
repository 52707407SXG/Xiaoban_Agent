        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.xiaobanDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Xiaoban to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.xiaobanDesktop?.watchPreviewFile ||
      !window.xiaobanDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.xiaobanDesktop.onPreviewFileChanged(payload => {
    ...
    void window.xiaobanDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.xiaobanDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.xiaobanDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:xiaoban-preview')
