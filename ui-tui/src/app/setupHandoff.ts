import type { RunExternalProcess } from '@xiaoban/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`xiaoban ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching xiaoban: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`xiaoban ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
