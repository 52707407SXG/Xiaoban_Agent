"""Regression test: `xiaoban dashboard --tui` must not hard-crash.

Older Xiaoban desktop app shells (<= 0.15.x) spawn the backend as::

    xiaoban dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Xiaoban couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real xiaoban_cli.main parser in a subprocess.
    ...
    return subprocess.run(
        [sys.executable, "-m", "xiaoban_cli.main", *args],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,
