...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m xiaoban_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Xiaoban_Gateway"
_TASK_DESCRIPTION = "Xiaoban-Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Xiaoban profile."""
    from xiaoban_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "xiaoban_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("XIAOBAN_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("XIAOBAN_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("XIAOBAN_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["XIAOBAN_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["XIAOBAN_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["XIAOBAN_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("XIAOBAN_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("XIAOBAN_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("XIAOBAN_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Xiaoban_Gateway``
    Named profile X: ``Xiaoban_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during xiaoban_cli boot.
    from xiaoban_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\xiaoban\\gateway-service\\<task_name>.cmd``
    (or ``<XIAOBAN_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Xiaoban installs stay self-contained).
    """
    _assert_windows()
    from xiaoban_cli.config import get_xiaoban_home

    script_dir = Path(get_xiaoban_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``XIAOBAN_HOME`` whenever it
    ...
    source checkout only if ``XIAOBAN_HOME`` cannot be resolved yet.
    """
    from xiaoban_cli.config import get_xiaoban_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports XIAOBAN_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m xiaoban_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "XIAOBAN_HOME={xiaoban_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "XIAOBAN_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports xiaoban_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "xiaoban_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    xiaoban_home = str(Path(get_xiaoban_home()).resolve())
    profile_arg = _profile_arg(xiaoban_home)

    content = _build_gateway_cmd_script(python_path, working_dir, xiaoban_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Xiaoban builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "xiaoban_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m xiaoban_cli.main gateway run``
    ...
    log_dir = Path(get_xiaoban_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("XIAOBAN_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("XIAOBAN_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from xiaoban_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `xiaoban -p <profile> gateway install` must be safe.
    ...
    from xiaoban_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"xiaoban {profile_arg} gateway start" if profile_arg else "xiaoban gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: xiaoban gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Xiaoban gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: xiaoban gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: xiaoban gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from xiaoban_cli.config import get_xiaoban_home
        print(f"    type {Path(get_xiaoban_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_xiaoban_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from xiaoban_cli.config import get_xiaoban_home

    xiaoban_home = Path(get_xiaoban_home()).resolve()
    print()
    print("Next steps:")
    print("  xiaoban gateway status                      # Check status")
    print(f"  type {xiaoban_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Xiaoban gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: xiaoban gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: xiaoban gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: xiaoban gateway start")
            return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from xiaoban_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
...
def restart() -> None:
    ...
    """
    _assert_windows()
    from xiaoban_cli.gateway import kill_gateway_processes
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `xiaoban gateway status`."
        )
