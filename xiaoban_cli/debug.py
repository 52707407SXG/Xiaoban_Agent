"""``xiaoban debug`` debug tools for Xiaoban-Agent.

Currently supports:
    xiaoban debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.xiaoban/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[xiaoban debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.xiaoban/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``xiaoban debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``xiaoban debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Xiaoban version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `xiaoban debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "xiaoban-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``xiaoban debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.xiaoban/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``xiaoban debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "xiaoban-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----XiaobanDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from xiaoban_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_xiaoban_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``xiaoban dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``xiaoban dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``xiaoban debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "xiaoban debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `xiaoban debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"To delete now:  xiaoban debug delete <url>")

    print(f"\nShare these links with the Xiaoban team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: xiaoban debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'xiaoban debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``xiaoban debug`` call.
    ...
    # best-effort — any failure is swallowed so ``xiaoban debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: xiaoban debug <command>")
        print()
        print("Commands:")
