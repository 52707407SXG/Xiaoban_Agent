...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_xiaoban_cli_expands_web(self):
        """xiaoban-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"xiaoban-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("xiaoban-cli", expanded)
    ...
    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['xiaoban-cli'] yields ['web']."""
        parent_toolsets = {"xiaoban-cli"}
