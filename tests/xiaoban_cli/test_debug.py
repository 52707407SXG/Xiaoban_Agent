"""Tests for ``xiaoban debug`` CLI command and debug utilities."""
...
@pytest.fixture
def xiaoban_home(tmp_path, monkeypatch):
    """Set up an isolated XIAOBAN_HOME with minimal logs."""
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO xiaoban_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from xiaoban_cli.debug import _upload_paste_rs
        ...
        with patch("xiaoban_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "xiaoban_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from xiaoban_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from xiaoban_cli.debug import upload_to_pastebin

        with patch("xiaoban_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("xiaoban_cli.debug._upload_paste_rs",
             ...
             patch("xiaoban_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, xiaoban_home):
        from xiaoban_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, xiaoban_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (xiaoban_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, xiaoban_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = xiaoban_home / "logs" / "agent.log"
        from xiaoban_cli import debug
    ...
    def test_truncates_large_file(self, xiaoban_home):
        """Files larger than max_bytes get tail-truncated."""
        from xiaoban_cli.debug import _capture_log_snapshot

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (xiaoban_home / "logs" / "agent.log").write_text(big_content * 200)

        snap = _capture_log_snapshot("agent", tail_lines=10, max_bytes=1024)
        assert snap.full_text is not None
        assert "truncated" in snap.full_text

    def test_keeps_first_line_when_truncation_on_boundary(self, xiaoban_home):
        ...
        (xiaoban_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, xiaoban_home):
    ...
    def test_unknown_log_returns_none(self, xiaoban_home):
        from xiaoban_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, xiaoban_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from xiaoban_cli.debug import _capture_log_snapshot

        logs_dir = xiaoban_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, xiaoban_home):
    ...
    def test_falls_back_when_primary_empty(self, xiaoban_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of XIAOBAN_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def xiaoban_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated XIAOBAN_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".xiaoban"
        home.mkdir()
        monkeypatch.setenv("XIAOBAN_HOME", str(home))
        ...
        monkeypatch.delenv("XIAOBAN_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, xiaoban_home_with_secret):
        from xiaoban_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, xiaoban_home_with_secret):
        from xiaoban_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, xiaoban_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when XIAOBAN_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("XIAOBAN_REDACT_SECRETS", "false")

        from xiaoban_cli.debug import _capture_log_snapshot

        assert os.environ.get("XIAOBAN_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, xiaoban_home_with_secret
    ):
        from xiaoban_cli.debug import _capture_log_snapshot

        log_path = xiaoban_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, xiaoban_home_with_secret):
        from xiaoban_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from xiaoban_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, xiaoban_home):
        from xiaoban_cli.debug import collect_debug_report

        with patch("xiaoban_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- xiaoban dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- xiaoban dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, xiaoban_home):
        from xiaoban_cli.debug import collect_debug_report

        with patch("xiaoban_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, xiaoban_home):
        from xiaoban_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, xiaoban_home):
        from xiaoban_cli.debug import collect_debug_report
    ...
    def test_report_includes_gui_log(self, xiaoban_home):
        from xiaoban_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, xiaoban_home):
        from xiaoban_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, xiaoban_home, capsys):
        """Slash-command path should sweep old pending deletes before uploading."""
        from xiaoban_cli.debug import run_debug_share
        ...
        with patch("xiaoban_cli.dump.run_dump"), \
             patch("xiaoban_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("xiaoban_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, xiaoban_home, capsys):
        ...
        with patch("xiaoban_cli.dump.run_dump"), \
             patch(
                 "xiaoban_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, xiaoban_home, capsys):
    ...
    def test_share_uploads_five_pastes(self, xiaoban_home, capsys):
        ...
        with patch("xiaoban_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- xiaoban dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- xiaoban dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- xiaoban dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste
        gui_paste = uploaded_content[3]
        assert "--- xiaoban dump ---" in gui_paste
        assert "--- full gui.log ---" in gui_paste
        desktop_paste = uploaded_content[4]
        assert "--- xiaoban dump ---" in desktop_paste
        assert "--- full desktop.log ---" in desktop_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, xiaoban_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from xiaoban_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("xiaoban_cli.dump.run_dump"), \
             patch("xiaoban_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("xiaoban_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, xiaoban_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, xiaoban_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, xiaoban_home_with_secret, capsys
    ):
        ...
        with patch("xiaoban_cli.dump.run_dump"), \
             patch("xiaoban_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("xiaoban_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "xiaoban debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, xiaoban_home):
        from xiaoban_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from xiaoban_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from xiaoban_cli.debug import delete_paste
        ...
        with patch("xiaoban_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.xiaoban/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``xiaoban debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, xiaoban_home):
        ...
        import ast
        import inspect
        from xiaoban_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, xiaoban_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from xiaoban_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, xiaoban_home):
    ...
    def test_merges_with_existing_pending(self, xiaoban_home):
        """Subsequent calls merge into existing pending.json."""
        from xiaoban_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, xiaoban_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, xiaoban_home):
    ...
    def test_sweep_deletes_expired_entries(self, xiaoban_home):
        from xiaoban_cli.debug import (
        ...
        with patch("xiaoban_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, xiaoban_home):
        from xiaoban_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("xiaoban_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, xiaoban_home):
        ...
        with patch(
            "xiaoban_cli.debug.delete_paste",
    ...
    def test_sweep_drops_entries_past_grace_window(self, xiaoban_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, xiaoban_home):
        from xiaoban_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("xiaoban_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, xiaoban_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: xiaoban debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from xiaoban_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("xiaoban_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("xiaoban_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, xiaoban_home, capsys):
        from xiaoban_cli.debug import run_debug_share
        ...
        with patch("xiaoban_cli.dump.run_dump"), \
             ...
             patch("xiaoban_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, xiaoban_home, capsys):
        from xiaoban_cli.debug import run_debug_share
        ...
        with patch("xiaoban_cli.dump.run_dump"), \
             patch("xiaoban_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("xiaoban_cli.debug._schedule_auto_delete"):
            run_debug_share(args)

        out = capsys.readouterr().out
        assert "public paste service" in out

    def test_local_no_privacy_notice(self, xiaoban_home, capsys):
        from xiaoban_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``xiaoban debug share`` (CLI) and ``POST /api/ops/debug-share``
    (dashboard). The dashboard renders ``urls`` as real, copyable links, so the
    contract here is the return value, not stdout.
    """

    def test_returns_structured_urls(self, xiaoban_home):
        from xiaoban_cli.debug import build_debug_share, DebugShareResult
        ...
        with patch("xiaoban_cli.dump.run_dump"), patch(
            "xiaoban_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("xiaoban_cli.debug._schedule_auto_delete"):
    ...
    def test_skips_missing_logs_without_failure(self, xiaoban_home):
        from xiaoban_cli.debug import build_debug_share

        # Remove desktop.log so it should be neither uploaded nor reported failed.
        (xiaoban_home / "logs" / "desktop.log").unlink()

        with patch("xiaoban_cli.dump.run_dump"), patch(
            "xiaoban_cli.debug.upload_to_pastebin",
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, xiaoban_home):
        from xiaoban_cli.debug import build_debug_share
    ...
    def test_optional_log_failure_is_collected_not_raised(self, xiaoban_home):
        from xiaoban_cli.debug import build_debug_share
    ...
    def test_required_report_failure_raises(self, xiaoban_home):
        from xiaoban_cli.debug import build_debug_share
