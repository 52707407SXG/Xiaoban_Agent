...
Usage:
    xiaoban                     # Interactive chat (default)
    xiaoban chat                # Interactive chat
    xiaoban gateway             # Run gateway in foreground
    xiaoban gateway start       # Start gateway as service
    xiaoban gateway stop        # Stop gateway service
    xiaoban gateway status      # Show gateway status
    xiaoban gateway install     # Install gateway service
    xiaoban gateway uninstall   # Uninstall gateway service
    xiaoban setup               # Interactive setup wizard
    xiaoban logout              # Clear stored authentication
    xiaoban status              # Show status of all components
    xiaoban cron                # Manage cron jobs
    xiaoban cron list           # List cron jobs
    xiaoban cron status         # Check if cron scheduler is running
...
# IMPORTANT: xiaoban_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``xiaoban_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``xiaoban update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``xiaoban_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, xiaoban crashes on import and the user can't run
# ``xiaoban update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import xiaoban_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'xiaoban' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``xiaoban tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``xiaoban.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("xiaoban")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"xiaoban", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"xiaoban")
        # Windows: the .exe name is already ``xiaoban.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* xiaoban_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("XIAOBAN_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".xiaoban", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``XIAOBAN_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("XIAOBAN_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``XIAOBAN_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("XIAOBAN_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`xiaoban --tui … >log`, CI capture):
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``xiaoban --version`` before config/logging imports on Termux."""
    if os.environ.get("XIAOBAN_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from xiaoban_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from xiaoban_cli.subcommands.cron import build_cron_parser
from xiaoban_cli.subcommands.gateway import build_gateway_parser
from xiaoban_cli.subcommands.profile import build_profile_parser
from xiaoban_cli.subcommands.model import build_model_parser
from xiaoban_cli.subcommands.setup import build_setup_parser
from xiaoban_cli.subcommands.postinstall import build_postinstall_parser
from xiaoban_cli.subcommands.whatsapp import build_whatsapp_parser
from xiaoban_cli.subcommands.slack import build_slack_parser
from xiaoban_cli.subcommands.login import build_login_parser
from xiaoban_cli.subcommands.logout import build_logout_parser
from xiaoban_cli.subcommands.auth import build_auth_parser
from xiaoban_cli.subcommands.status import build_status_parser
from xiaoban_cli.subcommands.webhook import build_webhook_parser
from xiaoban_cli.subcommands.hooks import build_hooks_parser
from xiaoban_cli.subcommands.doctor import build_doctor_parser
from xiaoban_cli.subcommands.security import build_security_parser
from xiaoban_cli.subcommands.dump import build_dump_parser
from xiaoban_cli.subcommands.debug import build_debug_parser
from xiaoban_cli.subcommands.backup import build_backup_parser
from xiaoban_cli.subcommands.import_cmd import build_import_cmd_parser
from xiaoban_cli.subcommands.config import build_config_parser
from xiaoban_cli.subcommands.version import build_version_parser
from xiaoban_cli.subcommands.update import build_update_parser
from xiaoban_cli.subcommands.uninstall import build_uninstall_parser
from xiaoban_cli.subcommands.dashboard import build_dashboard_parser
from xiaoban_cli.subcommands.gui import build_gui_parser
from xiaoban_cli.subcommands.logs import build_logs_parser
from xiaoban_cli.subcommands.prompt_size import build_prompt_size_parser
from xiaoban_cli.subcommands.memory import build_memory_parser
from xiaoban_cli.subcommands.acp import build_acp_parser
from xiaoban_cli.subcommands.tools import build_tools_parser
from xiaoban_cli.subcommands.insights import build_insights_parser
from xiaoban_cli.subcommands.skills import build_skills_parser
from xiaoban_cli.subcommands.pairing import build_pairing_parser
from xiaoban_cli.subcommands.plugins import build_plugins_parser
from xiaoban_cli.subcommands.mcp import build_mcp_parser
from xiaoban_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (xiaoban tools, xiaoban setup, xiaoban model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'xiaoban {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any xiaoban module import.
#
# Many modules cache XIAOBAN_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("XIAOBAN_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.xiaoban/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set XIAOBAN_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `xiaoban mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Xiaoban' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo xiaoban -p <name>` against the invoking user's home.
        ...
        candidate = home / ".xiaoban" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`xiaoban chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors xiaoban_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If XIAOBAN_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.xiaoban/profiles/coder or
    # /opt/data/profiles/coder).  If XIAOBAN_HOME points to the xiaoban root
    # instead (e.g. systemd hardcodes XIAOBAN_HOME=/root/.xiaoban), we must
    # still read active_profile — the user may have switched profiles via
    # `xiaoban profile use` and the gateway should honour that choice.
    # See issue #22502.
    xiaoban_home_env = os.environ.get("XIAOBAN_HOME", "")
    if profile_name is None and xiaoban_home_env:
        if Path(xiaoban_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the xiaoban root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as XIAOBAN_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``xiaoban gateway run`` to mean
    # "the root XIAOBAN_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("XIAOBAN_S6_SUPERVISED_CHILD"):
        try:
            from xiaoban_constants import get_default_xiaoban_root

            active_path = get_default_xiaoban_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set XIAOBAN_HOME
    if profile_name is not None:
        try:
            from xiaoban_cli.profiles import resolve_profile_env

            xiaoban_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            xiaoban_home = _resolve_sudo_user_profile_env(profile_name)
            if not xiaoban_home:
            ...
            # A bug in profiles.py must NEVER prevent xiaoban from starting
            print(
...
# Load .env from ~/.xiaoban/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from xiaoban_cli.config import get_xiaoban_home
from xiaoban_cli.env_loader import load_xiaoban_dotenv

load_xiaoban_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → XIAOBAN_REDACT_SECRETS env
# var BEFORE xiaoban_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_xiaoban_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from xiaoban_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "XIAOBAN_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["XIAOBAN_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `xiaoban` subcommands
...
try:
    from xiaoban_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from xiaoban_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if xiaoban_constants not importable yet
...
from xiaoban_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to xiaoban_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (xiaoban_cli.main._model_flow_*) keep resolving unchanged.
from xiaoban_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_xiaoban_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("XIAOBAN_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("XIAOBAN_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from xiaoban_cli.config import get_env_path, get_xiaoban_home, load_config
    from xiaoban_cli.auth import get_auth_status

    # Determine whether Xiaoban runtime has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from xiaoban_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_xiaoban_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from xiaoban_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_xiaoban_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Xiaoban has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Xiaoban to use their tokens.
    if _has_xiaoban_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, xiaoban_bin
        cli_args: the original CLI arguments (everything after 'xiaoban')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    xiaoban_bin = container_info["xiaoban_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo xiaoban {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo xiaoban {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  xiaoban --tui --resume {target}")
    if title:
        print(f'  xiaoban --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @xiaoban/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@xiaoban" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/xiaoban-ink/package.json",
    "packages/xiaoban-ink/index.js",
    "packages/xiaoban-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``XIAOBAN_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("XIAOBAN_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``XIAOBAN_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("XIAOBAN_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    xiaoban_home = os.environ.get("XIAOBAN_HOME") or str(Path.home() / ".xiaoban")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "XIAOBAN_HOME": xiaoban_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(xiaoban_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(xiaoban_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if xiaoban_cli_dir is None:
        xiaoban_cli_dir = Path(__file__).parent
    bundled = xiaoban_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``xiaoban update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("XIAOBAN_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Xiaoban checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `xiaoban update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Xiaoban checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `xiaoban --tui`\n"
        "If the checkout is still inconsistent, run `xiaoban update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (XIAOBAN_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("XIAOBAN_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from xiaoban_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("XIAOBAN_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with XIAOBAN_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset XIAOBAN_TUI_DIR (e.g. `unset XIAOBAN_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @xiaoban/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @xiaoban/ink resolves through
        # packages/xiaoban-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "xiaoban-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from xiaoban_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from xiaoban_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="xiaoban-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["XIAOBAN_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["XIAOBAN_PYTHON_SRC_ROOT"] = os.environ.get(
        "XIAOBAN_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("XIAOBAN_PYTHON", sys.executable)
    env.setdefault("XIAOBAN_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["XIAOBAN_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["XIAOBAN_MODEL"] = model
        env["XIAOBAN_INFERENCE_MODEL"] = model
    if provider:
        env["XIAOBAN_TUI_PROVIDER"] = provider
        env["XIAOBAN_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["XIAOBAN_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["XIAOBAN_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["XIAOBAN_TUI_SKILLS"] = value
    if query:
        env["XIAOBAN_TUI_QUERY"] = query
    if image:
        env["XIAOBAN_TUI_IMAGE"] = image
    if checkpoints:
        env["XIAOBAN_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["XIAOBAN_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["XIAOBAN_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["XIAOBAN_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["XIAOBAN_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["XIAOBAN_ACCEPT_HOOKS"] = "1"
    ...
    # XIAOBAN_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `xiaoban --tui` try to
    ...
    # still set XIAOBAN_TUI_RESUME themselves.
    env.pop("XIAOBAN_TUI_RESUME", None)
    if resume_session_id:
        env["XIAOBAN_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `xiaoban update` so
    ...
    if code == 42:
        from xiaoban_cli.relaunch import relaunch

        print()
        print("M Launching update...")
        print()
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``XIAOBAN_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``xiaoban kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``xiaoban kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("XIAOBAN_KANBAN_BOARD"):
        return
    try:
        from xiaoban_cli.kanban_db import get_current_board

        os.environ["XIAOBAN_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.xiaoban/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Xiaoban — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Xiaoban still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``XIAOBAN_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("XIAOBAN_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'xiaoban sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from xiaoban_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from xiaoban_cli.config import load_config as _load_config_for_xai_check
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Xiaoban isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  xiaoban setup")
        print()

        from xiaoban_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'xiaoban setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from xiaoban_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["XIAOBAN_YOLO_MODE"] = "1"
    ...
    # setup (config, rules files, plugins, MCP servers) or from Xiaoban runtime.
    # Implemented as a superset of --ignore-user-config + --ignore-rules plus
    # plugin/MCP discovery suppression (XIAOBAN_SAFE_MODE is checked by
    # xiaoban_cli/plugins.py and tools/mcp_tool.py).
    if getattr(args, "safe_mode", False):
        os.environ["XIAOBAN_SAFE_MODE"] = "1"
        os.environ["XIAOBAN_IGNORE_USER_CONFIG"] = "1"
        os.environ["XIAOBAN_IGNORE_RULES"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.xiaoban/config.yaml and return built-in defaults. Set BEFORE
    ...
    if getattr(args, "source", None):
        os.environ["XIAOBAN_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from xiaoban_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from xiaoban_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from xiaoban_constants import find_node_executable, with_xiaoban_node_path

    print()
    print("M WhatsApp Setup")
    print("=" * 50)
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Xiaoban?")
        print()
    ...
    # has no creds.json.  Every subsequent `xiaoban gateway` then paid a 30s
    ...
    session_dir = get_xiaoban_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: xiaoban gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `xiaoban gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  xiaoban gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '小伴'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  xiaoban gateway")
        ...
        print()
        print("  Or install as a service: xiaoban gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'xiaoban whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``xiaoban whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``xiaoban_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from xiaoban_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from xiaoban_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from xiaoban_cli.config import stamp_install_method
    from xiaoban_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("M Xiaoban post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from xiaoban_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``xiaoban model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from xiaoban_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    ...
    from xiaoban_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("XIAOBAN_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from xiaoban_cli.config import read_raw_config
    ...
    from xiaoban_cli.models import (
        CANONICAL_PROVIDERS,
        _PROVIDER_LABELS,
        group_providers,
    ...
    # PROVIDER_GROUPS in xiaoban_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.xiaoban/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.xiaoban/.env if the active provider is not 'custom'.
    ...
    """
    from xiaoban_cli.config import get_env_value, save_env_value, load_config
...
#
# Xiaoban uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `xiaoban model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `xiaoban model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`xiaoban_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from xiaoban_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from xiaoban_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Xiaoban only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``xiaoban model`` flow, then route aux tasks to them here.
    """
    from xiaoban_cli.config import load_config
    from xiaoban_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from xiaoban_cli.auth import _prompt_model_selection
    from xiaoban_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from xiaoban_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from xiaoban_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from xiaoban_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Xiaoban URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from xiaoban_cli.curses_ui import curses_radiolist
...
# downstream call sites read `xiaoban_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `xiaoban` invocation — including
# fast paths like `xiaoban --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from xiaoban_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``xiaoban setup`` / ``xiaoban model``.
    ...
    recover from a malformed paste without editing ``~/.xiaoban/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from xiaoban_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from xiaoban_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `xiaoban setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from xiaoban_constants import display_xiaoban_home as _dhh_fn

            print(
                f"    Xiaoban will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Xiaoban CLI with a provider."""
    from xiaoban_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from xiaoban_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from xiaoban_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from xiaoban_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from xiaoban_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from xiaoban_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``xiaoban slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: xiaoban slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `xiaoban slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from xiaoban_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from xiaoban_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from xiaoban_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from xiaoban_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `xiaoban security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from xiaoban_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from xiaoban_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from xiaoban_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from xiaoban_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Xiaoban home directory to a zip file."""
    if getattr(args, "quick", False):
        from xiaoban_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from xiaoban_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Xiaoban backup from a zip file."""
    from xiaoban_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from xiaoban_cli.banner import format_banner_version_label
    ...
    try:
        from xiaoban_cli.banner import check_for_updates
        from xiaoban_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    if getattr(args, "gui_summary", False):
        from xiaoban_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from xiaoban_cli.uninstall import run_gui_uninstall
    ...
    from xiaoban_cli.uninstall import run_uninstall
...
# Critical files that every ``xiaoban`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``xiaoban update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "xiaoban_cli/main.py",
    "xiaoban_cli/config.py",
    "xiaoban_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "xiaoban_constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``xiaoban`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="xiaoban-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``xiaoban update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``xiaoban_cli/web_dist/`` (per vite.config.ts
    outDir: "../xiaoban_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "xiaoban_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``xiaoban`` launcher present but ``xiaoban_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the xiaoban venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — xiaoban venv python3, no nix-shell overhead
    ...
    # a xiaoban venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``xiaoban update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``xiaoban web``).
    ...
    # build path usable on a stock `py -m xiaoban_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_xiaoban_node_path()
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (xiaoban web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``xiaoban update`` can unconditionally call ``xiaoban desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``xiaoban desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $XIAOBAN_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $XIAOBAN_HOME."""
    from xiaoban_constants import get_xiaoban_home
    return get_xiaoban_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``xiaoban update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Xiaoban.app/Contents/MacOS/Xiaoban"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Xiaoban.exe",
            release_dir / "win-ia32-unpacked" / "Xiaoban.exe",
            release_dir / "win-arm64-unpacked" / "Xiaoban.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "xiaoban",
            release_dir / "linux-unpacked" / "Xiaoban",
            release_dir / "linux-arm64-unpacked" / "xiaoban",
            release_dir / "linux-arm64-unpacked" / "Xiaoban",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Xiaoban'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_xiaoban_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Xiaoban.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Xiaoban.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Xiaoban.exe: Access is
    ...
    an unrelated "Xiaoban" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Xiaoban is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Xiaoban.app/Contents/MacOS/Xiaoban  ->  app bundle = .../Xiaoban.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Xiaoban Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Xiaoban Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from xiaoban_logging import setup_logging as _setup_logging_gui
    ...
    # with_xiaoban_node_path() copies os.environ when called with no arg.
    env = with_xiaoban_node_path()
    if getattr(args, "fake_boot", False):
        env["XIAOBAN_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["XIAOBAN_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "xiaoban_root", None):
        env["XIAOBAN_DESKTOP_XIAOBAN_ROOT"] = str(Path(args.xiaoban_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["XIAOBAN_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  xiaoban gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Xiaoban.exe locked on Windows, so the pack can't replace
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Xiaoban.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Xiaoban.exe, close any")
                    print("  running Xiaoban desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> xiaoban desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Xiaoban is
    ...
    if source_mode:
        print("→ Launching Xiaoban Desktop from source build...")
    ...
    print(f"→ Launching packaged Xiaoban Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``xiaoban dashboard`` processes other than ourselves.

    ``xiaoban dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``xiaoban update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Xiaoban Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``xiaoban dashboard`` as
    ...
    environment variable ``XIAOBAN_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "xiaoban dashboard",
        "xiaoban_cli.main dashboard",
        "xiaoban_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "xiaoban.*dashboard"`) keeps us consistent
            # with `xiaoban_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `xiaoban update`.
    ...
    print("  Preview now:  xiaoban curator run --dry-run")
    print("  Pause it:     xiaoban curator pause")
    print(
        "  Docs:         https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``xiaoban update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``xiaoban update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: xiaoban curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``xiaoban dashboard`` processes.

    Called at the end of ``xiaoban update`` (default ``reason``) and also
    from ``xiaoban dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Xiaoban Desktop Electron app spawns this dashboard as a
    # backend child, it sets XIAOBAN_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("XIAOBAN_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    xiaoban dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.xiaoban-update-staging"
    backup = f"{dst}.xiaoban-update-old"
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `xiaoban update "
            f"--branch {branch}`, or update against main with `xiaoban update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/52707407SXG/Xiaoban_Agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="xiaoban-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"xiaoban-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for Xiaoban-Agent itself
        ...
        # GitHub ZIPs extract to Xiaoban-Agent-<branch>/
        extracted = os.path.join(tmp_dir, f"Xiaoban-Agent-{branch}")
    ...
    from xiaoban_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: xiaoban skills list-modified  "
    ...
    try:
        from xiaoban_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "xiaoban-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Xiaoban behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes xiaoban completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Xiaoban couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Xiaoban couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Xiaoban behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Xiaoban couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Xiaoban couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `xiaoban update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/52707407SXG/Xiaoban_Agent.git",
    "git@github.com:52707407SXG/Xiaoban_Agent.git",
    "https://github.com/52707407SXG/Xiaoban_Agent",
    "git@github.com:52707407SXG/Xiaoban_Agent",
}
OFFICIAL_REPO_URL = "https://github.com/52707407SXG/Xiaoban_Agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_xiaoban_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_xiaoban_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Xiaoban repository.")
        print("  This means you may miss updates from 52707407SXG/Xiaoban_Agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/52707407SXG/Xiaoban_Agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/52707407SXG/Xiaoban_Agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``xiaoban update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from xiaoban_constants import get_default_xiaoban_root

    default_home = get_default_xiaoban_root()
...
# Install-scoped breadcrumb dropped right before ``xiaoban update`` mutates the
...
# survives and the next ``xiaoban`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $XIAOBAN_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``xiaoban update`` left half-done.
    ...
    protocol stream (``xiaoban acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `xiaoban update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from xiaoban_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``xiaoban update`` is still progressing even if pip/uv itself is silent.
    """
...
def _xiaoban_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "xiaoban.exe",
        scripts_dir / "xiaoban-gateway.exe",
    ]


def _detect_concurrent_xiaoban_instances(
    ...
    The Xiaoban Desktop Electron app spawns ``xiaoban.EXE`` as a backend child,
    so during ``xiaoban update`` the user-invoked process and the desktop's
    ...
    shims (``xiaoban.exe`` / ``xiaoban-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``xiaoban update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``xiaoban.exe``) is a separate process from the Python
    ...
    for shim in _xiaoban_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated xiaoban.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``xiaoban update``
    ...
    #      xiaoban.exe sitting *under* a non-Xiaoban parent (e.g. a Xiaoban
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "xiaoban.exe"
    lines = ["✗ Another xiaoban.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Xiaoban Desktop, exit any open `xiaoban` REPLs, and")
    lines.append("  stop the gateway (`xiaoban gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `xiaoban update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_xiaoban_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``xiaoban.exe``.
    ...
    when ``xiaoban update`` runs, ``xiaoban.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``xiaoban.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next xiaoban invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Xiaoban Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Xiaoban Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _xiaoban_exe_shims(scripts_dir):
        ...
        print(
            "    Close Xiaoban Desktop, exit other `xiaoban` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `xiaoban update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_xiaoban_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``xiaoban.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``xiaoban.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``xiaoban`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_xiaoban_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``xiaoban.exe.old.*`` left by prior updates.

    Called early on every xiaoban invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``xiaoban update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `xiaoban update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``xiaoban.exe`` / ``xiaoban-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_xiaoban_exe`` for the rationale.
    """
    ...
    # hours later inside ``xiaoban update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``xiaoban update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``xiaoban.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``xiaoban`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `xiaoban update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `xiaoban update --force` after closing other xiaoban processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from xiaoban_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$XIAOBAN_HOME/bin/uv``, but on Termux the official
    installer may not work (glibc vs bionic).  Fall back to ``pip install uv``
    which gets a Termux-compatible binary.
    """
    from xiaoban_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `xiaoban update`, so we install root-only first
    ...
    nixos_env = with_xiaoban_node_path(_nixos_build_env())
...
class _UpdateOutputStream:
    """Stream wrapper used during ``xiaoban update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.xiaoban/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``xiaoban update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``xiaoban update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.xiaoban/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``xiaoban update --gateway``) the update is already
    ...
    try:
        ...
        # xiaoban_cli.config.get_xiaoban_home to simulate setup failure.
        from xiaoban_cli.config import get_xiaoban_home as _get_xiaoban_home

        logs_dir = _get_xiaoban_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``xiaoban update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from xiaoban_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``xiaoban update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from xiaoban_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from xiaoban_cli.config import recommended_update_command
        from xiaoban_cli.banner import check_via_pypi
        ...
        if result is None:
            ...
            print("M Update available on PyPI.")
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            print(f"M Update available (behind {compare_branch}).")
            from xiaoban_cli.config import recommended_update_command
    ...
    if behind == 0:
        ...
        commits_word = "commit" if behind == 1 else "commits"
        print(f"M Update available: {behind} {commits_word} behind {compare_branch}.")
        from xiaoban_cli.config import recommended_update_command
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``xiaoban update`` without requiring a reinstall.  The
    ...
    ``xiaoban`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/xiaoban.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v xiaoban'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/xiaoban, code at /usr/local/lib/xiaoban-agent).
    fhs_link = Path("/usr/local/bin/xiaoban")
    ...
    path_comment = (
        "# Xiaoban-Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of XIAOBAN_HOME before running the update.
    ...
    XIAOBAN_HOME directories.  The ``--backup`` flag on ``xiaoban update``
    ...
    # ``xiaoban_cli/config.py``). Fall back to true if the key is missing
    ...
    try:
        from xiaoban_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_xiaoban_home so the user sees ~/.xiaoban/...
    try:
        from xiaoban_constants import get_xiaoban_home, display_xiaoban_home

        home = get_xiaoban_home()
        try:
            display_path = f"{display_xiaoban_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  xiaoban import {out_path}")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    xiaoban.exe concurrent-instance guard does not see them. They still import
    ...
    try:
        ...
        from xiaoban_cli.gateway import (
    ...
    if not running_pids:
        ...
        try:
            from xiaoban_cli import gateway_windows
    ...
    print("→ Stopping Windows gateway process(es) before updating Xiaoban...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m xiaoban_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: xiaoban gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``xiaoban gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from xiaoban_cli import gateway_windows
        from xiaoban_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``xiaoban update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    ...
    if is_managed():
        managed_error("update Xiaoban-Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `xiaoban update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    ...
    from xiaoban_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "xiaoban-agent"]
        ...
        cmd = [pipx, "upgrade", "xiaoban-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "xiaoban-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "xiaoban-agent"]
    ...
    print("✓ Update complete! Restart xiaoban to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("M Updating Xiaoban-Agent...")
    print()

    # On Windows, abort early if another xiaoban.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_xiaoban_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $XIAOBAN_HOME, this is cheap
        ...
        try:
            from xiaoban_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in xiaoban_cli/config.py bricked
        # every user who ran ``xiaoban update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``xiaoban
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``xiaoban update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_xiaoban_home added to xiaoban_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``xiaoban`` launch finishes the
        ...
        # build.  ``xiaoban desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``xiaoban desktop`` shouldn't be forced into a full
        # Electron build by ``xiaoban update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        from xiaoban_constants import find_node_executable

        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "xiaoban_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `xiaoban desktop` to retry)")
        ...
        # current — so copy it straight over ~/.xiaoban/cache/model_catalog.json
        ...
        # modules in this process.  Reload xiaoban_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_xiaoban_home() added since the last release.
        try:
            import importlib
            import xiaoban_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit XIAOBAN_HOME so
        # it is not affected by sync_skills()'s module-level XIAOBAN_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's XIAOBAN_HOME env var points at the default or a named profile.
        try:
            from xiaoban_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from xiaoban_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'xiaoban config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: xiaoban config migrate")
            else:
                print()
                print("Skipped. Run 'xiaoban config migrate' later to configure.")
        else:
        ...
        try:
            from xiaoban_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `xiaoban curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``xiaoban update`` gives users a
        ...
        try:
            if sys.platform in ("darwin", "win32", "linux") and shutil.which("cua-driver"):
                from xiaoban_cli.tools_config import install_cua_driver
        ...
        # When running as ``xiaoban update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_xiaoban_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... xiaoban-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... xiaoban-gateway*)
            ...
            # Discover all xiaoban-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. xiaoban-gateway.service or xiaoban-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `xiaoban update` restart it automatically, allow\n"
                            ...
                            # path in `xiaoban gateway restart`
                        ...
                        print(
                            ...
                            f"Check the gateway with: xiaoban gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: xiaoban gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: xiaoban -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Xiaoban gateway unit files are still installed.
        # When both xiaoban.service (from a pre-rename install) and the
        # current xiaoban-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `xiaoban update` surfaces the issue until the user migrates.
        try:
            from xiaoban_cli.gateway import (
                has_legacy_xiaoban_units,
                _find_legacy_xiaoban_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_xiaoban_units():
                print()
                print("⚠ Legacy Xiaoban gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_xiaoban_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (xiaoban.service) fight the current")
                print("  xiaoban-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    xiaoban gateway migrate-legacy")
                print()
        ...
        print("  xiaoban model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``xiaoban -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from xiaoban_constants import display_xiaoban_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `xiaoban profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_xiaoban_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → xiaoban -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.xiaoban)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  xiaoban profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     xiaoban -p {name} chat")
                else:
        ...
        from xiaoban_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from xiaoban_constants import get_xiaoban_home as _hh
        ...
        from xiaoban_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `xiaoban profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → xiaoban -p {name}  ({wrapper})")
        print()
        ...
        from xiaoban_cli.profiles import profile_exists
        ...
        from xiaoban_cli.profiles import rename_profile
        ...
        from xiaoban_cli.profiles import export_profile
        ...
        from xiaoban_cli.profiles import import_profile
        ...
        import tempfile
        from xiaoban_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="xiaoban_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  xiaoban -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      xiaoban -p {plan.manifest.name} chat")
        ...
        from xiaoban_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `xiaoban profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"xiaoban -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from xiaoban_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("xiaoban_requires"):
            print(f"Requires:     Xiaoban {data['xiaoban_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from xiaoban_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.xiaoban_requires:
        print(f"  Requires: Xiaoban {mf.xiaoban_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``xiaoban dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``xiaoban dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No xiaoban dashboard processes running.")
        return 0

    print(f"{len(pids)} xiaoban dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``xiaoban dashboard register`` for OAuth.
    ...
    try:
        from xiaoban_cli.web_server import should_require_auth
    ...
    try:
        from xiaoban_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `xiaoban dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    xiaoban dashboard register\n"
            "  It provisions a Nous Portal OAuth client and writes "
            "XIAOBAN_DASHBOARD_OAUTH_CLIENT_ID into ~/.xiaoban/.env for you.\n"
            "  Docs: https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/"
    ...
    try:
        from xiaoban_cli.plugins import discover_plugins
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No xiaoban dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `xiaoban update`.
    ...
    # launches the dashboard (`worker dashboard` → XIAOBAN_HOME points into
    ...
    if (
        ...
        and os.environ.get("XIAOBAN_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "xiaoban_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        # XIAOBAN_HOME.  We must resolve the root explicitly instead of just
        # dropping XIAOBAN_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV XIAOBAN_HOME=/opt/data`), so an unset
        # XIAOBAN_HOME falls back to $HOME/.xiaoban = /opt/data/.xiaoban — an
        ...
        # update-button guard also misfires).  get_default_xiaoban_root()
        # returns the root for both layouts: ~/.xiaoban for a standard install
        ...
        try:
            from xiaoban_constants import get_default_xiaoban_root
            env["XIAOBAN_HOME"] = str(get_default_xiaoban_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop XIAOBAN_HOME) rather than block the reroute.
            env.pop("XIAOBAN_HOME", None)
    ...
    # the same logs directory as every other Xiaoban surface.
    try:
        from xiaoban_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "XIAOBAN_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["XIAOBAN_WEB_DIST"])
            if "XIAOBAN_WEB_DIST" in os.environ
            else PROJECT_ROOT / "xiaoban_cli" / "web_dist"
        )
    ...
    try:
        from xiaoban_cli.mcp_startup import start_background_mcp_discovery
    ...
    from xiaoban_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from xiaoban_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from xiaoban_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from xiaoban_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from xiaoban_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Xiaoban log files."""
    from xiaoban_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from xiaoban_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``xiaoban -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``xiaoban_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``xiaoban -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``xiaoban`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("XIAOBAN_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from xiaoban_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from xiaoban_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["XIAOBAN_DEFER_AGENT_STARTUP"] = "1"
            os.environ["XIAOBAN_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["XIAOBAN_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `xiaoban --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_xiaoban_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_xiaoban_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_xiaoban_home()}/memories/\n")
    else:
        from xiaoban_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Xiaoban-Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from xiaoban_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from xiaoban_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from xiaoban_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from xiaoban_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from xiaoban_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from xiaoban_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from xiaoban_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from xiaoban_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from xiaoban_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for xiaoban CLI."""
    # Cosmetic: make the process show up as 'xiaoban' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from xiaoban_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``xiaoban.exe.old.*`` quarantine files left by previous
    # ``xiaoban update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_xiaoban_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``xiaoban update``
    ...
    # ``xiaoban skills install update``) merely defers recovery one launch;
    # under-matching (missing ``xiaoban -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in xiaoban_cli/subcommands/model.py)
    # =========================================================================
    ...
    # =========================================================================
    from xiaoban_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `xiaoban model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.xiaoban/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from xiaoban_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from xiaoban_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in xiaoban_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in xiaoban_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in xiaoban_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in xiaoban_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `xiaoban whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in xiaoban_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from xiaoban_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in xiaoban_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in xiaoban_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in xiaoban_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in xiaoban_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in xiaoban_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in xiaoban_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from xiaoban_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from xiaoban_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    # hooks command  (parser built in xiaoban_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in xiaoban_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in xiaoban_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in xiaoban_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in xiaoban_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in xiaoban_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.xiaoban/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo xiaoban uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from xiaoban_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in xiaoban_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in xiaoban_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in xiaoban_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in xiaoban_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from xiaoban_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in xiaoban_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``xiaoban --help``, ``xiaoban version``, ``xiaoban logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from xiaoban_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from xiaoban_cli.curator import register_cli as _register_curator_cli
    ...
    # =========================================================================
    # memory command  (parser built in xiaoban_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in xiaoban_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `xiaoban computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `xiaoban tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `xiaoban computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Xiaoban app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from xiaoban_cli.tools_config import _cua_driver_cmd
            # Honor XIAOBAN_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from xiaoban_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: xiaoban computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: xiaoban computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: xiaoban computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: xiaoban computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: xiaoban computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: xiaoban computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in xiaoban_cli/subcommands/mcp.py)
    # =========================================================================
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_xiaoban_home() / "sessions"
            ...
            # Launch xiaoban --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in xiaoban_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in xiaoban_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in xiaoban_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in xiaoban_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in xiaoban_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in xiaoban_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in xiaoban_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in xiaoban_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Xiaoban-Setup.exe success screen tells users to
    # run `xiaoban desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in xiaoban_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in xiaoban_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in xiaoban_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``xiaoban -c Pokemon Agent Dev`` → ``xiaoban -c 'Pokemon Agent Dev'``
    ...
    from xiaoban_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'xiaoban -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (xiaoban hooks list, cron
