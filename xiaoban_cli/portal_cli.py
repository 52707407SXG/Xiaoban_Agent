"""``xiaoban portal`` — the human-readable entry point for Nous Portal.

Running ``xiaoban portal`` with no subcommand performs the one-shot Portal
...
``xiaoban auth add nous --type oauth`` (which still works), is identical to
``xiaoban setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``xiaoban auth`` or ``xiaoban tools``. It's the onboarding + discovery
...
DOCS_URL = "https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `xiaoban model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `xiaoban portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `xiaoban auth add nous --type
    oauth`. It reuses the exact wiring behind `xiaoban setup --portal` (which in
    ...
    """
    from xiaoban_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `xiaoban portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `xiaoban portal` is the
        # human-readable alias for `xiaoban auth add nous --type oauth` /
    ...
    print("Run `xiaoban portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `xiaoban portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `xiaoban portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `xiaoban auth add "
            "nous --type oauth`, identical to `xiaoban setup --portal`). "
