...
The Xiaoban docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.xiaoban/cache/model_catalog.json``.
...
# Fallback fetch chain. The default Xiaoban catalog is already this raw JSON
# URL, but users may still carry an older docs-page URL in config; the fetcher
# dedupes identical primary/fallback URLs.
...
def _cache_path() -> Path:
    ...
    return get_xiaoban_home() / "cache" / "model_catalog.json"
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``xiaoban update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.xiaoban/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``xiaoban model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
