...
class TestBlueBubblesHelpers:
    ...
    def test_default_mention_patterns_match_xiaoban_variants(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter.require_mention is True
        assert adapter._message_matches_mention_patterns("Xiaoban, summarize this")
        assert adapter._message_matches_mention_patterns("@Xiaoban agent help")
        assert not adapter._message_matches_mention_patterns("casual family chatter")
        assert not adapter._message_matches_mention_patterns("antixiaoban should not match")
    ...
    def test_custom_mention_patterns_override_defaults(self, monkeypatch):
        ...
        assert not adapter._message_matches_mention_patterns("Xiaoban what is next?")

    def test_clean_mention_text_strips_leading_wake_word(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter._clean_mention_text("Xiaoban, summarize this") == "summarize this"
        assert adapter._clean_mention_text("Xiaoban agent: summarize this") == "summarize this"
        assert adapter._clean_mention_text("please ask Xiaoban about this") == "please ask Xiaoban about this"
