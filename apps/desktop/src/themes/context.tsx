...
const SKIN_KEY = 'xiaoban-desktop-theme-v2'
const MODE_KEY = 'xiaoban-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'xiaoban-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'xiaoban-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'xiaoban-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.xiaobanTheme = skinName
  root.dataset.xiaobanMode = rendered
  ...
  window.xiaobanDesktop?.setTitleBarTheme?.({
  ...
  try {
    window.localStorage.setItem('xiaoban-boot-background', c.background)
    window.localStorage.setItem('xiaoban-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.xiaobanThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.xiaobanDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Xiaoban backend theme on connect. */
