...
import type { ModelOptionProvider, ModelOptionsResponse } from '@/types/xiaoban'
...
export function ModelMenuPanel({ gateway, onSelectModel, requestGateway }: ModelMenuPanelProps) {
  ...
  // session (effort/fast), gated by capability. Unset → Xiaoban defaults.
          ...
          {groups.map(group => (
            <DropdownMenuGroup className="py-0.5" key={group.provider.slug}>
              ...
              {group.families.map(family => {
                ...
                // the active model, otherwise its remembered preset (Xiaoban
