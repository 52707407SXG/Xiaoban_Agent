...
Configuration
-------------
Secret (lives in $XIAOBAN_HOME/.env or the environment):
  MEM0_API_KEY       — Mem0 Platform API key (required for platform mode)

Behavioral settings (live in $XIAOBAN_HOME/mem0.json, set via `xiaoban memory
setup`):
  ...
  agent_id           — Agent identifier (default: xiaoban)
...
def _load_config() -> dict:
    """Load config from env vars, with $XIAOBAN_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "xiaoban"),
        "oss": {},
    }
    ...
    config_path = get_xiaoban_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, xiaoban_home):
        """Write config to $XIAOBAN_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, xiaoban_home: str, config: dict) -> None:
        ...
        post_setup(xiaoban_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $XIAOBAN_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "xiaoban")
