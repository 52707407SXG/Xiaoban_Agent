...
Xiaoban pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.xiaoban/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<xiaoban_home>/bin/bws`` on
  first use.  Xiaoban pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.xiaoban/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``xiaoban`` invocations don't
  hammer the API.
* Failures NEVER block Xiaoban startup.  Missing binary, no network,
...
# In-process cache so repeated load_xiaoban_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `xiaoban chat -q ...`
...
# <xiaoban_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.xiaoban/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under xiaoban_home/cache/.

    `home_path` is what `load_xiaoban_dotenv()` already resolved; falling back
    to `$XIAOBAN_HOME` / `~/.xiaoban` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("XIAOBAN_HOME", Path.home() / ".xiaoban"))
...
def _xiaoban_bin_dir() -> Path:
    """Where Xiaoban stores its managed binaries.  Profile-aware."""
    from xiaoban_constants import get_xiaoban_home

    return get_xiaoban_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<xiaoban_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _xiaoban_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``xiaoban secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _xiaoban_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="xiaoban-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "xiaoban-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<xiaoban_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from xiaoban_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_xiaoban_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `xiaoban secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `xiaoban secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `xiaoban secrets bitwarden setup` to install."
        )
        return result
