"""
MCP Server Management CLI — ``xiaoban mcp`` subcommand.

Implements ``xiaoban mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.xiaoban/config.yaml under the ``mcp_servers`` key.
"""
...
from xiaoban_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_xiaoban_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from xiaoban_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.xiaoban/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``xiaoban mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import XiaobanTokenStorage
        return XiaobanTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # xiaoban_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  xiaoban mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  xiaoban mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  xiaoban mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_xiaoban_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: xiaoban mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_xiaoban_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `xiaoban mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  xiaoban mcp add <name> --url <endpoint>')
        _info('  xiaoban mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `xiaoban mcp remove` + `xiaoban mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `xiaoban mcp login " + name + "`.")
            return
        if tools:
...
# ─── xiaoban mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'xiaoban mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``xiaoban mcp`` subcommands."""
    ...
    if action == "picker":
        from xiaoban_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from xiaoban_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from xiaoban_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `xiaoban plugin`.
        from xiaoban_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("xiaoban mcp                                    Open the catalog picker (default)")
        _info("xiaoban mcp catalog                            List Nous-approved MCPs")
        _info("xiaoban mcp install <name>                     Install a catalog MCP")
        _info("xiaoban mcp serve                              Run as MCP server")
        _info("xiaoban mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("xiaoban mcp add <name> --command <cmd>         Add a stdio server")
        _info("xiaoban mcp add <name> --preset <preset>       Add from a known preset")
        _info("xiaoban mcp remove <name>                      Remove a server")
        _info("xiaoban mcp list                               List configured servers")
        _info("xiaoban mcp test <name>                        Test connection")
        _info("xiaoban mcp configure <name>                   Toggle tools")
        _info("xiaoban mcp login <name>                       Re-authenticate OAuth")
        print()
