...
the volume.  XIAOBAN_UID/XIAOBAN_GID must still take precedence when both are
set.

The s6-overlay rework moved bootstrap from docker/entrypoint.sh (now a shim)
to docker/stage2-hook.sh, which is installed as /etc/cont-init.d/01-xiaoban-setup
...
def _alias_lines(text: str) -> list[str]:
    """The stage2 hook lines that resolve XIAOBAN_UID/XIAOBAN_GID from aliases."""
    return [
        line.strip()
        for line in text.splitlines()
        if line.strip().startswith(("XIAOBAN_UID=", "XIAOBAN_GID="))
    ]
...
def test_stage2_hook_resolves_puid_pgid_aliases(stage2_text: str) -> None:
    ...
    assert any("PUID" in line for line in alias_lines), (
        "docker/stage2-hook.sh must resolve XIAOBAN_UID from a PUID alias; see #15290"
    )
    assert any("PGID" in line for line in alias_lines), (
        "docker/stage2-hook.sh must resolve XIAOBAN_GID from a PGID alias; see #15290"
    )
...
def _resolve(stage2_text: str, env: dict[str, str]) -> str:
    ...
    resolved ``XIAOBAN_UID:XIAOBAN_GID`` pair."""
    bash = shutil.which("bash")
    if bash is None:
        pytest.skip("bash not available")
    script = "\n".join(_alias_lines(stage2_text))
    script += '\necho "${XIAOBAN_UID:-}:${XIAOBAN_GID:-}"\n'
    proc = subprocess.run(
        [bash, "-ec", script],
...
def test_puid_pgid_populate_xiaoban_uid_gid(stage2_text: str) -> None:
    assert _resolve(stage2_text, {"PUID": "1000", "PGID": "10"}) == "1000:10"


def test_xiaoban_uid_gid_take_precedence_over_aliases(stage2_text: str) -> None:
    resolved = _resolve(
        stage2_text,
        {"XIAOBAN_UID": "2000", "XIAOBAN_GID": "2001", "PUID": "1000", "PGID": "10"},
    )
    assert resolved == "2000:2001"


def test_no_uid_vars_leaves_values_empty(stage2_text: str) -> None:
    # An empty resolution means the stage2 hook keeps the default xiaoban user.
...
def test_stage2_hook_runs_config_migration_as_xiaoban(stage2_text: str) -> None:
    assert "scripts/docker_config_migrate.py" in stage2_text
    assert 's6-setuidgid xiaoban "$INSTALL_DIR/.venv/bin/python"' in stage2_text


def test_stage2_hook_documents_config_migration_opt_out(stage2_text: str) -> None:
    assert "XIAOBAN_SKIP_CONFIG_MIGRATION" in stage2_text
