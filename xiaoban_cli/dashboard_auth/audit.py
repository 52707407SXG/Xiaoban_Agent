...
Profile-aware location: ``$XIAOBAN_HOME/logs/dashboard-auth.log``.
...
from ``xiaoban_constants`` or other xiaoban_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$XIAOBAN_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``xiaoban_constants.get_xiaoban_home`` semantics: env var wins,
    else ``~/.xiaoban``. A local copy avoids an import cycle with the
    middleware which lives below ``xiaoban_cli``.
    """
    home = os.environ.get("XIAOBAN_HOME") or str(Path.home() / ".xiaoban")
