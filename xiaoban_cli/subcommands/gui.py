...
def build_gui_parser(subparsers, *, cmd_gui: Callable) -> None:
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the Xiaoban Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any xiaoban CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--xiaoban-root",
        help="Override the Xiaoban source root used by Desktop (sets XIAOBAN_DESKTOP_XIAOBAN_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets XIAOBAN_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",
