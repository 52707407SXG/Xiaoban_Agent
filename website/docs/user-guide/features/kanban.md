...
**Zero schema footprint on normal sessions.** A regular `xiaoban chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `HERMES_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
hermes kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `xiaoban gateway start` instead)
