...
- Per-user tokens (keyed by sender email):
    ``${XIAOBAN_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${XIAOBAN_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${XIAOBAN_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${XIAOBAN_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${XIAOBAN_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's XIAOBAN_HOME helper so the token follows the user's
# profile (e.g. tests can override via XIAOBAN_HOME=/tmp/...).
try:
    from xiaoban_constants import display_xiaoban_home, get_xiaoban_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where xiaoban_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _xiaoban_home.py shim).
...
def _xiaoban_home() -> Path:
    """Resolve XIAOBAN_HOME at call time (NOT module import).

    Tests and ``XIAOBAN_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.xiaoban/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _xiaoban_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _xiaoban_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _xiaoban_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _xiaoban_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _xiaoban_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install xiaoban-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'xiaoban-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into XIAOBAN_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_xiaoban_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_xiaoban_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Xiaoban (native attachment delivery)"
    )
