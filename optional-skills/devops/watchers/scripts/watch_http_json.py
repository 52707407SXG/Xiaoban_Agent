...
Usage (via cron with --no-agent):
    ...
    xiaoban cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$XIAOBAN_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Xiaoban-Watcher/1.0"})
