...
Older images made /opt/xiaoban/ui-tui and /opt/xiaoban/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV XIAOBAN_TUI_DIR=/opt/xiaoban/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R xiaoban:xiaoban /opt/xiaoban/ui-tui" not in text
    assert "chown -R xiaoban:xiaoban /opt/xiaoban/node_modules" not in text
