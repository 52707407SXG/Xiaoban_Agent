"""Tests for `xiaoban chat --safe-mode` — pristine troubleshooting runs.
...
_VARS = ("XIAOBAN_SAFE_MODE", "XIAOBAN_IGNORE_USER_CONFIG", "XIAOBAN_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in xiaoban_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("XIAOBAN_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under XIAOBAN_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("XIAOBAN_SAFE_MODE", "1")
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("XIAOBAN_SAFE_MODE", raising=False)
        from xiaoban_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under XIAOBAN_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `xiaoban chat`."""
