import { AlternateScreen, Box, NoSelect, ScrollBox, Text } from '@xiaoban/ink'
            ...
            <Box flexDirection="column" key={row.key} ref={transcript.virtualHistory.measureRef(row.key)}>
              ...
              {row.msg.kind === 'intro' ? (
                <Box flexDirection="column" paddingTop={1}>
                  {!row.msg.info && <Banner maxWidth={Math.max(1, composer.cols - 2)} t={ui.theme} />}
...
const ComposerPane = memo(function ComposerPane({
  ...
  return (
    ...
    >
      ...
      {!composer.empty && !ui.sid && <Text color={ui.theme.color.muted}>M {ui.status}</Text>}
