...
class LongPreviewAgent:
    ...
    LONG_CMD = "cd /home/teknium/.xiaoban/xiaoban-agent/.worktrees/xiaoban-d8860339 && source .venv/bin/activate && python -m pytest tests/gateway/test_run_progress_topics.py -n0 -q"
...
async def test_run_agent_progress_stays_in_originating_topic(monkeypatch, tmp_path):
    monkeypatch.setenv("XIAOBAN_TOOL_PROGRESS_MODE", "all")
...
async def test_terminal_progress_verbose_shows_full_command(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("XIAOBAN_TOOL_PROGRESS_MODE", "verbose")
