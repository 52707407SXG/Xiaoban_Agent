...
sidebar_position: 3
title: "Desktop App"
description: "The native Xiaoban desktop app — a polished experience for chatting with Xiaoban, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Xiaoban desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Xiaoban-Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `xiaoban` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Xiaoban has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`xiaoban`) and **[TUI](./tui.md)** (`xiaoban --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`xiaoban dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Xiaoban Desktop](../getting-started/installation.md).

If you already have Xiaoban installed, simply run
...
- **The same conversation history** as every other Xiaoban surface — sessions started here resume in the CLI/TUI and vice versa.
...
Chatting against a Xiaoban instance on another machine instead of the bundled local backend? See [Connecting to a remote backend](#connecting-to-a-remote-backend) below — and for the full picture of how the remote-hosted dashboard connection works (the auth gate, the `/api/ws` chat socket, and WebSocket close-code triage), see [Web Dashboard → Connecting Xiaoban Desktop to a remote backend](./features/web-dashboard.md#connecting-xiaoban-desktop-to-a-remote-backend).
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `xiaoban desktop --cwd <path>` (or the `XIAOBAN_DESKTOP_CWD` environment variable).

### Voice

Talk to Xiaoban and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `xiaoban model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Xiaoban management surface so you don't have to drop to a terminal:

- **Skills** — browse, install, and manage [skills](./features/skills.md).
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#xiaoban-cron).
- **Profiles** — switch between [Xiaoban profiles](./profiles.md) (isolated config/skills/sessions).
...
The [manual update process](https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the Xiaoban agent, your config, and your chats stay. (Same as `xiaoban uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `xiaoban uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `xiaoban uninstall --full`.)
...
You can do the same from the terminal — `xiaoban uninstall --gui` for the GUI alone, or `xiaoban uninstall` / `xiaoban uninstall --full` for the agent too.

:::note
Running `xiaoban uninstall --gui` from a **source checkout** (a `xiaoban desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `xiaoban desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `xiaoban desktop`

To launch via the CLI, simply run `xiaoban desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `xiaoban update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `XIAOBAN_DESKTOP_CWD`)           |
| `--xiaoban-root PATH` | Override the Xiaoban source root the app uses (sets `XIAOBAN_DESKTOP_XIAOBAN_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `xiaoban` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Xiaoban-Agent runtime into `XIAOBAN_HOME` (`~/.xiaoban`, or `%LOCALAPPDATA%\xiaoban` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `XIAOBAN_DESKTOP_XIAOBAN_ROOT`, then a completed managed install, then a probed `xiaoban` on `PATH` (unless `--ignore-existing` / `XIAOBAN_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `XIAOBAN_DESKTOP_XIAOBAN` command override for packagers such as Nix. The React renderer talks to a `xiaoban dashboard` backend over the `tui_gateway`/dashboard APIs and reuses the agent runtime rather than embedding `xiaoban --tui`. Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a Xiaoban backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.

:::info The remote backend is a running `xiaoban dashboard` process
"Remote backend" means a **`xiaoban dashboard`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that dashboard is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `xiaoban dashboard` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Set a username and password, then start the dashboard bound to a reachable address. The credentials live in `~/.xiaoban/.env` (the secrets file, mode 0600):
...
Keep that `xiaoban dashboard` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/xiaoban` work if you front it with a reverse proxy)
...
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `XIAOBAN_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `XIAOBAN_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateway settings panel.
...
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `XIAOBAN_DASHBOARD_BASIC_AUTH_USERNAME` / `XIAOBAN_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for an unknown user and a wrong password (no enumeration oracle), so double-check both. Confirm the gate is on with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.xiaoban/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `XIAOBAN_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Xiaoban Desktop to a remote backend](./features/web-dashboard.md#connecting-xiaoban-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Xiaoban Desktop](../reference/environment-variables.md#web-dashboard--xiaoban-desktop).

## Troubleshooting

Boot logs land in `XIAOBAN_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
rm "$HOME/.xiaoban/xiaoban-agent/.xiaoban-bootstrap-complete"

# Rebuild a broken Python venv (macOS/Linux)
rm -rf "$HOME/.xiaoban/xiaoban-agent/venv"

# Reset a stuck macOS microphone prompt
tccutil reset Microphone com.nousresearch.xiaoban
```
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.xiaoban/xiaoban-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
```bash
XIAOBAN_DESKTOP_XIAOBAN_ROOT=/path/to/clone npm run dev
XIAOBAN_HOME=/tmp/throwaway npm run dev
...
- [TUI](./tui.md) — the modern terminal UI used by `xiaoban --tui` and the dashboard chat tab
