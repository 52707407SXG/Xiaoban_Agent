"""Tests for xiaoban_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Xiaoban entry point
(xiaoban, xiaoban-agent, xiaoban-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Xiaoban entry point imports xiaoban_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported xiaoban_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("xiaoban_bootstrap", None)
    import xiaoban_bootstrap  # noqa: WPS433
    return xiaoban_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Xiaoban CLI
...
class TestEntryPointsImportBootstrap:
    """Every Xiaoban entry point must import xiaoban_bootstrap as its
    ...
    # Entry points that invoke Xiaoban as a process.  Each one must
    # import xiaoban_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "xiaoban_cli/main.py",   # xiaoban CLI (console_script)
        "run_agent.py",          # xiaoban-agent (console_script)
        "acp_adapter/entry.py",  # xiaoban-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import xiaoban_bootstrap' and that
        ...
        half-finished ``xiaoban update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``xiaoban_bootstrap`` as a top-level module) leaves xiaoban
        ...
        """
        # Resolve relative to the xiaoban-agent repo root.  Tests live
        # at tests/test_xiaoban_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # xiaoban start even when xiaoban_bootstrap hasn't been
        ...
        assert first_import_name == "xiaoban_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'xiaoban_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import xiaoban_bootstrap' line to be the first import."
        )
