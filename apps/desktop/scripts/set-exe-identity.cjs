#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Xiaoban icon + version metadata onto the
// built Xiaoban.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Xiaoban.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `xiaoban desktop`, the installer's
...
// path (which rebuilds via `xiaoban desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Xiaoban.exe>
//
...
// Stamp the Xiaoban icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Xiaoban',
      FileDescription: 'Xiaoban',
      CompanyName: 'Nous Research',
      LegalCopyright: 'Copyright (c) 2026 Nous Research'
    }
  })

  console.log('[set-exe-identity] done — Xiaoban icon + identity stamped')
}
