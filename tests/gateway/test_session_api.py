...
async def test_run_agent_binds_api_session_context_for_tool_env(adapter, monkeypatch):
    ...
    monkeypatch.setenv("XIAOBAN_SESSION_ID", "stale-session")
    observed = {}
    ...
    class FakeAgent:
        ...
        def run_conversation(self, user_message, conversation_history, task_id):
            ...
            observed["context_session_id"] = get_session_env("XIAOBAN_SESSION_ID")
            observed["context_platform"] = get_session_env("XIAOBAN_SESSION_PLATFORM")
            observed["context_session_key"] = get_session_env("XIAOBAN_SESSION_KEY")
            observed["context_user_id"] = get_session_env("XIAOBAN_SESSION_USER_ID")
            observed["context_message_id"] = get_session_env("XIAOBAN_SESSION_MESSAGE_ID")
            observed["trusted_user_message"] = get_session_user_message()
            observed["child_session_id"] = child_env.get("XIAOBAN_SESSION_ID")
            observed["child_user_id"] = child_env.get("XIAOBAN_SESSION_USER_ID")
            observed["child_message_id"] = child_env.get("XIAOBAN_SESSION_MESSAGE_ID")
            observed["child_user_message"] = child_env.get(
                "XIAOBAN_SESSION_USER_MESSAGE"
            )
    ...
    assert observed == {
        ...
        "context_user_id": "ZYJ005",
        "context_message_id": "msg-001",
        "trusted_user_message": "hello",
        "child_session_id": "request-session",
        "child_user_id": "ZYJ005",
        "child_message_id": "msg-001",
        "child_user_message": None,
    }
...
async def test_session_crud_and_message_history(adapter, session_db):
    ...
    async with TestClient(TestServer(app)) as cli:
        ...
        assert created["object"] == "xiaoban.session"
        assert created["session"]["title"] == "Mobile chat"

        session_db.append_message(session_id, "user", "hello from phone")
        session_db.append_message(session_id, "assistant", "hello from xiaoban")
        ...
        assert deleted == {"object": "xiaoban.session.deleted", "id": session_id, "deleted": True}
...
async def test_session_fork_uses_current_sessiondb_branch_primitives(adapter, session_db):
    ...
    assert payload["object"] == "xiaoban.session"
    ...
    with patch.object(auth_adapter, "_run_agent", mock_run):
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                ...
                headers={"Authorization": "Bearer sk-test", "X-Xiaoban-Session-Key": "client-42"},
            )
            assert resp.status == 200
            payload = await resp.json()

    assert resp.headers["X-Xiaoban-Session-Id"] == session_id
    assert resp.headers["X-Xiaoban-Session-Key"] == "client-42"
    assert payload["object"] == "xiaoban.session.chat.completion"
...
async def test_session_header_rejected_without_api_key(adapter, session_db):
    ...
    async with TestClient(TestServer(app)) as cli:
        resp = await cli.post(
            f"/api/sessions/{session_id}/chat",
            json={"message": "hello"},
            headers={"X-Xiaoban-Session-Key": "client-42"},
        )
        assert resp.status == 403
        data = await resp.json()
        assert "X-Xiaoban-Session-Key requires API key" in data["error"]["message"]
