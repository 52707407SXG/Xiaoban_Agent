"""``xiaoban debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Xiaoban-Agent. Use 'xiaoban debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    xiaoban debug share              Upload debug report and print URL
    xiaoban debug share --lines 500  Include more log lines
    xiaoban debug share --expire 30  Keep paste for 30 days
    xiaoban debug share --local      Print report locally (no upload)
    xiaoban debug share --no-redact  Disable upload-time secret redaction
    xiaoban debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'xiaoban debug share'",
    )
