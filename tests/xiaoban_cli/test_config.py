"""Tests for xiaoban_cli configuration management."""
...
class TestGetXiaobanHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert home == Path.home() / ".xiaoban"

    def test_env_override(self):
        with patch.dict(os.environ, {"XIAOBAN_HOME": "/custom/path"}):
            home = get_xiaoban_home()
            assert home == Path("/custom/path")


class TestEnsureXiaobanHome:
    def test_creates_subdirs(self, tmp_path):
        with patch.dict(os.environ, {"XIAOBAN_HOME": str(tmp_path)}):
            ensure_xiaoban_home()
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``xiaoban logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from xiaoban_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"XIAOBAN_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="xiaoban_cli.config"):
                config = load_config()
            ...
            # stands out at xiaoban startup before logging is configured)
            captured = capsys.readouterr()
            assert "xiaoban config:" in captured.err
    ...
    def test_dedup_on_repeated_load_same_file(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"XIAOBAN_HOME": str(tmp_path)}):
            ...
            assert "xiaoban config:" in first
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"XIAOBAN_HOME": str(tmp_path)}):
            ...
            assert "xiaoban config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"XIAOBAN_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"XIAOBAN_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"XIAOBAN_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"XIAOBAN_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"XIAOBAN_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"XIAOBAN_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from xiaoban_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """XIAOBAN_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `xiaoban setup`) lets a
        ...
        via config.yaml; XIAOBAN_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from xiaoban_cli.config import OPTIONAL_ENV_VARS
        assert "XIAOBAN_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``XIAOBAN_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Xiaoban
    process on next startup via the dotenv → ``os.environ`` chain in
    ``xiaoban_cli/env_loader.py``.
    ...
    def _xiaoban_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "XIAOBAN_SPOTIFY_CLIENT_ID",
            "XIAOBAN_QWEN_BASE_URL",
            "XIAOBAN_MAX_ITERATIONS",
        ],
    )
    def test_xiaoban_integration_keys_still_writable(self, allowed_key):
        """``XIAOBAN_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``XIAOBAN_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``XIAOBAN_*``) keep working. The denylist
