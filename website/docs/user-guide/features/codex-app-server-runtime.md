...
(Hermes will preserve your override on re-migration as long as it lives outside the `# managed by xiaoban-agent` markers.)
...
```toml
# managed by xiaoban-agent — `hermes codex-runtime migrate` regenerates this section
...
CODEX_HOME=~/.hermes/profiles/work/codex xiaoban chat
```
...
If you find a bug, [open an issue](https://github.com/52707407SXG/Xiaoban-Agent/issues) with the output of `xiaoban logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.
...
For implementation details, see [PR #24182](https://github.com/52707407SXG/Xiaoban-Agent/pull/24182) and the [Codex app-server protocol README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md).
