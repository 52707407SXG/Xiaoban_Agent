import { type ScrollBoxHandle, useApp, useHasSelection, useSelection, useStdout, useTerminalTitle } from '@xiaoban/ink'
...
export function useMainApp(gw: GatewayClient) {
  ...
  useTerminalTitle(
    model ? composeTabTitle(marker, ui.sessionTitle, model, tabCwd ? shortCwd(tabCwd, 24) : '') : 'Xiaoban'
  )
  ...
  const cwd = ui.info?.cwd || process.env.XIAOBAN_CWD || process.cwd()
