...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_XIAOBAN_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Xiaoban expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``xiaoban_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_xiaoban_home()`` themselves.
        """
        if "xiaoban_home" not in kwargs:
            from xiaoban_constants import get_xiaoban_home
            kwargs["xiaoban_home"] = str(get_xiaoban_home())
        for provider in self._providers:
            try:
