...
def _resolve_xiaoban_home() -> Path:
...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.xiaoban",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``XIAOBAN_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside XIAOBAN_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    xiaoban_home = _resolve_xiaoban_home()

    # Reject absolute paths — they bypass the XIAOBAN_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to XIAOBAN_HOME)",
            relative_path,
        )
        return False

    host_path = xiaoban_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape XIAOBAN_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, xiaoban_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.xiaoban/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="xiaoban-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching xiaoban_constants.get_xiaoban_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_xiaoban_dir()`` for backward compatibility with old directory layouts.
    """
    from xiaoban_constants import get_xiaoban_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_xiaoban_dir(new_subpath, old_name)
        if host_dir.is_dir():
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.xiaoban``,
    SSH ``<remote_home>/.xiaoban``, etc.) and whether translation is wanted.
