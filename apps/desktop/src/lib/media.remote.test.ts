...
describe('filePathFromMediaPath', () => {
  it('passes through a plain path', () => {
    expect(filePathFromMediaPath('/home/u/.xiaoban/images/a.png')).toBe('/home/u/.xiaoban/images/a.png')
  })
...
describe('gatewayMediaDataUrl', () => {
  ...
  beforeEach(() => {
    api.mockClear()
    vi.stubGlobal('window', { xiaobanDesktop: { api } })
  })
  ...
  it('requests the encoded gateway path and returns the data URL', async () => {
    const url = await gatewayMediaDataUrl('/home/u/.xiaoban/images/a b.png')

    expect(url).toBe('data:image/png;base64,ZHVtbXk=')
    expect(api).toHaveBeenCalledWith({
      path: '/api/media?path=%2Fhome%2Fu%2F.xiaoban%2Fimages%2Fa%20b.png'
    })
  })
})
