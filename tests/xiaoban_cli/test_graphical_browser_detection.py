"""Tests for `_can_open_graphical_browser()` in xiaoban_cli.auth.

Guards the fix for the May 2026 report where `xiaoban auth add xai-oauth`
...
from xiaoban_cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("xiaoban_cli.auth.sys.platform", "linux")
...
def test_non_linux_with_gui_allows(monkeypatch):
    ...
    monkeypatch.setattr("xiaoban_cli.auth.sys.platform", "darwin")
