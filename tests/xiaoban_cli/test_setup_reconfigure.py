...
- Bare `xiaoban setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `xiaoban setup --quick` runs the narrower "fill in missing items" flow.
- `xiaoban setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("xiaoban_cli.setup.ensure_xiaoban_home", {}),
        ("xiaoban_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("xiaoban_cli.config.is_managed", {"return_value": False}),
        ("xiaoban_cli.setup.load_config", {"return_value": {}}),
        ("xiaoban_cli.setup.save_config", {}),
        ("xiaoban_cli.setup.get_env_value", {"return_value": None}),
        ("xiaoban_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("xiaoban_cli.setup._print_setup_summary", {}),
        ("xiaoban_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("xiaoban_cli.setup.load_config", {"return_value": {}}),
        ("xiaoban_cli.setup.save_config", {}),
        ("xiaoban_cli.auth.get_active_provider", {"return_value": None}),
        ("xiaoban_cli.setup.get_env_value", {"return_value": None}),
        ("xiaoban_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `xiaoban setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="xiaoban_cli.setup.prompt_choice",
                quick="xiaoban_cli.setup._run_quick_setup",
                model="xiaoban_cli.setup.setup_model_provider",
                terminal="xiaoban_cli.setup.setup_terminal_backend",
                agent="xiaoban_cli.setup.setup_agent_settings",
                gateway="xiaoban_cli.setup.setup_gateway",
                tools="xiaoban_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`xiaoban setup --reconfigure` behaves the same as bare `xiaoban setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("xiaoban_cli.setup.prompt_choice", {"return_value": 0}),
                first="xiaoban_cli.setup._run_first_time_quick_setup",
            )
            from xiaoban_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "xiaoban_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["xiaoban", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["xiaoban", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["xiaoban", "setup"])
        try:
            main()
        except SystemExit:
            pass
