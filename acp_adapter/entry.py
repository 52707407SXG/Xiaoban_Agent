"""CLI entry point for the xiaoban-agent ACP adapter.

Loads environment variables from ``~/.xiaoban/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from XIAOBAN_HOME (default ``~/.xiaoban``)."""
    from xiaoban_cli.env_loader import load_xiaoban_dotenv

    xiaoban_home = get_xiaoban_home()
    loaded = load_xiaoban_dotenv(xiaoban_home=xiaoban_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", xiaoban_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="xiaoban-acp",
        description="Run Xiaoban-Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Xiaoban version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Xiaoban provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.xiaoban/node/ "
...
def _print_version() -> None:
    ...
    print(xiaoban_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import XiaobanACPAgent  # noqa: F401

    print("Xiaoban ACP check OK")


def _run_setup() -> None:
    from xiaoban_cli.main import main as xiaoban_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "xiaoban", "model"]
        xiaoban_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``xiaoban postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting xiaoban-agent ACP adapter")
    ...
    import acp
    from .server import XiaobanACPAgent
