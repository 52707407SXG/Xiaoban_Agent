...
from xiaoban_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdatePip:
    ...
    def test_update_pip_exports_virtualenv_from_sys_prefix(
        ...
        from xiaoban_cli import main as hm

        mock_run.return_value = subprocess.CompletedProcess([], 0, stdout="", stderr="")
        monkeypatch.delenv("VIRTUAL_ENV", raising=False)
        monkeypatch.setattr(hm.sys, "prefix", "/tmp/xiaoban-launcher-venv")
        monkeypatch.setattr(hm.sys, "base_prefix", "/usr")

        hm._cmd_update_pip(mock_args)

        assert mock_run.call_count == 1
        assert mock_run.call_args.args[0] == ["/usr/bin/uv", "pip", "install", "--upgrade", "xiaoban-agent"]
        assert mock_run.call_args.kwargs["env"]["VIRTUAL_ENV"] == "/tmp/xiaoban-launcher-venv"
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind 52707407SXG/Xiaoban-Agent silently misses updates.
        """
        from xiaoban_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/xiaoban-agent.git",
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "xiaoban_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "xiaoban_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("xiaoban_cli.config.check_config_version", return_value=(1, 2)), patch(
            "xiaoban_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("xiaoban_cli.main.sys") as mock_sys:
            ...
            from xiaoban_cli.config import migrate_config
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "xiaoban_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "xiaoban_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "xiaoban_cli.config.check_config_version", return_value=(5, 24)
        ), patch(
            "xiaoban_cli.config.migrate_config",
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "xiaoban_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "xiaoban_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "xiaoban_cli.config.check_config_version", return_value=(1, 24)
        ), patch(
            "xiaoban_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("xiaoban_cli.main.sys") as mock_sys:
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.xiaoban"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.xiaoban/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.xiaoban/profiles/work"))
        ...
        with (
            patch("xiaoban_cli.profiles.list_profiles", return_value=all_profiles),
            patch("xiaoban_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("xiaoban_cli.profiles.list_profiles", return_value=[default_p]),
            patch("xiaoban_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateBranchFlag:
    """``xiaoban update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``xiaoban update --check --branch <name>`` honors the branch override.
    ...
    @patch("xiaoban_cli.config.detect_install_method", return_value="pip")
    @patch("xiaoban_cli.banner.check_via_pypi", return_value=0)
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    """``xiaoban update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from xiaoban_cli.main import _update_via_zip
