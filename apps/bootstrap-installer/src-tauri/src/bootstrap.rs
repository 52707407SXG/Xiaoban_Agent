...
//! XIAOBAN_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for XIAOBAN_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub xiaoban_home: Option<String>,
}
...
/// Spawn the locally-built Xiaoban desktop binary, then close the installer
...
#[tauri::command]
pub async fn launch_xiaoban_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_xiaoban_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Xiaoban desktop at {}. The desktop build step \
             may have been skipped or failed. Run `xiaoban desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching Xiaoban desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Xiaoban
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_xiaoban_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Xiaoban.exe"),
            ("win-arm64-unpacked", "Xiaoban.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Xiaoban.app/Contents/MacOS", "Xiaoban"),
            ("mac-arm64/Xiaoban.app/Contents/MacOS", "Xiaoban"),
        ]
    } else {
        &[("linux-unpacked", "xiaoban")]
    };
...
pub(crate) fn resolve_xiaoban_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_xiaoban_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../Xiaoban.app/Contents/MacOS/Xiaoban -> .../Xiaoban.app
...
/// path so a bare re-open just opens Xiaoban instead of re-running setup.
pub(crate) fn xiaoban_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".xiaoban-bootstrap-complete").exists()
        && resolve_xiaoban_desktop_exe(install_root).is_some()
}
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_xiaoban_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Xiaoban desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_xiaoban_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::xiaoban_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $XiaobanHome which Stage-Repository clones
    // the repo INTO at $XiaobanHome\xiaoban-agent. Mirrors xiaoban_constants.
    let xiaoban_home = args
        .xiaoban_home
        .clone()
        .unwrap_or_else(|| crate::paths::xiaoban_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&xiaoban_home).join("xiaoban-agent");

    // Copy ourselves to XIAOBAN_HOME/xiaoban-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_xiaoban_home() {
        tracing::warn!(?err, "failed to copy installer into XIAOBAN_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    args: &[String],
    xiaoban_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, xiaoban_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Xiaoban"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Xiaoban.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Xiaoban.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("xiaoban");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/Xiaoban.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_xiaoban_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_xiaoban_desktop_app(&root)
    ...
    #[test]
    fn resolve_xiaoban_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_xiaoban_desktop_app(&root).is_none(),
