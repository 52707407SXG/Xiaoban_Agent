"""
Backup and import commands for xiaoban CLI.

`xiaoban backup` creates a zip archive of the entire ~/.xiaoban/ directory
(excluding the xiaoban-agent repo and transient files).

`xiaoban import` restores from a backup zip, overlaying onto the current
XIAOBAN_HOME root.
"""
...
from xiaoban_constants import get_default_xiaoban_root, get_xiaoban_home, display_xiaoban_home
...
# ``xiaoban-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/xiaoban-agent/``
...
# XIAOBAN_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "xiaoban-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under XIAOBAN_HOME) —
...
# File names that ``xiaoban import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE XIAOBAN_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside XIAOBAN_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to xiaoban root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``xiaoban-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/xiaoban-agent/`` — must be preserved.
        if part == "xiaoban-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside XIAOBAN_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Xiaoban home directory."""
    xiaoban_root = get_default_xiaoban_root()

    if not xiaoban_root.is_dir():
        print(f"Error: Xiaoban home directory not found at {xiaoban_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"xiaoban-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"xiaoban-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_xiaoban_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(xiaoban_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(xiaoban_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``xiaoban-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "xiaoban-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(xiaoban_root)
    ...
    # outside XIAOBAN_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_xiaoban_home()}."
        )
    ...
    print(f"\nRestore with: xiaoban import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Xiaoban backup.
    ...
    # Look for telltale files that a xiaoban home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Xiaoban backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.xiaoban/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a xiaoban dir name
        if prefix in {".xiaoban", "xiaoban"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_xiaoban_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (xiaoban_root / "config.yaml").exists()
        has_env = (xiaoban_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Xiaoban configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under XIAOBAN_HOME.
            ...
            target = xiaoban_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(xiaoban_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_xiaoban_home()}."
            )
        ...
        profiles_dir = xiaoban_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # xiaoban_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: xiaoban profile list  (after installing xiaoban)")

        # Guidance
        print()
        if not (xiaoban_root / "xiaoban-agent").is_dir():
            print("Note: The xiaoban-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: xiaoban update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  xiaoban -p {pname} gateway install")

        print("Done. Your Xiaoban configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and xiaoban backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to XIAOBAN_HOME).
...
# — `xiaoban update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(xiaoban_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(xiaoban_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside XIAOBAN_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``xiaoban update``.
    ...
    Args:
        ...
        xiaoban_home: Override for the Xiaoban home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(xiaoban_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for xiaoban backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_xiaoban_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, xiaoban_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``xiaoban_root`` to ``out_path``.
...
def _pre_update_backup_dir(xiaoban_home: Optional[Path] = None) -> Path:
    home = xiaoban_home or get_xiaoban_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of XIAOBAN_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<XIAOBAN_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``xiaoban update``) should continue even if the backup fails.
    """
    xiaoban_root = xiaoban_home or get_default_xiaoban_root()
    if not xiaoban_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(xiaoban_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, xiaoban_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `xiaoban claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of XIAOBAN_HOME under ``backups/`` before a
    ``xiaoban claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``xiaoban import <archive>``.  Writes to
    ``<XIAOBAN_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `xiaoban import` and the
