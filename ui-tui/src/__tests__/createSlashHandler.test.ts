...
describe('createSlashHandler', () => {
  ...
  it('falls through to command.dispatch for skill commands and sends the message', async () => {
      ...
      gateway: {
        gw: {
          ...
          request: vi.fn((method: string) => {
            ...
            if (method === 'command.dispatch') {
              return Promise.resolve({ type: 'skill', message: skillMessage, name: 'xiaoban-agent-dev' })
            }
    ...
    const h = createSlashHandler(ctx)
    expect(h('/xiaoban-agent-dev')).toBe(true)
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('⚡ loading skill: xiaoban-agent-dev')
    })
