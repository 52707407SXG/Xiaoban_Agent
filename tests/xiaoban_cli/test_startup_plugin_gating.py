...
``xiaoban_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``xiaoban --help``, ``xiaoban version``,
``xiaoban logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``xiaoban --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from xiaoban_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["xiaoban", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["xiaoban", "-w", "--tui"], None),
        (["xiaoban", "version"], "version"),
        (["xiaoban", "--tui", "chat"], "chat"),
        (["xiaoban", "-w", "logs"], "logs"),
        (["xiaoban", "chat", "hello world"], "chat"),
        (["xiaoban", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["xiaoban", "-m", "gpt5", "chat"], "chat"),
        (["xiaoban", "--model", "gpt5", "chat", "hi"], "chat"),
        (["xiaoban", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["xiaoban", "-z", "hello world"], None),
        (["xiaoban", "-z", "hello", "chat"], "chat"),
        (["xiaoban", "--model=gpt5", "chat"], "chat"),     # inline form
        (["xiaoban", "--", "chat"], "chat"),               # -- terminator
        (["xiaoban", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["xiaoban", "some-plugin-cmd"], "some-plugin-cmd"),
        (["xiaoban", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["xiaoban"],                          # bare → chat
        ["xiaoban", "--help"],                # top-level help
        ["xiaoban", "-h"],
        ["xiaoban", "version"],               # known built-in
        ["xiaoban", "logs"],
        ["xiaoban", "gateway", "run"],
        ["xiaoban", "--tui"],
        ["xiaoban", "-w", "--tui"],
        ["xiaoban", "chat", "hi"],
        ["xiaoban", "help"],                  # accepted built-in-ish
        ["xiaoban", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["xiaoban", "meet", "join"],          # potential google_meet plugin
        ["xiaoban", "honcho", "status"],      # potential memory plugin
        ["xiaoban", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``xiaoban help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"xiaoban_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
