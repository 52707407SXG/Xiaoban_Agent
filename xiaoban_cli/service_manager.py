...
the existing module-level functions in xiaoban_cli.gateway and
xiaoban_cli.gateway_windows directly. This protocol is a thin facade
...
``xiaoban gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``xiaoban gateway start/stop/restart``).
    """
    ...
    # `xiaoban gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged xiaoban
    ...
    detection always fails. Since every Xiaoban runtime call inside the
    container drops to xiaoban via ``s6-setuidgid``, that silent failure
...
# in ``xiaoban_cli.gateway`` (systemd/launchd) and ``xiaoban_cli.gateway_windows``
...
# active (set via the ``xiaoban -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in xiaoban_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from xiaoban_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from xiaoban_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from xiaoban_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from xiaoban_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in xiaoban_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from xiaoban_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from xiaoban_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from xiaoban_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from xiaoban_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``xiaoban_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `xiaoban profile create`
# runs inside the container (Phase 4). Static services (main-xiaoban, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's XIAOBAN_HOME blindly;
    derive the shared profile root from the current XIAOBAN_HOME and map the
    ...
    xiaoban_home = Path(os.environ.get("XIAOBAN_HOME", "/opt/data"))
    if xiaoban_home.parent.name == "profiles":
        root = xiaoban_home.parent.parent
    else:
        root = xiaoban_home
...
# UID/GID of the in-image ``xiaoban`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the xiaoban user.
    ...
    (the ``xiaoban`` user — UID 10000 — running every Xiaoban runtime
    ...
    is dispatched as the xiaoban user.
    ...
    with xiaoban ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           xiaoban:xiaoban, 0755 (parent must already exist)
    ``svc_dir/event/``                     xiaoban:xiaoban, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 xiaoban:xiaoban, 0755
    ``svc_dir/supervise/event/``           xiaoban:xiaoban, 03730
    ``svc_dir/supervise/control``          xiaoban:xiaoban, 0660    (FIFO)
    ...
    slot's supervise/ was xiaoban-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _XIAOBAN_UID, _XIAOBAN_GID)
        except PermissionError:
            # Running as the xiaoban user already — directory is xiaoban-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _XIAOBAN_UID, _XIAOBAN_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is xiaoban-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _XIAOBAN_UID, _XIAOBAN_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``xiaoban -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`xiaoban profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-xiaoban, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources XIAOBAN_HOME (and any extra env) via with-contenv —
             so e.g. ``-e XIAOBAN_HOME=/data/xiaoban`` is honored at run
          ...
          4. Drops to the xiaoban user and exec's
             ``xiaoban -p <profile> gateway run`` (or just ``xiaoban
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``xiaoban gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root XIAOBAN_HOME profile" (the implicit profile that exists at
        the top of $XIAOBAN_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``xiaoban_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$XIAOBAN_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``XIAOBAN_HOME``,
        ...
        allocator in ``xiaoban_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export XIAOBAN_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's XIAOBAN_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``xiaoban gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-XIAOBAN_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The XIAOBAN_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own XIAOBAN_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "xiaoban gateway run --replace"
        else:
            gateway_cmd = f"xiaoban -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid xiaoban {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${XIAOBAN_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the XIAOBAN_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e XIAOBAN_HOME=/data/xiaoban``
        gets its logs under /data/xiaoban/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``xiaoban logs`` reads and what
        ...
        return (
            ...
            f': "${{XIAOBAN_HOME:=/opt/data}}"\n'
            f'log_dir="$XIAOBAN_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as xiaoban and crash-loops on
            ...
            f'chown xiaoban:xiaoban "$XIAOBAN_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R xiaoban:xiaoban "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid xiaoban s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``xiaoban gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``xiaoban -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with xiaoban ownership
            ...
            # the xiaoban user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is xiaoban-owned (see ``_seed_supervise_skeleton``), and on
