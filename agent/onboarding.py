...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Xiaoban starts and finds ``~/.openclaw/``.

    Points users at ``xiaoban claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``xiaoban claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Xiaoban, run "
        "`xiaoban claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `xiaoban claw cleanup` (renames it to ~/.openclaw.pre-migration — "
