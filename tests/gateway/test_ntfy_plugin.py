...
class TestNtfyRequirements:
    ...
    def test_returns_false_when_httpx_unavailable(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "xiaoban-test")
...
class TestNtfyAdapterInit:
    ...
    def test_default_server_url(self, monkeypatch):
        ...
        config = PlatformConfig(enabled=True, extra={"topic": "xiaoban-in"})
    ...
    def test_publish_topic_falls_back_to_topic(self, monkeypatch):
        ...
        config = PlatformConfig(enabled=True, extra={"topic": "xiaoban-in"})
        adapter = NtfyAdapter(config)
        assert adapter._publish_topic == "xiaoban-in"

    def test_publish_topic_uses_extra_value(self):
        config = PlatformConfig(
            enabled=True,
            extra={"topic": "xiaoban-in", "publish_topic": "xiaoban-out"},
        )
        adapter = NtfyAdapter(config)
        assert adapter._publish_topic == "xiaoban-out"
...
class TestConnect:
    ...
    def test_connect_starts_stream_task(self, monkeypatch):
        monkeypatch.setattr(_ntfy, "HTTPX_AVAILABLE", True)
        config = PlatformConfig(enabled=True, extra={"topic": "xiaoban-test"})
        adapter = NtfyAdapter(config)
...
class TestSend:
    ...
    def _make_adapter(self, topic="xiaoban-in", publish_topic="", token="", markdown=False):
    ...
    def test_send_fails_without_http_client(self):
        adapter = self._make_adapter()
        result = _run(adapter.send("xiaoban-in", "hello"))
        assert result.success is False
        assert "not initialized" in result.error.lower()

    def test_send_posts_to_publish_topic(self):
        adapter = self._make_adapter(topic="xiaoban-in", publish_topic="xiaoban-out")
        ...
        result = _run(adapter.send("xiaoban-in", "Hello ntfy!"))
        assert result.success is True
        assert result.message_id == "abc123"

        posted_url = mock_client.post.call_args[0][0]
        assert posted_url.endswith("/xiaoban-out")

    def test_send_falls_back_to_subscribe_topic(self):
        adapter = self._make_adapter(topic="xiaoban-in")
        ...
        assert result.success is True
        posted_url = mock_client.post.call_args[0][0]
        assert posted_url.endswith("/xiaoban-in")
    ...
    def test_send_uses_metadata_publish_topic(self):
        ...
        result = _run(adapter.send(
            "xiaoban-in", "Hi!", metadata={"publish_topic": "override-out"}
        ))
        assert result.success is True
        posted_url = mock_client.post.call_args[0][0]
    ...
    def test_get_chat_info_returns_dict(self):
        adapter = NtfyAdapter(PlatformConfig(enabled=True, extra={"topic": "t"}))
        info = _run(adapter.get_chat_info("xiaoban-in"))
        assert info["name"] == "xiaoban-in"
        assert info["type"] == "dm"

    def test_send_includes_bearer_auth_header(self):
        adapter = self._make_adapter(topic="xiaoban-in", token="mytoken")
        ...
        _run(adapter.send("xiaoban-in", "secure message"))
    ...
    def test_send_emits_markdown_header_when_enabled(self):
        adapter = self._make_adapter(topic="xiaoban-in", markdown=True)
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.json.return_value = {}
        mock_client = AsyncMock()
    ...
    def test_send_omits_markdown_header_when_disabled(self):
        ...
        _run(adapter.send("xiaoban-in", "plain"))
        call_headers = mock_client.post.call_args[1]["headers"]
    ...
    def test_send_emits_echo_tag_header(self):
        ...
        _run(adapter.send("xiaoban-in", "Hello!"))
        call_headers = mock_client.post.call_args[1]["headers"]
...
class TestOnMessage:
    ...
    def _make_adapter(self):
        return NtfyAdapter(PlatformConfig(enabled=True, extra={"topic": "xiaoban-in"}))
    ...
    def test_duplicate_message_skipped(self):
        ...
        adapter.set_message_handler(handler)
        event = {"id": "dup-1", "event": "message", "topic": "xiaoban-in", "message": "hi", "time": None}
        _run(adapter._on_message(event))
        _run(adapter._on_message(event))
        assert len(calls) == 1
    ...
    def test_title_not_used_as_user_id(self):
        ...
        assert captured[0].source.user_id == "xiaoban-in"
        assert captured[0].source.user_name == "xiaoban-in"
    ...
    def test_source_chat_id_is_topic(self):
        ...
        assert captured[0].source.chat_id == "xiaoban-in"
...
class TestEnvEnablement:
    ...
    def test_seeds_topic_and_server(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "xiaoban-in")
        monkeypatch.delenv("NTFY_SERVER_URL", raising=False)
        seed = _env_enablement()
        assert seed is not None
        assert seed["topic"] == "xiaoban-in"
    ...
    def test_publish_topic_seeded(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "xiaoban-in")
        monkeypatch.setenv("NTFY_PUBLISH_TOPIC", "xiaoban-out")
        seed = _env_enablement()
        assert seed["publish_topic"] == "xiaoban-out"
    ...
    def test_home_channel_defaults_to_topic(self, monkeypatch):
        ...
        seed = _env_enablement()
        assert seed["home_channel"]["chat_id"] == "xiaoban-in"
        assert seed["home_channel"]["name"] == "xiaoban-in"
...
class TestStandaloneSend:
    ...
    def test_posts_to_server(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "xiaoban-in")
        pconfig = MagicMock()
        pconfig.extra = {"server": "https://ntfy.example.com", "topic": "xiaoban-in"}
        ...
        with patch.object(_ntfy, "httpx") as mock_httpx:
            ...
            result = _run(_standalone_send(pconfig, "xiaoban-in", "hello"))

        assert result.get("success") is True
        assert result["platform"] == "ntfy"
        assert result["message_id"] == "id-42"
        posted_url = mock_client.post.call_args[0][0]
        assert posted_url == "https://ntfy.example.com/xiaoban-in"
    ...
    def test_emits_echo_tag_header(self, monkeypatch):
        ...
        monkeypatch.setenv("NTFY_TOPIC", "xiaoban-in")
        pconfig = MagicMock()
        pconfig.extra = {"topic": "xiaoban-in"}
        ...
        with patch.object(_ntfy, "httpx") as mock_httpx:
            mock_httpx.AsyncClient.return_value = mock_client
            _run(_standalone_send(pconfig, "xiaoban-in", "hi"))
    ...
    def test_emits_bearer_token_when_configured(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "xiaoban-in")
        pconfig = MagicMock()
        pconfig.extra = {"topic": "xiaoban-in", "token": "tk_xyz"}
    ...
    def test_basic_auth_when_token_has_colon(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "xiaoban-in")
        pconfig = MagicMock()
        pconfig.extra = {"topic": "xiaoban-in", "token": "user:pass"}
    ...
    def test_returns_error_on_http_failure(self, monkeypatch):
        ...
        with patch.object(_ntfy, "httpx") as mock_httpx:
            mock_httpx.AsyncClient.return_value = mock_client
            result = _run(_standalone_send(pconfig, "xiaoban-in", "hi"))
