"""Tests for xiaoban_cli.security_advisories.
...
import xiaoban_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect XIAOBAN_HOME so banner cache and config writes are sandboxed."""
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full xiaoban_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "xiaoban_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "xiaoban_cli.config.save_config",
...
class TestRendering:
    def test_short_banner_lines_includes_id_and_version(self, fake_advisory):
        ...
        assert "xiaoban doctor" in joined
