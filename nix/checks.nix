...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      ...
      hermesVenv = xiaoban-agent.hermesVenv;
    ...
    in {
      ...
      checks = {
        ...
        package-contents = pkgs.runCommand "hermes-package-contents" { } ''
          ...
          test -x ${xiaoban-agent}/bin/hermes || (echo "FAIL: hermes binary missing"; exit 1)
          test -x ${xiaoban-agent}/bin/xiaoban-agent || (echo "FAIL: xiaoban-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${xiaoban-agent}/bin/xiaoban version 2>&1 | grep -qi "hermes" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "hermes-entry-points-sync" { } ''
          ...
          for bin in hermes xiaoban-agent hermes-acp; do
        ...
        cli-commands = pkgs.runCommand "hermes-cli-commands" { } ''
          ...
          ${xiaoban-agent}/bin/hermes --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${xiaoban-agent}/bin/hermes --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "hermes-bundled-skills" { } ''
          ...
          grep -q "HERMES_BUNDLED_SKILLS" ${xiaoban-agent}/bin/hermes || \
        ...
        bundled-plugins = pkgs.runCommand "hermes-bundled-plugins" { } ''
          ...
          grep -q "HERMES_BUNDLED_PLUGINS" ${xiaoban-agent}/bin/hermes || \
        ...
        bundled-locales = pkgs.runCommand "hermes-bundled-locales" { } ''
          ...
          grep -q "HERMES_BUNDLED_LOCALES" ${xiaoban-agent}/bin/hermes || \
          ...
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && HERMES_BUNDLED_LOCALES=${xiaoban-agent}/share/xiaoban-agent/locales \
        ...
        bundled-tui = pkgs.runCommand "hermes-bundled-tui" { } ''
          ...
          grep -q "HERMES_TUI_DIR" ${xiaoban-agent}/bin/hermes || \
        ...
        hermes-node = pkgs.runCommand "hermes-node-version" { } ''
          ...
          grep -q "HERMES_NODE" ${xiaoban-agent}/bin/hermes || \
            (echo "FAIL: HERMES_NODE not set in wrapper"; exit 1)
          echo "PASS: HERMES_NODE present in wrapper"

          HERMES_NODE=$(sed -n "s/^export HERMES_NODE='\(.*\)'/\1/p" ${xiaoban-agent}/bin/hermes)
        ...
        managed-guard = pkgs.runCommand "hermes-managed-guard" { } ''
          ...
          check_blocked "config set" ${xiaoban-agent}/bin/hermes config set model foo
          check_blocked "config edit" ${xiaoban-agent}/bin/hermes config edit
        ...
        extra-python-packages = let
          ...
          hermesWithExtra = xiaoban-agent.override {
        ...
        in pkgs.runCommand "hermes-extra-python-packages" { } ''
          ...
          if grep -q "PYTHONPATH" ${xiaoban-agent}/bin/hermes; then
        ...
        extra-dependency-groups = let
          hermesWithGroups = xiaoban-agent.override {
