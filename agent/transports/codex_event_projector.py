"""Projects codex app-server events into Xiaoban' messages list.

The translator that lets Xiaoban' memory/skill review keep working under the
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Xiaoban' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Xiaoban stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Xiaoban already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Xiaoban' messages store text only.
        text_parts: list[str] = []
