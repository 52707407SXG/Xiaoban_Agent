#!/usr/bin/env python3
"""Canvas LMS API CLI for Xiaoban-Agent.
...
def _check_config():
    ...
    if missing:
        xiaoban_env = os.path.join(
            os.environ.get("XIAOBAN_HOME", os.path.expanduser("~/.xiaoban")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {xiaoban_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Xiaoban-Agent"
    )
