"""
Xiaoban Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with xiaoban-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.xiaoban/plugins/<name>/``
3. **Project plugins** – ``./.xiaoban/plugins/<name>/`` (opt-in via
   ``XIAOBAN_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``xiaoban_agent.plugins``
   entry-point group.
...
from xiaoban_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``XIAOBAN_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("XIAOBAN_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``XIAOBAN_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.xiaoban/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("XIAOBAN_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When XIAOBAN_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Xiaoban loggers.
    """
    ...
    logger.debug(
        "XIAOBAN_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # Kanban task lifecycle hooks. Fired by xiaoban_cli.kanban_db when a task
    ...
    # separate `xiaoban -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `xiaoban kanban dispatch`),
...
ENTRY_POINTS_GROUP = "xiaoban_agent.plugins"

_NS_PARENT = "xiaoban_plugins"
...
class PluginManifest:
    ...
    #              in ~/.xiaoban/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``xiaoban plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Xiaoban profile name (e.g. ``"default"``).

        Derived from ``XIAOBAN_HOME`` via
        :func:`xiaoban_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.xiaoban/profiles/<name>``, or ``"custom"`` when
        ``XIAOBAN_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``xiaoban honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``xiaoban <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`xiaoban_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from xiaoban_cli.dashboard_auth import (
    ...
    def register_slack_action_handler(
        ...
        Xiaoban' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``xiaoban model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: xiaoban_cli.main imports plugins indirectly
        from xiaoban_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.xiaoban/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        # Safe mode (--safe-mode / XIAOBAN_SAFE_MODE=1): troubleshooting run
        ...
        if env_var_enabled("XIAOBAN_SAFE_MODE"):
            logger.info("XIAOBAN_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to xiaoban_cli/. Two layouts are
        ...
        # 2. User plugins (~/.xiaoban/plugins/)
        user_dir = get_xiaoban_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.xiaoban/plugins/)
        if _env_enabled("XIAOBAN_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".xiaoban" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set XIAOBAN_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with xiaoban and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Xiaoban ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `xiaoban plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `xiaoban plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``xiaoban_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``xiaoban_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="xiaoban-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``xiaoban tools`` TUI so plugin-provided toolsets appear
