...
def test_xiaoban_client_tag_includes_current_version():
    """The client tag must reflect xiaoban_cli.__version__ verbatim."""
    from xiaoban_cli import __version__
    from agent.portal_tags import xiaoban_client_tag

    assert xiaoban_client_tag() == f"client=xiaoban-client-v{__version__}"


def test_xiaoban_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import xiaoban_client_tag

    tag = xiaoban_client_tag()
    assert tag.startswith("client=xiaoban-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import xiaoban_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=xiaoban-agent" in tags
    assert xiaoban_client_tag() in tags
    assert len(tags) == 2
