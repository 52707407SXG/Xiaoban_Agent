...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/xiaoban'
...
const CONFIGURED_CACHE_KEY = 'xiaoban-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'xiaoban-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Xiaoban is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through the Nous Tool Gateway,
// mirroring xiaoban_cli/nous_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
async function fetchProviderDefaultModel(
  ...
  // `xiaoban model` does (for Nous it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Xiaoban still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Xiaoban still cannot resolve a usable provider.'
}
...
async function openSignInUrl(url: string) {
  if (window.xiaobanDesktop?.openExternal) {
    try {
      await window.xiaobanDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Xiaoban still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Xiaoban still cannot reach ${url}.` }
    }
