...
| | |
|---|---|
| Source | Optional — install with `xiaoban skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Xiaoban-Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Xiaoban terminal calls (desktop app flow only)
...
### Service Account (recommended for Xiaoban)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${XIAOBAN_HOME:-~/.xiaoban}/.env` (the skill will prompt for this on first load).
...
## Xiaoban Execution Pattern (desktop app flow)

Xiaoban terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/xiaoban-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/xiaoban-op.sock"
