"""Tests for xiaoban_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $XIAOBAN_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Xiaoban-managed node dir.
"""
...
def _make_xiaoban_node(xiaoban_home: Path) -> Path:
    """Create a fake $XIAOBAN_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = xiaoban_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_xiaoban_node(fake_home):
    xiaoban_home = fake_home / ".xiaoban"
    node_bin = _make_xiaoban_node(xiaoban_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(xiaoban_home)
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    xiaoban_home = fake_home / ".xiaoban"
    _make_xiaoban_node(xiaoban_home)
    local_bin = fake_home / ".local" / "bin"
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(xiaoban_home) == []


def test_removes_dangling_symlink_into_xiaoban_node(fake_home):
    """A link into the Xiaoban node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Xiaoban links that exist; ignores the ones that don't."""
    xiaoban_home = fake_home / ".xiaoban"
    node_bin = _make_xiaoban_node(xiaoban_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Xiaoban-managed; node is a real user binary.
