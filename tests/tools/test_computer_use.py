...
def _reset_backend():
    ...
    with patch.dict(os.environ, {"XIAOBAN_COMPUTER_USE_BACKEND": "noop"}, clear=False):
        yield
    reset_backend_for_tests()
...
class TestCuaEnvironmentScrubbing:
    ...
    def test_cua_session_sanitizes_provider_env_vars(self):
        ...
        The cua-driver MCP subprocess should not inherit Xiaoban-managed
...
class TestClickButtonPassthrough:
    """Surface 5 (52707407SXG/Xiaoban-Agent#47072) — `middle_click` must
...
class TestImageMimeTypePropagation:
    """Surface 7 (52707407SXG/Xiaoban-Agent#47072): trycua/cua#1961 made
...
class TestMcpInvocationResolution:
    """Surface 8 (52707407SXG/Xiaoban-Agent#47072): instead of hardcoding
    ...
    the MCP subcommand doesn't require a Xiaoban patch.
    ...
    def test_future_renamed_subcommand_is_honored(self):
        ...
        instead of `mcp` keeps working without a Xiaoban patch."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend import _resolve_mcp_invocation
    ...
    def test_falls_back_when_manifest_missing_command(self):
        ...
        resolved driver path (so XIAOBAN_CUA_DRIVER_CMD still wins)."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend import _resolve_mcp_invocation
...
class TestStructuredElementsConsumption:
    """Surface 2 (52707407SXG/Xiaoban-Agent#47072): trycua/cua#1961 made
...
class TestCapabilityDiscovery:
    """Surface 4 (52707407SXG/Xiaoban-Agent#47072): the wrapper learns
...
class TestElementTokenAttachment:
    """Surface 6 (52707407SXG/Xiaoban-Agent#47072): trycua/cua#1961 added
...
class TestSessionLifecycle:
    """Surface gap (audit June 2026): Xiaoban never declared a cua-driver
    ...
    def test_session_id_format(self):
        from tools.computer_use.cua_backend import CuaDriverBackend
        backend = CuaDriverBackend()
        # xiaoban-{12 hex chars} — short enough to surface in logs
        # without being a privacy hazard, unique enough for concurrent runs.
        assert backend._session_id.startswith("xiaoban-")
        assert len(backend._session_id) == len("xiaoban-") + 12

    def test_session_id_unique_per_backend(self):
        from tools.computer_use.cua_backend import CuaDriverBackend
        a = CuaDriverBackend()._session_id
        b = CuaDriverBackend()._session_id
        assert a != b, "each Xiaoban run should mint its own session id"
