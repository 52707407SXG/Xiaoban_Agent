...
import { getCronJobs, getSessionMessages, listAllProfileSessions, type SessionInfo, triggerCronJob } from '../xiaoban'
...
import { useXiaobanConfig } from './session/hooks/use-xiaoban-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.xiaobanDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.xiaobanDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.xiaobanDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.xiaobanDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // xiaoban:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.xiaobanDesktop?.onDeepLink?.(payload => {
    ...
    void window.xiaobanDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.xiaobanDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshXiaobanConfig, sttEnabled, voiceMaxRecordingSeconds } = useXiaobanConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshXiaobanConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshXiaobanConfig])
