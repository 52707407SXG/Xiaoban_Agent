...
Before the s6 migration, ``docker run 52707407SXG/xiaoban-agent gateway
...
crash and a supervised dashboard alongside (when ``XIAOBAN_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``XIAOBAN_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``XIAOBAN_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``xiaoban`` per the conftest module
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``xiaoban gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "XIAOBAN_GATEWAY_NO_SUPERVISE=1",
    ...
    if inspect.stdout.strip() == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "XIAOBAN_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The XIAOBAN_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``xiaoban gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``xiaoban gateway
    ...
    # Count python processes running `xiaoban gateway run`. If the
    ...
    r = _sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*xiaoban.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `xiaoban gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-xiaoban`
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``XIAOBAN_DASHBOARD=1`` is set, ``docker run <image> gateway
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${XIAOBAN_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Xiaoban Gateway Starting…"
    ...
    assert "⚕" in combined or "Xiaoban Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Xiaoban Gateway Starting" in file_contents, (
