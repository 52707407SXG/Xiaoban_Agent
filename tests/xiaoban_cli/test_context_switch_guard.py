"""Tests for xiaoban_cli.context_switch_guard."""
...
def test_no_warning_when_below_new_threshold(monkeypatch):
    monkeypatch.setattr(
        "xiaoban_cli.context_switch_guard.resolve_display_context_length",
        lambda *a, **k: 32_000,
    )
...
def test_warns_when_estimate_exceeds_new_threshold(monkeypatch):
    ...
    monkeypatch.setattr(
        "xiaoban_cli.context_switch_guard._estimate_tokens",
        lambda *a, **k: 90_000,
    )
    cc = _compressor(monkeypatch)
    agent = SimpleNamespace(
