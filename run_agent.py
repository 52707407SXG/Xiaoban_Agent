...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``xiaoban -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("XIAOBAN_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("XIAOBAN_SESSION_SOURCE", "")
...
from xiaoban_cli.timeouts import (
...
_loaded_env_paths = load_xiaoban_dotenv(xiaoban_home=_xiaoban_home, project_env=_project_env)
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"XiaobanAgent/{_XIAOBAN_VERSION}",
    }
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[xiaoban-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least Xiaoban' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from xiaoban_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``xiaoban chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``XIAOBAN_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``XIAOBAN_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("XIAOBAN_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``XIAOBAN_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("XIAOBAN_API_CALL_STALE_TIMEOUT")
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        xiaoban-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See xiaoban-agent#21444 for symptom history."
        )
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from xiaoban_cli.models import _should_use_copilot_responses_api
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Xiaoban against xai-oauth.  Lots of
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("XIAOBAN_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        try:
            from xiaoban_cli import plugins as _plugins
    ...
    def _redact_message_content(content):
        ...
        Respects ``XIAOBAN_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.xiaoban/sessions/session_{sid}.json``
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects XIAOBAN_REDACT_SECRETS via
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.xiaoban/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``XIAOBAN_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("XIAOBAN_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        process is a dispatcher-spawned worker (XIAOBAN_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (XIAOBAN_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset XIAOBAN_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("XIAOBAN_DEV_CREDITS"))
        ...
        if _dev:
            # XIAOBAN_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `xiaoban logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def _hydrate_todo_store(self, history: List[Dict[str, Any]]) -> None:
        ...
        if last_todo_response:
            ...
            from tools.todo_tool import MultipleInProgressError

            try:
                self._todo_store.write(last_todo_response, merge=False)
            except MultipleInProgressError:
                logger.debug("Ignored invalid historical todo snapshot")
            else:
                if not self.quiet_mode:
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``xiaoban auth add xai-oauth``) whose tokens belong to
        ...
        try:
            if self.provider == "openai-codex":
                ...
                from xiaoban_cli.auth import resolve_xai_oauth_runtime_credentials
        ...
        *,
        ...
        try:
            ...
            creds = resolve_nous_runtime_credentials(
                timeout_seconds=env_float("XIAOBAN_NOUS_TIMEOUT_SECONDS", 15),
                force_refresh=force,
            )
        except Exception as exc:
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from xiaoban_cli.copilot_auth import resolve_copilot_token
    ...
    def _apply_client_headers_for_base_url(self, base_url: str) -> None:
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            from xiaoban_cli.models import copilot_default_headers
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from xiaoban_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from xiaoban_cli.models import lmstudio_model_reasoning_options
    ...
    def _append_guardrail_observation(
        ...
        *,
        ...
        if decision.action in {"warn", "halt"} and isinstance(function_result, str):
            function_result = append_toolguard_guidance(function_result, decision)
        if decision.should_halt:
            self._set_tool_guardrail_halt(decision)
        if return_decision:
            return function_result, decision
