...
xiaoban_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `xiaoban config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_xiaoban_home = get_xiaoban_home()`` at import time (module
    singleton), so monkeypatching XIAOBAN_HOME after import doesn't move it.
    ...
    this only matters for tests that swap XIAOBAN_HOME.)
    """
    import cli

    cli._xiaoban_home = home
