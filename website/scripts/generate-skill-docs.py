...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/52707407SXG/Xiaoban_Agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    xiaoban_meta = (fm.get("metadata") or {}).get("xiaoban") or {}
    tags = xiaoban_meta.get("tags") or []
    related = xiaoban_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `xiaoban skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Xiaoban-Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Xiaoban ships with a large built-in skill library copied into `~/.xiaoban/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Xiaoban also syncs bundled skills on `xiaoban update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.xiaoban/skills/` tree, it is still shipped with Xiaoban; restore it with `xiaoban skills reset <name> --restore`.",
        "",
