"""
Configuration management for Xiaoban-Agent.

Config files are stored in ~/.xiaoban/ for easy access:
- ~/.xiaoban/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.xiaoban/.env         - API keys and secrets

This module provides:
- xiaoban config          - Show current configuration
- xiaoban config edit     - Open config in editor
- xiaoban config set      - Set a specific value
- xiaoban config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``xiaoban config set`` (which rewrites
    ...
    ``config.yaml`` in place: xiaoban never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.xiaoban/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``xiaoban logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  xiaoban config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Xiaoban makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Xiaoban runtime starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``xiaoban update``, the TUI build.
...
#   on every plugin install / ``xiaoban update``.
...
# * ``XIAOBAN_HOME`` / ``XIAOBAN_PROFILE`` / ``XIAOBAN_CONFIG`` /
#   ``XIAOBAN_ENV`` — Xiaoban runtime location flags. Writing these into
...
#
# IMPORTANT: ``XIAOBAN_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (XIAOBAN_LANGFUSE_PUBLIC_KEY,
# XIAOBAN_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Xiaoban runtime location — never via dashboard env writer.
    # NOT a XIAOBAN_* blanket: integration credentials (XIAOBAN_GEMINI_*,
    # XIAOBAN_LANGFUSE_*, XIAOBAN_SPOTIFY_*, ...) ARE allowed.
    "XIAOBAN_HOME", "XIAOBAN_PROFILE", "XIAOBAN_CONFIG", "XIAOBAN_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Xiaoban runtime location "
            "(XIAOBAN_HOME, XIAOBAN_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.xiaoban/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "XIAOBAN_TOOL_PROGRESS", "XIAOBAN_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `xiaoban plugins enable
    # observability/langfuse` or `xiaoban tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "XIAOBAN_LANGFUSE_ENV",
    "XIAOBAN_LANGFUSE_RELEASE",
    "XIAOBAN_LANGFUSE_SAMPLE_RATE",
    "XIAOBAN_LANGFUSE_MAX_CHARS",
    "XIAOBAN_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from xiaoban_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("XIAOBAN_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_xiaoban_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Xiaoban is running in package-manager-managed mode.

    Two signals: the XIAOBAN_MANAGED env var (set by the systemd service),
    or a .managed marker file in XIAOBAN_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade xiaoban-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``xiaoban_cli/`` — i.e. the git checkout for source
    installs, ``/opt/xiaoban`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$XIAOBAN_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Xiaoban was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$XIAOBAN_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. XIAOBAN_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.xiaoban``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$XIAOBAN_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.xiaoban:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$XIAOBAN_HOME``, a
    ...
    ``docker`` and ``xiaoban update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``52707407SXG/xiaoban-agent`` image bakes a ``docker``
        stamp into ``/opt/xiaoban`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $XIAOBAN_HOME.
    try:
    ...
    #    host install whose shared $XIAOBAN_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``xiaoban update``.
    try:
        method = (
            (get_xiaoban_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``xiaoban_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$XIAOBAN_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/xiaoban`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Xiaoban lives in a ``uv tool`` layout.

    ``uv tool install xiaoban-agent`` places the install at
    ``.../uv/tools/xiaoban-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``xiaoban-agent`` happens to be uv-tool-installed on the machine while
    the *active* Xiaoban is a regular pip/venv install, causing
    ``xiaoban update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/xiaoban-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade xiaoban-agent"
    if method == "docker":
        return "docker pull 52707407SXG/xiaoban-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade xiaoban-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade xiaoban-agent"
        return "pip install --upgrade xiaoban-agent"
    return "xiaoban update"
...
# Long-form text for ``xiaoban update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# xiaoban_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Xiaoban, it doesn't update
...
✗ ``xiaoban update`` doesn't apply inside the Docker container.

Xiaoban-Agent runs as a published image (52707407SXG/xiaoban-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull 52707407SXG/xiaoban-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate xiaoban-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm 52707407SXG/xiaoban-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/52707407SXG/xiaoban-agent/tags
  • Your config and session history live under ``$XIAOBAN_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``xiaoban update`` inside Docker.
...
def format_managed_message(action: str = "modify this Xiaoban installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("XIAOBAN_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Xiaoban installation is managed by NixOS "
            f"(XIAOBAN_MANAGED={env_hint}).\n"
            "Edit services.xiaoban-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Xiaoban installation is managed by Homebrew "
            f"(XIAOBAN_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade xiaoban-agent"
        )

    return (
        f"Cannot {action}: this Xiaoban installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Xiaoban."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from XIAOBAN_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, xiaoban_bin
    or None if container mode is not active, we're already inside the
    container, or XIAOBAN_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("XIAOBAN_DEV") == "1":
        return None
    ...
    container_mode_file = get_xiaoban_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "xiaoban-agent")
    exec_user = info.get("exec_user", "xiaoban")
    xiaoban_bin = info.get("xiaoban_bin", "/data/current-package/bin/xiaoban")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "xiaoban_bin": xiaoban_bin,
    }
...
# Re-export from xiaoban_constants — canonical definition lives there.
from xiaoban_constants import get_xiaoban_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_xiaoban_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_xiaoban_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_xiaoban_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the XIAOBAN_UID / XIAOBAN_GID env vars set by Docker deployments.

    Docker containers running Xiaoban commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level XIAOBAN_HOME once, but
    subdirectories created at runtime by ``ensure_xiaoban_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("XIAOBAN_UID", "").strip()
    gid_str = os.environ.get("XIAOBAN_GID", "").strip()
    try:
...
def _chown_to_xiaoban_uid(path) -> None:
    """Chown ``path`` to ``XIAOBAN_UID:XIAOBAN_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_xiaoban_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_xiaoban_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the xiaoban group can
    share state with the gateway service.

    The mode can be overridden via the XIAOBAN_HOME_MODE environment variable
    (e.g. XIAOBAN_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse XIAOBAN_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``XIAOBAN_UID``/``XIAOBAN_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("XIAOBAN_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_xiaoban_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Xiaoban runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("XIAOBAN_CONTAINER") or os.environ.get("XIAOBAN_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set XIAOBAN_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into XIAOBAN_HOME if the user doesn't have one yet."""
...
def ensure_xiaoban_home():
    """Ensure ~/.xiaoban directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_xiaoban_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_xiaoban_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"XIAOBAN_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Xiaoban-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Xiaoban
        ...
        # identity slot (SOUL.md). Empty by default. The XIAOBAN_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Xiaoban adds a coding operating brief
    ...
    "terminal": {
        ...
        #             XIAOBAN_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force XIAOBAN_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Xiaoban auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Xiaoban sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Xiaoban
        ...
        # bundled Xiaoban image, which drops to the `xiaoban` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Xiaoban sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Xiaoban should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``xiaoban chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.xiaoban/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: xiaoban sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .xiaoban.md, or .cursorrules before Xiaoban applies
    ...
    # single read_file page exceeds these limits, Xiaoban truncates the
    ...
    # See tools/kanban_tools.py and xiaoban_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``xiaoban dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``xiaoban kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``xiaoban kanban decompose`` and the kanban
        ...
        # ``xiaoban profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `xiaoban model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `xiaoban` (and `xiaoban chat`) launches by default:
        ...
        # REPL and `--tui` (or XIAOBAN_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `xiaoban --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `xiaoban -c` muscle memory.  Default off so existing
        # users aren't surprised.  XIAOBAN_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `xiaoban --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/xiaoban`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``XIAOBAN_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``XIAOBAN_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``XIAOBAN_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``XIAOBAN_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/xiaoban``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Xiaoban appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.xiaoban/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.xiaoban/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Xiaoban feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Xiaoban auto-pauses the goal and
    ...
    # always goes to ~/.xiaoban/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${XIAOBAN_SKILL_DIR} and ${XIAOBAN_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `xiaoban curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `xiaoban curator run --consolidate` overrides
        ...
        # normally restored on every `xiaoban update`, so pruning them only
        ...
        # skipped), snapshot ~/.xiaoban/skills/ into
        # ~/.xiaoban/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `xiaoban curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for xiaoban-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "M *Xiaoban-Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (XIAOBAN_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Xiaoban' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.xiaoban/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or XIAOBAN_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `xiaoban doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``xiaoban_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Xiaoban to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $XIAOBAN_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via XIAOBAN_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `xiaoban -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `xiaoban` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `xiaoban kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     xiaoban-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Xiaoban tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.xiaoban/logs/.
    ...
    # update model picker lists without shipping a xiaoban-agent release.
    # The default URL is the raw JSON snapshot in the canonical Xiaoban_Agent repo.
    "model_catalog": {
        "enabled": True,
        "url": "https://raw.githubusercontent.com/52707407SXG/Xiaoban_Agent/main/website/static/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `xiaoban model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.xiaoban/.env,
        ...
        # behavior: files must live under the Xiaoban cache, under
        ...
        # user. Bridged to XIAOBAN_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Xiaoban
        # cache (~/.xiaoban/cache/{documents,images,audio,video,screenshots})
        ...
        # to XIAOBAN_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # XIAOBAN_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to XIAOBAN_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.xiaoban/state.db.
    ...
    "sessions": {
        ...
        # default of ``xiaoban sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.xiaoban/sessions/session_{sid}.json`` on every turn
    ...
    # ``xiaoban update`` behaviour.
    "updates": {
        # Run a full ``xiaoban backup``-style zip of XIAOBAN_HOME before every
        # ``xiaoban update``.  Backups land in ``<XIAOBAN_HOME>/backups/`` and
        # can be restored with ``xiaoban import <path>``.  Defaults to true
        # after the #48200 incident: a ``xiaoban update --yes`` run that
        ...
        "backup_keep": 5,
        # What `xiaoban update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<XIAOBAN_HOME>/lsp/bin/`` on first use.
    ...
    # `xiaoban tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.xiaoban/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.xiaoban/.env (or your shell) and never in config.yaml.
            ...
            # ~/.xiaoban/bin/ on first use.  When False you must install
            ...
            # `xiaoban secrets bitwarden setup`.
            "server_url": "",
        },
    },
    ...
    "computer_use": {
        ...
        # by default upstream. Xiaoban disables it for our users unless they
        # explicitly opt in here. When false (default), Xiaoban sets
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'xiaoban model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Xiaoban auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Xiaoban, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @xiaoban:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. XIAOBAN_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #xiaoban)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: xiaoban-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'xiaoban-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Xiaoban API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Xiaoban API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Xiaoban API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # XIAOBAN_TOOL_PROGRESS and XIAOBAN_TOOL_PROGRESS_MODE are deprecated —
    ...
    "XIAOBAN_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.xiaoban.config`` entries, then checks
    ...
    try:
        ...
        # should never break `xiaoban update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Xiaoban versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``xiaoban_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'xiaoban setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Xiaoban won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'xiaoban doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("XIAOBAN_HOME", "~/.xiaoban")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("XIAOBAN_TOOL_PROGRESS")
            old_mode = get_env_value("XIAOBAN_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from XIAOBAN_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from XIAOBAN_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("XIAOBAN_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from XIAOBAN_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$XIAOBAN_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_xiaoban_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `xiaoban plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `xiaoban curator status` has no
    ...
    #   3. Creates `~/.xiaoban/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_xiaoban_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_xiaoban_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from xiaoban_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: xiaoban config set <key> <value>")
    ...
    # metadata.xiaoban.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``xiaoban config set``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _xiaoban_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.xiaoban/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.xiaoban/config.yaml.
    ...
    (which change ``XIAOBAN_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/xiaoban/config.yaml invalidates the
...
#   openai-codex (OAuth — xiaoban auth) — OpenAI Codex
#   nous         (OAuth — xiaoban auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.xiaoban/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.xiaoban/.env.
    ...
    interactive menu render (`xiaoban tools`, `xiaoban setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `xiaoban tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Xiaoban env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.xiaoban/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.xiaoban/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.xiaoban/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.xiaoban/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Xiaoban — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Xiaoban vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.xiaoban/.env or environment."""
...
def show_config():
    ...
    print(color("│              M Xiaoban Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from xiaoban_cli.auth import get_anthropic_key
    ...
    # Warn on stale XIAOBAN_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("XIAOBAN_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale XIAOBAN_MAX_ITERATIONS={_env_ghost} "
                f"(run 'xiaoban doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  xiaoban config edit     # Edit config file", Colors.DIM))
    print(color("  xiaoban config set <key> <value>", Colors.DIM))
    print(color("  xiaoban setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `xiaoban config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `xiaoban config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: xiaoban config set <key> <value>")
            print()
            print("Examples:")
            print("  xiaoban config set model anthropic/claude-sonnet-4")
            print("  xiaoban config set terminal.backend docker")
            print("  xiaoban config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'xiaoban config migrate' to add them")
        ...
        print("  xiaoban config           Show current configuration")
        print("  xiaoban config edit      Open config in editor")
        print("  xiaoban config set <key> <value>   Set a config value")
        print("  xiaoban config check     Check for missing/outdated config")
        print("  xiaoban config migrate   Update config with new options")
        print("  xiaoban config path      Show config file path")
        print("  xiaoban config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``xiaoban config`` UI so users
