...
Xiaoban-Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.xiaoban/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Xiaoban stores session history so it can resume conversations, but it does not
...
Xiaoban explicitly injects for that turn.
...
For example, if a user sends an image and asks Xiaoban to make a meme from it,
Xiaoban may inspect that image once with vision and run an image-processing
...
`xiaoban sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`xiaoban` or `xiaoban chat`) |
...
xiaoban -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
xiaoban -c "my project"   # → resumes "my project #3"
```
...
xiaoban --resume 20250305_091523_a1b2c3d4
xiaoban -r 20250305_091523_a1b2c3d4

# Resume by title
xiaoban --resume "refactoring auth"

# Or with the chat subcommand
xiaoban chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `xiaoban sessions list`.

### Conversation Recap on Resume

When you resume a session, Xiaoban displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Xiaoban session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.xiaoban/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `xiaoban -r "<title>"` from the shell) and pick up where the platform left off.
...
Xiaoban automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `xiaoban sessions list` or `xiaoban sessions browse`.
...
```bash
xiaoban sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Xiaoban creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`xiaoban -c "my project"`), it automatically picks the most recent session in the lineage.
...
Xiaoban provides a full set of session management commands via `xiaoban sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
xiaoban sessions list

# Filter by platform
xiaoban sessions list --source telegram

# Show more sessions
xiaoban sessions list --limit 50
```
...
xiaoban sessions export backup.jsonl

# Export sessions from a specific platform
xiaoban sessions export telegram-history.jsonl --source telegram

# Export a single session
xiaoban sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
xiaoban sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
xiaoban sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
xiaoban sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
xiaoban sessions prune

# Custom age threshold
xiaoban sessions prune --older-than 30

# Only prune sessions from a specific platform
xiaoban sessions prune --source telegram --older-than 60

# Skip confirmation
xiaoban sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`xiaoban insights`](/reference/cli-commands#xiaoban-insights).
...
When Xiaoban cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Xiaoban uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Xiaoban in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.xiaoban/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.xiaoban/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.xiaoban/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.xiaoban/sessions/`. They are no longer written or
read by Xiaoban. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Xiaoban process in the same `XIAOBAN_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.xiaoban/config.yaml`:
...
xiaoban sessions delete <session_id>
