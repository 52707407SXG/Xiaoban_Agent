...
Auto-tracks and cleans up ephemeral files created during Xiaoban-Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$XIAOBAN_HOME` and `/tmp/xiaoban-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `XIAOBAN_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `XIAOBAN_HOME` or `/tmp/xiaoban-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$XIAOBAN_HOME/disk-cleanup/` is itself excluded
- `$XIAOBAN_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
