"""xiaoban hooks — inspect and manage shell-script hooks.

Usage::

    xiaoban hooks list
    xiaoban hooks test <event> [--for-tool X] [--payload-file F]
    xiaoban hooks revoke <command>
    xiaoban hooks doctor

Consent records live under ``~/.xiaoban/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.xiaoban/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``xiaoban hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: xiaoban hooks {list|test|revoke|doctor}")
        print("Run 'xiaoban hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.xiaoban/config.yaml.")
        print("See `xiaoban hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `xiaoban hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `xiaoban hooks test` and `xiaoban hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Xiaoban wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `xiaoban hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `xiaoban hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `xiaoban hooks doctor`.")
