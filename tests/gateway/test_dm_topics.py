...
def test_persist_dm_topic_thread_id_writes_config(tmp_path):
    ...
    config_file = tmp_path / ".xiaoban" / "config.yaml"
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"XIAOBAN_HOME": str(tmp_path / ".xiaoban")}):
...
def test_group_topic_chat_id_int_string_coercion():
    ...
    adapter = _make_adapter(group_topics_config=[
        {
            ...
            "topics": [
                {"name": "Dev", "thread_id": "7", "skill": "xiaoban-agent-dev"},
            ],
        }
    ])
    ...
    assert event.auto_skill == "xiaoban-agent-dev"
