"""
Xiaoban runtime CLI compatibility package.

Provides subcommands for:
- xiaoban chat          - Interactive chat
- xiaoban gateway       - Run gateway in foreground
- xiaoban gateway start - Start gateway service
...
def _ensure_utf8():
    ...
    The CLI prints box-drawing characters (┌│├└─) and the M glyph in the setup
    ...
    raises an unhandled UnicodeEncodeError that crashes the command before it
      can even start — e.g. `xiaoban setup` on a fresh Pi.
    ...
    xiaoban_cli/stdio.py::configure_windows_stdio() runs later from the entry
