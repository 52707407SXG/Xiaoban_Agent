...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/xiaoban-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/xiaoban-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/xiaoban-results"
HEREDOC_MARKER = "XIAOBAN_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/xiaoban-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"XIAOBAN_PERSIST_{uuid.uuid4().hex[:8]}"
