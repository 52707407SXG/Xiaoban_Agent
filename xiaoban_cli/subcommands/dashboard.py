"""``xiaoban dashboard`` subcommand parser.

Extracted verbatim from ``xiaoban_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Xiaoban web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `xiaoban dashboard` cmdlines and SIGTERM them directly — the same
    # path `xiaoban update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running xiaoban dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running xiaoban dashboard processes and exit",
    )
    # Backward-compat shim: older Xiaoban desktop app shells (<= 0.15.x) spawn the
    # backend as `xiaoban dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Xiaoban couldn't start" with no actionable cause. Accept and silently
    ...
    # `xiaoban dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.xiaoban/.env.
    # Nested subparser so bare `xiaoban dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "XIAOBAN_DASHBOARD_OAUTH_CLIENT_ID into ~/.xiaoban/.env, and prints "
            "how to engage the login gate. Requires being logged in (xiaoban setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://xiaoban.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via XIAOBAN_DASHBOARD_PORTAL_URL. Mainly for "
