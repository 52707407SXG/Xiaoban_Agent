...
// (e.g. https://mission-control.tilos.com/xiaoban/). The Python backend
// injects ``window.__XIAOBAN_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__XIAOBAN_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const XIAOBAN_BASE_PATH = readBasePath();
const BASE = XIAOBAN_BASE_PATH;
...
declare global {
  interface Window {
    __XIAOBAN_SESSION_TOKEN__?: string;
    __XIAOBAN_BASE_PATH__?: string;
    ...
    __XIAOBAN_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Xiaoban-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__XIAOBAN_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``xiaoban update``, ``xiaoban gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__XIAOBAN_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__XIAOBAN_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("xiaoban.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("xiaoban.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__XIAOBAN_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("xiaoban.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__XIAOBAN_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Xiaoban dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``xiaoban_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__XIAOBAN_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Xiaoban-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``xiaoban_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__XIAOBAN_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateXiaoban: () =>
    fetchJSON<ActionResponse>("/api/xiaoban/update", { method: "POST" }),
  checkXiaobanUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/xiaoban/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** xiaoban-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``xiaoban update``. */
  can_update_xiaoban?: boolean;
  config_path: string;
