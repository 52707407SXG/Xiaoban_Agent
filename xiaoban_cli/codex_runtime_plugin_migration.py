"""Migrate Xiaoban' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Xiaoban' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Xiaoban mcp_servers.<n>.command/args/env  → codex stdio transport
  Xiaoban mcp_servers.<n>.url/headers       → codex streamable_http transport
  Xiaoban mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Xiaoban mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Xiaoban-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Xiaoban' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Xiaoban config.")
...
# Xiaoban keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Xiaoban' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, xiaoban_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Xiaoban MCP server config to the codex inline-table dict
    ...
    if not isinstance(xiaoban_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(xiaoban_cfg.get("command"))
    has_url = bool(xiaoban_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (XIAOBAN_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Xiaoban)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Xiaoban' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Xiaoban' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Xiaoban
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="xiaoban-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a XIAOBAN_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed xiaoban-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.xiaoban`` symlink that happens to live under ``/private/var/folders``
...
def _build_xiaoban_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Xiaoban' own
    ...
    so a xiaoban installed under /opt/, /usr/local/, or a venv all work.
    XIAOBAN_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # XIAOBAN_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_xiaoban_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever XIAOBAN_HOME its launcher sets
    ...
    # — that would override the launcher's XIAOBAN_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("XIAOBAN_HOME", tmp_path) would
    ...
    xiaoban_home = os.environ.get("XIAOBAN_HOME") or ""
    if xiaoban_home and _looks_like_test_tempdir(xiaoban_home):
        xiaoban_home = ""
    if xiaoban_home:
        env["XIAOBAN_HOME"] = xiaoban_home
    # PYTHONPATH passes through so a worktree-launched xiaoban finds the
    ...
    env["XIAOBAN_QUIET"] = "1"
    env["XIAOBAN_REDACT_SECRETS"] = env.get("XIAOBAN_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_xiaoban_tools: bool = True,
) -> MigrationReport:
    """Translate Xiaoban mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        xiaoban_config: full ~/.xiaoban/config.yaml dict
        ...
        expose_xiaoban_tools: when True (default), register Xiaoban' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Xiaoban for tools
    ...
    xiaoban_servers = (xiaoban_config or {}).get("mcp_servers") or {}
    if not isinstance(xiaoban_servers, dict):
        report.errors.append(
            "mcp_servers in Xiaoban config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in xiaoban_servers.items():
    ...
    # Inject Xiaoban' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Xiaoban for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/xiaoban_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_xiaoban_tools:
        translated["xiaoban-tools"] = _build_xiaoban_tools_mcp_entry()
        if "xiaoban-tools" not in report.migrated:
            report.migrated.append("xiaoban-tools")
