...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Xiaoban Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_xiaoban_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("XIAOBAN_HOME", str(tmp_path / ".xiaoban"))

    xiaoban_env = tmp_path / ".xiaoban" / ".env"
    xiaoban_env.parent.mkdir(parents=True)
    xiaoban_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.xiaoban/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
