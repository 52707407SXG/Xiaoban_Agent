...
export function GatewaySettings() {
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.xiaobanDesktop.applyConnectionConfig(payload())
        : await window.xiaobanDesktop.saveConnectionConfig(payload())
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.xiaobanDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.xiaobanDesktop.oauthLoginConnectionConfig(trimmedUrl)

      if (result.connected) {
        const refreshed = await window.xiaobanDesktop.getConnectionConfig(scope)
        setState(refreshed)
  ...
  const signOut = async () => {
    ...
    try {
      await window.xiaobanDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.xiaobanDesktop.getConnectionConfig(scope)
      setState(refreshed)
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.xiaobanDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
  ...
  if (!window.xiaobanDesktop?.getConnectionConfig) {
    return (
      <EmptyState
  ...
  return (
    <SettingsContent>
        ...
        <ListRow
          action={
            <Input
              ...
              placeholder="https://gateway.example.com/xiaoban"
      ...
      <div className="mt-6 grid gap-1">
        <ListRow
          action={
            <Button onClick={() => void window.xiaobanDesktop?.revealLogs()} size="sm" variant="textStrong">
              <FileText />
              {g.openLogs}
            </Button>
          }
