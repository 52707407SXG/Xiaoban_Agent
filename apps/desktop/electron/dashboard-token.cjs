 ...
 *
 * The desktop main process can pass XIAOBAN_DASHBOARD_SESSION_TOKEN when it
...
async function fetchPublicText(url, options = {}) {
  ...
  if (protocol !== 'http:' && protocol !== 'https:') {
    throw new Error(`Unsupported Xiaoban backend URL protocol: ${protocol}`)
  }

  const timeoutMs = options.timeoutMs ?? DEFAULT_TOKEN_FETCH_TIMEOUT_MS
  const res = await fetch(url, { signal: AbortSignal.timeout(timeoutMs) }).catch(error => {
    if (error.name === 'TimeoutError') {
      throw new Error(`Timed out connecting to Xiaoban backend after ${timeoutMs}ms`)
    }
    throw error
  })
...
function extractInjectedDashboardToken(html) {
  const match = /window\.__XIAOBAN_SESSION_TOKEN__\s*=\s*("(?:\\.|[^"\\])*")/.exec(String(html || ''))
 ...
 */
async function adoptServedDashboardToken(baseUrl, spawnToken, { childAlive, label = 'Xiaoban backend', ...options }) {
