...
code paths Xiaoban takes on native Windows.
...
class TestConfigureWindowsStdio:
    """``xiaoban_cli.stdio.configure_windows_stdio`` wiring.
    ...
    - respect XIAOBAN_DISABLE_WINDOWS_UTF8 opt-out
    """
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("xiaoban_cli.stdio", None)
        # Fresh import now; tests import from xiaoban_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import xiaoban_cli.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("xiaoban_cli.stdio", None)

    def test_no_op_on_posix(self):
        from xiaoban_cli import stdio
    ...
    def test_windows_path_sets_env_and_reconfigures_streams(self, monkeypatch):
        ...
        monkeypatch.delenv("XIAOBAN_DISABLE_WINDOWS_UTF8", raising=False)
    ...
    def test_disable_flag_short_circuits(self, monkeypatch, optout):
        ...
        monkeypatch.setattr(stdio, "is_windows", lambda: True)
        monkeypatch.setenv("XIAOBAN_DISABLE_WINDOWS_UTF8", optout)
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("xiaoban_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "xiaoban_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, xiaoban_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "xiaoban_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call xiaoban_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """xiaoban_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from xiaoban_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from xiaoban_cli._subprocess_compat import resolve_node_command
    ...
    def test_windows_detach_flags_includes_breakaway_from_job(self, monkeypatch):
        ...
        Without it, the gateway-respawn watcher spawned by ``xiaoban update``
        (which runs under xiaoban-setup.exe, itself a grandchild of the
...
# ---------------------------------------------------------------------------
# xiaoban_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "xiaoban_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the XIAOBAN_RPC_SOCKET format.
    """
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / xiaoban_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "xiaoban_cli/tools_config.py",
            "xiaoban_cli/doctor.py",
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_source_has_windows_branch_using_xiaoban_home(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "tools" / "environments" / "local.py").read_text(encoding="utf-8")
        assert "if _IS_WINDOWS:" in source
        assert "get_xiaoban_home" in source
        assert 'cache_dir = get_xiaoban_home() / "cache" / "terminal"' in source
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_xiaoban_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "xiaoban_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "xiaoban_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "xiaoban_cli" / "gateway.py").read_text(encoding="utf-8")
