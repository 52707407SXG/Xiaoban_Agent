...
All skills live in **`~/.xiaoban/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Xiaoban at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `xiaoban update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.xiaoban` profile):

```bash
curl -fsSL https://raw.githubusercontent.com/52707407SXG/Xiaoban_Agent/main/scripts/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
xiaoban profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
xiaoban skills opt-out            # stop future seeding — nothing on disk is touched
xiaoban skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
xiaoban skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `xiaoban update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `xiaoban skills opt-in`) to re-enable.

:::note Safe by default
`xiaoban skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Xiaoban installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Xiaoban to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.xiaoban/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
xiaoban chat --toolsets skills -q "What skills do you have?"
xiaoban chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.xiaoban/cache/chart-q4-2025.png
...
When a missing value is encountered, Xiaoban asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `xiaoban setup` or `~/.xiaoban/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `xiaoban config migrate` prompts for unconfigured settings, and `xiaoban config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.xiaoban/skills/                  # Single source of truth
...
If you maintain skills outside of Xiaoban — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Xiaoban to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.xiaoban/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.xiaoban/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Xiaoban process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Xiaoban ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.xiaoban/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.xiaoban/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `xiaoban bundles list`.
...
xiaoban bundles create research

# Overwrite an existing bundle
xiaoban bundles create backend-dev --skill ... --force

# Delete a bundle
xiaoban bundles delete backend-dev

# Re-scan ~/.xiaoban/skill-bundles/ and report changes
xiaoban bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.xiaoban/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.xiaoban/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.xiaoban/pending/skills/` and are
...
```bash
xiaoban skills browse                              # Browse all hub skills (official first)
xiaoban skills browse --source official            # Browse only official optional skills
xiaoban skills search kubernetes                   # Search all sources
xiaoban skills search react --source skills-sh     # Search the skills.sh directory
xiaoban skills search https://mintlify.com/docs --source well-known
xiaoban skills inspect openai/skills/k8s           # Preview before installing
xiaoban skills install openai/skills/k8s           # Install with security scan
xiaoban skills install official/security/1password
xiaoban skills install skills-sh/vercel-labs/json-render/json-render-react --force
xiaoban skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
xiaoban skills list --source hub                   # List hub-installed skills
xiaoban skills check                               # Check installed hub skills for upstream updates
xiaoban skills update                              # Reinstall hub skills with upstream changes when needed
xiaoban skills audit                               # Re-scan all hub skills for security
xiaoban skills uninstall k8s                       # Remove a hub skill
xiaoban skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
xiaoban skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
xiaoban skills publish skills/my-skill --to github --repo owner/repo
xiaoban skills snapshot export setup.json          # Export skill config
xiaoban skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Xiaoban. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `xiaoban skills search <query> --source skills-sh`. Xiaoban resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Xiaoban currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Xiaoban repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Xiaoban can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
xiaoban skills search https://mintlify.com/docs --source well-known
xiaoban skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
xiaoban skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Xiaoban can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
xiaoban skills install openai/skills/k8s
xiaoban skills tap add myorg/skills-repo
```
...
[Skills Hub](https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Xiaoban-side changes required.
...
- Xiaoban source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Xiaoban supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Xiaoban source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Xiaoban can search and convert agent entries from LobeHub's public catalog into installable Xiaoban skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Xiaoban source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Xiaoban integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Xiaoban' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Xiaoban source id: `browse-sh`
- Trust level: `community`

```bash
xiaoban skills search airbnb --source browse-sh
xiaoban skills inspect browse-sh/airbnb.com/search-listings-ddgioa
xiaoban skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Xiaoban fetches the URL, parses the YAML frontmatter, security-scans it, and installs.

- Xiaoban source id: `url`
...
```bash
xiaoban skills install https://sharethis.chat/SKILL.md
xiaoban skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
xiaoban skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `xiaoban skills update` re-fetches from the same URL automatically when you want to refresh.
...
`xiaoban skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Xiaoban | Always trusted |
...
```bash
xiaoban skills check          # Report which installed hub skills changed upstream
xiaoban skills update         # Reinstall only the skills with updates available
xiaoban skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Xiaoban users add with `xiaoban skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.xiaoban.tags`, `version`, `author`, `platforms`, `metadata.xiaoban.config`).
...
Xiaoban discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/xiaoban-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any Xiaoban user can subscribe and install:

```bash
xiaoban skills tap add my-org/xiaoban-skills
xiaoban skills search deploy
xiaoban skills install my-org/xiaoban-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.xiaoban/.hub/taps.json`:
...
The `xiaoban skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `xiaoban skills tap list` shows the effective path per tap.
...
```bash
xiaoban skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.xiaoban/.hub/taps.json` (created on demand).

## Bundled skill updates (`xiaoban skills reset`)

Xiaoban ships with a set of bundled skills in `skills/` inside the repo. On install and on every `xiaoban update`, a sync pass copies those into `~/.xiaoban/skills/` and records a manifest at `~/.xiaoban/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Xiaoban recomputes the hash of your local copy and compares it to the origin hash:
...
xiaoban skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `XIAOBAN_HOME`, so `xiaoban -p coder skills reset <name>` only affects that profile.
:::
