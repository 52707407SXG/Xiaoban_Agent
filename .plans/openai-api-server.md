# OpenAI-Compatible API Server for Xiaoban-Agent
...
SSE streaming. By exposing this endpoint, xiaoban-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  xiaoban-agent   │
│  NextChat        │  │                                    │  gateway        │
...
| GET | `/v1/models` | List available "models" (returns xiaoban-agent as a model) |
...
But xiaoban-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "xiaoban-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured xiaoban-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of xiaoban-agent's core prompt
...
Once implemented, xiaoban-agent works as a drop-in backend for:
