...
Optional Xiaoban observability plugin that maps Xiaoban observer hooks to
...
not replace Xiaoban-Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Xiaoban emit NeMo Relay lifecycle events for
the work Xiaoban already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Xiaoban-Agent can:

- Preserve Xiaoban execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
xiaoban plugins enable observability/nemo_relay
```

The `XIAOBAN_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `XIAOBAN_HOME` that the
agent run will use:

```bash
env XIAOBAN_HOME=/tmp/xiaoban-nemo-relay-test \
  xiaoban plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`XIAOBAN_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`XIAOBAN_HOME` is the Xiaoban profile/config home used by both
`xiaoban plugins enable ...` and the later `xiaoban chat ...` run. If unset,
Xiaoban uses the user's default home, usually `~/.xiaoban`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export XIAOBAN_HOME=/tmp/xiaoban-nemo-relay-test
xiaoban plugins enable observability/nemo_relay
xiaoban chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `xiaoban` command you run is built from the
...
uv run xiaoban plugins enable observability/nemo_relay
uv run xiaoban chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/xiaoban_agent-*.whl
...
The plugin can configure exporters directly from `XIAOBAN_NEMO_RELAY_*`
...
```bash
export XIAOBAN_NEMO_RELAY_ATOF_ENABLED=1
export XIAOBAN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export XIAOBAN_NEMO_RELAY_ATIF_ENABLED=1
export XIAOBAN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `XIAOBAN_NEMO_RELAY_ATOF_FILENAME`
- `XIAOBAN_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `XIAOBAN_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `XIAOBAN_NEMO_RELAY_ATIF_AGENT_NAME`
- `XIAOBAN_NEMO_RELAY_ATIF_AGENT_VERSION`
- `XIAOBAN_NEMO_RELAY_ATIF_MODEL_NAME`
- `XIAOBAN_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Xiaoban at it:

```bash
export XIAOBAN_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `XIAOBAN_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`XIAOBAN_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`XIAOBAN_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Xiaoban does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Xiaoban keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Xiaoban routes LLM and tool
...
For the full generic Xiaoban middleware contract, see
...
export XIAOBAN_HOME=/tmp/xiaoban-nemo-relay-docs/xiaoban-home
mkdir -p "$XIAOBAN_HOME"

cat > "$XIAOBAN_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Xiaoban session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export XIAOBAN_NEMO_RELAY_ATOF_ENABLED=1
export XIAOBAN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/xiaoban-nemo-relay-docs/subagent/atof
export XIAOBAN_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export XIAOBAN_NEMO_RELAY_ATOF_MODE=overwrite
export XIAOBAN_NEMO_RELAY_ATIF_ENABLED=1
export XIAOBAN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/xiaoban-nemo-relay-docs/subagent/atif
export XIAOBAN_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export XIAOBAN_NEMO_RELAY_ATIF_AGENT_NAME='Xiaoban-Agent E2E'
export XIAOBAN_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export XIAOBAN_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"xiaoban.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Xiaoban-Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Xiaoban dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/xiaoban-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/xiaoban-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/xiaoban-nemo-relay-docs/workdir/beta.txt
cd /tmp/xiaoban-nemo-relay-docs/workdir

export XIAOBAN_NEMO_RELAY_ATOF_ENABLED=1
export XIAOBAN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/xiaoban-nemo-relay-docs/parallel/atof
export XIAOBAN_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export XIAOBAN_NEMO_RELAY_ATOF_MODE=overwrite
export XIAOBAN_NEMO_RELAY_ATIF_ENABLED=1
export XIAOBAN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/xiaoban-nemo-relay-docs/parallel/atif
export XIAOBAN_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export XIAOBAN_NEMO_RELAY_ATIF_AGENT_NAME='Xiaoban-Agent E2E'
export XIAOBAN_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Xiaoban sessions map to `agent` scopes.
- Xiaoban API request hooks map to `llm` scope start/end events.
- Xiaoban tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Xiaoban execution middleware to hand
...
Enable it for Xiaoban:

```bash
export XIAOBAN_NEMO_RELAY_PLUGINS_TOML=/tmp/xiaoban-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Xiaoban routes execution
through these boundaries:

```text
Xiaoban provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Xiaoban provider adapter next_call(...)

Xiaoban tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Xiaoban tool dispatcher next_call(...)
```
...
middleware for a local Xiaoban run. This path requires a NeMo Relay runtime that
...
```bash
export XIAOBAN_HOME=/tmp/xiaoban-middleware-test/xiaoban-home
mkdir -p "$XIAOBAN_HOME" /tmp/xiaoban-middleware-test/nemo-relay
...
cat > /tmp/xiaoban-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/xiaoban-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/xiaoban-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Xiaoban Middleware E2E"
agent_version = "local"
...
export XIAOBAN_NEMO_RELAY_PLUGINS_TOML=/tmp/xiaoban-middleware-test/nemo-relay/plugins.toml
