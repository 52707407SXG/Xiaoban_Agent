...
binary download are exercised manually by `xiaoban secrets bitwarden
...
def xiaoban_home(tmp_path, monkeypatch):
    """Point Xiaoban at an isolated home directory."""
    home = tmp_path / ".xiaoban"
    home.mkdir()
    monkeypatch.setenv("XIAOBAN_HOME", str(home))
    # Some modules cache get_xiaoban_home; clear if needed.
    import xiaoban_constants
    if hasattr(xiaoban_constants, "_XIAOBAN_HOME_CACHE"):
        xiaoban_constants._XIAOBAN_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(xiaoban_home, monkeypatch):
...
def test_install_bws_happy_path(xiaoban_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(xiaoban_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(xiaoban_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from xiaoban_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under xiaoban_home/cache/."""
    home = tmp_path / ".xiaoban"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
