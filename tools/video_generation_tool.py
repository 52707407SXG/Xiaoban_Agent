...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/xiaoban-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `xiaoban tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    ...
    msg = (
        "No video generation backend is configured. Run `xiaoban tools` → "
...
# `xiaoban tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`xiaoban tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `xiaoban tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
