...
If you only run one Xiaoban agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Xiaoban agents that should all
...
(`ai.xiaoban.gateway-<name>.plist`) or systemd user service
(`xiaoban-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
xiaoban profile create coder
xiaoban profile create personal-bot
xiaoban profile create research
...
```bash
xiaoban config set gateway.multiplex_profiles true
xiaoban gateway restart
```

Equivalently, in the default profile's `~/.xiaoban/config.yaml`:
...
You do **not** run `xiaoban gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `xiaoban gateway start` / `run` is a
...
home). `xiaoban status` reports the multiplexer and the profiles it serves;
`xiaoban status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/xiaoban-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: xiaoban-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    xiaoban gateway "$action"
  else
    xiaoban -p "$profile" gateway "$action"
  fi
}
...
```bash
xiaoban-gateways start      # start every configured profile
xiaoban-gateways stop       # stop every configured profile
xiaoban-gateways restart    # restart all
xiaoban-gateways status     # status across all
xiaoban-gateways list       # delegates to `xiaoban gateway list`
```

:::tip
The `default` profile is targeted with `xiaoban gateway <action>` (no `-p`),
not `xiaoban -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `xiaoban -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.xiaoban.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/xiaoban-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.xiaoban.gateway.plist` /
`xiaoban-gateway.service`.
...
tail -f ~/.xiaoban/logs/gateway.error.log

# Named profile
tail -f ~/.xiaoban/profiles/<name>/logs/gateway.log
tail -f ~/.xiaoban/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.xiaoban/logs/gateway.log ~/.xiaoban/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
xiaoban logs -f                  # follow default profile
xiaoban -p coder logs -f         # follow one profile
xiaoban logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
xiaoban profile list             # profiles + model + gateway state
xiaoban-gateways status          # full status across every profile
launchctl list | grep xiaoban    # macOS — PIDs and labels
systemctl --user list-units 'xiaoban-gateway-*'   # Linux — units
```
...
```
~/.xiaoban/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.xiaoban/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
xiaoban config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.xiaoban/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=xiaoban --why="gateways running" \
  sleep infinity &
...
`xiaoban-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.xiaoban/.env ~/.xiaoban/profiles/*/.env
```

## Updating the code

`xiaoban update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `xiaoban gateway start` after a previous `xiaoban gateway stop`. The
...
```bash
ps -ef | grep "xiaoban_cli.*-p <profile>"
cat ~/.xiaoban/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.xiaoban.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.xiaoban.gateway-<profile>.plist

# Linux
systemctl --user restart xiaoban-gateway-<profile>.service
```

### Health check

```bash
xiaoban doctor                  # default profile
xiaoban -p <profile> doctor     # one profile
```
