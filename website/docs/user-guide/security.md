...
Xiaoban-Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Xiaoban checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.xiaoban/config.yaml`:
...
| `timeout` | `60` | Seconds Xiaoban waits for an approval reply before timing out. |
...
1. **CLI flag**: Start a session with `xiaoban --yolo` or `xiaoban chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `XIAOBAN_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `XIAOBAN_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, Xiaoban shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Xiaoban refuses to run them **regardless** of:
...
Configure the timeout in `~/.xiaoban/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.xiaoban/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.xiaoban/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` xiaoban/gateway | Self-termination prevention |
...
The `XIAOBAN_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.xiaoban/config.yaml`:
...
:::tip
Use `xiaoban config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Xiaoban controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.xiaoban/.env`:
...
For more flexible authorization, Xiaoban includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `xiaoban pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.xiaoban/config.yaml`:
...
xiaoban pairing approve telegram ABC12DEF

# Revoke a user's access
xiaoban pairing revoke telegram 123456789

# Clear all pending codes
xiaoban pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.xiaoban/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Xiaoban applies strict security hardening to every container.
...
Container resources are configurable in `~/.xiaoban/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.xiaoban/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `XIAOBAN_HOME`. Skills declare these in frontmatter:
...
When loaded, Xiaoban checks if these files exist in the active profile's `XIAOBAN_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Xiaoban infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Xiaoban infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Xiaoban integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Xiaoban under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.xiaoban/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.xiaoban/logs/` for unauthorized access attempts
10. **Keep updated** — run `xiaoban update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.xiaoban/.env`:
...
TERMINAL_SSH_USER=xiaoban
TERMINAL_SSH_KEY=~/.ssh/xiaoban_agent_key
```
...
Xiaoban ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `xiaoban_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `xiaoban doctor` for the full remediation.
- **`xiaoban doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
```bash
xiaoban doctor --ack <advisory-id>
```
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Xiaoban installs these **lazily** on first use rather than eagerly under `xiaoban-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `xiaoban tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `xiaoban tools`.
