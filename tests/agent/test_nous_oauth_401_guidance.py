...
to a generic "Your API key was rejected... run xiaoban setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give a concrete re-auth command, not a generic "xiaoban setup".
    assert "xiaoban portal" in source
