...
Copy-paste blueprints for common automation patterns. Each blueprint uses Xiaoban's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`xiaoban webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `xiaoban webhook subscribe`) |
...
```bash
xiaoban cron create "0 2 * * *" \
  "You are a project manager triaging the 52707407SXG/Xiaoban-Agent GitHub repo.

1. Run: gh issue list --repo 52707407SXG/Xiaoban-Agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
xiaoban webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
xiaoban cron create "0 9 * * 1" \
  "Scan the 52707407SXG/Xiaoban-Agent repo for documentation drift.

1. Run: gh pr list --repo 52707407SXG/Xiaoban-Agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (xiaoban_cli/commands.py, xiaoban_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (xiaoban_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
xiaoban cron create "0 6 * * *" \
  "Run a dependency security audit on the xiaoban-agent project.

1. cd ~/.xiaoban/xiaoban-agent && source .venv/bin/activate
...
```bash
xiaoban webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
xiaoban webhook subscribe alert-triage \
...
```python title="~/.xiaoban/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Xiaoban-Monitor/1.0"})
...
xiaoban cron create "every 30m" \
  ...
  --script ~/.xiaoban/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
xiaoban cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Xiaoban-Agent development." \
...
```bash
xiaoban webhook subscribe github-issues \
  --events "issues" \
...
```bash
xiaoban webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
xiaoban webhook subscribe stripe-payments \
...
```bash
xiaoban cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the xiaoban-agent codebase.
