...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or xiaoban -c.",
    ...
    "Ctrl+Z suspends Xiaoban to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "xiaoban -c resumes your most recent CLI session. xiaoban -c \"project name\" resumes by title.",
    "xiaoban -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "xiaoban -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "xiaoban chat -t web,terminal enables only specific toolsets for a focused session.",
    "xiaoban chat -s github-pr-workflow preloads a skill at launch.",
    "xiaoban chat -q \"query\" runs a single non-interactive query and exits.",
    "xiaoban chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "xiaoban chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "xiaoban --yolo bypasses all dangerous command approval prompts for the entire session.",
    "xiaoban chat --source telegram tags the session for filtering in xiaoban sessions list.",
    "xiaoban -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "xiaoban doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "xiaoban dump outputs a compact setup summary — great for bug reports.",
    "xiaoban config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "xiaoban config edit opens config.yaml in your default editor.",
    "xiaoban config check scans for missing or stale configuration options.",
    "xiaoban sessions browse opens an interactive session picker with search.",
    "xiaoban sessions stats shows session counts by platform and database size.",
    "xiaoban sessions prune --older-than 30 cleans up old sessions.",
    "xiaoban skills search react --source skills-sh searches the skills.sh public directory.",
    "xiaoban skills check scans installed hub skills for upstream updates.",
    "xiaoban skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "xiaoban skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "xiaoban mcp add github --command npx adds MCP servers from the command line.",
    "xiaoban mcp serve runs Xiaoban runtime as an MCP server for other agents.",
    "xiaoban auth add lets you add multiple API keys for credential pool rotation.",
    "xiaoban completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "xiaoban logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "xiaoban backup creates a zip backup of your entire Xiaoban home directory.",
    "xiaoban profile create coder creates an isolated profile that becomes its own command.",
    "xiaoban profile create work --clone copies your current config and keys to a new profile.",
    "xiaoban update syncs new bundled skills to ALL profiles automatically.",
    "xiaoban gateway install sets up Xiaoban as a system service (systemd/launchd).",
    "xiaoban memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "xiaoban webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: xiaoban tools disables unused tools, xiaoban skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "xiaoban models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as xiaoban -w).",
    ...
    "XIAOBAN_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Xiaoban tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'xiaoban profile create coder' creates the 'coder' command.",
    "xiaoban profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Xiaoban prints a resume command with session ID and stats.",
    "xiaoban sessions export backup.jsonl exports all sessions for backup or analysis.",
    "xiaoban -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "xiaoban skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .xiaoban/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: xiaoban cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Xiaoban runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "xiaoban gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Xiaoban via a local macOS server.",
    ...
    "Context files (.xiaoban.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Xiaoban loads project context from .xiaoban.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "xiaoban mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "xiaoban mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "xiaoban mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — xiaoban tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.xiaoban/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "xiaoban chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "xiaoban plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "xiaoban acp runs Xiaoban as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "XIAOBAN_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "xiaoban auth add nous or xiaoban auth add openai-codex sets up OAuth-based providers.",
    ...
    "xiaoban status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.xiaoban/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Xiaoban your own.",
    ...
    "Before a gateway session reset, Xiaoban auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.xiaoban/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Xiaoban state via XIAOBAN_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "XIAOBAN_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in XIAOBAN_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.xiaoban/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'xiaoban profile create ls' would be rejected.",
    "xiaoban profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "xiaoban claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "XIAOBAN_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via xiaoban tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.xiaoban/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.xiaoban/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Xiaoban auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Xiaoban config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Xiaoban is working.',
    ...
    '/reload reloads ~/.xiaoban/.env into the running session — pick up new API keys without restarting.',
    ...
    "XIAOBAN_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.xiaoban/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.xiaoban/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'xiaoban curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "xiaoban curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'xiaoban curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'xiaoban auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'XIAOBAN_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "XIAOBAN_TUI_THEME=light|dark|<hex> forces the TUI theme; legacy XIAOBAN_TUI_THEME still works.",
    ...
    'xiaoban dashboard embeds the full Xiaoban TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.xiaoban/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.xiaoban/dashboard-plugins/ — no npm build required.',
    ...
    'XIAOBAN_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'XIAOBAN_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'XIAOBAN_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'XIAOBAN_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.xiaoban/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.xiaoban/skills/ so drop-in skills appear without restarting the session.',
    ...
    'xiaoban -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'xiaoban chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'xiaoban chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'xiaoban chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "xiaoban chat --source tool tags programmatic chats so they don't clutter xiaoban sessions list.",
    'xiaoban dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'xiaoban sessions rename <ID> "new title" renames any past session; xiaoban sessions delete <ID> removes one.',
    'xiaoban import restores a session export or profile archive produced by sessions export or profile export.',
    'xiaoban fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'xiaoban pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'xiaoban setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'xiaoban status --deep runs the full health sweep across every component; plain xiaoban status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'XIAOBAN_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'XIAOBAN_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.xiaoban/plugins/ — trust-gated by design.',
    "XIAOBAN_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'XIAOBAN_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'XIAOBAN_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'XIAOBAN_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'XIAOBAN_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'XIAOBAN_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'XIAOBAN_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'XIAOBAN_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'XIAOBAN_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'XIAOBAN_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'XIAOBAN_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Xiaoban block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from xiaoban sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.xiaoban/dashboard-plugins/.',
]
...
def get_random_tip(exclude_recent: int = 0) -> str:
    ...
    """
    visible_tips = [tip for tip in TIPS if "xiaoban" not in tip.lower()]
