...
Connect Xiaoban to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Xiaoban — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Xiaoban runs it in a
...
xiaoban photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Xiaoban-Agent` project on your account.
...
Runtime credentials are written to `~/.xiaoban/.env`
...
(device token, dashboard project id) lives in `~/.xiaoban/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Xiaoban
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Xiaoban replies with a pairing code. Approve it with:

```bash
xiaoban pairing approve photon <CODE>
```

Use `xiaoban pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.xiaoban/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.xiaoban/.env`):
...
By default Xiaoban responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Xiaoban` and
`@Xiaoban agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Xiaoban will reply.

## Status & troubleshooting

```bash
xiaoban photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Xiaoban uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run xiaoban photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `xiaoban photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `xiaoban photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `xiaoban photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Xiaoban sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Xiaoban wake words  | JSON list / comma / newline regex patterns for group mentions |
