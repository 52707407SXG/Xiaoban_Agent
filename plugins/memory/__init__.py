...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with xiaoban-agent)
2. User-installed providers: ``$XIAOBAN_HOME/plugins/<name>/``
...
_USER_NAMESPACE = "_xiaoban_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_xiaoban_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_xiaoban_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$XIAOBAN_HOME/plugins/`` or None if unavailable."""
    try:
        from xiaoban_constants import get_xiaoban_home
        d = get_xiaoban_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$XIAOBAN_HOME/plugins/<name>/``) directories.  Bundled takes
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _xiaoban_user_memory.<name>.cli, usually
