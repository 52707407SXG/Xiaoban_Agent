...
`mcp_tool.py`, and `xiaoban_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# XiaobanMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_xiaoban_provider_class() -> Optional[type]:
    ...
    class XiaobanMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._xiaoban_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`XiaobanTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, XiaobanTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._xiaoban_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import XiaobanTokenStorage
                        if isinstance(storage, XiaobanTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._xiaoban_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import XiaobanTokenStorage
            if not isinstance(storage, XiaobanTokenStorage):
                return
    ...
    return XiaobanMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_XIAOBAN_PROVIDER_CLS: Optional[type] = _make_xiaoban_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`XiaobanMCPOAuthProvider` directly using the helpers
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `xiaoban mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``xiaoban mcp remove <name>`` and (indirectly) by
        ``xiaoban mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
