...
description: "Operate the Teams meeting summary pipeline via Xiaoban CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions"
---
...
Operate the Teams meeting summary pipeline via Xiaoban CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions.
...
Everything operator-facing is a `xiaoban teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${XIAOBAN_HOME:-~/.xiaoban}/.env`:
...
```bash
xiaoban teams-pipeline validate              # config snapshot — run first after any change
xiaoban teams-pipeline token-health          # Graph token status
xiaoban teams-pipeline token-health --force-refresh   # force a fresh token acquisition
xiaoban teams-pipeline list                  # recent meeting jobs
xiaoban teams-pipeline list --status failed  # only failed jobs
xiaoban teams-pipeline show <job-id>         # full detail of one job
xiaoban teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
xiaoban teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
xiaoban teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
xiaoban teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
xiaoban teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

xiaoban teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
xiaoban teams-pipeline delete-subscription <sub-id>
xiaoban teams-pipeline maintain-subscriptions            # renew near-expiry ones
xiaoban teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `xiaoban teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `xiaoban cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
