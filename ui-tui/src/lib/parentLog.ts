...
const logDir = join(process.env.XIAOBAN_HOME?.trim() || join(homedir(), '.xiaoban'), 'logs')
...
// a real ~/.xiaoban (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('xiaoban-tui: parent lifecycle log unavailable\n')
    }
  }
}
