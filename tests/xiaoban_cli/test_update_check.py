"""Tests for the update check mechanism in xiaoban_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "xiaoban-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("xiaoban_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    fake_banner = tmp_path / "xiaoban_cli" / "banner.py"
    ...
    monkeypatch.setenv("XIAOBAN_HOME", str(tmp_path))
    monkeypatch.delenv("XIAOBAN_REVISION", raising=False)
    with patch("xiaoban_cli.banner.subprocess.run") as mock_run, \
         patch("xiaoban_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XIAOBAN_HOME", str(tmp_path))
    with patch("xiaoban_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_official_ssh_origin_uses_https_probe(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="git@github.com:52707407SXG/Xiaoban-Agent.git\n")
    ...
    with patch("xiaoban_cli.banner.subprocess.run", side_effect=fake_run):
...
def test_check_via_local_git_shallow_clone_behind_reports_no_count(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append(cmd)
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="https://github.com/52707407SXG/Xiaoban_Agent.git\n")
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XIAOBAN_HOME", str(tmp_path))
    with patch("xiaoban_cli.banner.subprocess.run") as mock_run:
        with patch("xiaoban_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when XIAOBAN_HOME has no git repo."""
    import xiaoban_cli.banner as banner
    ...
    # Point XIAOBAN_HOME at a temp dir with no xiaoban-agent/.git
    monkeypatch.setenv("XIAOBAN_HOME", str(tmp_path))
    with patch("xiaoban_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    XIAOBAN_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    container, and `xiaoban update` correctly refusing to run there. The guard
    ...
    with patch("xiaoban_cli.config.detect_install_method", return_value="docker"), \
         patch("xiaoban_cli.banner.subprocess.run") as mock_run, \
         patch("xiaoban_cli.banner.check_via_pypi") as mock_pypi:
        result = banner.check_for_updates()
...
def test_check_for_updates_non_docker_still_checks(tmp_path, monkeypatch):
    ...
    with patch("xiaoban_cli.config.detect_install_method", return_value="pip"), \
         patch("xiaoban_cli.banner.subprocess.run") as mock_run, \
         patch("xiaoban_cli.banner.check_via_pypi", return_value=1) as mock_pypi:
        result = banner.check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from xiaoban_cli.main import _invalidate_update_cache

    # Build a fake ~/.xiaoban with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"XIAOBAN_HOME": str(default_home)}):
