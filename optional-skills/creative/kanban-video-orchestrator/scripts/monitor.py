#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `xiaoban kanban list` and
...
def xiaoban_available() -> bool:
    return shutil.which("xiaoban") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed xiaoban CLI."""
    try:
        out = subprocess.run(
            ["xiaoban", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `xiaoban kanban list`
    out = subprocess.run(
        ["xiaoban", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["xiaoban", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not xiaoban_available():
        print("ERROR: 'xiaoban' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
