"""``xiaoban slack ...`` CLI subcommands.

Today only ``xiaoban slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ xiaoban slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of Xiaoban. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Xiaoban deployment — users can tweak them in the Slack UI after
    pasting.
    """
    from xiaoban_cli.commands import slack_app_manifest
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Xiaoban agent on Slack")[:140],
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": "Chat with Xiaoban in threads and DMs.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``xiaoban_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$XIAOBAN_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Xiaoban")
      --description DESC  Override the bot description
      --slashes-only  Emit only the ``features.slash_commands`` array (for
                      merging into an existing manifest manually)
    """
    name = getattr(args, "name", None) or "Xiaoban"
    description = getattr(args, "description", None) or "Your Xiaoban agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("XIAOBAN_HOME") or str(Path.home() / ".xiaoban")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Xiaoban app\n"
