"""Tests for xiaoban_constants module."""
...
from xiaoban_constants import (
    ...
    find_xiaoban_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_xiaoban_root,
    get_xiaoban_home,
    iter_xiaoban_node_dirs,
    is_container,
    parse_reasoning_effort,
    secure_parent_dir,
    with_xiaoban_node_path,
)


class TestGetDefaultXiaobanRoot:
    """Tests for get_default_xiaoban_root() — Docker/custom deployment awareness."""

    def test_no_xiaoban_home_returns_native(self, tmp_path, monkeypatch):
        """When XIAOBAN_HOME is not set, returns ~/.xiaoban."""
        monkeypatch.delenv("XIAOBAN_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_xiaoban_root() == tmp_path / ".xiaoban"

    def test_xiaoban_home_is_native(self, tmp_path, monkeypatch):
        """When XIAOBAN_HOME = ~/.xiaoban, returns ~/.xiaoban."""
        native = tmp_path / ".xiaoban"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XIAOBAN_HOME", str(native))
        assert get_default_xiaoban_root() == native

    def test_xiaoban_home_is_profile(self, tmp_path, monkeypatch):
        """When XIAOBAN_HOME is a profile under ~/.xiaoban, returns ~/.xiaoban."""
        native = tmp_path / ".xiaoban"
    ...
    def test_xiaoban_home_is_docker(self, tmp_path, monkeypatch):
        """When XIAOBAN_HOME points outside ~/.xiaoban (Docker), returns XIAOBAN_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XIAOBAN_HOME", str(docker_home))
        assert get_default_xiaoban_root() == docker_home

    def test_xiaoban_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any XIAOBAN_HOME outside ~/.xiaoban is treated as the root."""
        custom = tmp_path / "my-xiaoban-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XIAOBAN_HOME", str(custom))
        assert get_default_xiaoban_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (XIAOBAN_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XIAOBAN_HOME", str(profile))
        assert get_default_xiaoban_root() == docker_root

    def test_no_xiaoban_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\xiaoban, not ~/.xiaoban."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("XIAOBAN_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(xiaoban_constants.sys, "platform", "win32")

        assert get_default_xiaoban_root() == local_appdata / "xiaoban"

    def test_no_xiaoban_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/xiaoban without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("XIAOBAN_HOME", raising=False)
        ...
        assert get_default_xiaoban_root() == home / "AppData" / "Local" / "xiaoban"


class TestGetXiaobanHome:
    """Tests for get_xiaoban_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When XIAOBAN_HOME is unset on Windows, use %LOCALAPPDATA%\\xiaoban."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("XIAOBAN_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(xiaoban_constants.sys, "platform", "win32")
        monkeypatch.setattr(xiaoban_constants, "_profile_fallback_warned", False)

        assert get_xiaoban_home() == local_appdata / "xiaoban"


class TestXiaobanManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_xiaoban_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        assert find_xiaoban_node_executable("npm") == str(npm_cmd)
    ...
    def test_with_xiaoban_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "xiaoban"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_xiaoban_node_path({"PATH": "system-node"})
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(xiaoban_constants, "_container_detected", None)
    ...
    def test_detects_cgroup_v2_via_mountinfo(self, monkeypatch, tmp_path):
        """cgroup v2 falls back to a containerd-backed root overlay mount."""
        import builtins
        self._reset_cache(monkeypatch)
        monkeypatch.delenv("KUBERNETES_SERVICE_HOST", raising=False)
        monkeypatch.setattr(os.path, "exists", lambda p: False)
    ...
    def test_ignores_host_container_runtime_mounts(self, monkeypatch, tmp_path):
        """Nested Docker/containerd mounts do not make the ext4 host a container."""
        import builtins
        self._reset_cache(monkeypatch)
        monkeypatch.delenv("KUBERNETES_SERVICE_HOST", raising=False)
        monkeypatch.setattr(os.path, "exists", lambda p: False)
        ...
        mountinfo_file = tmp_path / "mountinfo"
        mountinfo_file.write_text(
            "31 1 252:3 / / rw,relatime - ext4 /dev/vda3 rw\n"
            "886 31 0:47 / /var/lib/docker/rootfs/overlayfs/abc rw,relatime "
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(xiaoban_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".xiaoban"
