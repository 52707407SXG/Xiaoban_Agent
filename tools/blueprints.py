...
`xiaoban skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.xiaoban.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.xiaoban.blueprint`` is a mapping containing
    ...
    xiaoban = meta.get("xiaoban") if isinstance(meta, dict) else None
    blueprint = xiaoban.get("blueprint") if isinstance(xiaoban, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.xiaoban.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.xiaoban.blueprint`` block) they can hand
    to ``xiaoban skills publish`` to share. ``body`` is the plain-language
