...
closure the PR changed, against a real temp ``XIAOBAN_HOME``.
"""
...
def _fake_switch_result():
    ...
    from xiaoban_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "xiaoban_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_xiaoban_home() / config.yaml`` — point it here.
    monkeypatch.setattr("xiaoban_constants.get_xiaoban_home", lambda: xiaoban_home)
    monkeypatch.setattr("xiaoban_cli.config.get_xiaoban_home", lambda: xiaoban_home)
    return cfg_path
