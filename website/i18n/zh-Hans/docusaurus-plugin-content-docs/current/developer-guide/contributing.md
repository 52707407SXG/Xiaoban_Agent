...
对大多数贡献者来说，最好的开发启动方式和用户安装方式相同：运行标准安装器，然后在它克隆出的仓库里开发。安装器会创建 Hermes venv、配置 `hermes` 命令、为 `xiaoban update` 写入安装方式标记，并把完整 git 项目克隆到 `$HERMES_HOME/xiaoban-agent`（通常是 `~/.hermes/xiaoban-agent`）。这样你的开发环境会和 CLI、updater、lazy dependency installer、gateway、docs 默认假设的布局一致。
...
- **Profile 安全路径**：不得硬编码 `~/.hermes` — 代码路径使用 `hermes_constants` 中的 `get_hermes_home()`，面向用户的消息使用 `display_hermes_home()`。完整规则参见 [AGENTS.md](https://github.com/52707407SXG/Xiaoban-Agent/blob/main/AGENTS.md#profiles-multi-instance-support)。
...
- 使用 [GitHub Issues](https://github.com/52707407SXG/Xiaoban-Agent/issues)
- 请包含：操作系统、Python 版本、Hermes 版本（`xiaoban version`）、完整错误堆栈
- 包含复现步骤
- 创建前请检查是否已有重复 issue
- 安全漏洞请私下报告
...
提交贡献即表示您同意您的贡献将以 [MIT 许可证](https://github.com/52707407SXG/Xiaoban-Agent/blob/main/LICENSE) 授权。