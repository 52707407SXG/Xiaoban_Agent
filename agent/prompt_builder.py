...
KANBAN_GUIDANCE = (
    ...
    "every assignee in a real profile (`xiaoban profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `xiaoban kanban <verb>` for board operations. Use "
...
TOOL_USE_ENFORCEMENT_MODELS = ("gpt", "codex", "gemini", "gemma", "grok", "glm", "qwen", "deepseek", "kimi", "k3")
...
#
# The Xiaoban-Agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to Xiaoban-Agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `xiaoban computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Xiaoban "
...
PLATFORM_HINTS = {
    ...
    "weixin": (
        "You are on Weixin/WeChat. Treat it as a compact chat-text channel, not "
        "a rich Markdown renderer. Use plain Chinese text, short paragraphs, "
        "simple numbered/bullet lists, and separators when useful. Do not rely "
        "on Markdown tables, colors, custom font sizes, emoji/icon-heavy layouts, "
        "or rich cards. Keep structured information readable as labeled lines. "
        "You can send media files natively when appropriate: include "
    ...
    "wecom": (
        "You are on WeCom (企业微信 / Enterprise WeChat). Keep replies compact "
        "and chat-friendly. Simple Markdown-style headings, separators, and "
        "short lists are acceptable, but do not rely on Markdown tables, colors, "
        "custom font sizes, emoji/icon-heavy layouts, or rich cards. Present "
        "structured information as labeled lines unless the connector explicitly "
        "supports richer rendering. You CAN send media files natively — to "
        "deliver a file to the user, include MEDIA:/absolute/path/to/file in "
        "your response. The file will be sent as a native WeCom attachment: "
        "images (.jpg, .png, .webp) are sent as photos (up to 10 MB), other "
        "files (.pdf, .docx, .xlsx, .md, .txt, etc.) arrive as downloadable "
        "documents (up to 20 MB), and videos (.mp4) play inline."
    ),
    "qqbot": (
    ...
    "api_server": (
        "You're responding through an API server. Rendering depends on the caller, "
        "so follow any caller/channel-specific system instructions first. If no "
        "rendering guidance is provided, use concise plain text. For My Stand "
        "desktop-pet/web chat, Markdown cards are supported: casual short chat can "
        "stay plain, while structured answers should use compact Markdown headings, "
        "lists, tables, and code fences when they genuinely improve scanning. Do "
        "not prefix structured headings with emoji/icons. For "
        "Weixin/WeCom-style channels, prefer compact text and simple Markdown; "
        "separators are fine, but avoid Markdown tables, color-dependent layouts, "
        "custom font-size assumptions, emoji/icon-heavy formatting, and rich-card "
        "assumptions."
    ),
    "webui": (
        "You are in the Xiaoban WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Xiaoban runtime
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Xiaoban runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Xiaoban runtime is running. The host OS, home, and cwd "
                f"of the Xiaoban runtime process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Xiaoban "
                f"runtime runs. The backend probe didn't respond at "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Xiaoban
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Xiaoban-Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, connectors, or any feature — prefer Xiaoban docs "
            "and local source clues. Use `xiaoban ...` commands in user-facing instructions.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: xiaoban setup, xiaoban setup tools, xiaoban setup terminal, xiaoban status.",
        ]
    )
    return "\n".join(lines)
