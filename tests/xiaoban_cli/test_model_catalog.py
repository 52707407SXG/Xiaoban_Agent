"""Tests for xiaoban_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate XIAOBAN_HOME + reset any module-level catalog cache per test."""
    ...
    import importlib
    from xiaoban_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from xiaoban_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/52707407SXG/Xiaoban_Agent"
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in xiaoban_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from xiaoban_cli import model_catalog
        from xiaoban_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from xiaoban_cli import model_catalog
        from xiaoban_cli.models import get_curated_nous_model_ids
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``xiaoban model`` CLI — not the live
        ...
        # ``XIAOBAN_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` XIAOBAN_HOME directly instead.
        import importlib
        from xiaoban_cli import model_catalog
        from xiaoban_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from xiaoban_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["XIAOBAN_HOME"])
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("xiaoban_cli.models.check_nous_free_tier", return_value=False), patch(
                "xiaoban_cli.models.union_with_portal_free_recommendations",
