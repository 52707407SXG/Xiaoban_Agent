"""CLI subcommand: ``xiaoban send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"xiaoban send: {file_path} is not a text file. --file reads the "
                ...
                f'  xiaoban send --to telegram "MEDIA:{file_path}"\n'
                f'  xiaoban send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  xiaoban send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"xiaoban send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"xiaoban send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"xiaoban send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"xiaoban send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"xiaoban send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `xiaoban gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.xiaoban/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_xiaoban_env() -> None:
    """Populate ``os.environ`` from ``~/.xiaoban/.env`` AND bridge top-level
    ...
    startup that ``xiaoban send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.xiaoban/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.xiaoban/config.yaml`` into
       ...
       via ``xiaoban config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``xiaoban send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.xiaoban/.env and ~/.xiaoban/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "xiaoban send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  xiaoban send --to telegram \"hello\"\n"
            "  xiaoban send --to discord:#ops --file report.md\n"
            "  xiaoban send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "xiaoban send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `xiaoban send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Xiaoban "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.xiaoban/.env + ~/.xiaoban/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  xiaoban send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `xiaoban send --list telegram`.",
    )
