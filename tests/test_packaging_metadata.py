...
def test_every_on_disk_subpackage_is_covered_by_packages_find():
    ...
    top-level package is listed twice — bare (``xiaoban_cli``) for the package
    itself and ``xiaoban_cli.*`` for its subpackages — EXCEPT when someone
    forgets the wildcard. v0.15.x listed ``xiaoban_cli`` without ``xiaoban_cli.*``,
    so the wheel shipped ``xiaoban_cli/*.py`` but dropped the ``dashboard_auth``
    and ``proxy`` subpackages. The dashboard then died on every install with
    ``ModuleNotFoundError: No module named 'xiaoban_cli.dashboard_auth'``.
...
def test_packaging_declared_as_core_dependency():
    ...
    xiaoban_cli/main.py) yet was undeclared, so it only reached users
...
def test_bundled_plugin_manifests_ship_in_both_wheel_and_sdist():
    ...
    Plugin discovery (xiaoban_cli/plugins.py) registers each bundled plugin by
