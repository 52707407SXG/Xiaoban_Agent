...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `xiaoban tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``xiaoban tools`` → Video Generation."
        ),
    ...
    # Coding posture (base Xiaoban — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    # ==========================================================================
    # Full Xiaoban toolsets (CLI + messaging platforms)
    #
    ...
    "xiaoban-cli": {
        ...
        "tools": _XIAOBAN_CORE_TOOLS,
        "includes": []
    },

    "xiaoban-cron": {
        # Mirrors xiaoban-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `xiaoban tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as xiaoban-cli; gated by `xiaoban tools`",
        "tools": _XIAOBAN_CORE_TOOLS,
        "includes": []
    },
    ...
    "xiaoban-email": {
        "description": "Email bot toolset - interact with Xiaoban via email (IMAP/SMTP)",
        "tools": _XIAOBAN_CORE_TOOLS,
        "includes": []
    },
    ...
    "xiaoban-sms": {
        "description": "SMS bot toolset - interact with Xiaoban via SMS (Twilio)",
        "tools": _XIAOBAN_CORE_TOOLS,
        "includes": []
    },
    ...
    "xiaoban-webhook": {
        ...
        "tools": _XIAOBAN_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``xiaoban-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_XIAOBAN_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``xiaoban-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (xiaoban-<name>).
        # Gives them _XIAOBAN_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("xiaoban-"):
            platform_name = name[len("xiaoban-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_XIAOBAN_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
