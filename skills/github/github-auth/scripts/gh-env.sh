#!/usr/bin/env bash
# GitHub environment detection helper for Xiaoban-Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _xiaoban_env="${XIAOBAN_HOME:-$HOME/.xiaoban}/.env"; [ -f "$_xiaoban_env" ] && grep -q "^GITHUB_TOKEN=" "$_xiaoban_env" 2>/dev/null; then
