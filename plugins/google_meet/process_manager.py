...
session-scoped state file under ``$XIAOBAN_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $XIAOBAN_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_xiaoban_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this xiaoban install, leave it first —
    ...
    env = os.environ.copy()
    env["XIAOBAN_MEET_URL"] = url
    env["XIAOBAN_MEET_OUT_DIR"] = str(out)
    env["XIAOBAN_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["XIAOBAN_MEET_HEADED"] = "1"
    if auth_state:
        env["XIAOBAN_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["XIAOBAN_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if XIAOBAN_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["XIAOBAN_MEET_MODE"] = mode
    if realtime_model:
        env["XIAOBAN_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["XIAOBAN_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["XIAOBAN_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["XIAOBAN_MEET_REALTIME_KEY"] = realtime_api_key
