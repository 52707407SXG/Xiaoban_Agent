...
Xiaoban runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Xiaoban
...
1. Xiaoban captures a baseline of current diagnostics for the file.
...
…). Xiaoban auto-detects the binary on PATH or in
`<XIAOBAN_HOME>/lsp/bin/`.
...
`node_modules` tree — Xiaoban installs both packages together when you
run `xiaoban lsp install typescript` or auto-install fires on first
use.

## CLI

```
xiaoban lsp status          # service state + per-server install status
xiaoban lsp list            # registry, optionally --installed-only
xiaoban lsp install <id>    # eagerly install one server
xiaoban lsp install-all     # try every server with a known recipe
xiaoban lsp restart         # tear down running clients
xiaoban lsp which <id>      # print resolved binary path
```

`xiaoban lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <XIAOBAN_HOME>/lsp/bin
...
When `install_strategy: auto`, Xiaoban installs binaries into
`<XIAOBAN_HOME>/lsp/bin/`. NPM packages land in
`<XIAOBAN_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Xiaoban-owned and is
...
Servers are kept alive for the life of the Xiaoban process. There's
...
**`xiaoban lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<XIAOBAN_HOME>/lsp/bin/`. Run
`xiaoban lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `xiaoban lsp status`**
...
When `xiaoban lsp status` shows a `Backend warnings` section, install
...
`~/.xiaoban/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.xiaoban/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `xiaoban lsp restart` to clear the set;
