...
// even though a fresh shell — and the Xiaoban CLI — sees it immediately. The
// desktop's XIAOBAN_HOME resolution relies on process.env, so that stale-snapshot
// gap silently sends the backend to the default %LOCALAPPDATA%\xiaoban. Reading
