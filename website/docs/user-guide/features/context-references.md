...
Type `@` followed by a reference to inject content directly into your message. Xiaoban expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Xiaoban env: `$XIAOBAN_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$XIAOBAN_HOME/skills/.hub/`
