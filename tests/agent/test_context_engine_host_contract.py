...
engine plugins (e.g. xiaoban-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``xiaoban plugins`` picker."""
    from xiaoban_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "xiaoban_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "xiaoban_cli.plugins.get_plugin_context_engine",
