...
description: "Use when a Xiaoban Kanban worker wants to run Codex CLI as an isolated implementation lane while Xiaoban keeps ownership of task lifecycle, reconciliation, tes..."
---
...
Use when a Xiaoban Kanban worker wants to run Codex CLI as an isolated implementation lane while Xiaoban keeps ownership of task lifecycle, reconciliation, testing, and handoff.
...
| Related skills | [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`xiaoban-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-xiaoban-agent) |
...
This skill defines the lightweight Xiaoban+Codex dual-lane convention for Kanban workers. Xiaoban is always the task owner: it calls `kanban_show`, decides whether Codex is appropriate, creates or selects an isolated workspace, starts and monitors Codex, reconciles any diff, runs verification, and writes the final `kanban_complete` or `kanban_block` handoff. Codex is an input lane only. Codex output is not a task completion signal, not a trusted reviewer, and not allowed to write durable Kanban state directly.

The convention exists so a Xiaoban worker can use Codex for bounded implementation help without changing the dispatcher. The dispatcher must still spawn Xiaoban workers. A worker may optionally spawn Codex inside its own run, then accept, partially accept, or reject the lane after independent review and tests.
...
- A bounded diff can be evaluated by Xiaoban in one run.
- The repo can be copied or checked out in an isolated git worktree/branch.
- Xiaoban can run the relevant tests itself after Codex exits.
...
1. Xiaoban owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any Xiaoban board CLI as a substitute for the worker.
2. Xiaoban owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. Xiaoban owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from Xiaoban with the repo's canonical wrapper.
4. Xiaoban owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. Xiaoban owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
```bash
TASK_ID="${XIAOBAN_KANBAN_TASK:-t_manual}"
REPO="/path/to/repo"
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if Xiaoban can do the task directly.
...
Xiaoban owns the Kanban lifecycle; do not call Xiaoban kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on Xiaoban reconciliation.
...
- Explicit statement: Xiaoban owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands Xiaoban will run afterward.
...
Xiaoban must perform this checklist before accepting any Codex lane result:

- [ ] `git -C <WORKTREE> status --short --branch` shows only expected files.
- [ ] `git -C <WORKTREE> diff --stat` and `git diff` were reviewed by Xiaoban.
...
- [ ] Xiaoban ran the canonical tests itself, using `scripts/run_tests.sh` for Xiaoban-Agent or the repo's documented wrapper for other repos.
- [ ] Any Codex-run tests are listed separately from Xiaoban-run tests.
- [ ] Accepted commits/diffs were applied to the Xiaoban-owned workspace/branch.
...
```json
{
  "codex_lane": {
    ...
    "tests_run": [
      {"command": "scripts/run_tests.sh tests/tools/test_x.py", "exit_code": 0, "owner": "xiaoban"},
...
```json
{
  "codex_lane": {
    ...
    "result": "rejected",
    "accepted_commits": [],
    "rejected_reason": "Direct Xiaoban edit was smaller and safer than spawning Codex.",
    "tests_run": [],
    "artifacts": []
  }
}
...
1. Treating Codex self-report as verification. Always inspect the diff and rerun tests from Xiaoban.
2. Running Codex in the user's dirty main checkout. Always isolate in a worktree/branch.
3. Letting Codex own Kanban. Codex may summarize progress, but Xiaoban writes board state.
...
- [ ] Xiaoban reviewed `git diff` and safety-sensitive files.
- [ ] Xiaoban ran canonical tests independently.
