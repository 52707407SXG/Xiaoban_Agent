...
def test_seeded_directory_list_covers_legacy_and_new_pairing_dirs(stage2_text: str) -> None:
    seed_block = stage2_text.split("as_xiaoban mkdir -p \\", 1)[1].split(
        "# --- Install-method stamp",
        1,
    )[0]
    assert '"$XIAOBAN_HOME/pairing"' in seed_block
    assert '"$XIAOBAN_HOME/platforms/pairing"' in seed_block
