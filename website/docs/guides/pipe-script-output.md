...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `xiaoban send`."
---

# Pipe Script Output to Messaging Platforms

`xiaoban send` is a small, scriptable CLI that pushes a message to any
messaging platform Xiaoban is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | xiaoban send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `xiaoban
...
xiaoban send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
xiaoban send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
xiaoban send --list

# Filter by platform
xiaoban send --list telegram
```
...
Any platform Xiaoban ships adapters for works as a target:
...
`xiaoban send` resolves the message body in this order:

1. **Positional argument** — `xiaoban send --to telegram "hi"`
2. **`--file PATH`** — `xiaoban send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | xiaoban send --to telegram`

When stdin is a TTY (no pipe), Xiaoban does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  xiaoban send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `xiaoban send` reuses your Xiaoban config, the same script works on
any host where Xiaoban is installed — no need to export bot tokens into
...
`xiaoban send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  xiaoban send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | xiaoban send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.xiaoban/bin/xiaoban send \
...
```bash
./train.py --epochs 200 && \
  xiaoban send --to telegram "training done" || \
  xiaoban send --to telegram "training failed (exit $?)"
```
...
xiaoban send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(xiaoban send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `xiaoban send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `xiaoban send` calls
the platform's REST endpoint directly using credentials from
`~/.xiaoban/.env` and `~/.xiaoban/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `xiaoban gateway start` and retry.
...
# Machine-readable
xiaoban send --list --json
```

The listing is built from `~/.xiaoban/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`xiaoban gateway
...
| Approach | Multi-platform | Reuses Xiaoban creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `xiaoban send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`xiaoban send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `xiaoban send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `xiaoban send` shares with cron delivery.
