...
import { XiaobanGateway } from '@/xiaoban'
...
import type { RpcEvent } from '@/types/xiaoban'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.xiaobanDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: XiaobanGateway | null) => void
  refreshXiaobanConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: XiaobanConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Xiaoban...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Xiaoban…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Xiaoban gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshXiaobanConfig().catch(() => undefined)
    ...
    const gateway = new XiaobanGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshXiaobanConfig()
