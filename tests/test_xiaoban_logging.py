"""Tests for xiaoban_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import xiaoban_logging
# Use whatever RotatingFileHandler class xiaoban_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. xiaoban_logging
...
from xiaoban_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    xiaoban_logging._logging_initialized = False
    ...
    xiaoban_logging._install_session_record_factory()
    yield
    ...
    xiaoban_logging._logging_initialized = False
    xiaoban_logging.clear_session_context()


@pytest.fixture
def xiaoban_home(tmp_path, monkeypatch):
    """Provide an isolated XIAOBAN_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_xiaoban_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["XIAOBAN_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, xiaoban_home):
        log_dir = xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home)
        assert log_dir == xiaoban_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, xiaoban_home):
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home)
        root = logging.getLogger()
    ...
    def test_creates_errors_log_handler(self, xiaoban_home):
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, xiaoban_home):
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home)
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, xiaoban_home):
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home, force=True)
    ...
    def test_custom_log_level(self, xiaoban_home):
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, xiaoban_home):
        xiaoban_logging.setup_logging(
            xiaoban_home=xiaoban_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, xiaoban_home):
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home)
    ...
    def test_writes_to_agent_log(self, xiaoban_home):
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home)

        test_logger = logging.getLogger("test_xiaoban_logging.write_test")
        ...
        agent_log = xiaoban_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, xiaoban_home):
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home)

        test_logger = logging.getLogger("test_xiaoban_logging.warning_test")
        ...
        agent_log = xiaoban_home / "logs" / "agent.log"
        errors_log = xiaoban_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, xiaoban_home):
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home)

        test_logger = logging.getLogger("test_xiaoban_logging.info_test")
    ...
    def test_explicit_params_override_config(self, xiaoban_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (xiaoban_home / "config.yaml").write_text(yaml.dump(config))

        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, xiaoban_home):
        """The custom record factory injects session_tag on all records."""
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_xiaoban_session_injector", False), (
            "Record factory should have _xiaoban_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, xiaoban_home):
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, xiaoban_home):
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, xiaoban_home):
        ...
        gw_log = xiaoban_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, xiaoban_home):
    ...
    def test_gateway_log_receives_gateway_records(self, xiaoban_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, xiaoban_home):
    ...
    def test_agent_log_still_receives_all(self, xiaoban_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, xiaoban_home):
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_created_after_cli_init(self, xiaoban_home):
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home, mode="cli")
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home, mode="gui")
    ...
    def test_gui_log_receives_only_gui_components(self, xiaoban_home):
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home, mode="gui")

        logging.getLogger("xiaoban_cli.web_server").info("dashboard online")
        ...
        gui_log = xiaoban_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, xiaoban_home):
        """When session context is set, log lines include [session_id]."""
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home)
        xiaoban_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, xiaoban_home):
    ...
    def test_clear_session_context(self, xiaoban_home):
        """After clearing, session tag disappears."""
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home)
        xiaoban_logging.set_session_context("xyz789")
        xiaoban_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, xiaoban_home):
        ...
        def thread_a():
            xiaoban_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            xiaoban_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        xiaoban_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        xiaoban_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = xiaoban_logging._ComponentFilter(("gateway",))
    ...
    def test_passes_nested_matching_prefix(self):
        ...
        f = xiaoban_logging._ComponentFilter(
            xiaoban_logging.COMPONENT_PREFIXES["gateway"]
        )
        record = logging.LogRecord(
    ...
    def test_multiple_prefixes(self):
        f = xiaoban_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in xiaoban_logging.COMPONENT_PREFIXES
        # The gateway component captures core gateway logs, the xiaoban_plugins
        ...
        # break this test.
        gateway_prefixes = xiaoban_logging.COMPONENT_PREFIXES["gateway"]
        assert "gateway" in gateway_prefixes
        assert "xiaoban_plugins" in gateway_prefixes
        assert "plugins.platforms" in gateway_prefixes

    def test_agent_prefix(self):
        prefixes = xiaoban_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == xiaoban_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = xiaoban_logging.COMPONENT_PREFIXES["cli"]
        assert "xiaoban_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == xiaoban_logging.COMPONENT_PREFIXES["cron"]

    def test_gui_prefix(self):
        prefixes = xiaoban_logging.COMPONENT_PREFIXES["gui"]
        assert "xiaoban_cli.web_server" in prefixes
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, xiaoban_home):
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home)
        xiaoban_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_xiaoban_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, xiaoban_home):
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home)
        xiaoban_logging.setup_verbose_logging()
        xiaoban_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        xiaoban_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = xiaoban_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        xiaoban_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("xiaoban_cli.config.is_managed", return_value=True):
                xiaoban_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, xiaoban_home):
        level, max_size, backup = xiaoban_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, xiaoban_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, xiaoban_home):
        import yaml
        config = {"model": "test"}
        (xiaoban_home / "config.yaml").write_text(yaml.dump(config))
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> xiaoban_logging._ManagedRotatingFileHandler:
        handler = xiaoban_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that xiaoban_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        handler.flush()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        xiaoban_logging.setup_logging(xiaoban_home=xiaoban_home, mode="gateway")
        gw_path = xiaoban_home / "logs" / "gateway.log"
        rotated = xiaoban_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_returns_stderr_on_utf8_system(self, monkeypatch):
        ...
        result = xiaoban_logging._safe_stderr()
