...
import { getSessionMessages, listAllProfileSessions } from '@/xiaoban'
...
import type { SessionInfo, SessionMessage } from '@/types/xiaoban'
  ...
  const openArtifact = useCallback(async (href: string) => {
    try {
      if (window.xiaobanDesktop?.openExternal) {
        await window.xiaobanDesktop.openExternal(href)
      } else {
