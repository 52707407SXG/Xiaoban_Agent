...
const checkXiaobanUpdateSpy = vi.fn()
const updateXiaobanSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/xiaoban', () => ({
  checkXiaobanUpdate: (...args: unknown[]) => checkXiaobanUpdateSpy(...args),
  updateXiaoban: (...args: unknown[]) => updateXiaobanSpy(...args),
...
describe('checkBackendUpdates', () => {
  beforeEach(() => {
    storage.clear()
    notifySpy.mockClear()
    checkXiaobanUpdateSpy.mockReset()
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkXiaobanUpdateSpy.mockResolvedValue({
      install_method: 'git',
      current_version: '0.16.0',
      behind: 2,
      update_available: true,
      can_apply: true,
      update_command: 'xiaoban update',
      message: null,
    ...
    expect(checkXiaobanUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkXiaobanUpdateSpy).not.toHaveBeenCalled()
  })
})
...
describe('applyUpdates terminal state', () => {
  ...
  beforeEach(() => {
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      xiaobanDesktop: { updates: { apply: applyMock } }
    }
    vi.useRealTimers()
  })
  ...
  it('keeps the manual command state for CLI installs with no staged updater', async () => {
    applyMock.mockResolvedValue({ ok: true, manual: true, command: 'xiaoban update' })

    await applyUpdates()

    expect($updateApply.get().stage).toBe('manual')
    expect($updateApply.get().command).toBe('xiaoban update')
    expect($updateOverlayOpen.get()).toBe(true)
    expect(notifySpy).not.toHaveBeenCalled()
  })
  ...
  it('lands on a closeable manual-restart state when the rebuilt sandbox blocks auto-relaunch', async () => {
    ...
    applyMock.mockResolvedValue({
      ...
      manualRestart: true,
      sandboxBlocked: true,
      message: 'Backend updated. Quit and reopen Xiaoban to finish.'
    })
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    updateXiaobanSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkXiaobanUpdateSpy.mockResolvedValue({ install_method: 'git', current_version: '0.16.0', behind: 0, update_available: false, can_apply: true, update_command: 'xiaoban update', message: null })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updateXiaobanSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkXiaobanUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
