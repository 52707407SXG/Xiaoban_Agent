...
Connect Xiaoban to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Xiaoban supports two WeCom integration modes:
...
> Run `xiaoban gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Xiaoban decrypts the message, queues it for the agent
...
(Use `xiaoban gateway start` only after `xiaoban gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Xiaoban and the token the
...
in `~/.xiaoban/.env` values around `=` will also break signature checks. After
fixing, restart `xiaoban gateway run`.
...
Check `xiaoban gateway run` logs for the bound host/port. If the adapter bound to
