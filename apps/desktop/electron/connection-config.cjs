 ...
 *     `X-Xiaoban-Session-Token` header; WS uses `?token=`.
...
// xiaoban_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - xiaoban_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - xiaoban_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / xiaoban #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-xiaoban_session_at', '__Secure-xiaoban_session_at', 'xiaoban_session_at']
const RT_COOKIE_VARIANTS = ['__Host-xiaoban_session_rt', '__Secure-xiaoban_session_rt', 'xiaoban_session_rt']
...
function pathWithGlobalRemoteProfile(path, profile, opts = {}) {
  ...
  let parsed
  try {
    parsed = new URL(rawPath, 'http://xiaoban.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a xiaoban session ACCESS-token cookie
