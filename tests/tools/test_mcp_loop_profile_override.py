"""Regression tests for XIAOBAN_HOME override propagation onto the MCP loop.
...
get_xiaoban_home() inside the coroutine, most visibly OAuth token-store
...
def test_override_propagates_to_mcp_loop(tmp_path, monkeypatch, mcp_loop):
    ...
    process_home.mkdir()
    profile_home.mkdir()
    monkeypatch.setenv("XIAOBAN_HOME", str(process_home))
    ...
    token = set_xiaoban_home_override(str(profile_home))
    try:
...
def test_oauth_token_paths_follow_override(tmp_path, monkeypatch, mcp_loop):
    """The actual symptom path: XiaobanTokenStorage resolving inside the
    ...
    async def token_path():
        ...
        return str(XiaobanTokenStorage("probe-srv")._tokens_path())
...
def test_concurrent_scopes_do_not_interfere(tmp_path, monkeypatch, mcp_loop):
    ...
    def scoped_call(key, home):
        token = set_xiaoban_home_override(str(home))
        try:
