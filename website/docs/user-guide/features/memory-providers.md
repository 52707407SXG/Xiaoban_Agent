...
Xiaoban-Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
xiaoban memory setup      # interactive picker + configuration
xiaoban memory status     # check what's active
xiaoban memory off        # disable external provider
```

You can also select the active memory provider via `xiaoban plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.xiaoban/config.yaml`:
...
When a memory provider is active, Xiaoban automatically:
...
**Setup Wizard:**
```bash
xiaoban memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `xiaoban honcho setup` command still works (it now redirects to `xiaoban memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$XIAOBAN_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$XIAOBAN_HOME/honcho.json` > `~/.xiaoban/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/52707407SXG/Xiaoban_Agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/xiaoban).
...
:::tip Migrating from `xiaoban honcho`
If you previously used `xiaoban honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Xiaoban profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Xiaoban profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Xiaoban profile. Host key `xiaoban` → default; `xiaoban.<profile>` for others. |
...
```bash
xiaoban profile create coder --clone
```

`--clone` creates a `xiaoban.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Xiaoban profile, creates host blocks for any profile without one, inherits settings from the default `xiaoban` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `xiaoban memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/52707407SXG/Xiaoban_Agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/xiaoban).
...
xiaoban memory setup    # select "openviking"
# Or manually:
xiaoban config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.xiaoban/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.xiaoban/.env
```
...
`OPENVIKING_AGENT` is Xiaoban' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
xiaoban memory setup    # select "mem0" → "Platform"
# Or manually:
xiaoban config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.xiaoban/.env
```

**Setup (OSS):**
```bash
xiaoban memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
xiaoban memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
xiaoban memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Config:** `$XIAOBAN_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.xiaoban/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `xiaoban-user` | User identifier |
| `agent_id` | `xiaoban` | Agent identifier |
...
**Switching modes:** Re-run `xiaoban memory setup mem0 --mode <platform|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
xiaoban memory setup    # select "hindsight"
# Or manually:
xiaoban config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.xiaoban/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p xiaoban ui start`

**Config:** `$XIAOBAN_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `xiaoban` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Xiaoban-Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/52707407SXG/Xiaoban_Agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
xiaoban memory setup    # select "holographic"
# Or manually:
xiaoban config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.xiaoban-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$XIAOBAN_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
xiaoban memory setup    # select "retaindb"
# Or manually:
xiaoban config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.xiaoban/.env
```
...
# Then configure Xiaoban
xiaoban memory setup    # select "byterover"
# Or manually:
xiaoban config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$XIAOBAN_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
xiaoban memory setup    # select "supermemory"
# Or manually:
xiaoban config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.xiaoban/.env
```

**Config:** `$XIAOBAN_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `xiaoban` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `xiaoban-{identity}` → `xiaoban-coder`) to isolate memories per Xiaoban profile
...
| **Requires** | `pip install xiaoban-memori` + `xiaoban-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install xiaoban-memori
xiaoban-memori install
xiaoban config set memory.provider memori
xiaoban memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `xiaoban-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$XIAOBAN_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$XIAOBAN_HOME/` so each profile has its own credentials
