"""
Tests for mcp_serve — Xiaoban MCP server.
...
def _isolate_xiaoban_home(tmp_path, monkeypatch):
    """Redirect XIAOBAN_HOME to a temp directory."""
    monkeypatch.setenv("XIAOBAN_HOME", str(tmp_path))
    try:
        import xiaoban_constants
        monkeypatch.setattr(xiaoban_constants, "get_xiaoban_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking xiaoban_state schema."""
