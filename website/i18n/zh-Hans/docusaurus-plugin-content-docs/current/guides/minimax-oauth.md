...
Xiaoban-Agent 通过基于浏览器的 OAuth 登录流程支持 **MiniMax**，使用与 [MiniMax 门户](https://www.minimax.io) 相同的凭据。无需 API 密钥或信用卡——登录一次，Hermes 即可自动刷新您的会话。
...
**解决方法：** 重新运行 `hermes auth add minimax-oauth`（或 `xiaoban model`）。流程将重新开始。
...
**解决方法：** 运行 `xiaoban model` 并选择 MiniMax (OAuth)，或运行 `hermes auth add minimax-oauth`。
