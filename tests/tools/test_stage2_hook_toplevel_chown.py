"""Contract test: the s6-overlay stage2 hook resets ownership of xiaoban-owned
top-level state files in $XIAOBAN_HOME — but only those, never arbitrary
host-owned files.

Regression guard for the gateway restart loop reported in #35098: files such
as gateway.lock / state.db / auth.json live directly under $XIAOBAN_HOME (not in
a subdir), so the targeted subdir chown misses them. When created or rewritten
by `docker exec <container> xiaoban …` (root unless `-u` is passed) they land
root-owned and the unprivileged xiaoban runtime then hits PermissionError on next
startup.
...
bind-mounted $XIAOBAN_HOME may contain host-owned files Xiaoban does not manage,
and those must never be chowned.

The s6-overlay rework moved bootstrap from docker/entrypoint.sh (now a shim) to
docker/stage2-hook.sh, installed as /etc/cont-init.d/01-xiaoban-setup. This test
...
def _toplevel_chown_loop(text: str) -> str:
    """Extract the `for f in … chown xiaoban:xiaoban "$XIAOBAN_HOME/$f" … done`
    ...
    block = m.group(1)
    assert 'chown xiaoban:xiaoban "$XIAOBAN_HOME/$f"' in block, (
        "the top-level-file loop must chown each allowlisted file to xiaoban"
    )
    return block
...
def test_no_blanket_find_user_root_sweep(stage2_text: str) -> None:
    ...
    $XIAOBAN_HOME contents — that would clobber host-owned files in a bind mount
    (#19788 / PR #19795)."""
    assert not re.search(r"find\s+\"?\$\{?XIAOBAN_HOME\}?\"?[^\n]*-user\s+root", stage2_text), (
        "stage2-hook.sh must not blanket-chown root-owned files under "
        "$XIAOBAN_HOME via `find -user root`; use the targeted allowlist instead "
        "so host-owned bind-mounted files are preserved (#19788, #19795)."
    )


def _run_loop(text: str, present_files: list[str]) -> list[str]:
    """Run the extracted chown loop in a sandbox $XIAOBAN_HOME, with `chown`
