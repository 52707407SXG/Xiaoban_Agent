...
title: "Git Worktrees"
description: "Run multiple Xiaoban agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Xiaoban-Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Xiaoban so each session has a clean, isolated working directory.

## Why Use Worktrees with Xiaoban?

Xiaoban treats the **current working directory** as the project root:

- CLI: the directory where you run `xiaoban` or `xiaoban chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.xiaoban/config.yaml`
...
git worktree add ../repo-feature feature/xiaoban-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/xiaoban-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Xiaoban there:

```bash
cd ../repo-feature

# Start Xiaoban in the worktree
xiaoban
```
...
git worktree add ../repo-experiment-a feature/xiaoban-a
git worktree add ../repo-experiment-b feature/xiaoban-b
```
...
Each Xiaoban process:

- Works on its own branch (`feature/xiaoban-a` vs `feature/xiaoban-b`).
...
- Xiaoban checkpoint data under `~/.xiaoban/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Xiaoban experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/xiaoban-checkpoints-docs`, `feature/xiaoban-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Xiaoban from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `xiaoban -w` (Automatic Worktree Mode)

Xiaoban has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `xiaoban/xiaoban-<hash>`).
...
```bash
xiaoban -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `xiaoban -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Xiaoban session its own clean checkout.
