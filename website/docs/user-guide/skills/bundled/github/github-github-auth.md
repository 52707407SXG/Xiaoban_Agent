...
- Give it a name like "xiaoban-agent"
- Select scopes:
...
- Give it a title like "xiaoban-agent-&lt;machine-name>"
...
elif _xiaoban_env="${XIAOBAN_HOME:-$HOME/.xiaoban}/.env"; [ -f "$_xiaoban_env" ] && grep -q "^GITHUB_TOKEN=" "$_xiaoban_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_xiaoban_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
