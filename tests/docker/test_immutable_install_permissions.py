...
def test_container_sets_hosted_write_policy_env(built_image: str) -> None:
    script = (
        'test "$XIAOBAN_HOME" = "/opt/data" && '
        'test "$XIAOBAN_WRITE_SAFE_ROOT" = "/opt/data" && '
        'test "$XIAOBAN_DISABLE_LAZY_INSTALLS" = "1" && '
...
def test_xiaoban_user_cannot_modify_install_but_can_write_data(built_image: str) -> None:
    script = textwrap.dedent(
        ...
        install_file = Path("/opt/xiaoban/agent/message_sanitization.py")
        try:
