...
import {
  ...
  getXiaobanConfigRecord,
  getRecommendedDefaultModel,
  saveXiaobanConfig,
  setEnvVar,
  setModelAssignment
} from '@/xiaoban'
import type { AuxiliaryModelsResponse, ModelOptionProvider, StaleAuxAssignment } from '@/xiaoban'
import { useI18n } from '@/i18n'
import { AlertTriangle, Cpu, Loader2 } from '@/lib/icons'
import { cn } from '@/lib/utils'
import { notifyError } from '@/store/notifications'
import { startManualLocalEndpoint, startManualProviderOAuth } from '@/store/onboarding'
import type { XiaobanConfigRecord } from '@/types/xiaoban'

import { CONTROL_TEXT } from './constants'
import { getNested, setNested } from './helpers'
import { ListRow, LoadingState, Pill, SectionHeading } from './primitives'

// Xiaoban' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Xiaoban default (medium), shown as Medium.
...
// backend now surfaces the full `xiaoban model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in xiaoban_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const [config, setConfig] = useState<XiaobanConfigRecord | null>(null)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `xiaoban model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Xiaoban runs the flow for you.`}
          </p>
        )}
