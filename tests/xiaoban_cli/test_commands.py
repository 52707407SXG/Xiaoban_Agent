...
class TestSlackNativeSlashes:
    ...
    def test_xiaoban_catchall_is_first(self):
        """``/xiaoban`` must be reserved as the first slot so the legacy
        ``/xiaoban <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "xiaoban"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /xiaoban <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
    ...
    def test_includes_aliases_as_first_class_slashes(self):
        ...
        aliases stay reachable via ``/xiaoban <alias>``.
        """
        slashes = slack_native_slashes()
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /xiaoban <command> on Slack only
        # (Slack's 50-slash cap) are expected to be absent from native slashes.
        via_xiaoban_norm = {_norm(n) for n in _SLACK_VIA_XIAOBAN_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_xiaoban_norm
        assert not missing, (
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `xiaoban slack manifest`)."""
...
class TestSlashCommandCompleter:
    ...
    def test_builtin_completion_display_meta_shows_description(self):
        ...
        assert completions[0].display_meta_text == "查看可用命令说明"
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_completes_toolset_names(self, monkeypatch):
        ...
        from xiaoban_cli import commands as commands_mod
        ...
        monkeypatch.setattr(
            "xiaoban_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
from xiaoban_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch xiaoban_cli.plugins.get_plugin_commands() to a fixed dict."""
        from xiaoban_cli import plugins as _plugins_mod
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/xiaoban metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
