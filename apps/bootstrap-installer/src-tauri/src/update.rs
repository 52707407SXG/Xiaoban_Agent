...
/// `xiaoban update` exit code meaning "another hermes process is holding the
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    let install_root = hermes_home.join("xiaoban-agent");
    ...
    // time `xiaoban update` runs there is no legitimate hermes.exe to protect,
