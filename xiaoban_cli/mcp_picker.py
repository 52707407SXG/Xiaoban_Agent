"""MCP picker — interactive `xiaoban mcp picker` (also the default `xiaoban mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``xiaoban mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `xiaoban plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from xiaoban_cli.cli_output import prompt_yes_no
from xiaoban_cli.curses_ui import curses_single_select
from xiaoban_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Xiaoban session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from xiaoban_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `xiaoban mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: xiaoban mcp install <name>    Picker: xiaoban mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Xiaoban is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Xiaoban — run `xiaoban update` "
...
def show_catalog() -> None:
    """`xiaoban mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`xiaoban mcp picker` (and default `xiaoban mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`xiaoban mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `xiaoban mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
