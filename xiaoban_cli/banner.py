...
_RUNTIME_PACKAGE = "xiaoban_cli"
_HOME_MODULE = "xiaoban_constants"
_ENV_PREFIX = "XIAOBAN"
...
def _get_runtime_home():
    ...
    return getattr(module, "get_xiaoban_home")()
...
STARTUP_LEFT_WIDTH = 42
XIAOBAN_MARK_LABEL_START = 16
XIAOBAN_MARK_LINES = [
    ("#C7A06A", "My Stand", XIAOBAN_MARK_LABEL_START),
    ("#F4E7C1", "███╗     ███╗", None),
    ("#F4E7C1", "████╗   ████║", None),
    ("#F4E7C1", "██╔██╗ ██╔██║", None),
    ("#F4E7C1", "██║╚████╔╝██║", None),
...
def _startup_pad(text: str, width: int = STARTUP_LEFT_WIDTH, start: Optional[int] = None) -> str:
    if len(text) >= width:
        return text
    left = (width - len(text)) // 2 if start is None else max(0, min(start, width - len(text)))
    right = max(0, width - len(text) - left)
    return f"{' ' * left}{text}{' ' * right}"


XIAOBAN_MARK = "\n".join(
    f"[{color}]{_startup_pad(line, start=start)}[/]"
    for color, line, start in XIAOBAN_MARK_LINES
)
...
_UPSTREAM_REPO_URL = "https://github.com/52707407SXG/Xiaoban_Agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/52707407SXG/Xiaoban_Agent".lower()
...
_RELEASE_URL_BASE = "https://github.com/52707407SXG/Xiaoban_Agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical 52707407SXG/Xiaoban_Agent repo (forks don't get a link).
    """
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    layout_table.add_column("left", justify="left", min_width=STARTUP_LEFT_WIDTH, no_wrap=True)
    ...
    left_lines = [
        "",
        f"[{text}]{_startup_pad('Welcome back!')}[/]",
        "",
        _hero,
        "",
    ]
    ...
    ctx_text = f" · {_format_context_length(context_length)} context" if context_length else ""
    ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
    model_suffix_text = " · API Usage Billing"
    model_line_text = f"{model_short}{ctx_text}{model_suffix_text}"
    model_left = max(0, (STARTUP_LEFT_WIDTH - len(model_line_text)) // 2)
    model_right = max(0, STARTUP_LEFT_WIDTH - len(model_line_text) - model_left)
    left_lines.append(
        f"{' ' * model_left}[{accent}]{model_short}[/]{ctx_str} "
        f"[dim {dim}]·[/] [dim {dim}]API Usage Billing[/]{' ' * model_right}"
    )

    if _runtime_env("YOLO_MODE"):
        left_lines.append(f"[bold red]⚠ YOLO mode[/] [dim {dim}]— all approval prompts bypassed[/]")
    left_lines.append(f"[dim {dim}]{_startup_pad(cwd)}[/]")
    left_content = "\n".join(left_lines)
