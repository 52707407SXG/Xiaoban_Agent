...
def container_env(tmp_path, monkeypatch):
    """Set up a fake XIAOBAN_HOME with .container-mode file."""
    xiaoban_home = tmp_path / ".xiaoban"
    xiaoban_home.mkdir()
    monkeypatch.setenv("XIAOBAN_HOME", str(xiaoban_home))
    monkeypatch.delenv("XIAOBAN_DEV", raising=False)

    container_mode = xiaoban_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=xiaoban-agent\n"
        "exec_user=xiaoban\n"
        "xiaoban_bin=/data/current-package/bin/xiaoban\n"
    )
    return xiaoban_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("xiaoban_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "xiaoban-agent"
    assert info["exec_user"] == "xiaoban"
    assert info["xiaoban_bin"] == "/data/current-package/bin/xiaoban"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("xiaoban_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_xiaoban_dev(container_env, monkeypatch):
    """Returns None when XIAOBAN_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("XIAOBAN_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_xiaoban_dev_zero(container_env, monkeypatch):
    """XIAOBAN_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("XIAOBAN_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        xiaoban_home = Path(tmpdir) / ".xiaoban"
        xiaoban_home.mkdir()
        (xiaoban_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("xiaoban_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_xiaoban_home": lambda: xiaoban_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("XIAOBAN_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=xiaoban-custom\n"
        "exec_user=myuser\n"
        "xiaoban_bin=/opt/xiaoban/bin/xiaoban\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "xiaoban-custom"
    assert info["exec_user"] == "myuser"
    assert info["xiaoban_bin"] == "/opt/xiaoban/bin/xiaoban"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from xiaoban_cli.main import _exec_in_container
