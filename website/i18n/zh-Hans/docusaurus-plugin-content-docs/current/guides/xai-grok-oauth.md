...
Xiaoban-Agent 通过基于浏览器的 OAuth 登录流程支持 xAI Grok，认证服务器为 [accounts.x.ai](https://accounts.x.ai)，支持 **SuperGrok 订阅**（[grok.com](https://x.ai/grok)）或 **X Premium+ 订阅**（已关联的 X 账号）。无需 `XAI_API_KEY`——登录一次后，Hermes 会在后台自动刷新会话。
...
xAI 的后端对 OAuth API 接口维护自己的白名单，已有记录显示即使应用内订阅处于激活状态，标准 SuperGrok 订阅者也会收到 `HTTP 403`（见 issue [#26847](https://github.com/52707407SXG/Xiaoban-Agent/issues/26847)）。如果浏览器中 OAuth 登录成功但推理返回 403，请设置 `XAI_API_KEY` 并切换到 API 密钥路径（`provider: xai`）——该接口目前不受相同限制。
:::
...
完整操作说明请参阅 [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md#browser-only-remote-cloud-shell--codespaces--ec2-instance-connect)。此为 [#26923](https://github.com/52707407SXG/Xiaoban-Agent/issues/26923) 的回归修复。
...
**修复方法：** 重新运行 `hermes auth add xai-oauth`（或 `xiaoban model`）。流程重新开始。
...
这**不是** token 过期问题——重新运行 `xiaoban model` 不会改变结果。xAI 的后端已被观察到将 OAuth API 访问限制在特定 SuperGrok 套餐，即使应用内订阅处于激活状态（issue [#26847](https://github.com/52707407SXG/Xiaoban-Agent/issues/26847)）。
...
**修复方法：** 运行 `xiaoban model` 并选择 xAI Grok OAuth provider，或运行 `hermes auth add xai-oauth`。
