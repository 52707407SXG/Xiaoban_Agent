...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate XIAOBAN_HOME under tmp_path."""
    monkeypatch.setenv("XIAOBAN_HOME", str(tmp_path / ".xiaoban"))
    (tmp_path / ".xiaoban").mkdir()

    # Force the constants/image cache helpers to re-read XIAOBAN_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("xiaoban_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_xiaoban_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under XIAOBAN_HOME — gateway cleanup
