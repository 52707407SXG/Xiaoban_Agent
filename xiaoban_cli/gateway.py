"""
Gateway subcommand for xiaoban CLI.

Handles: xiaoban gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_xiaoban_home is imported lazily at call sites to avoid ImportError
# when xiaoban_constants is cached from a pre-update version during `xiaoban update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "xiaoban CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``xiaoban gateway status`` from falsely counting the ``xiaoban`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``xiaoban gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_xiaoban_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"xiaoban_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. XIAOBAN_HOME
        ...
        # XIAOBAN_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "xiaoban_home=" in command_lc
            and f"xiaoban_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``xiaoban update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Xiaoban profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Xiaoban profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from xiaoban_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "xiaoban_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m xiaoban_cli.main gateway
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose XIAOBAN_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``xiaoban update`` users
    # with no running gateway until they re-invoke ``xiaoban gateway``
    ...
    from xiaoban_cli._subprocess_compat import (
...
def _sync_xiaoban_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``XIAOBAN_HOME``.

    Under ``sudo``, ``XIAOBAN_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_xiaoban_home` falls back to ``/root/.xiaoban`` — the wrong
    profile. The unit file pins ``XIAOBAN_HOME`` for the actual gateway
    ...
    unit_home = env.get("XIAOBAN_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("XIAOBAN_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["XIAOBAN_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}xiaoban gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}xiaoban gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from xiaoban_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `xiaoban gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``xiaoban gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from xiaoban_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (XIAOBAN_HOME-scoped).
...
from xiaoban_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Xiaoban Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``xiaoban gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``XIAOBAN_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("XIAOBAN_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "xiaoban-gateway"
SERVICE_DESCRIPTION = "Xiaoban-Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current XIAOBAN_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (XIAOBAN_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from xiaoban_constants import get_default_xiaoban_root

    home = get_xiaoban_home().resolve()
    default = get_default_xiaoban_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary XIAOBAN_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(xiaoban_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when XIAOBAN_HOME is a named profile.

    For ``~/.xiaoban/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        xiaoban_home: Optional explicit XIAOBAN_HOME path. Defaults to the current
            ``get_xiaoban_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Xiaoban root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_xiaoban_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from xiaoban_constants import get_default_xiaoban_root

    home = Path(xiaoban_home or str(get_xiaoban_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_xiaoban_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(xiaoban_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".xiaoban"
    try:
        Path(xiaoban_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(xiaoban_home, default_root=target_root)
    except ValueError:
        return _profile_arg(xiaoban_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this XIAOBAN_HOME.

    Default ``~/.xiaoban`` returns ``xiaoban-gateway`` (backward compatible).
    Profile ``~/.xiaoban/profiles/coder`` returns ``xiaoban-gateway-coder``.
    Any other XIAOBAN_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Xiaoban installs that predate the
# xiaoban-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (xiaoban-gateway-*.service) and unrelated third-party
# "xiaoban" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("xiaoban.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "xiaoban_cli.main gateway",
    "xiaoban_cli/main.py gateway",
    "gateway/run.py",
    " xiaoban gateway ",
    "/xiaoban gateway ",
)
...
def _find_legacy_xiaoban_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Xiaoban gateway units.

    Detects unit files installed by older Xiaoban versions that used a
    different service name (e.g. ``xiaoban.service`` before the rename to
    ``xiaoban-gateway.service``). When both a legacy unit and the current
    ``xiaoban-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``xiaoban-gateway-coder.service`` and unrelated third-party
      ``xiaoban-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``xiaoban.service`` running an unrelated
...
def has_legacy_xiaoban_units() -> bool:
    """Return True when any legacy Xiaoban gateway unit files exist."""
    return bool(_find_legacy_xiaoban_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Xiaoban gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_xiaoban_units()
    if not legacy:
        return
    print_warning("Legacy Xiaoban gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current xiaoban-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    xiaoban gateway migrate-legacy")


def remove_legacy_xiaoban_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Xiaoban gateway unit files.

    Iterates over whatever ``_find_legacy_xiaoban_units()`` returns — which is
    ...
    legacy = _find_legacy_xiaoban_units()
    if not legacy:
        print("No legacy Xiaoban gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Xiaoban gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: xiaoban gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo xiaoban gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    xiaoban gateway uninstall")
    print_info("    sudo xiaoban gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Xiaoban can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo xiaoban gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo xiaoban gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo xiaoban gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Xiaoban often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.xiaoban`` → ``ai.xiaoban.gateway.plist`` (backward compatible).
    Profile ``~/.xiaoban/profiles/coder`` → ``ai.xiaoban.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.xiaoban.gateway-{suffix}" if suffix else "ai.xiaoban.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.xiaoban/xiaoban-agent  -> /home/alice/.xiaoban/xiaoban-agent
      /opt/xiaoban                 -> /opt/xiaoban  (kept as-is)
...
def _xiaoban_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current XIAOBAN_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_xiaoban_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.xiaoban                    → /home/alice/.xiaoban
      /root/.xiaoban/profiles/coder     → /home/alice/.xiaoban/profiles/coder
      /opt/custom-xiaoban               → /opt/custom-xiaoban  (kept as-is)
    """
    current_xiaoban = get_xiaoban_home().resolve()
    current_default = (Path.home() / ".xiaoban").resolve()
    target_default = Path(target_home_dir) / ".xiaoban"

    # Default ~/.xiaoban → remap to target user's default
    if current_xiaoban == current_default:
        return str(target_default)

    # Profile or subdir of ~/.xiaoban → preserve the relative structure
    try:
        relative = current_xiaoban.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.xiaoban) — keep as-is
        return str(current_xiaoban)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    xiaoban_home = get_xiaoban_home()
    xiaoban_node = xiaoban_home / "node" / "bin"
    if _is_dir(xiaoban_node):
        candidates.append(str(xiaoban_node))
    xiaoban_nm = xiaoban_home / "node_modules" / ".bin"
    if _is_dir(xiaoban_nm):
        candidates.append(str(xiaoban_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m xiaoban_cli.main``, so module
    ...
    dir, or a clone that ``xiaoban update`` later relocates/removes — the path
    ...
    ``XIAOBAN_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if XIAOBAN_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        xiaoban_home = _xiaoban_home_for_target_user(home_dir)
        profile_arg = _profile_arg_for_target_user(xiaoban_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's XIAOBAN_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(xiaoban_home) if xiaoban_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m xiaoban_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="XIAOBAN_HOME={xiaoban_home}"
Restart=always
RestartSec=5
...
"""
    ...
    xiaoban_home = str(get_xiaoban_home().resolve())
    profile_arg = _profile_arg(xiaoban_home)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir XIAOBAN_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved XIAOBAN_HOME
    ...
    exported a throwaway ``XIAOBAN_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``xiaoban gateway restart``
    with ``XIAOBAN_HOME=/tmp/xiaoban-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="XIAOBAN_HOME=..."`` lines and launchd
    ``<key>XIAOBAN_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'XIAOBAN_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>XIAOBAN_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp XIAOBAN_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: XIAOBAN_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported XIAOBAN_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only XIAOBAN_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp XIAOBAN_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../xiaoban_test`` path into
    # ``Environment="XIAOBAN_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/xiaoban_test"' in new_unit
        or "/xiaoban_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # XIAOBAN_HOME into the unit (manual E2E homes like /tmp/xiaoban-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Xiaoban install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo xiaoban gateway uninstall --system")
    print_info("         xiaoban gateway install")
    print_info("         xiaoban gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("XIAOBAN_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(
    ...
    # Offer to remove legacy units (xiaoban.service from pre-rename installs)
    # before installing the new xiaoban-gateway.service. If both remain, they
    ...
    if has_legacy_xiaoban_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_xiaoban_units(interactive=False)
            print()
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}xiaoban gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}xiaoban gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}xiaoban gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_xiaoban_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `xiaoban gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `xiaoban gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}xiaoban gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}xiaoban gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}xiaoban gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}xiaoban gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.xiaoban.gateway-{suffix}" if suffix else "ai.xiaoban.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``xiaoban gateway start/stop/status`` call).
...
# (the documented `nohup xiaoban gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m xiaoban_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active XIAOBAN_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "xiaoban_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup xiaoban gateway run --replace`
    ...
    """
    from xiaoban_cli._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_xiaoban_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    """
    from xiaoban_constants import display_xiaoban_home as _dhh
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: xiaoban gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup xiaoban gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>xiaoban_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>XIAOBAN_HOME</key>
        <string>{xiaoban_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Xiaoban install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  xiaoban gateway status             # Check status")
    from xiaoban_constants import display_xiaoban_home as _dhh
...
def launchd_stop():
    ...
    # `xiaoban gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Xiaoban install")
    else:
        print("⚠ Service definition is stale relative to the current Xiaoban install")
        print("  Run: xiaoban gateway start")
    ...
    if deep:
        log_file = get_xiaoban_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``XIAOBAN_S6_SUPERVISED_CHILD``.
    """
    if os.environ.get("INVOCATION_ID"):
        return True
    if os.environ.get("XIAOBAN_S6_SUPERVISED_CHILD"):
        return True
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``xiaoban gateway run`` is always a misconfiguration, so we
    ...
    try:
        from xiaoban_constants import get_default_xiaoban_root
        default_root = get_default_xiaoban_root()
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``xiaoban gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``xiaoban gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other XIAOBAN_HOME roots.
    """
    ...
    print("  Use 'xiaoban gateway restart' to replace it,")
    print("  or 'xiaoban gateway stop' first.")
    print("  Or use 'xiaoban gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("XIAOBAN_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Xiaoban gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'xiaoban' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/xiaoban/docker/entrypoint.sh before the Xiaoban command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$XIAOBAN_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set XIAOBAN_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `xiaoban gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set XIAOBAN_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `xiaoban gateway restart` which already
    ...
    # `xiaoban gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           M Xiaoban Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # XIAOBAN_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("XIAOBAN_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from xiaoban_constants import get_xiaoban_home as _ghh
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. xiaoban) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Xiaoban delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Xiaoban connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: xiaoban pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Xiaoban will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``xiaoban setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run xiaoban under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.xiaoban/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_xiaoban_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from xiaoban_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'xiaoban pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: xiaoban pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'xiaoban gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Xiaoban will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Xiaoban will store the returned account_id/token in ~/.xiaoban/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `xiaoban gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_xiaoban_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `xiaoban pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `xiaoban pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "XiaobanAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``xiaoban_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from xiaoban_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.xiaoban/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.xiaoban/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    print(
        color(
            "│             M Gateway Setup                            │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: xiaoban gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: xiaoban gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: xiaoban gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo xiaoban gateway install --system"
                        )
                    print_info("  Or run in foreground:  xiaoban gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: xiaoban gateway run")
                print_info(
                    "  For persistence:   tmux new -s xiaoban 'xiaoban gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: xiaoban gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup xiaoban gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'xiaoban gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`xiaoban_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from xiaoban_cli.service_manager import (
    ...
    if profile is None:
        ...
        # XIAOBAN_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``xiaoban gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``xiaoban gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``xiaoban gateway run`` are unaffected.
      2. ``XIAOBAN_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``xiaoban gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``XIAOBAN_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("XIAOBAN_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("XIAOBAN_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${XIAOBAN_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if XIAOBAN_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or XIAOBAN_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-xiaoban service's
    # pattern in docker/s6-rc.d/main-xiaoban/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: xiaoban gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: xiaoban gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s xiaoban 'xiaoban gateway run'"
                )
                print()
            ...
            print()
            print(
                "  xiaoban gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s xiaoban 'xiaoban gateway run'         # persistent via tmux"
            )
            print(
                "  nohup xiaoban gateway run > ~/.xiaoban/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from xiaoban_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  xiaoban profile create <name>     # creates the s6 service slot")
                print("  xiaoban -p <name> gateway start   # bring it up via s6")
                print("  xiaoban status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: xiaoban gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: xiaoban gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: xiaoban gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  xiaoban profile delete <name>     # tears down the s6 service slot")
                print("  xiaoban -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `xiaoban -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: xiaoban gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_XIAOBAN_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `xiaoban gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_XIAOBAN_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `xiaoban gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: xiaoban gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  xiaoban gateway install")
                else:
                    print("To install as a service:")
                    print("  xiaoban gateway install")
                    print("  sudo xiaoban gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  xiaoban gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup xiaoban gateway run > ~/.xiaoban/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s xiaoban 'xiaoban gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  xiaoban gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  xiaoban gateway install  # Install as user service")
                    print(
                        "  sudo xiaoban gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Xiaoban gateway unit files from
        # pre-rename installs (e.g. xiaoban.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_xiaoban_units(interactive=not yes, dry_run=dry_run)
