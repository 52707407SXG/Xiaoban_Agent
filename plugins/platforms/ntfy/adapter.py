"""ntfy platform adapter (Xiaoban plugin).
...
a Xiaoban dependency.

This adapter ships as a Xiaoban platform plugin under
``plugins/platforms/ntfy/``. The Xiaoban plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "xiaoban-in"              # subscribe topic (incoming)
          publish_topic: "xiaoban-out"     # optional — defaults to topic
...
_ECHO_TAG = "xiaoban-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``xiaoban cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Xiaoban dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `xiaoban gateway status` without
