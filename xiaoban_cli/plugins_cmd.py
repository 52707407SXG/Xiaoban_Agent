"""``xiaoban plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.xiaoban/plugins/``.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Xiaoban subprocess resolution: :func:`shutil.which` first,
...
def _plugins_dir() -> Path:
    ...
    plugins = get_xiaoban_home() / "plugins"
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.xiaoban/.env``."""
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from xiaoban_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_xiaoban_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_xiaoban_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_xiaoban_home()}/.env later)[/dim]")
...
def _install_plugin_core(identifier: str, *, force: bool) -> tuple[Path, dict, str]:
    """Clone Git plugin into ``~/.xiaoban/plugins``.
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update Xiaoban.",
                ) from None
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `xiaoban plugins update {plugin_name}`.",
                )
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid Xiaoban plugin.",
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `xiaoban plugins enable {installed_name}` to activate.[/dim]",
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  xiaoban gateway restart[/dim]")
    console.print()
...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``xiaoban plugins enable`` /
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``xiaoban plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] xiaoban plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] xiaoban plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] xiaoban plugins")
    console.print("[dim]Enable/disable:[/dim] xiaoban plugins enable/disable <name>")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Xiaoban
    ...
    try:
        from xiaoban_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def _run_composite_ui(curses, plugin_names, plugin_labels, plugin_selected,
    ...
    from xiaoban_cli.curses_ui import flush_stdin
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.xiaoban/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.xiaoban/plugins/<name>``."""
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.xiaoban/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def plugins_command(args) -> None:
    """Dispatch xiaoban plugins subcommands."""
