...
description: Control your smart home with Xiaoban-Agent via Home Assistant integration.
...
Xiaoban-Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Xiaoban-Agent"
5. Copy the token
...
Xiaoban-Agent registers four tools for smart home control:
...
Configure which events the agent sees in `~/.xiaoban/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Xiaoban-Agent".
...
The adapter reads credentials from `~/.xiaoban/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Xiaoban profile
...
running Xiaoban — `curl -H "Authorization: Bearer <token>" <url>/api/` should
