...
def _clean_state():
    ...
    saved = {}
    for k in ("XIAOBAN_INTERACTIVE", "XIAOBAN_GATEWAY_SESSION", "XIAOBAN_EXEC_ASK", "XIAOBAN_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["XIAOBAN_INTERACTIVE"] = "1"
        session_key = os.getenv("XIAOBAN_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No XIAOBAN_INTERACTIVE or XIAOBAN_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
...
class TestGatewayApprovalAllowPermanent:
    ...
    def _capture_gateway_payload(self, command, session_key):
        ...
        os.environ["XIAOBAN_SESSION_KEY"] = session_key
        try:
            check_all_command_guards(command, "local")
        finally:
            os.environ.pop("XIAOBAN_GATEWAY_SESSION", None)
            os.environ.pop("XIAOBAN_EXEC_ASK", None)
            os.environ.pop("XIAOBAN_SESSION_KEY", None)
