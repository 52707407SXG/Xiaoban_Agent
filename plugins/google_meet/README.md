...
Let the xiaoban agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where xiaoban runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `xiaoban meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `xiaoban meet` CLI |
...
| `cli.py` | `xiaoban meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$XIAOBAN_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `xiaoban meet node {run,list,approve,remove,status,ping}` |
...
xiaoban meet install                                      # pip + Chromium
xiaoban meet setup                                        # preflight
xiaoban meet auth                                         # optional
xiaoban meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
xiaoban meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.xiaoban/.env
xiaoban meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
xiaoban meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
xiaoban meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.xiaoban/.env
xiaoban meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, xiaoban will **not** switch your system audio input automatically — the
...
xiaoban plugins enable google_meet
xiaoban meet node run --display-name my-mac --host 0.0.0.0 --port 18789
