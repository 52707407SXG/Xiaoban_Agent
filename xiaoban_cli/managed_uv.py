...
Xiaoban owns its own uv binary at ``$XIAOBAN_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$XIAOBAN_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    """Return the path where Xiaoban keeps *its* uv binary.

    ``$XIAOBAN_HOME/bin/uv`` on POSIX, ``$XIAOBAN_HOME\\bin\\uv.exe`` on
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``xiaoban update`` runs
    the call site from the *old*, already-imported ``xiaoban_cli.main`` against
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``xiaoban update`` so the managed copy stays current.
...
def _install_uv(target: Path) -> None:
    ...
    ``$XIAOBAN_HOME/bin/`` instead of ``~/.local/bin/``.
    """
