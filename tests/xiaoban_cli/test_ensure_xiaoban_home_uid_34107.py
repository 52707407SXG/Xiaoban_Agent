"""Regression tests for #34107 — Docker UID/GID handling in ensure_xiaoban_home.

When Xiaoban runs in Docker with ``XIAOBAN_UID=1000`` / ``XIAOBAN_GID=911``,
the entrypoint chowns the top-level ``XIAOBAN_HOME`` once at startup. But
...
The fix is a ``_chown_to_xiaoban_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_xiaoban_uid_gid
# ---------------------------------------------------------------------------


class TestResolveXiaobanUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("XIAOBAN_UID", "1000")
        monkeypatch.setenv("XIAOBAN_GID", "911")
        from xiaoban_cli.config import _resolve_xiaoban_uid_gid
        uid, gid = _resolve_xiaoban_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("XIAOBAN_UID", raising=False)
        monkeypatch.delenv("XIAOBAN_GID", raising=False)
        from xiaoban_cli.config import _resolve_xiaoban_uid_gid
        uid, gid = _resolve_xiaoban_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("XIAOBAN_UID", "not-a-number")
        monkeypatch.setenv("XIAOBAN_GID", "911")
        from xiaoban_cli.config import _resolve_xiaoban_uid_gid
        uid, gid = _resolve_xiaoban_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("XIAOBAN_UID", "")
        monkeypatch.setenv("XIAOBAN_GID", "")
        from xiaoban_cli.config import _resolve_xiaoban_uid_gid
        uid, gid = _resolve_xiaoban_uid_gid()
        assert uid is None
        assert gid is None
...
# ---------------------------------------------------------------------------
# _chown_to_xiaoban_uid
# ---------------------------------------------------------------------------


class TestChownToXiaobanUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("XIAOBAN_UID", "1000")
        monkeypatch.setenv("XIAOBAN_GID", "911")
        from xiaoban_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_xiaoban_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_xiaoban_uid(d)  # must not raise
