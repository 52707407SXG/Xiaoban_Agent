...
import { disconnectOAuthProvider, listOAuthProviders } from '@/xiaoban'
import { useI18n } from '@/i18n'
import { Check, ChevronDown, ChevronRight, KeyRound, Loader2, Terminal, Trash2 } from '@/lib/icons'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import { $desktopOnboarding, startManualProviderOAuth } from '@/store/onboarding'
import type { EnvVarInfo, OAuthProvider } from '@/types/xiaoban'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.xiaobanDesktop?.terminal)
...
//      in xiaoban_cli/provider_catalog.py) — the SAME provider identity
//      `xiaoban model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Xiaoban can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Xiaoban can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Xiaoban never deletes creds another tool owns behind a silent API call.
