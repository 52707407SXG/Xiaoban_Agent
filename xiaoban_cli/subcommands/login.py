"""``xiaoban login`` subcommand parser.
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``xiaoban login`` was removed in favor of ``xiaoban auth`` / ``xiaoban model``
    (the runtime handler in ``xiaoban_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``xiaoban login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``xiaoban --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``xiaoban login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``xiaoban model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `xiaoban auth` to manage credentials, "
            "`xiaoban model` to select a provider, or `xiaoban setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `xiaoban model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: xiaoban-cli)"
    )
