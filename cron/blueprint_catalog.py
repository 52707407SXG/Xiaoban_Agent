...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``xiaoban://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``xiaoban://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"xiaoban://blueprint/{quote(blueprint.key)}{qs}"
