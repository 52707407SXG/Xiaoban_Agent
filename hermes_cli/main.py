...
def cmd_postinstall(args):
    ...
    print("⚕ Xiaoban post-install bootstrap")
    print()
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``xiaoban`` launcher present but ``hermes_cli`` not importable.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    if exe is None:
        return
    # exe = .../Hermes.app/Contents/MacOS/Xiaoban  ->  app bundle = .../Hermes.app
    app = exe.parents[2]
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         https://xiaoban-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        sys.exit(1)
    zip_url = (
        f"https://github.com/52707407SXG/Xiaoban-Agent/archive/refs/heads/{branch}.zip"
    )
...
OFFICIAL_REPO_URLS = {
    "https://github.com/52707407SXG/Xiaoban-Agent.git",
    "git@github.com:52707407SXG/Xiaoban-Agent.git",
    "https://github.com/52707407SXG/Xiaoban-Agent",
    "git@github.com:52707407SXG/Xiaoban-Agent",
}
OFFICIAL_REPO_URL = "https://github.com/52707407SXG/Xiaoban-Agent.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from 52707407SXG/Xiaoban-Agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/52707407SXG/Xiaoban-Agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/52707407SXG/Xiaoban-Agent.git' to add later."
            )
...
def _detect_concurrent_hermes_instances(
    ...
    The Xiaoban Desktop Electron app spawns ``hermes.EXE`` as a backend child,
    ...
    #      hermes.exe sitting *under* a non-Xiaoban parent (e.g. a Xiaoban
...
def _quarantine_running_hermes_exe(
    ...
    when ``xiaoban update`` runs, ``hermes.exe`` IS the live process, and uv
...
def _ensure_fhs_path_guard() -> None:
    ...
    # /usr/local/bin/hermes, code at /usr/local/lib/xiaoban-agent).
...
def _run_pre_update_backup(args) -> None:
    ...
    HERMES_HOME directories.  The ``--backup`` flag on ``xiaoban update``
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://xiaoban-agent.nousresearch.com/install.sh | bash"
            )
            sys.exit(1)
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    if choice == "2":
        ...
        print(
            ...
            "  Docs: https://xiaoban-agent.nousresearch.com/docs/"
...
def main():
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://xiaoban-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://xiaoban-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
