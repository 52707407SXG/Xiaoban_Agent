...
The PID file lives at ``{XIAOBAN_HOME}/gateway.pid``.  XIAOBAN_HOME defaults to
``~/.xiaoban`` but can be overridden via the environment variable.  This means
separate XIAOBAN_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting XIAOBAN_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("XIAOBAN_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "xiaoban" / _LOCKS_DIRNAME
...
def looks_like_gateway_command_line(command: str | None) -> bool:
    ...
    test also matched ``xiaoban_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\xiaoban-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Xiaoban's
    ...
    for token in tokens:
        ...
        if basename in ("xiaoban-gateway", "xiaoban-gateway.exe"):
            return True

    joined = " ".join(tokens)
    has_gateway_entry = (
        "xiaoban_cli.main" in joined
        or "xiaoban_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("xiaoban", "xiaoban.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return True  # bare `xiaoban gateway` defaults to `run`
        return filtered[i + 1] == "run"
    return False


def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Xiaoban gateway."""
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `xiaoban status` show per-profile
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different XIAOBAN_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``xiaoban.service`` + ``xiaoban-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``xiaoban gateway stop`` on Windows would be misclassified as an
