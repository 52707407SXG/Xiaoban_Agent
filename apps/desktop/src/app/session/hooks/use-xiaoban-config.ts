...
import { getXiaobanConfig, getXiaobanConfigDefaults } from '@/xiaoban'
...
interface XiaobanConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useXiaobanConfig({ activeSessionIdRef, refreshProjectBranch }: XiaobanConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshXiaobanConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getXiaobanConfig(), getXiaobanConfigDefaults().catch(() => ({}))])
  ...
  return { refreshXiaobanConfig, sttEnabled, voiceMaxRecordingSeconds }
}
