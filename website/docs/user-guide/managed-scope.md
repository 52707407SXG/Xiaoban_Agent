...
`~/.xiaoban/config.yaml`, `~/.xiaoban/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/xiaoban`:

```text
/etc/xiaoban/
├── config.yaml     # managed config layer (wins over ~/.xiaoban/config.yaml)
└── .env            # managed env layer (wins over ~/.xiaoban/.env + shell)
```
...
The location can be relocated with the `XIAOBAN_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `XIAOBAN_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Xiaoban.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export XIAOBAN_MANAGED_DIR=/opt/org/xiaoban-policy
```

:::warning
A user who can set `XIAOBAN_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `xiaoban doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/xiaoban/config.yaml` (managed) | `/etc/xiaoban/.env` (managed) |
| 2 | `~/.xiaoban/config.yaml` (user) | `~/.xiaoban/.env` (user) |
...
```bash
xiaoban config        # shows a header naming the managed source + the pinned keys
xiaoban doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Xiaoban refuses and names the source:

```bash
$ xiaoban config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/xiaoban/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `xiaoban config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/xiaoban

# Pin some config values for every user on this machine
sudo tee /etc/xiaoban/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/xiaoban/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/xiaoban
sudo chmod 0644 /etc/xiaoban/config.yaml /etc/xiaoban/.env
```

Changes take effect on the next Xiaoban start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`xiaoban doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Xiaoban as `root`), managed scope is advisory.
