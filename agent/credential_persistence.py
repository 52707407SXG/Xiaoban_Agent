...
``auth.json``.  They intentionally have no dependency on ``xiaoban_cli.auth`` so
...
# Sources Xiaoban owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Xiaoban-owned OAuth/device-code state)
