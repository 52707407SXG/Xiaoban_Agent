...
Before the fix, ``xiaoban whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``xiaoban gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    xiaoban = home / ".xiaoban"
    xiaoban.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("XIAOBAN_HOME", str(xiaoban))
...
def _env_value(xiaoban_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from xiaoban_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("xiaoban_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``xiaoban whatsapp`` with an existing paired session and
