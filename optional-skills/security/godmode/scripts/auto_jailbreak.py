...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("XIAOBAN_HOME", os.path.expanduser("~/.xiaoban")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("XIAOBAN_HOME", Path.home() / ".xiaoban")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Xiaoban config paths
# ═══════════════════════════════════════════════════════════════════

XIAOBAN_HOME = Path(os.getenv("XIAOBAN_HOME", Path.home() / ".xiaoban"))
CONFIG_PATH = XIAOBAN_HOME / "config.yaml"
PREFILL_PATH = XIAOBAN_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Xiaoban models — already uncensored, just needs clean prompt
    "xiaoban": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "xiaoban" in model_lower or "nous" in model_lower:
        return "xiaoban"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Xiaoban config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.xiaoban/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Xiaoban for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Xiaoban for changes to take effect.")
