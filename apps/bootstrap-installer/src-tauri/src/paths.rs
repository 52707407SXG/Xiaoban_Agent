...
//!
//! Mirrors `xiaoban_constants.get_xiaoban_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\xiaoban
//!   macOS:   ~/.xiaoban
//!   Linux:   ~/.xiaoban  (override via $XIAOBAN_HOME)
//!
//! NOTE (macOS): Python's get_xiaoban_home(), scripts/install.sh, and the
//! Electron desktop's resolveXiaobanHome() ALL use ~/.xiaoban on macOS — there
...
/// Returns the canonical Xiaoban home directory, respecting $XIAOBAN_HOME if set.
pub fn xiaoban_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("XIAOBAN_HOME") {
    ...
    {
        // %LOCALAPPDATA%\xiaoban — matches scripts/install.ps1's $XiaobanHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("xiaoban");
        }
    }

    // macOS + Linux + fallback: ~/.xiaoban (matches Python get_xiaoban_home(),
    // install.sh, and the Electron desktop's resolveXiaobanHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".xiaoban");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".xiaoban")
}

pub fn log_dir() -> PathBuf {
    xiaoban_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    xiaoban_home().join("bootstrap-cache")
}
...
/// XIAOBAN_HOME so it survives repo checkout deletion (unlike anything under
/// xiaoban-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\xiaoban\xiaoban-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_xiaoban`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under XIAOBAN_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves XIAOBAN_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    xiaoban_home().join(".xiaoban-update-in-progress")
}
...
pub fn copy_self_to_xiaoban_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to XIAOBAN_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_XIAOBAN_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".xiaoban-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under XIAOBAN_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[xiaoban-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("XIAOBAN_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_xiaoban_home() -> String {
    xiaoban_home().to_string_lossy().into_owned()
}
