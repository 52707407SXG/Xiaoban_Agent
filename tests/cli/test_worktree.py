...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("xiaoban/xiaoban-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned xiaoban/* and pr-* branches."""

    def test_prunes_orphaned_xiaoban_branch(self, git_repo):
        """xiaoban/xiaoban-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "xiaoban/xiaoban-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "xiaoban/xiaoban-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "xiaoban/xiaoban-deadbeef" in result.stdout
        ...
        assert "xiaoban/xiaoban-deadbeef" in orphaned
        ...
        assert "xiaoban/xiaoban-deadbeef" not in result.stdout
