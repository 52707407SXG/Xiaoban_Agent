"""Helpers for loading Xiaoban .env files consistently across entrypoints."""
...
# load_xiaoban_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_xiaoban_dotenv().
# Used by setup / `xiaoban model` flows to label detected credentials so
# users understand WHERE a key came from when their .env doesn't contain it
# directly (otherwise the "credentials detected ✓" line looks identical to
# the .env case and they don't know Bitwarden is wired up).
_SECRET_SOURCES: dict[str, str] = {}

# XIAOBAN_HOME paths we've already pulled external secrets for during this
# process.  ``load_xiaoban_dotenv()`` is called at module-import time from
# several hot modules (cli.py, xiaoban_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_xiaoban_dotenv()`` call.  Returns
...
def reset_secret_source_cache() -> None:
    """Forget which XIAOBAN_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `xiaoban setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``xiaoban_cli.config._sanitize_env_lines`` which
    already knows all valid Xiaoban env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from xiaoban_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_xiaoban_dotenv(
    *,
    xiaoban_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Xiaoban environment files with user config taking precedence.

    Behavior:
    - `~/.xiaoban/.env` overrides stale shell-exported values when present.
    - project `.env` acts as a dev fallback and only fills missing values when
      the user env exists.
    - if no user env exists, the project `.env` also overrides stale shell vars,
      except explicit home/profile/config selectors such as XIAOBAN_HOME.
    """
    loaded: list[Path] = []
    protected_existing_env = {
        key: os.environ[key]
        for key in ("XIAOBAN_HOME", "XIAOBAN_PROFILE", "XIAOBAN_CONFIG", "XIAOBAN_ENV")
        if key in os.environ
    }

    home_path = Path(xiaoban_home or os.getenv("XIAOBAN_HOME", Path.home() / ".xiaoban"))
    ...
    if project_env_path and project_env_path.exists():
        ...
        for key, value in protected_existing_env.items():
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of XIAOBAN_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of Xiaoban reads
    ``os.environ`` for credentials.  Any failure here is logged and
    swallowed — external secret sources must never block startup.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_xiaoban_dotenv()`` runs at import
    time from several hot modules (cli.py, xiaoban_cli/main.py,
    run_agent.py, trajectory_compressor.py, ...), so without this guard
    the Bitwarden status line would print 3-5x per CLI startup.  Use
    ``reset_secret_source_cache()`` if you need to force a re-pull
    (tests, future ``xiaoban secrets bitwarden sync`` from a long-running
    process).
    """
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `xiaoban model`
