...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("xiaoban_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("xiaoban_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"XIAOBAN_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_xiaoban_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.xiaoban/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_xiaoban_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $XIAOBAN_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_xiaoban_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$XIAOBAN_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestXiaobanConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.xiaoban/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.xiaoban/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.xiaoban/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.xiaoban/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.xiaoban/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.xiaoban/config.yaml")
        assert dangerous is True
        assert "xiaoban config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.xiaoban/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_xiaoban_home_config(self):
        config_path = get_xiaoban_home() / "config.yaml"
    ...
    def test_sed_in_place_absolute_xiaoban_home_env(self):
        env_path = get_xiaoban_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_xiaoban_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $XIAOBAN_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.xiaoban/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_xiaoban_home_config(self):
        config_path = get_xiaoban_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.xiaoban/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_xiaoban_home_env(self):
        env_path = get_xiaoban_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.xiaoban/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.xiaoban/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.xiaoban/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.xiaoban/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.xiaoban/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Xiaoban config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_xiaoban_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $XIAOBAN_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.xiaoban/config.yaml/.env must require approval — the
    ...
    def test_cp_to_xiaoban_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.xiaoban/config.yaml")
        assert dangerous is True
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.xiaoban/xiaoban-agent && source venv/bin/activate && python -m xiaoban_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m xiaoban_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m xiaoban_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "xiaoban_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart xiaoban-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_pkill_xiaoban_detected(self):
        """pkill targeting xiaoban/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_xiaoban_detected(self):
        cmd = "killall xiaoban"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep xiaoban) bypasses the pkill/killall name-matching
    ...
    def test_pkill_xiaoban_still_detected(self):
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("XIAOBAN_GATEWAY_SESSION", "XIAOBAN_CRON_SESSION",
                      "XIAOBAN_YOLO_MODE",
                      "XIAOBAN_SESSION_KEY", "XIAOBAN_INTERACTIVE")
        }
        os.environ.pop("XIAOBAN_YOLO_MODE", None)
        os.environ.pop("XIAOBAN_INTERACTIVE", None)
        # XIAOBAN_CRON_SESSION takes priority over XIAOBAN_GATEWAY_SESSION in
        ...
        os.environ.pop("XIAOBAN_CRON_SESSION", None)
        os.environ["XIAOBAN_GATEWAY_SESSION"] = "1"
        os.environ["XIAOBAN_SESSION_KEY"] = self.SESSION_KEY
