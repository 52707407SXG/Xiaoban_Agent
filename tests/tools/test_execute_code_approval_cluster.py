...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad XIAOBAN_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: XIAOBAN_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("XIAOBAN_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("XIAOBAN_CRON_SESSION", "1")
    monkeypatch.delenv("XIAOBAN_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_xiaoban_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "XIAOBAN_HOME": "/h", "XIAOBAN_PROFILE": "p",
        "XIAOBAN_CONFIG": "/c.yaml", "XIAOBAN_ENV": "/e",
        # other XIAOBAN_* → dropped (broad prefix removed)
        "XIAOBAN_BASE_URL": "https://x", "XIAOBAN_INTERACTIVE": "1",
        "XIAOBAN_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("XIAOBAN_HOME", "XIAOBAN_PROFILE", "XIAOBAN_CONFIG", "XIAOBAN_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "XIAOBAN_BASE_URL", "XIAOBAN_INTERACTIVE", "XIAOBAN_KANBAN_DB",
...
def test_env_scrub_logs_dropped_xiaoban_vars(caplog):
    """Dropping a non-allowlisted, non-secret XIAOBAN_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset XIAOBAN_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "XIAOBAN_HOME": "/h",          # allowlisted → kept, not logged
        "XIAOBAN_BASE_URL": "https://x",   # dropped → logged
        "XIAOBAN_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "XIAOBAN_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "XIAOBAN_HOME" in out and "PATH" in out
    assert "XIAOBAN_BASE_URL" not in out and "XIAOBAN_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "XIAOBAN_BASE_URL" in msgs and "XIAOBAN_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "XIAOBAN_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped XIAOBAN_* vars."""
    import logging
