...
import { formatRefValue, xiaobanDirectiveFormatter } from './directive-text'
...
describe('xiaobanDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = xiaobanDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = xiaobanDirectiveFormatter.parse('@file:src/main.tsx the entry point')
