...
-----------------------------------------------------------------------------
    ...
    ~/.xiaoban/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/xiaoban/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_xiaoban_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/xiaoban"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Xiaoban convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("XIAOBAN_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal Xiaoban infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``xiaoban checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `xiaoban checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "xiaoban@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Xiaoban Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``XIAOBAN_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at XIAOBAN_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "XIAOBAN_WORKDIR").write_text(
    ...
    * its ``last_touch`` is older than ``retention_days`` days.
    ...
    for child in base.iterdir():
        ...
        if delete_orphans:
            ...
            wd_marker = child / "XIAOBAN_WORKDIR"
            if wd_marker.exists():
                try:
...
# ---------------------------------------------------------------------------
# Public helpers for `xiaoban checkpoints` CLI
# ---------------------------------------------------------------------------
