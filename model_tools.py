...
#   - cli.py, xiaoban_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("XIAOBAN_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("XIAOBAN_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by XIAOBAN_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like xiaoban-cli)
    # is enabled, any tools belonging to a disabled toolset are strictly
    # stripped out. See issue #17309.
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                if toolset_name.startswith("xiaoban-"):
                    # Platform bundles (xiaoban-*) include _XIAOBAN_CORE_TOOLS, so
    ...
    # window). Core Xiaoban tools (toolsets._XIAOBAN_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from xiaoban_cli.config import load_config as _load
        cfg = _load() or {}
