...
Pure logic: walks a Xiaoban config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `xiaoban doctor` and a future `xiaoban migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Xiaoban config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Xiaoban config and return retirement issues.
