...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Xiaoban'
...
The actual runtime resolution happens in xiaoban_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "xiaoban"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Xiaoban' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Xiaoban for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from xiaoban_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the xiaoban-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "xiaoban-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Xiaoban tool callback are always-on production
            ...
            if "xiaoban-tools" in mig_report.migrated:
                msg_lines.append(
                    "Xiaoban tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Xiaoban runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Xiaoban tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Xiaoban runtime.")
