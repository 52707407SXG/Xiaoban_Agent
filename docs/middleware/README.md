...
Xiaoban middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Xiaoban'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Xiaoban calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Xiaoban retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Xiaoban guardrails,
...
- `telemetry_schema_version`: currently `xiaoban.observer.v1`
- `middleware_schema_version`: currently `xiaoban.middleware.v1`
...
Xiaoban stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Xiaoban runs
them as a nested chain in registration order. Middleware failures are fail-open:
Xiaoban logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Xiaoban applies middleware in this order:
...
For each tool call, Xiaoban applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Xiaoban pre-execution path against the effective arguments:
...
```bash
xiaoban plugins enable <plugin-name>
```

For isolated local testing, use one `XIAOBAN_HOME` for plugin enablement and the
agent run:

```bash
export XIAOBAN_HOME=/tmp/xiaoban-middleware-test
mkdir -p "$XIAOBAN_HOME"
xiaoban plugins enable <plugin-name>
xiaoban chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run xiaoban plugins enable <plugin-name>
uv run xiaoban chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["xiaoban_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/xiaoban-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Xiaoban expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Xiaoban treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Xiaoban preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Xiaoban does not convert downstream
