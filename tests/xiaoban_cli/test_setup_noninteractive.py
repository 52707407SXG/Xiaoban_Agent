...
import pytest
from xiaoban_cli.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_cmd_setup_allows_noninteractive_flag_without_tty(self):
        ...
        from xiaoban_cli.main import cmd_setup

        args = _make_setup_args(non_interactive=True)

        with (
            patch("xiaoban_cli.setup.run_setup_wizard") as mock_run_setup,
    ...
    def test_cmd_setup_defers_no_tty_handling_to_setup_wizard(self):
        """Bare `xiaoban setup` should reach setup.py, which prints headless guidance."""
        from xiaoban_cli.main import cmd_setup
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("xiaoban_cli.setup.ensure_xiaoban_home"),
            patch("xiaoban_cli.setup.load_config", return_value={}),
            patch("xiaoban_cli.setup.get_xiaoban_home", return_value="/tmp/.xiaoban"),
            patch("xiaoban_cli.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            run_setup_wizard(args)

        out = capsys.readouterr().out
        assert "xiaoban config set model.provider custom" in out
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `xiaoban` should not prompt for input when no provider exists and stdin is headless."""
        from xiaoban_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("xiaoban_cli.main._has_any_provider_configured", return_value=False),
            patch("xiaoban_cli.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
    ...
    def test_main_accepts_tts_setup_section(self, monkeypatch):
        """`xiaoban setup tts` should parse and dispatch like other setup sections."""
        ...
        monkeypatch.setattr("sys.argv", ["xiaoban", "setup", "tts"])
