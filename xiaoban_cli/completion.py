"""Shell completion script generation for xiaoban CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_xiaoban_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Xiaoban-Agent bash completion
# Add to ~/.bashrc:
#   eval "$(xiaoban completion bash)"

_xiaoban_profiles() {{
    local profiles_dir="$HOME/.xiaoban/profiles"
...
_xiaoban_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_xiaoban_profiles)" -- "$cur"))
        return
    fi
...
complete -F _xiaoban_completion xiaoban
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef xiaoban
# Xiaoban-Agent zsh completion
# Add to ~/.zshrc:
...
_xiaoban_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.xiaoban/profiles" ]]; then
        profiles+=($HOME/.xiaoban/profiles/*(N/:t))
    fi
...
_xiaoban() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_xiaoban_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'xiaoban command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Xiaoban-Agent fish completion",
        "# Add to your config:",
        "#   xiaoban completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __xiaoban_profiles",
        "    echo default",
        "    if test -d $HOME/.xiaoban/profiles",
        "        for d in $HOME/.xiaoban/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c xiaoban -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c xiaoban -f -s p -l profile"
        " -d 'Profile name' -xa '(__xiaoban_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__xiaoban_profiles)' -d 'Profile name'"
                )
