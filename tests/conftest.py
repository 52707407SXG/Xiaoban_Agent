"""Shared fixtures for the xiaoban-agent test suite.
...
2. **Isolated XIAOBAN_HOME.** XIAOBAN_HOME points to a per-test tempdir so
   code reading ``~/.xiaoban/*`` via ``get_xiaoban_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".xiaoban"`` instead of the canonical
   ``get_xiaoban_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No XIAOBAN_SESSION_* inheritance** — the agent's current gateway
...
# XIAOBAN_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_XIAOBAN_BEHAVIORAL_VARS = frozenset({
    "XIAOBAN_YOLO_MODE",
    "XIAOBAN_INTERACTIVE",
    "XIAOBAN_QUIET",
    "XIAOBAN_TOOL_PROGRESS",
    "XIAOBAN_TOOL_PROGRESS_MODE",
    "XIAOBAN_MAX_ITERATIONS",
    "XIAOBAN_SESSION_PLATFORM",
    "XIAOBAN_SESSION_CHAT_ID",
    "XIAOBAN_SESSION_CHAT_NAME",
    "XIAOBAN_SESSION_THREAD_ID",
    "XIAOBAN_SESSION_SOURCE",
    "XIAOBAN_SESSION_KEY",
    "XIAOBAN_GATEWAY_SESSION",
    "XIAOBAN_CRON_SESSION",
    "_XIAOBAN_GATEWAY",
    "XIAOBAN_PLATFORM",
    "XIAOBAN_MODEL",
    "XIAOBAN_INFERENCE_MODEL",
    "XIAOBAN_INFERENCE_PROVIDER",
    "XIAOBAN_TUI_PROVIDER",
    "XIAOBAN_MANAGED",
    "XIAOBAN_MANAGED_DIR",
    "XIAOBAN_DEV",
    "XIAOBAN_CONTAINER",
    ...
    "XIAOBAN_TIMEZONE",
    "XIAOBAN_REDACT_SECRETS",
    "XIAOBAN_BACKGROUND_NOTIFICATIONS",
    "XIAOBAN_EXEC_ASK",
    "XIAOBAN_HOME_MODE",
    "XIAOBAN_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.xiaoban/kanban.db instead of the per-test XIAOBAN_HOME.
    "XIAOBAN_KANBAN_DB",
    "XIAOBAN_KANBAN_BOARD",
    "XIAOBAN_KANBAN_HOME",
    "XIAOBAN_KANBAN_WORKSPACES_ROOT",
    "XIAOBAN_KANBAN_LOGS_ROOT",
    "XIAOBAN_KANBAN_TASK",
    "XIAOBAN_KANBAN_WORKSPACE",
    "XIAOBAN_KANBAN_RUN_ID",
    "XIAOBAN_KANBAN_CLAIM_LOCK",
    "XIAOBAN_KANBAN_DISPATCH_IN_GATEWAY",
    "XIAOBAN_TENANT",
    ...
    "XIAOBAN_DASHBOARD_OAUTH_CLIENT_ID",
    "XIAOBAN_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and XIAOBAN_HOME to per-test tempdirs so code that
    reads ``~/.xiaoban/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral XIAOBAN_* vars that could change test semantics.
    for name in _XIAOBAN_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect XIAOBAN_HOME to a per-test tempdir. Code that reads
    #    ``~/.xiaoban/*`` via ``get_xiaoban_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.xiaoban/*`` via
    #    ``Path.home() / ".xiaoban"`` instead of ``get_xiaoban_home()``
    #    is a bug to fix at the callsite.
    fake_xiaoban_home = tmp_path / "xiaoban_test"
    fake_xiaoban_home.mkdir()
    (fake_xiaoban_home / "sessions").mkdir()
    (fake_xiaoban_home / "cron").mkdir()
    (fake_xiaoban_home / "memories").mkdir()
    (fake_xiaoban_home / "skills").mkdir()
    monkeypatch.setenv("XIAOBAN_HOME", str(fake_xiaoban_home))
    ...
    #    ~/.xiaoban/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import xiaoban_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_xiaoban_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal xiaoban config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``xiaoban-gateway`` process
...
# correlated with a ``tests/xiaoban_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> xiaoban-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart xiaoban-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting xiaoban/python patterns are also blocked.
    """
    ...
    def _matches_xiaoban_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _XIAOBAN_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_xiaoban_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch xiaoban-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m xiaoban_cli.main".
                if (
                    "xiaoban" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live xiaoban-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting xiaoban/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `xiaoban update` (or the
        # equivalent `python -m xiaoban_cli.main update`).  These commands
        ...
        if "update" in low and (
            # xiaoban update / xiaoban update --gateway / setsid bash -c ... xiaoban update
            ("xiaoban" in low and "update" in low.split())
            or
            # python -m xiaoban_cli.main update --gateway
            ("xiaoban_cli" in low and "update" in low.split())
            or
            # venv/bin/xiaoban update  (absolute path variant used in tests)
            (".venv/bin/xiaoban" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`xiaoban update` against the real checkout, fetching "
