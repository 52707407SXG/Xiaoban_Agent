...
// bug that drives the REAL useGatewayBoot hook + REAL XiaobanGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshXiaobanConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { xiaobanDesktop?: unknown }).xiaobanDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { xiaobanDesktop?: unknown }).xiaobanDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForXiaoban) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startXiaoban()'s remote branch awaits waitForXiaoban() for 45s before it
    ...
    ;(window as { xiaobanDesktop?: unknown }).xiaobanDesktop = desktop
    ...
    // After ~45s waitForXiaoban gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Xiaoban backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
