"""
Xiaoban MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Xiaoban-specific extra)

Usage:
    xiaoban mcp serve
    xiaoban mcp serve --verbose
...
logger = logging.getLogger("xiaoban.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using XIAOBAN_HOME."""
    try:
        from xiaoban_constants import get_xiaoban_home
        return get_xiaoban_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("XIAOBAN_HOME", Path.home() / ".xiaoban")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from xiaoban_constants import get_xiaoban_home
        directory_file = get_xiaoban_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("XIAOBAN_HOME", Path.home() / ".xiaoban")
...
class EventBridge:
    ...
    This is the Xiaoban equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from xiaoban_constants import get_xiaoban_home
            db_file = get_xiaoban_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("XIAOBAN_HOME", Path.home() / ".xiaoban")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Xiaoban MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "xiaoban",
        instructions=(
            "Xiaoban-Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Xiaoban MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
