"""
Xiaoban-Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m xiaoban_cli.main web          # Start on http://127.0.0.1:9119
    python -m xiaoban_cli.main web --port 8080
"""
...
from xiaoban_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
try:
    ...
    # running `xiaoban dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["XIAOBAN_WEB_DIST"]) if "XIAOBAN_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``xiaoban gateway run`` — but the
    desktop app spawns a ``xiaoban dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same XIAOBAN_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import xiaoban_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from xiaoban_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire xiaoban_cli.gateway import into a background thread so the event
    # loop is not blocked and XIAOBAN_DASHBOARD_READY fires without delay.
    ...
    # Desktop-spawned backends (XIAOBAN_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `xiaoban
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("XIAOBAN_DESKTOP") == "1":
...
app = FastAPI(title="Xiaoban-Agent", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from xiaoban_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# XIAOBAN_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("XIAOBAN_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Xiaoban-Session-Token"
...
#
# This list is defined in ``xiaoban_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from xiaoban_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Xiaoban-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``xiaoban-0day``
...
async def _dashboard_auth_gate(request: Request, call_next):
    from xiaoban_cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Xiaoban (no terminal prompt), "
...
class ModelAssignment(BaseModel):
    ...
    # ``xiaoban model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Xiaoban provider slug — fine.
    ...
    1. Vendor-name → Xiaoban-provider mapping: when the provider string is
       not a known Xiaoban provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from xiaoban_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from xiaoban_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "XIAOBAN_DASHBOARD_FILES_ROOT"
    ...
    for _ in range(50):
        try:
            if (directory / ".git").exists():
                if directory.resolve(strict=False) in ignored_roots:
                    return None
...
def _default_xiaoban_root_is_opt_data() -> bool:
    raw = os.environ.get("XIAOBAN_HOME", "").strip()
    if not raw:
        return False
    try:
        from xiaoban_constants import get_default_xiaoban_root

        root = get_default_xiaoban_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``xiaoban update``.
    ...
    a container — e.g. the xiaoban-webui image sharing the Xiaoban source tree),
    the dashboard's ``xiaoban update`` button is the correct update path and
    ...
    """
    if _default_xiaoban_root_is_opt_data():
        return True
    try:
        from xiaoban_constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Xiaoban root is actually /opt/data
    # (the container/hosted layout) or when XIAOBAN_DASHBOARD_FILES_ROOT is set.
    if _default_xiaoban_root_is_opt_data():
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_xiaoban_home() at call time
    ...
    try:
        ...
        # Windows install the first import of xiaoban_cli.gateway blocks the
        ...
        # and which providers are registered so ``xiaoban status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
        # "loopback only — no auth gate" with no extra round trips.
        auth_required = bool(getattr(app.state, "auth_required", False))
        auth_providers: list[str] = []
        try:
            from xiaoban_cli.dashboard_auth import list_providers as _list_providers
        ...
        status = {
            ...
            "can_update_xiaoban": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
        ...
        if not auth_required:
            status.update({
                "xiaoban_home": str(get_xiaoban_home()),
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the xiaoban home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_xiaoban_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_xiaoban_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from xiaoban_cli.auth import get_nous_auth_status
...
async def run_prompt_size():
    try:
        proc = _spawn_xiaoban_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_xiaoban_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_xiaoban_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from xiaoban_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.xiaoban/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_xiaoban_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "xiaoban-update": "xiaoban-update.log",
...
def _spawn_xiaoban_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``xiaoban <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``xiaoban_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "xiaoban_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "XIAOBAN_NONINTERACTIVE": "1"},
    }
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["xiaoban", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``xiaoban gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``xiaoban gateway restart`` children race each other on the
    ...
    return _spawn_xiaoban_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``xiaoban gateway restart`` in the background."""
    try:
...
@app.post("/api/xiaoban/update")
async def update_xiaoban():
    """Kick off ``xiaoban update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Xiaoban updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("xiaoban-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "xiaoban-update",
    ...
    try:
        proc = _spawn_xiaoban_action(["update"], "xiaoban-update")
    except Exception as exc:
        _log.exception("Failed to spawn xiaoban update")
...
@app.get("/api/xiaoban/update/check")
async def check_xiaoban_update(force: bool = False):
    """Report whether a Xiaoban update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/xiaoban/update`` actually runs ``xiaoban update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Xiaoban version string
    ...
    try:
        ...
        if force:
            try:
                (get_xiaoban_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.xiaoban/config.yaml`` under ``tts.``.
    """
...
async def get_profiles_sessions(
    ...
    from xiaoban_state import SessionDB
    from xiaoban_cli import profiles as profiles_mod
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Xiaoban surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Xiaoban supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _memory_provider_config_path(provider: MemoryProvider) -> Path:
    return get_xiaoban_home() / provider.name / "config.json"
...
# in xiaoban_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    """
    try:
        from xiaoban_cli.inventory import build_models_payload, load_picker_context
        ...
        # onboarding picker) sees the SAME full provider universe `xiaoban model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `xiaoban model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from xiaoban_cli.auth import get_provider_auth_state
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.xiaoban/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from xiaoban_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (xiaoban_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from xiaoban_cli.nous_subscription import apply_nous_managed_defaults
        ...
        # endpoint, mirroring the ``xiaoban model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from xiaoban_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``xiaoban model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from xiaoban_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from xiaoban_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `xiaoban model` provider — would be
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict) -> dict:
        ...
        return {
            ...
            # CLI `xiaoban model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Xiaoban from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Xiaoban to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Xiaoban from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Xiaoban to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Xiaoban in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Xiaoban through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Xiaoban via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Xiaoban through an IMAP/SMTP mailbox.",
        "docs_url": "https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Xiaoban to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Xiaoban inside Feishu / Lark.",
    ...
    "weixin": {
        ...
        "docs_url": "https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Xiaoban through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Xiaoban to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Xiaoban to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Xiaoban as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/user-guide/messaging/webhooks/",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in xiaoban gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in xiaoban gateway setup",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.xiaoban-agent.nousresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"XiaobanDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Xiaoban-Agent").strip() or "Xiaoban-Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Xiaoban appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's XIAOBAN_HOME.
...
# the canonical ``xiaoban auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.xiaoban/.anthropic_oauth.json`` — Xiaoban-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_xiaoban_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_xiaoban_oauth_credentials,
            _XIAOBAN_OAUTH_FILE,
        )
    except ImportError:
        read_xiaoban_oauth_credentials = None  # type: ignore
        _XIAOBAN_OAUTH_FILE = None  # type: ignore

    xiaoban_creds = None
    if read_xiaoban_oauth_credentials:
        try:
            xiaoban_creds = read_xiaoban_oauth_credentials()
        except Exception:
            xiaoban_creds = None
    if xiaoban_creds and xiaoban_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "xiaoban_pkce",
            "source_label": f"Xiaoban PKCE ({_XIAOBAN_OAUTH_FILE})",
            "token_preview": _truncate_token(xiaoban_creds.get("accessToken")),
            "expires_at": xiaoban_creds.get("expiresAt"),
            "has_refresh_token": bool(xiaoban_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Xiaoban even
    when they also have a separate Xiaoban-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Xiaoban never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "xiaoban auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "xiaoban auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "xiaoban auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "xiaoban auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "xiaoban auth add xai-oauth",
        "docs_url": "https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "xiaoban auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from xiaoban_cli import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Xiaoban, so the disconnect
    ...
    user sees exactly what executes, and Xiaoban then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Xiaoban — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``xiaoban model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `xiaoban model` universe (zero-edit for new plugins).
    try:
        from xiaoban_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"xiaoban auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("xiaoban_pkce", "claude_code", ...)
    ...
    sync with the `xiaoban model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Xiaoban-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _XIAOBAN_OAUTH_FILE
                if _XIAOBAN_OAUTH_FILE.exists():
                    _XIAOBAN_OAUTH_FILE.unlink()
                    cleared = True
            except Exception:
                pass
            # Also clear the credential pool entry if present.
            try:
                from xiaoban_cli.auth import clear_provider_auth
        ...
        try:
            from xiaoban_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.xiaoban/.anthropic_oauth.json AND credential pool
...
# Guarded so xiaoban web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Xiaoban file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``xiaoban auth add anthropic``.
    """
    from agent.anthropic_adapter import _XIAOBAN_OAUTH_FILE
    payload = {
    ...
    _XIAOBAN_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _XIAOBAN_OAUTH_FILE.with_name(
        f"{_XIAOBAN_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _XIAOBAN_OAUTH_FILE)
        try:
            _XIAOBAN_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _submit_anthropic_pkce(
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "xiaoban-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("XIAOBAN_PORTAL_BASE_URL")
...
# the tokens exactly like `xiaoban auth add xai-oauth`.
...
def _xai_loopback_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = hauth._xai_validate_inference_base_url(
            os.getenv("XIAOBAN_XAI_BASE_URL", "").strip().rstrip("/")
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `xiaoban auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from xiaoban_cli.auth import persist_nous_credentials
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``xiaoban auth add minimax-oauth``.
    """
    from xiaoban_cli.auth import (
        _minimax_poll_token,
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from xiaoban_cli.auth import _save_codex_tokens
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `xiaoban sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `xiaoban sessions prune`)."""
    if body.older_than_days < 1:
        raise HTTPException(status_code=400, detail="older_than_days must be >= 1")
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_xiaoban_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from xiaoban_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_xiaoban_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from xiaoban_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, XIAOBAN_HOME)."""
    from xiaoban_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["xiaoban_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process XIAOBAN_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (xiaoban_cli.mcp_config), so
# servers managed here show up under `xiaoban mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from xiaoban_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from xiaoban_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from xiaoban_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from xiaoban_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # XIAOBAN_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds XIAOBAN_HOME-derived paths in the child.
    if entry.install is not None:
        try:
            proc = _spawn_xiaoban_action(
...
#
# Wraps the same JSON store the CLI uses (xiaoban_cli.webhook); the webhook
...
async def list_webhooks():
    import xiaoban_cli.webhook as wh
...
# spawn the real `xiaoban gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_xiaoban_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_xiaoban_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from xiaoban_cli.auth import read_credential_pool
...
# `xiaoban memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `xiaoban memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_xiaoban_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_xiaoban_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_xiaoban_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `xiaoban import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_xiaoban_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from xiaoban_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from xiaoban_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <xiaoban_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_xiaoban_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_xiaoban_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``xiaoban`` subprocess, and
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from xiaoban_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`xiaoban -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the XIAOBAN_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_xiaoban_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "xiaoban setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local XIAOBAN_HOME override so the write lands in the target
    ...
    """
    from xiaoban_constants import set_xiaoban_home_override, reset_xiaoban_home_override

    token = set_xiaoban_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local XIAOBAN_HOME override (same mechanism as
    ...
    """
    from xiaoban_constants import set_xiaoban_home_override, reset_xiaoban_home_override
    from xiaoban_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the XIAOBAN_HOME override. Returns the
    number of skills newly disabled.
    """
    from xiaoban_constants import set_xiaoban_home_override, reset_xiaoban_home_override
    from xiaoban_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's XIAOBAN_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``xiaoban profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from XIAOBAN_HOME).
    """
    from xiaoban_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``xiaoban profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the XIAOBAN_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``xiaoban profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from xiaoban_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_xiaoban_home()`` at call
       time — the context-local override from ``set_xiaoban_home_override``
    ...
    retargeted to the *current* ``get_xiaoban_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a XIAOBAN_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
    ...
    from xiaoban_constants import (
        get_xiaoban_home,
        set_xiaoban_home_override,
        reset_xiaoban_home_override,
    )
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_xiaoban_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_xiaoban_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from xiaoban_cli.skills_config import get_disabled_skills
    with _profile_scope(profile):
        config = load_config()
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``xiaoban tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``xiaoban tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``xiaoban tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.xiaoban/.env`` via ``save_env_value`` —
    the same store ``xiaoban tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``xiaoban tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``xiaoban -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same XIAOBAN_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Xiaoban,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``xiaoban computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``xiaoban --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from xiaoban_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from xiaoban_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from xiaoban_cli.dashboard_auth.audit import AuditEvent, audit_log
        from xiaoban_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``xiaoban --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``XIAOBAN_TUI_RESUME`` env var —
    matching what ``xiaoban_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``XIAOBAN_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``XIAOBAN_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `profile` (when set) scopes the ENTIRE chat to that profile by pointing
    ``XIAOBAN_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_xiaoban_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``xiaoban -p <name>`` performs. The in-process
    ``XIAOBAN_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from xiaoban_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("XIAOBAN_TUI_DISABLE_MOUSE", "1")
    env.setdefault("XIAOBAN_TUI_INLINE", "1")
    env["XIAOBAN_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["XIAOBAN_HOME"] = str(profile_dir)

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["XIAOBAN_TUI_RESUME"] = resume

    if sidecar_url:
        env["XIAOBAN_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile XIAOBAN_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["XIAOBAN_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from xiaoban_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Xiaoban inside WSL2 to use the dashboard's /chat "
...
# XIAOBAN_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`xiaoban_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from xiaoban_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/xiaoban/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /xiaoban`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__XIAOBAN_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/xiaoban``)
        ...
        ``__XIAOBAN_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__XIAOBAN_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__XIAOBAN_BASE_PATH__="{prefix}";'
                f"window.__XIAOBAN_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__XIAOBAN_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__XIAOBAN_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__XIAOBAN_BASE_PATH__="{prefix}";'
                f"window.__XIAOBAN_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/xiaoban`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Xiaoban backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Xiaoban Teal",         "description": "Classic dark teal — the canonical Xiaoban look"},
    {"name": "default-large", "label": "Xiaoban Teal (Large)", "description": "Xiaoban Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.xiaoban/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.xiaoban/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_xiaoban_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.xiaoban/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    2. User plugins:    ~/.xiaoban/plugins/<name>/dashboard/manifest.json
    3. Project plugins: ./.xiaoban/plugins/  (only if XIAOBAN_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from xiaoban_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
        (get_xiaoban_home() / "plugins", "user"),
    ]
    ...
    # ``xiaoban_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("XIAOBAN_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".xiaoban" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from xiaoban_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_xiaoban_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"xiaoban auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from xiaoban_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from xiaoban_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from xiaoban_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from xiaoban_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"xiaoban_dashboard_plugin_{plugin['name']}"
...
from xiaoban_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    try:
        from xiaoban_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the xiaoban-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing XIAOBAN_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Xiaoban-Agent exposes a
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `xiaoban dashboard register` (Nous Portal) or "
    ...
    # XIAOBAN_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"XIAOBAN_DASHBOARD_READY port={actual_port}", flush=True)
            print(f"  Xiaoban Web UI -> http://{host}:{actual_port}")
