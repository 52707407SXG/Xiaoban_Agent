...
# when hermes_constants is cached from a pre-update version during `xiaoban update`.
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.hermes/xiaoban-agent  -> /home/alice/.hermes/xiaoban-agent
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print(
        ...
        "/opt/hermes/docker/entrypoint.sh before the Xiaoban command."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Xiaoban Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
def _setup_weixin():
    ...
    print_info(
        "  3. Xiaoban will store the returned account_id/token in ~/.hermes/.env."
    )
    print_info(
    ...
    if any_configured:
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                print_info(
                    "  For persistence:   tmux new -s hermes 'xiaoban gateway run'"
                )
                print_info(
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            if is_wsl():
                ...
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s hermes 'xiaoban gateway run'"
                )
                print()
            ...
            print(
                "  tmux new -s hermes 'xiaoban gateway run'         # persistent via tmux"
            )
            print(
                "  nohup xiaoban gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    print(
                        "  nohup xiaoban gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hermes 'xiaoban gateway run'         # persistent via tmux"
