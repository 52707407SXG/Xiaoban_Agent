...
Set your provider with `xiaoban model` or by editing `~/.hermes/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Xiaoban-Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.hermes/`.
...
tmux new -s hermes 'xiaoban gateway run'
# Reattach later: tmux attach -t hermes

# Option 3: Background via nohup
nohup xiaoban gateway run > ~/.hermes/logs/gateway.log 2>&1 &
```
...
cd ~/.hermes/xiaoban-agent && uv pip install -e ".[mcp]"
...
```bash
rsync -av --exclude='xiaoban-agent' ~/.hermes/ newmachine:~/.hermes/
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/52707407SXG/Xiaoban-Agent/issues)
