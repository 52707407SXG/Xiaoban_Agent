...
``xiaoban_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``XIAOBAN_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # XIAOBAN_* handling (#27303): only the operational allowlist passes;
        # every other XIAOBAN_* is dropped (the broad prefix was removed).
        "XIAOBAN_HOME": "/home/alice/.xiaoban",        # allowlisted → kept
        "XIAOBAN_PROFILE": "default",                 # allowlisted → kept
        "XIAOBAN_INTERACTIVE": "1",                   # not allowlisted → dropped
        "XIAOBAN_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "XIAOBAN_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``xiaoban_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``xiaoban_tools.py`` and ``script.py`` writes in
    ...
    def test_file_rpc_stub_uses_utf8(self):
        ...
        from tools.code_execution_tool import generate_xiaoban_tools_module
        stub = generate_xiaoban_tools_module(["terminal"], transport="file")
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        from tools.code_execution_tool import generate_xiaoban_tools_module
        import tempfile, ast
        stub = generate_xiaoban_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_xiaoban_tools_module(["terminal"], transport="uds")
...
# let the child import xiaoban_tools, a user script that printed non-ASCII
