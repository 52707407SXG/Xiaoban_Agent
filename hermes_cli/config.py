...
#   ``HERMES_ENV`` — Xiaoban runtime location flags. Writing these into
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/xiaoban-agent`` image bakes a ``docker``
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade xiaoban-agent"
    if method == "docker":
        return "docker pull nousresearch/xiaoban-agent:latest"
    if method == "pip":
...
Xiaoban-Agent runs as a published image (nousresearch/xiaoban-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/xiaoban-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate xiaoban-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/xiaoban-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/xiaoban-agent/tags
...
DEFAULT_CONFIG = {
    ...
    # AGENTS.md, CLAUDE.md, .hermes.md, or .cursorrules before Xiaoban applies
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Xiaoban-Agent*" header.
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://xiaoban-agent.nousresearch.com/docs/api/model-catalog.json",
    ...
    "updates": {
        # Run a full ``xiaoban backup``-style zip of HERMES_HOME before every
        # ``xiaoban update``.  Backups land in ``<HERMES_HOME>/backups/`` and
...
def show_config():
    ...
    print(color("│              ⚕ Xiaoban Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
