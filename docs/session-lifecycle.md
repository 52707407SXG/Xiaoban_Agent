...
The canonical transcript store is SQLite via `SessionDB` (from `xiaoban_state`). The
...
Counts consecutive restarts via a JSON file (`{XIAOBAN_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `xiaoban update`, `xiaoban gateway restart`,
or `/restart`.
