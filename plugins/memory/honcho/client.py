...
Resolution order for config file:
  1. $XIAOBAN_HOME/honcho.json  (instance-local, enables isolated Xiaoban instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Xiaoban profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Xiaoban profile.

    Resolution order:
      1. XIAOBAN_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``xiaoban.<profile>``
      3. Fallback: ``"xiaoban"`` (default profile)
    """
    explicit = os.environ.get("XIAOBAN_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $XIAOBAN_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.xiaoban/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_xiaoban_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_xiaoban_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Xiaoban system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.xiaoban
    ...
    def from_global_config(
        ...
        Resolution: $XIAOBAN_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Xiaoban profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.xiaoban block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Xiaoban session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Xiaoban session title (from /title command; non-per-session)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'xiaoban honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        # the "go run xiaoban honcho setup" hint they used to.
        try:
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `xiaoban honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                xiaoban_cfg = load_config()
                honcho_cfg = xiaoban_cfg.get("honcho", {})
