...
the actual `setup.sh` that creates the project workspace, configures Xiaoban
...
2. **Create profiles** — `xiaoban profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.xiaoban/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `xiaoban kanban create` assigned to the director
...
```bash
xiaoban profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.xiaoban/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.xiaoban/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Xiaoban
...
Each profile gets a `SOUL.md` at `~/.xiaoban/profiles/<name>/SOUL.md` that
...
```bash
xiaoban kanban create "Direct production of <video title>" \
    --assignee director \
...
the Xiaoban `.env` (`${XIAOBAN_HOME:-$HOME/.xiaoban}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _xiaoban_env="${XIAOBAN_HOME:-$HOME/.xiaoban}/.env"
    if grep -q "^${var}=" "$_xiaoban_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_xiaoban_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_xiaoban_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY xiaoban ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY xiaoban OPENROUTER_API_KEY || exit 1
# ...
```
