...
from xiaoban_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from xiaoban_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("XIAOBAN_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_xiaoban_ca_bundle_env_falls_back(self, monkeypatch):
        from xiaoban_cli.auth import _resolve_verify

        monkeypatch.setenv("XIAOBAN_CA_BUNDLE", "/nonexistent/xiaoban-ca.pem")
...
def test_resolve_nous_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    ...
    import xiaoban_cli.auth as auth_mod
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XIAOBAN_HOME", str(xiaoban_home))
    monkeypatch.setenv("XIAOBAN_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.INFO, logger="xiaoban_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_nous_auth_status_pool_opaque_key_is_not_inference_credential(tmp_path, monkeypatch):
    from xiaoban_cli.auth import get_nous_auth_status, invalidate_nous_auth_status_cache
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    xiaoban_home = tmp_path / "xiaoban"
    _setup_nous_auth(xiaoban_home, access_token="at-123")
    monkeypatch.setenv("XIAOBAN_HOME", str(xiaoban_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    xiaoban_home = tmp_path / "xiaoban"
    _setup_nous_auth(xiaoban_home, access_token="at-fresh")
    monkeypatch.setenv("XIAOBAN_HOME", str(xiaoban_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("xiaoban_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_refreshed_jwt_lacks_invoke_scope(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("xiaoban_cli.auth._refresh_access_token", _fake_refresh_access_token)
...
def test_terminal_refresh_failure_quarantines_tokens(
    ...
    from xiaoban_cli import auth as auth_mod
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    xiaoban_home = tmp_path / "xiaoban"
    _setup_nous_auth(xiaoban_home, refresh_token="refresh-old")
    monkeypatch.setenv("XIAOBAN_HOME", str(xiaoban_home))
    from agent.credential_pool import load_pool
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `xiaoban model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return xiaoban_home, config_path, auth_path
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from xiaoban_cli.auth import PROVIDER_REGISTRY, _login_nous

        xiaoban_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `xiaoban auth add nous`
    ...
    """
    from xiaoban_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Xiaoban is not logged into Nous Portal").
    ...
    # doesn't raise "Xiaoban is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `xiaoban auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Xiaoban must surface an
    ...
    re-authenticate" string led users to report this as a Xiaoban persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from xiaoban_cli.auth import _refresh_access_token
    ...
    assert "xiaoban auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect XIAOBAN_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("XIAOBAN_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without XIAOBAN_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.xiaoban/shared/`` across CI runs.
    """
    from xiaoban_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("XIAOBAN_SHARED_AUTH_DIR", raising=False)
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """XIAOBAN_SHARED_AUTH_DIR must redirect the path."""
    from xiaoban_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("XIAOBAN_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from xiaoban_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from xiaoban_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from xiaoban_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `xiaoban auth add nous
    ...
    xiaoban_home = tmp_path / "xiaoban"
    xiaoban_home.mkdir(parents=True, exist_ok=True)
    (xiaoban_home / "auth.json").write_text(
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from xiaoban_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "xiaoban-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different XIAOBAN_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("XIAOBAN_HOME", str(profile_a))
    ...
    # Profile B: fresh XIAOBAN_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("XIAOBAN_HOME", str(profile_b))
