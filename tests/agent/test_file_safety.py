...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_xiaoban_env(self):
        """Xiaoban' own .env inside XIAOBAN_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: xiaoban internal .env is in ~/.xiaoban/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.xiaoban/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.xiaoban/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Xiaoban cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        xiaoban_home = tmp_path / ".xiaoban"
        cache = xiaoban_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._xiaoban_home_path", return_value=xiaoban_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Xiaoban cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        xiaoban_home = tmp_path / ".xiaoban"
        hub = xiaoban_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_xiaoban_home(self, tmp_path):
        """The env basename guard does not depend on XIAOBAN_HOME resolution."""
        xiaoban_home = tmp_path / ".xiaoban"
        xiaoban_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        xiaoban_home = tmp_path / ".xiaoban"
        cache = xiaoban_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
