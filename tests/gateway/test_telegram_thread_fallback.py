...
Supergroup forum topics route with ``message_thread_id``. Xiaoban-created
...
async def test_send_typing_attempts_api_call_for_dm_topic_reply_fallback():
    """Xiaoban-created DM topic lanes should still attempt scoped typing.
...
@pytest.mark.asyncio
async def test_send_uses_reply_fallback_for_xiaoban_dm_topics():
    """Xiaoban-created Telegram DM topics route with thread id plus reply anchor."""
    adapter = _make_adapter()
    call_log = []
...
async def test_send_uses_metadata_reply_fallback_for_streaming_dm_topics():
    """Metadata-only sends still stay in Xiaoban-created Telegram DM topics."""
    adapter = _make_adapter()
    call_log = []
