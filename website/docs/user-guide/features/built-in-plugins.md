...
Session grouping keys off the Hermes session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `xiaoban chat` session lives under one Langfuse session.

**Verify:**

```bash
hermes plugins list                 # observability/langfuse should show "enabled"
xiaoban chat -q "hello"              # check the Langfuse UI for a "Hermes turn" trace
```
