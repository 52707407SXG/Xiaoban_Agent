...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the XIAOBAN_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Xiaoban runtime / deployment
    "XIAOBAN_HOME", "XIAOBAN_PROFILE", "XIAOBAN_GATEWAY_LOCK_DIR",
    "XIAOBAN_MAX_ITERATIONS", "XIAOBAN_MAX_TOKENS", "XIAOBAN_API_TIMEOUT",
    "XIAOBAN_REDACT_SECRETS", "XIAOBAN_NOUS_TIMEOUT_SECONDS",
    "_XIAOBAN_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "XIAOBAN_KANBAN_DB", "XIAOBAN_KANBAN_WORKSPACES_ROOT", "XIAOBAN_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "XIAOBAN_KANBAN_",
    "XIAOBAN_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(xiaoban_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(xiaoban_home) / ".env")
