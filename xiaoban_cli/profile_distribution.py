"""Profile distributions — shareable, packaged Xiaoban profiles via git.

A distribution is a Xiaoban profile published as a git repository (or
...
* ``xiaoban profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``xiaoban skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``xiaoban profile``, not a parallel tree):

    xiaoban profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "xiaoban_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".xiaoban_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "xiaoban-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            xiaoban_requires=str(data.get("xiaoban_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.xiaoban_requires:
            out["xiaoban_requires"] = self.xiaoban_requires
        if self.author:
...
def check_xiaoban_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Xiaoban {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``xiaoban profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Xiaoban profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Xiaoban distribution."
        )

    # Version check up-front so we fail fast
    check_xiaoban_requires(manifest.xiaoban_requires, xiaoban_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.xiaoban).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="xiaoban_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `xiaoban profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`xiaoban profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="xiaoban_dist_update_") as tmp:
        plan = plan_install(
