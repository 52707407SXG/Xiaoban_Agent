...
class TestUnifiedDashboardRouting:
    def test_profile_launch_attaches_to_running_dashboard(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "xiaoban_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        # XIAOBAN_HOME.  For a standard install (XIAOBAN_HOME unset) that root is
        # the platform-native default (~/.xiaoban), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from xiaoban_constants import get_default_xiaoban_root
        assert env.get("XIAOBAN_HOME") == str(get_default_xiaoban_root())

    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        """In the Docker layout (XIAOBAN_HOME=/opt/data, profiles under
        /opt/data/profiles/<name>) the reroute must pin the child to the
        machine root /opt/data — NOT drop XIAOBAN_HOME.

        Dropping it makes the child fall back to $HOME/.xiaoban
        (= /opt/data/.xiaoban), an empty auto-seeded home, so the dashboard
        ...
        """
        monkeypatch.setenv("XIAOBAN_HOME", "/opt/data/profiles/oracle")
        monkeypatch.setattr(
            "xiaoban_cli.profiles.get_active_profile_name", lambda: "oracle"
        )
        monkeypatch.setattr(main_mod, "_dashboard_listening", lambda host, port: False)
        execs = []
    ...
    def test_default_profile_launch_skips_routing(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "xiaoban_cli.profiles.get_active_profile_name", lambda: "default"
        )
        listening_calls = []
        monkeypatch.setattr(
            main_mod, "_dashboard_listening",
    ...
    def test_dashboard_starts_mcp_discovery_for_ws_backend(self, main_mod, monkeypatch):
        ...
        monkeypatch.delenv("XIAOBAN_WEB_DIST", raising=False)
        ...
        calls = []
        monkeypatch.setattr(
            "xiaoban_cli.mcp_startup.start_background_mcp_discovery",
            lambda **kwargs: calls.append(kwargs),
        )
        monkeypatch.setitem(
            sys.modules,
            "xiaoban_cli.web_server",
