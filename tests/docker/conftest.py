...
Override the image with ``XIAOBAN_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("XIAOBAN_TEST_IMAGE", "xiaoban-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``XIAOBAN_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("XIAOBAN_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged xiaoban user
# ---------------------------------------------------------------------------
#
# Background: every Xiaoban runtime path inside the container drops to UID
# 10000 (the ``xiaoban`` user) via ``s6-setuidgid xiaoban``. ``docker exec``
...
# fails (PermissionError swallowed) for xiaoban, so a test that ran as root
...
# helpers below run every probe under ``-u xiaoban`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: xiaoban).
