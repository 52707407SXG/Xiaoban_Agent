...
function markerPath(xiaobanHome) {
  return path.join(xiaobanHome, '.xiaoban-update-in-progress')
}
 ...
 * clock for tests.
 */
function readLiveUpdateMarker(xiaobanHome, { kill, now = Date.now, maxAgeMs = UPDATE_MARKER_MAX_AGE_MS } = {}) {
  const file = markerPath(xiaobanHome)
  let raw
  try {
