...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.XIAOBAN_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Xiaoban on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via XIAOBAN_TUI_TRUECOLOR=1.
