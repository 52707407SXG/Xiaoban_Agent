...
The shim (docker/xiaoban-exec-shim.sh, installed at /opt/xiaoban/bin/xiaoban)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> xiaoban login` would write /opt/data/auth.json as
...
failed: Xiaoban is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> xiaoban …`` (defaulting to root) gets dropped to the
   xiaoban user before the real binary runs.
2. ``docker exec --user xiaoban <c> xiaoban …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $XIAOBAN_HOME from a ``docker exec`` session land
   as xiaoban:xiaoban — the actual user-visible invariant.
4. The XIAOBAN_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def test_shim_drops_root_to_xiaoban_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `xiaoban` with a Python-style `-c` shim equivalent — there's no
    pure-xiaoban "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `xiaoban` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `xiaoban`
    do anything that writes to disk, then check the file's owner.

    Use `xiaoban config set` which writes config.yaml under XIAOBAN_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "xiaoban", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by xiaoban, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "xiaoban:xiaoban", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected xiaoban:xiaoban. "
        "The shim did not drop privileges before invoking xiaoban."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user xiaoban already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "xiaoban", sleep_container,
         "xiaoban", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user xiaoban failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up xiaoban:xiaoban — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    """XIAOBAN_DOCKER_EXEC_AS_ROOT=1 should suppress the privilege drop.
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", "XIAOBAN_DOCKER_EXEC_AS_ROOT=1",
         sleep_container,
         "xiaoban", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "root:root", (
        f"With XIAOBAN_DOCKER_EXEC_AS_ROOT=1, expected root:root, "
...
def test_shim_opt_out_strict_truthiness(
    ...
    Strict truthiness so a typo (``XIAOBAN_DOCKER_EXEC_AS_ROOT=0``) doesn't
    silently keep the user as root. Mirrors the policy used by
    ``XIAOBAN_GATEWAY_NO_SUPERVISE`` in #33583.
    """
    subprocess.run(
        ["docker", "exec", "--user", "root", sleep_container,
         "rm", "-f", "/opt/data/config.yaml"],
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", f"XIAOBAN_DOCKER_EXEC_AS_ROOT={falsy_value}",
         sleep_container,
         "xiaoban", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "xiaoban:xiaoban", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected xiaoban:xiaoban"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/xiaoban/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid xiaoban xiaoban <args>` which resolves `xiaoban`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> xiaoban login`` (root) wrote
    ...
    "Xiaoban is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `xiaoban config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is xiaoban-readable.

    Specifically: pretend the operator ran `xiaoban login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the xiaoban UID. We use `xiaoban auth list` since that touches the
    ...
    # `xiaoban auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `xiaoban config set` first to
    # provoke a write into XIAOBAN_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "xiaoban", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # XIAOBAN_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "xiaoban", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the xiaoban user "
