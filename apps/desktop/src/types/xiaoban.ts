...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Xiaoban doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in xiaoban_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `xiaoban model` uses — instead of
...
export interface XiaobanConfig {
  agent?: {
...
export type XiaobanConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `xiaoban model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
export interface ToolProvider {
   ...
   *  CLI `xiaoban tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Xiaoban); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/xiaoban/update/check` — the backend's own update state.
