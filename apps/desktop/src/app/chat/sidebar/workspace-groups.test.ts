...
import type { XiaobanWorktreeInfo } from '@/global'
import type { SessionInfo } from '@/types/xiaoban'
...
describe('workspaceGroupsFor', () => {
  it('groups by full cwd, not by basename — same-named folders are separate groups', () => {
    const groups = workspaceGroupsFor(
      [makeSession('/a/xiaoban-agent/apps/desktop'), makeSession('/a/xiaoban-agent-wt-rtl/apps/desktop')],
      'No workspace'
    )
  ...
  it('disambiguates colliding basenames by walking up the path', () => {
    expect(
      labels([makeSession('/a/xiaoban-agent/apps/desktop'), makeSession('/a/xiaoban-agent-wt-rtl/apps/desktop')])
    ).toEqual(['xiaoban-agent/apps/desktop', 'xiaoban-agent-wt-rtl/apps/desktop'])
  })

  it('leaves a unique basename as its short label', () => {
    expect(labels([makeSession('/a/xiaoban-agent/apps/desktop'), makeSession('/b/heval-py')])).toEqual([
      'desktop',
      'heval-py'
    ])
  })
...
const info = (over: Partial<XiaobanWorktreeInfo> & Pick<XiaobanWorktreeInfo, 'repoRoot' | 'worktreeRoot'>): XiaobanWorktreeInfo => ({
  branch: null,
...
describe('workspaceTreeFor', () => {
  it('heuristic nests `<repo>-wt-<branch>` under its sibling repo', () => {
    const tree = workspaceTreeFor(
      [makeSession('/www/xiaoban-agent'), makeSession('/www/xiaoban-agent-wt-rtl')],
      'No workspace'
    )

    expect(tree).toHaveLength(1)
    expect(tree[0].label).toBe('xiaoban-agent')
    expect(tree[0].groups.map(g => g.label).sort()).toEqual(['xiaoban-agent', 'rtl'])
  })
  ...
  it('git metadata is authoritative — worktrees group by repoRoot regardless of directory naming', () => {
    const resolver: WorktreeResolver = cwd => {
      if (cwd === '/www/xiaoban-agent') {
        return info({ repoRoot: '/www/xiaoban-agent', worktreeRoot: '/www/xiaoban-agent', isMainWorktree: true, branch: 'main' })
      }

      if (cwd === '/elsewhere/ha-rtl') {
        return info({ repoRoot: '/www/xiaoban-agent', worktreeRoot: '/elsewhere/ha-rtl', branch: 'rtl' })
      }
    ...
    const tree = workspaceTreeFor(
      [makeSession('/www/xiaoban-agent'), makeSession('/elsewhere/ha-rtl')],
      'No workspace',
      resolver
    )
    ...
    expect(tree[0].groups.map(g => g.label)).toEqual(['xiaoban-agent', 'rtl'])
  })
