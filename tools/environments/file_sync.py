...
_FORCE_SYNC_ENV = "XIAOBAN_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.xiaoban") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.xiaoban to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``XIAOBAN_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, xiaoban_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.xiaoban/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="xiaoban-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.xiaoban/skills/a.md`` →
        ``~/.xiaoban/skills/a.md``, a new remote file at
        ``/root/.xiaoban/skills/b.md`` maps to ``~/.xiaoban/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
