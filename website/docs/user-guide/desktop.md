...
The [manual update process](https://github.com/52707407SXG/Xiaoban-Agent/docs/getting-started/updating) also works with the GUI.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Xiaoban Desktop to a remote backend](./features/web-dashboard.md#connecting-hermes-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Xiaoban Desktop](../reference/environment-variables.md#web-dashboard--hermes-desktop).
...
rm "$HOME/.hermes/xiaoban-agent/.hermes-bootstrap-complete"

# Rebuild a broken Python venv (macOS/Linux)
rm -rf "$HOME/.hermes/xiaoban-agent/venv"
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.hermes/xiaoban-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
