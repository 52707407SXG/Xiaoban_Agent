...
xiaoban kanban watch --tenant <project-slug>

# Snapshot of the board
xiaoban kanban list --tenant <project-slug>
xiaoban kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
xiaoban kanban stats --tenant <project-slug>

# Visual dashboard (browser)
xiaoban dashboard

# Inspect a specific task (includes comments + events)
xiaoban kanban show <task-id>

# Follow a single task's event stream
xiaoban kanban tail <task-id>
```

Verify available subcommands with `xiaoban kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `xiaoban kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `xiaoban kanban block <id>` then `xiaoban kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `xiaoban kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `xiaoban kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `xiaoban profile list` (profile exists?), `xiaoban status` (gateway/dispatcher up?), and verify tenant. |
...
xiaoban kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
xiaoban kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(xiaoban kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
xiaoban kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
xiaoban kanban block <task-id>
xiaoban kanban archive <task-id>

# Diagnose what happened
xiaoban kanban show <task-id>      # task body, comments, recent events
xiaoban kanban tail <task-id>      # follow the live event stream
xiaoban kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `xiaoban kanban create` for the director
...
while true; do
    clear
    xiaoban kanban list --tenant <slug>
    echo "---"
    xiaoban kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `xiaoban kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `xiaoban profile list` and start
