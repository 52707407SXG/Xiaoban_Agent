...
# ``sessions_dir / f"{session_id}.json"`` in xiaoban_state, request-dump
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `xiaoban tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `xiaoban tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_xiaoban_home()}/cron/output/)"
    )
