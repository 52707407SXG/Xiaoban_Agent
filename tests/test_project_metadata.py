...
def test_matrix_extra_not_in_all():
    ...
    assert "matrix" in optional_dependencies, "[matrix] extra must still exist for explicit `pip install xiaoban-agent[matrix]`"
...
def test_lazy_installable_extras_excluded_from_all():
    ...
    for extra in lazy_covered_extras:
        offending = [
            ...
            if f"xiaoban-agent[{extra}]" in spec
        ]
...
def test_pyproject_aiohttp_pins_match_lazy_slack_pin():
    ...
    If the two drift, `xiaoban update` resolves the pyproject pin and
    ...
    assert not mismatches, (
        ...
        "LAZY_DEPS['platform.slack'] to avoid xiaoban update downgrading "
...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `xiaoban update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `xiaoban update` downgrades the "
...
def test_dingtalk_extra_includes_qrcode_for_qr_auth():
    """DingTalk's QR-code device-flow auth (xiaoban_cli/dingtalk_auth.py)
