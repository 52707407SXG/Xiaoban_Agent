...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_xiaoban_home_override(self, tmp_path):
        """Auth lookup should read from XIAOBAN_HOME/auth.json, not ~/.xiaoban/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".xiaoban").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``xiaoban tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "xiaoban_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
