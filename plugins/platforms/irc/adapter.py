"""
IRC Platform Adapter for Xiaoban-Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Xiaoban agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "xiaoban-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Xiaoban-Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Xiaoban-Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: xiaoban_, xiaoban_1, xiaoban_2...
...
def check_requirements() -> bool:
    ...
    # xiaoban setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `xiaoban gateway setup` flow for the IRC platform.

    Lazy-imports ``xiaoban_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Xiaoban to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #xiaoban — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.xiaoban/.env")
    print_info("Restart the gateway for changes to take effect: xiaoban gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "xiaoban-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "xiaoban-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Xiaoban-Agent (cron)")
