...
def _xai_user_agent() -> str:
    try:
        ...
        return xiaoban_xai_user_agent()
    except Exception:
        return "xiaoban-agent/video_gen"
...
class XAIVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        # hook (``xiaoban_cli/tools_config.py``) so the picker doesn't blindly
    ...
    async def _generate_async(
        ...
        *,
        ...
        if not api_key:
            return error_response(
                error=(
                    "No xAI credentials found. Sign in via `xiaoban auth add xai-oauth` "
