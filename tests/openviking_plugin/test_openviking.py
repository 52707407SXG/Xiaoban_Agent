...
class TestOpenVikingSummaryUriNormalization:
    def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/xiaoban/.overview.md") == "viking://user/xiaoban"
        ...
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/xiaoban/memories/profile.md") == "viking://user/xiaoban/memories/profile.md"
...
class TestOpenVikingSkillQuerySafety:
    ...
    def test_skill_markers_match_xiaoban_scaffolding(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XIAOBAN_BUNDLES_DIR", str(bundles_dir))
...
class TestOpenVikingTurnConversion:
            ...
            turn = [
                ...
                },
                {
                    ...
                    "content": json.dumps({
                        "results": [
                            {
                                "uri": "viking://user/xiaoban/memories/context",
    ...
    def test_messages_to_openviking_batch_adds_assistant_peer_id_when_requested(self):
        ...
        batch = OpenVikingMemoryProvider._messages_to_openviking_batch(
            turn,
            assistant_peer_id="xiaoban",
        )

        assert batch == [
            {"role": "user", "parts": [{"type": "text", "text": "hello"}]},
            {"role": "assistant", "parts": [{"type": "text", "text": "answer"}], "peer_id": "xiaoban"},
        ]
...
class TestOpenVikingRead:
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    (("uri", "viking://user/xiaoban"),),
                ): {"result": {"content": "overview text"}},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/xiaoban/.overview.md", "level": "overview"}))

        assert result["uri"] == "viking://user/xiaoban/.overview.md"
        assert result["resolved_uri"] == "viking://user/xiaoban"
        ...
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/content/read",
                    (("uri", "viking://user/xiaoban/memories/profile.md"),),
                ): {"result": "full text"},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/xiaoban/memories/profile.md", "level": "full"}))

        assert result["uri"] == "viking://user/xiaoban/memories/profile.md"
        assert result["resolved_uri"] == "viking://user/xiaoban/memories/profile.md"
    ...
    def test_overview_file_uri_routes_straight_to_content_read_via_stat_probe(self):
        ...
        provider = OpenVikingMemoryProvider()
        file_uri = "viking://user/xiaoban/memories/entities/mem_abc.md"
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/fs/stat",
    ...
    def test_overview_dir_uri_skips_stat_when_pseudo_summary(self):
        ...
        assert provider._client.calls == [
            ("/api/v1/content/overview", {"uri": "viking://user/xiaoban"}),
        ]
    ...
    def test_overview_directory_uri_uses_stat_probe_then_overview(self):
        ...
        provider = OpenVikingMemoryProvider()
        dir_uri = "viking://user/xiaoban/memories"
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/fs/stat",
    ...
    def test_summary_uri_error_does_not_fallback_and_raises(self):
        ...
        try:
            provider._tool_read({"uri": "viking://user/xiaoban/.overview.md", "level": "overview"})
...
class TestOpenVikingBrowse:
    def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    "result": {
                        "entries": [
                            {"name": "memories", "uri": "viking://user/xiaoban/memories", "type": "dir"},
                            {"rel_path": "profile.md", "uri": "viking://user/xiaoban/memories/profile.md", "isDir": False, "abstract": "Profile"},
                        ]
                    }
                },
            }
        ...
        result = json.loads(provider._tool_browse({"action": "list", "path": "viking://user/xiaoban"}))

        assert result["path"] == "viking://user/xiaoban"
        assert result["entries"] == [
            {"name": "memories", "uri": "viking://user/xiaoban/memories", "type": "dir", "abstract": ""},
            {"name": "profile.md", "uri": "viking://user/xiaoban/memories/profile.md", "type": "file", "abstract": "Profile"},
        ]
        assert provider._client.calls == [(
            "/api/v1/fs/ls",
            {"uri": "viking://user/xiaoban"},
