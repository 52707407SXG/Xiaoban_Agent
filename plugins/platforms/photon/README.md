...
This plugin connects Xiaoban-Agent to iMessage (and other Spectrum
...
TypeScript-only, Xiaoban runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Xiaoban-Sidecar-Token`.
...
There is no separate `login` command; like every other Xiaoban channel,
...
Run `xiaoban photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.xiaoban/.env` (the same place every other
...
Management metadata lives in `~/.xiaoban/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `xiaoban photon telemetry on\|off` (restart the gateway to apply) |
...
5. Verify end-to-end: `xiaoban photon status`, a DM and a group roundtrip,
