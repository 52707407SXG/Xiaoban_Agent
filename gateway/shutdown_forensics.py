...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if xiaoban_home_str:
            takeover_path = Path(xiaoban_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(xiaoban_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded xiaoban-agent but never re-ran ``xiaoban setup`` to regenerate
    ...
    cgroup_path = ""
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../xiaoban-gateway.service"
    ...
    # on which manager actually owns the unit.  Prefer the manager indicated
    # by cgroup so a same-named user unit cannot shadow a system service.
    ...
    # constant in xiaoban_cli/gateway.py.
    headroom = 30.0
