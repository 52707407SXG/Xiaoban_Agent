...
- **`xiaoban update`** — 更新 Hermes 本身时，如果 `cua-driver` 在 PATH 中，更新结束时会重新运行上游安装程序。对非 macOS 用户及未安装 cua-driver 的用户无操作。
...
**`computer_use backend unavailable: cua-driver is not installed`** — 运行 `hermes computer-use install` 获取 cua-driver 二进制文件，或运行 `xiaoban tools` 并启用 Computer Use 工具集。
...
- [通用技能：`macos-computer-use`](https://github.com/52707407SXG/Xiaoban-Agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
