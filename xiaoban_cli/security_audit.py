"""On-demand supply-chain audit for Xiaoban-Agent installs.

Scans three surfaces a Xiaoban user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Xiaoban venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.xiaoban/plugins``
...
def _discover_plugins(xiaoban_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.xiaoban/plugins``.
...
def run_audit(
    *,
    ...
    home = xiaoban_home or Path(get_xiaoban_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, xiaoban_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(xiaoban_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `xiaoban security audit`."""
    home = Path(get_xiaoban_home())
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, xiaoban_home=home
    )
    if total == 0:
