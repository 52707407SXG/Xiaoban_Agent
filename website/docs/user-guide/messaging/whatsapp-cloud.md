...
3. Start the Hermes gateway: `xiaoban gateway`.
...
Message-template support (the workaround for outside-window sends) is not yet implemented in Hermes. If you need it, please [open an issue](https://github.com/52707407SXG/Xiaoban-Agent/issues) — it's planned but waiting on a clear demand signal.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `xiaoban tools list` and verify the platform is using `hermes-whatsapp` (the default Cloud adapter toolset, same as Baileys).
