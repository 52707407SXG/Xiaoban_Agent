...
class TestIsBotMentioned:
    ...
    def test_full_user_id_in_body(self):
        assert self.adapter._is_bot_mentioned("hey @xiaoban:example.org help")

    def test_localpart_in_body(self):
        assert self.adapter._is_bot_mentioned("xiaoban can you help?")

    def test_localpart_case_insensitive(self):
        assert self.adapter._is_bot_mentioned("XIAOBAN can you help?")

    def test_matrix_pill_in_formatted_body(self):
        html = '<a href="https://matrix.to/#/@xiaoban:example.org">Xiaoban</a> help'
        assert self.adapter._is_bot_mentioned("Xiaoban help", html)
    ...
    def test_partial_localpart_no_match(self):
        # "xiaobanbot" should not match word-boundary check for "xiaoban"
        assert not self.adapter._is_bot_mentioned("xiaobanbot is here")
    ...
    def test_m_mentions_user_ids_authoritative(self):
        ...
        assert self.adapter._is_bot_mentioned(
            "please reply",  # no @xiaoban anywhere in body
            mention_user_ids=["@xiaoban:example.org"],
        )

    def test_m_mentions_user_ids_with_body_mention(self):
        """Both m.mentions and body mention — should still be True."""
        assert self.adapter._is_bot_mentioned(
            "hey @xiaoban:example.org help",
            mention_user_ids=["@xiaoban:example.org"],
        )
...
class TestStripMention:
    ...
    def test_strip_full_user_id(self):
        result = self.adapter._strip_mention("@xiaoban:example.org help me")
        assert result == "help me"

    def test_localpart_preserved(self):
        """Bare localpart (no @) is preserved — avoids false positives in paths."""
        result = self.adapter._strip_mention("xiaoban help me")
        assert result == "xiaoban help me"

    def test_localpart_in_path_preserved(self):
        """Localpart inside a file path must not be damaged."""
        result = self.adapter._strip_mention("read /home/xiaoban/config.yaml")
        assert result == "read /home/xiaoban/config.yaml"
    ...
    def test_does_not_strip_bare_localpart_word(self):
        # Regression: plain words like "Xiaoban-Agent" should not be mutated.
        result = self.adapter._strip_mention("Xiaoban-Agent")
        assert result == "Xiaoban-Agent"

    def test_strip_returns_empty_for_mention_only(self):
        result = self.adapter._strip_mention("@xiaoban:example.org")
        assert result == ""
...
async def test_require_mention_default_processes_mentioned(monkeypatch):
    ...
    adapter = _make_adapter()
    event = _make_event("@xiaoban:example.org help me")
...
async def test_require_mention_html_pill(monkeypatch):
    ...
    adapter = _make_adapter()
    formatted = '<a href="https://matrix.to/#/@xiaoban:example.org">Xiaoban</a> help'
    event = _make_event("Xiaoban help", formatted_body=formatted)
...
async def test_dm_preserves_localpart_in_body(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("xiaoban help me")

    await adapter._on_room_message(event)
    adapter.handle_message.assert_awaited_once()
    msg = adapter.handle_message.await_args.args[0]
    assert msg.text == "xiaoban help me"
...
async def test_bare_mention_passes_empty_string(monkeypatch):
    ...
    adapter = _make_adapter()
    event = _make_event("@xiaoban:example.org")
...
async def test_require_mention_disabled_skips_stripping(monkeypatch):
    ...
    adapter.handle_message.assert_awaited_once()
    msg = adapter.handle_message.await_args.args[0]
    assert msg.text == "@xiaoban:example.org help me"
...
async def test_dm_mention_thread_disabled_by_default(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("@xiaoban:example.org help me", event_id="$dm1")
...
async def test_dm_mention_thread_preserves_existing_thread(monkeypatch):
    ...
    event = _make_event("@xiaoban:example.org help me", thread_id="$existing_thread")
...
async def test_dm_mention_thread_tracks_participation(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("@xiaoban:example.org help", event_id="$dm1")
