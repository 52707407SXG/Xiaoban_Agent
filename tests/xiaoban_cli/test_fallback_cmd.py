"""Tests for `xiaoban fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate XIAOBAN_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".xiaoban" / "config.yaml"
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from xiaoban_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "nous", "model": "Xiaoban-4-Llama-3.1-405B"},
            ]
        }
    ...
    def test_merges_new_and_legacy_formats(self):
        ...
        cfg = {
            ...
            "fallback_model": {"provider": "nous", "model": "Xiaoban-4"},
        }
        assert _read_chain(cfg) == [
            {"provider": "openrouter", "model": "anthropic/claude-sonnet-4.6"},
            {"provider": "nous", "model": "Xiaoban-4"},
        ]
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from xiaoban_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("xiaoban_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("xiaoban_cli.main._require_tty"):
            from xiaoban_cli.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    def test_remove_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from xiaoban_cli.fallback_cmd import cmd_fallback_remove
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "nous / Xiaoban-4")
        with patch("xiaoban_cli.setup._curses_prompt_choice", return_value=1):
            from xiaoban_cli.fallback_cmd import cmd_fallback_remove
            cmd_fallback_remove(types.SimpleNamespace())
...
class TestClearCommand:
    def test_clear_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from xiaoban_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    """Verify `xiaoban fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "xiaoban_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
