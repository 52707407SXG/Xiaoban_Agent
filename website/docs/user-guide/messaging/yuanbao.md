...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Xiaoban-Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Xiaoban to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.xiaoban/.env`:
...
All standard Xiaoban commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.xiaoban/logs/gateway.log`
...
4. Consider enabling verbose logging: `XIAOBAN_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Xiaoban automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
XIAOBAN_LOG_LEVEL=debug xiaoban gateway
```
...
```bash
xiaoban chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
