...
load_xiaoban_dotenv(
    xiaoban_home=_xiaoban_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.xiaoban/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_xiaoban_home, "logs", "tui_gateway_crash.log")
...
try:
    _slash_timeout = float(os.environ.get("XIAOBAN_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("XIAOBAN_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("XIAOBAN_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent XiaobanCLI subprocess for slash commands."""
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `xiaoban --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("XIAOBAN_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `xiaoban --tui`, so it must NOT count as dead here (doing
...
# ``profile`` on those calls; we open that profile's db and bind its XIAOBAN_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_xiaoban_home).resolve():
        return None
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `xiaoban` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # XIAOBAN_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_xiaoban_home_override(profile_home)
                try:
                    from xiaoban_state import SessionDB
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from xiaoban_cli.runtime_provider import canonical_custom_identity
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _xiaoban_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_xiaoban_home_override()
        home = override if isinstance(override, str) and override else _xiaoban_home
...
def _apply_managed(cfg: dict) -> dict:
    ...
    xiaoban_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _save_cfg(cfg: dict):
    ...
    path = _xiaoban_home / "config.yaml"
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["XIAOBAN_GATEWAY_SESSION"] = "1"
...
def resolve_skin() -> dict:
    try:
        from xiaoban_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("XIAOBAN_MODEL", "")
        or os.environ.get("XIAOBAN_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _config_model_target() -> tuple[str, str]:
    ...
    config.yaml wins over XIAOBAN_MODEL / XIAOBAN_INFERENCE_MODEL here, the
    reverse of `_resolve_model()`'s startup order. Those env vars are a
    provision-time seed (hosted instances set XIAOBAN_INFERENCE_MODEL in the
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    model = _resolve_model()
    explicit_provider = os.environ.get("XIAOBAN_TUI_PROVIDER", "").strip()
    ...
    try:
        from xiaoban_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("XIAOBAN_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("XIAOBAN_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("XIAOBAN_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Xiaoban): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `xiaoban --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] XIAOBAN_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from xiaoban_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown XIAOBAN_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in XIAOBAN_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid XIAOBAN_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid XIAOBAN_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _persist_model_switch(result) -> None:
    from xiaoban_cli.config import save_config
...
def _apply_model_switch(
    ...
    *,
    ...
    custom_provs = None
    try:
        from xiaoban_cli.config import get_compatible_custom_providers, load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (XIAOBAN_MODEL /
    # XIAOBAN_INFERENCE_MODEL / XIAOBAN_TUI_PROVIDER / XIAOBAN_INFERENCE_PROVIDER)
...
def _get_usage(agent) -> dict:
    ...
    # XIAOBAN_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("XIAOBAN_DEV_CREDITS")):
        try:
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from xiaoban_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from xiaoban_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from xiaoban_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from xiaoban_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("XIAOBAN_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("XIAOBAN_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``XiaobanCLI.__init__`` and ``gateway/run.py``:
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses xiaoban_cli.mcp_startup; TUI stdio keeps
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("XIAOBAN_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("XIAOBAN_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("XIAOBAN_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("XIAOBAN_IGNORE_RULES")),
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind XIAOBAN_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `XIAOBAN_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_xiaoban_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_xiaoban_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_xiaoban_home_override(init_home_token)
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds XIAOBAN_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``xiaoban --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``xiaoban gateway`` process, whose
...
# (agent/billing_view.py + xiaoban_cli/nous_billing.py) — same as /credits.
...
def _(rid, params: dict) -> dict:
    ...
    """
    from xiaoban_cli.nous_billing import BillingError, post_charge
...
def _(rid, params: dict) -> dict:
    ...
    """
    from xiaoban_cli.nous_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from xiaoban_cli.nous_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Xiaoban profile home
    # (~/.xiaoban/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $XIAOBAN_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_xiaoban_home() / "spawn-trees"
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn XIAOBAN_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_xiaoban_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from xiaoban_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from xiaoban_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: xiaoban_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from xiaoban_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _xiaoban_home / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".xiaoban" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.xiaoban/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.xiaoban/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Xiaoban Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Xiaoban/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Xiaoban Desktop. If it serves Xiaoban/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from xiaoban_cli.model_switch import parse_model_flags
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("XIAOBAN_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["XIAOBAN_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("XIAOBAN_YOLO_MODE", None)
                    nv = "0"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from xiaoban_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_xiaoban_home), "display": display_xiaoban_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    try:
        from xiaoban_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Xiaoban provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.xiaoban/.env`` into the gateway process via
    ``xiaoban_cli.config.reload_env``, matching classic CLI's ``/reload``
...
_TUI_EXTRA: list[tuple[str, str, str]] = [
    ...
    (
        "/mouse",
        "设置鼠标跟踪模式 [on|off|toggle|wheel|buttons|all]",
        "TUI",
    ),
    ("/sessions", "切换当前 TUI 里的实时会话", "TUI"),
]
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "直接运行 `xiaoban` 会进入交互模式；请用 `/xiaoban chat -q …`，或在另一个终端运行 `xiaoban`"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`xiaoban setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`xiaoban gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`xiaoban sessions browse` 是交互式命令；当前界面请用 /resume，或在另一个终端运行 browse"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`xiaoban config edit` 需要真实终端里的 $EDITOR"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m xiaoban_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "xiaoban_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _details_completions(text: str) -> list[dict] | None:
        ...
        return [
            *[
                _details_root_completion_item(
                    mode, "全局展示模式", not has_trailing_space
                )
                for mode in modes
            ],
            _details_root_completion_item(
                "cycle", "循环切换全局模式", not has_trailing_space
            ),
            *[
                _details_root_completion_item(
                    section, "单独设置这一块", not has_trailing_space
                )
        ...
        return [
            _details_completion_item(
                ...
                (
                    ...
                    else "循环切换全局模式" if candidate == "cycle" else "全局展示模式"
                ),
            )
    ...
    if len(parts) == 1 and has_trailing_space and parts[0].lower() in sections:
        return [
            *[
                _details_completion_item(mode, f"设置 {parts[0].lower()}")
                for mode in modes
            ],
            _details_completion_item("reset", f"清除 {parts[0].lower()} 单独设置"),
        ]
    ...
    if len(parts) == 2 and not has_trailing_space and parts[0].lower() in sections:
        ...
        return [
            _details_completion_item(
                candidate,
                (
                    f"清除 {parts[0].lower()} 单独设置"
                    if candidate == "reset"
                    else f"设置 {parts[0].lower()}"
                ),
            )
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from xiaoban_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from xiaoban_cli.config import is_managed, save_env_value
        from xiaoban_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `xiaoban model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.xiaoban/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from xiaoban_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from xiaoban_cli.config import remove_env_value
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("XIAOBAN_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("XIAOBAN_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["XIAOBAN_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from xiaoban_cli.voice import stop_continuous
            ...
            os.environ["XIAOBAN_VOICE_TTS"] = "0"
    ...
    if action == "tts":
        ...
        os.environ["XIAOBAN_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from xiaoban_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from xiaoban_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from xiaoban_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from xiaoban_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("XIAOBAN_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("XIAOBAN_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_xiaoban_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from xiaoban_cli.skills_hub import do_install
        ...
        if action == "browse":
            from xiaoban_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from xiaoban_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``xiaoban plugins`` / the dashboard, so the three surfaces
