 ...
 * the three user-facing uninstall modes to the `xiaoban uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m xiaoban_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `xiaoban_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'xiaoban_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Xiaoban.app/Contents/MacOS/Xiaoban  → …/Xiaoban.app
 *   Windows: …\Xiaoban\Xiaoban.exe                 → …\Xiaoban  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/Xiaoban.app/Contents/MacOS/Xiaoban → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Xiaoban.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs Xiaoban.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]Xiaoban$/i.test(dir) || /[\\/]xiaoban-desktop$/i.test(dir)) return dir
    return null
  }

  // Linux: an AppImage exposes its own path via the APPIMAGE env var.
  if (env.APPIMAGE) return env.APPIMAGE
  // Unpacked electron-builder tree: …/linux-unpacked/xiaoban
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import xiaoban_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, xiaobanHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export XIAOBAN_HOME=${q(xiaobanHome)}`
  ]
  if (pythonPath) {
 ...
 * modes + `pythonPath`=agentRoot so `import xiaoban_cli` resolves from source
 ...
 */
function buildWindowsCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, xiaobanHome }) {
  const pid = Number(desktopPid) || 0
  // cmd.exe has no string escaping inside quotes; strip embedded quotes (paths
  // under %LOCALAPPDATA% never contain them). `&`/`^` in a path would still be
  // a problem, but Xiaoban install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "XIAOBAN_HOME=${String(xiaobanHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
  if (pythonPath) {
