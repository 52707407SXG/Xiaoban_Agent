...
from xiaoban_cli.memory_setup import _CANCELLED
...
def _provider_for_mode(tmp_path, monkeypatch, mode: str):
    ...
    monkeypatch.setattr(
        "plugins.memory.hindsight.get_xiaoban_home", lambda: tmp_path
    )

    provider = HindsightMemoryProvider()
    provider.initialize(session_id="test-session", xiaoban_home=str(tmp_path), platform="cli")
    return provider
...
def provider(tmp_path, monkeypatch):
    ...
    p.initialize(session_id="test-session", xiaoban_home=str(tmp_path), platform="cli")
...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags("agent:fakeassistantname, source_system:xiaoban-agent, agent:fakeassistantname") == [
        "agent:fakeassistantname",
        "source_system:xiaoban-agent",
    ]


def test_normalize_retain_tags_accepts_json_array_string():
    value = json.dumps(["agent:fakeassistantname", "source_system:xiaoban-agent"])
    assert _normalize_retain_tags(value) == ["agent:fakeassistantname", "source_system:xiaoban-agent"]
...
class TestConfig:
    ...
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Xiaoban-Agent and the User"
    ...
    def test_custom_config_values(self, provider_with_config):
        ...
        assert p._retain_source == "xiaoban"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "plugins.memory.hindsight.get_xiaoban_home",
        ...
        assert cfg["banks"]["xiaoban"]["bankId"] == "env-bank"
        assert cfg["banks"]["xiaoban"]["budget"] == "high"
...
class TestPostSetup:
    def test_setup_cancel_at_mode_picker_writes_nothing(self, tmp_path, monkeypatch):
        xiaoban_home = tmp_path / "xiaoban-home"
        user_home = tmp_path / "user-home"
        user_home.mkdir()
        monkeypatch.setenv("HOME", str(user_home))
        monkeypatch.setattr("plugins.memory.hindsight.get_xiaoban_home", lambda: xiaoban_home)

        save_config = MagicMock()
        which = MagicMock(return_value="/usr/bin/uv")
        run = MagicMock()
        monkeypatch.setattr("xiaoban_cli.memory_setup._curses_select", lambda *args, **kwargs: _CANCELLED)
        ...
        monkeypatch.setattr("xiaoban_cli.config.save_config", save_config)

        provider = HindsightMemoryProvider()
        provider.post_setup(str(xiaoban_home), {"memory": {"provider": "builtin"}})

        save_config.assert_not_called()
        which.assert_not_called()
        run.assert_not_called()
        assert not (xiaoban_home / ".env").exists()
        assert not (xiaoban_home / "hindsight" / "config.json").exists()
        assert not (user_home / ".hindsight" / "profiles" / "xiaoban.env").exists()
    ...
    def test_local_embedded_setup_cancel_at_llm_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        which = MagicMock(return_value="/usr/bin/uv")
        run = MagicMock()
        monkeypatch.setattr("xiaoban_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", which)
        monkeypatch.setattr("subprocess.run", run)
        monkeypatch.setattr("builtins.input", MagicMock(side_effect=AssertionError("prompt should not run")))
        monkeypatch.setattr("getpass.getpass", MagicMock(side_effect=AssertionError("prompt should not run")))
    ...
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        saved_configs = []
        monkeypatch.setattr("xiaoban_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))

        provider = HindsightMemoryProvider()
        provider.post_setup(str(xiaoban_home), {"memory": {}})

        assert saved_configs[-1]["memory"]["provider"] == "hindsight"
        env_text = (xiaoban_home / ".env").read_text()
        assert "HINDSIGHT_LLM_API_KEY=sk-local-test\n" in env_text
        assert "HINDSIGHT_TIMEOUT=120\n" in env_text
        assert "HINDSIGHT_IDLE_TIMEOUT=300\n" in env_text

        profile_env = user_home / ".hindsight" / "profiles" / "xiaoban.env"
    ...
    def test_local_embedded_setup_respects_existing_profile_name(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config({"profile": "coder"}, str(xiaoban_home))
        provider.post_setup(str(xiaoban_home), {"memory": {}})

        coder_env = user_home / ".hindsight" / "profiles" / "coder.env"
        xiaoban_env = user_home / ".hindsight" / "profiles" / "xiaoban.env"
        assert coder_env.exists()
        assert not xiaoban_env.exists()
    ...
    def test_local_embedded_setup_blank_inputs_preserve_existing_config(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config(existing_config, str(xiaoban_home))
        ...
        monkeypatch.setattr("xiaoban_cli.memory_setup._curses_select", lambda *args, **kwargs: kwargs.get("default", 0))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "")
        ...
        saved = json.loads((xiaoban_home / "hindsight" / "config.json").read_text())
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        item = call_kwargs["items"][0]
        assert item["context"] == "conversation between Xiaoban-Agent and the User"
        ...
        assert item["metadata"]["source"] == "xiaoban"
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["retain_async"] is True
        assert len(call_kwargs["items"]) == 1
        assert call_kwargs["items"][0]["context"] == "conversation between Xiaoban-Agent and the User"
    ...
    def test_resume_creates_new_document(self, tmp_path, monkeypatch):
        ...
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        monkeypatch.setattr("plugins.memory.hindsight.get_xiaoban_home", lambda: tmp_path)

        p1 = HindsightMemoryProvider()
        p1.initialize(session_id="resumed-session", xiaoban_home=str(tmp_path), platform="cli")
        ...
        p2.initialize(session_id="resumed-session", xiaoban_home=str(tmp_path), platform="cli")
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("xiaoban") == "xiaoban"
    ...
    def test_resolve_empty_template_uses_fallback(self):
        result = _resolve_bank_id_template(
            "", fallback="xiaoban", profile="coder"
        )
        assert result == "xiaoban"

    def test_resolve_with_profile(self):
        result = _resolve_bank_id_template(
            "xiaoban-{profile}", fallback="xiaoban",
            profile="coder", workspace="", platform="", user="", session="",
        )
        assert result == "xiaoban-coder"
    ...
    def test_resolve_collapses_empty_placeholders(self):
        # When user is empty, "xiaoban-{user}" becomes "xiaoban-" -> trimmed to "xiaoban"
        result = _resolve_bank_id_template(
            "xiaoban-{user}", fallback="default",
    ...
    def test_resolve_invalid_template_returns_fallback(self):
        ...
        result = _resolve_bank_id_template(
            "xiaoban-{unknown}", fallback="xiaoban",
            profile="", workspace="", platform="", user="", session="",
        )
        assert result == "xiaoban"
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "xiaoban-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        p.initialize(
            ...
            agent_workspace="xiaoban",
        )
        assert p._bank_id == "xiaoban-coder"
        assert p._bank_id_template == "xiaoban-{profile}"
    ...
    def test_provider_template_with_missing_profile_falls_back(self, tmp_path, monkeypatch):
        ...
        p = HindsightMemoryProvider()
        # No agent_identity passed — template renders to "xiaoban-" which collapses to "xiaoban"
        p.initialize(session_id="s1", xiaoban_home=str(tmp_path), platform="cli")
        assert p._bank_id == "xiaoban"
