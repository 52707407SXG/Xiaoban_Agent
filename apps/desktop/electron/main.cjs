...
const ACTIVE_HERMES_ROOT = path.join(HERMES_HOME, 'xiaoban-agent')
...
// `xiaoban update`). Instead we spawn the staged Hermes-Setup binary with
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // whole update itself: `xiaoban update` (backend) + `hermes desktop
...
async function applyUpdatesPosixInApp() {
  ...
  if (!hermes) {
    ...
    return { ok: true, manual: true, command: 'xiaoban update', hermesRoot: updateRoot }
  }
...
function resolveHermesBackend(dashboardArgs) {
  ...
  //    %LOCALAPPDATA%\hermes\xiaoban-agent (Windows) or ~/.hermes/xiaoban-agent.
  ...
  if (process.env.HERMES_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Xiaoban CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }
    ...
    if (hermesCommand) {
      ...
      if (verifyHermesCli(hermesCommand, { shell: shellForProbe })) {
        return {
          label: `existing Xiaoban CLI at ${hermesCommand}`,
      ...
      rememberLog(
        `Ignoring existing Xiaoban CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
  ...
  if (python) {
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\hermes\xiaoban-agent\venv.
