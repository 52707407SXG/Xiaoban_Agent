...
| | |
|---|---|
| Source | Optional — install with `xiaoban skills install official/productivity/shopify` |
...
4. Save to `${XIAOBAN_HOME:-~/.xiaoban}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Xiaoban","productType":"Apparel","tags":["test"]}}'
```
