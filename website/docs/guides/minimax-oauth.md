...
Xiaoban-Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Hermes automatically refreshes your session.
...
**Fix:** re-run `hermes auth add minimax-oauth` (or `xiaoban model`). The flow starts fresh.
...
**Fix:** run `xiaoban model` and select MiniMax (OAuth), or run `hermes auth add minimax-oauth`.
