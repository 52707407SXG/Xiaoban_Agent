"""
Cron subcommand for xiaoban CLI.
...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(xiaoban\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*xiaoban)"
    r"|(systemctl\s+(-\S+\s+)*(restart|stop|start)\s+.*xiaoban)"
    r"|(p?kill\s+.*xiaoban.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'xiaoban cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: xiaoban gateway install", Colors.DIM))
        print(color("                    sudo xiaoban gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: xiaoban gateway restart")
        ...
        print("    xiaoban gateway install    # Install as a user service")
        print("    sudo xiaoban gateway install --system  # Linux servers: boot-time system service")
        print("    xiaoban gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `xiaoban gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: xiaoban cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
