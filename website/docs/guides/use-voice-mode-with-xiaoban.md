---
sidebar_position: 8
title: "Use Voice Mode with Xiaoban"
description: "A practical guide to setting up and using Xiaoban voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Xiaoban
...
- you want Xiaoban sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Xiaoban.
...
## Step 1: make sure normal Xiaoban works first
...
Xiaoban supports both local and cloud speech stacks.
...
### If you use `xiaoban setup`

If you choose NeuTTS in the setup wizard, Xiaoban checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Xiaoban transcribes and responds
...
- asking Xiaoban to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Xiaoban loop.

## Tuning CLI behavior

### Silence threshold

If Xiaoban starts/stops too aggressively, tune:
...
Xiaoban stays a normal chat bot, but can speak replies.
...
- you want Xiaoban to function like a portable research or ops assistant
...
Xiaoban joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Xiaoban detects speech boundaries
- transcripts are posted in the associated text channel
- Xiaoban responds in text and audio
...
1. get text Xiaoban working
2. install `xiaoban-agent[voice]`
