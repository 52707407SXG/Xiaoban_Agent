...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._xiaoban_home / "logs" / "errors.log").resolve()
...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host XIAOBAN_REDACT_SECRETS state.
...
class TestTaskCompletionGuidance:
    ...
    def test_guidance_is_goal_level_not_tool_status_level(self):
        from agent.prompt_builder import TASK_COMPLETION_GUIDANCE

        assert "judge the original goal rather than the tool call" in (
            TASK_COMPLETION_GUIDANCE
        )
        assert "not merely acknowledge that a lookup or action ran" in (
            TASK_COMPLETION_GUIDANCE
        )
        assert "actual attempts and final cause" in TASK_COMPLETION_GUIDANCE
...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("xiaoban_cli.plugins.invoke_hook", _capture_hook)
        monkeypatch.setattr("xiaoban_cli.plugins.has_hook", lambda name: True)
...
class TestConcurrentToolExecution:
    ...
    def test_invoke_tool_agent_level_tool_emits_terminal_post_tool_hook(self, agent, monkeypatch):
        ...
        hook_calls = []
        monkeypatch.setattr(
            "xiaoban_cli.plugins.get_pre_tool_call_block_message",
    ...
    def test_sequential_agent_level_tool_execution_middleware_wraps_inline_dispatch(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "xiaoban_cli.plugins.invoke_middleware",
...
class TestRunConversation:
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        assert "Ollama runtime context too small for Xiaoban tool use" in caplog.text
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch("xiaoban_cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("xiaoban_cli.plugins.has_hook", lambda name: False)
        monkeypatch.setattr("xiaoban_cli.plugins.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("xiaoban_cli.plugins.has_hook", _has_hook)
        ...
        with (
            patch("xiaoban_cli.plugins.invoke_hook", return_value=[]),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("XIAOBAN_KANBAN_TASK", "t_test_task_123")
        ...
        with (
            ...
            patch("xiaoban_cli.kanban_db._record_task_failure",
                  mock_record_failure),
            patch("xiaoban_cli.kanban_db.connect", mock_connect),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """The exhaustion bridge must NOT fire when XIAOBAN_KANBAN_TASK
...
class TestNousCredentialRefresh:
    ...
    def test_try_refresh_nous_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "xiaoban_cli.auth.resolve_nous_runtime_credentials", _fake_resolve
        )
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Xiaoban-Agent" in agent._cached_system_prompt
