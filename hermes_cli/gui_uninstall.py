...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``xiaoban desktop`` / ``xiaoban gui``)
     Built inside the agent checkout under ``$HERMES_HOME/xiaoban-agent/``:
...
def _agent_root(hermes_home: Path) -> Path:
    ...
    return hermes_home / "xiaoban-agent"
...
def uninstall_gui(hermes_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``xiaoban-agent/hermes_cli`` (agent source), ``venv/``, or any
