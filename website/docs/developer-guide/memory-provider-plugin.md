...
description: "How to build a memory provider plugin for Xiaoban-Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Xiaoban-Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `xiaoban plugins`.
:::
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          xiaoban_home (str): Active XIAOBAN_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `xiaoban memory setup` | **Yes** |
| `save_config(values, xiaoban_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `xiaoban memory setup`:
...
the `messages` parameter; Xiaoban will continue calling them with the legacy
signature.
...
All storage paths **must** use the `xiaoban_home` kwarg from `initialize()`, not hardcoded `~/.xiaoban`:

```python
# CORRECT — profile-scoped
from xiaoban_constants import get_xiaoban_home
data_dir = get_xiaoban_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.xiaoban/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `xiaoban my-provider status`, `xiaoban my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `xiaoban <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `xiaoban --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: xiaoban my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the xiaoban my-provider argparse tree.
