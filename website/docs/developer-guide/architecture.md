...
sidebar_position: 1
title: "Architecture"
description: "Xiaoban-Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Xiaoban-Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ xiaoban_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # XiaobanCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── xiaoban_state.py           # SQLite session/state database with FTS5
├── xiaoban_constants.py       # XIAOBAN_HOME, profile-aware paths
...
│
├── xiaoban_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `xiaoban` subcommands (large file)
...
│   ├── skills_config.py      # xiaoban skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # xiaoban tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # xiaoban gateway start/stop
│
...
```text
User input → XiaobanCLI.process_input()
...
Three discovery sources: `~/.xiaoban/plugins/` (user), `.xiaoban/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `xiaoban plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-xiaoban-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Xiaoban as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`xiaoban -p <name>`) gets its own XIAOBAN_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
