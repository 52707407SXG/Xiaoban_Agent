"""Tests for acp_adapter.server — XiaobanACPAgent ACP server."""
...
from acp_adapter.server import XiaobanACPAgent, XIAOBAN_VERSION
...
def agent(mock_manager):
    """XiaobanACPAgent backed by a mock session manager."""
    return XiaobanACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "xiaoban-agent"
        assert resp.agent_info.version == XIAOBAN_VERSION
        ...
        assert payloads == [
            {
                "args": ["--setup"],
                "description": (
                    "Open Xiaoban' interactive model/provider setup in a terminal. "
                    "Use this when Xiaoban has not been configured on this machine yet."
                ),
                "id": TERMINAL_SETUP_AUTH_METHOD_ID,
                "name": "Configure Xiaoban provider",
                "type": "terminal",
            }
        ]
...
class TestSessionOps:
    ...
    async def test_new_session_returns_model_state(self):
        ...
        acp_agent = XiaobanACPAgent(session_manager=manager)

        with patch(
            "xiaoban_cli.models.curated_models_for_provider",
    ...
    async def test_load_session_replays_persisted_history_to_client(self, agent):
        ...
        state.history = [
            ...
            {"role": "assistant", "content": "XiaobanACPAgent._ADVERTISED_COMMANDS controls them."},
            {
                "role": "assistant",
                "content": "",
                "tool_calls": [
        ...
        assert replay_calls[1].kwargs["update"].content.text.startswith("XiaobanACPAgent")
...
class TestSessionConfiguration:
    ...
    async def test_set_session_model_accepts_provider_prefixed_choice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "xiaoban_cli.models.parse_model_input",
...
class TestPrompt:
    ...
    @pytest.mark.asyncio
    async def test_prompt_propagates_xiaoban_session_id_env(self, agent, monkeypatch):
        """ACP must propagate the originating session id to the agent loop
        via ``XIAOBAN_SESSION_ID`` so tools that want to stamp side-effects
        ...
        def mock_run(user_message, conversation_history=None, task_id=None, **kwargs):
            ...
            captured["env"] = os.environ.get("XIAOBAN_SESSION_ID")
        ...
        assert captured["env"] == new_resp.session_id, (
            "XIAOBAN_SESSION_ID must be set to the originating ACP session id "
        ...
        assert os.environ.get("XIAOBAN_SESSION_ID") is None, (
            "XIAOBAN_SESSION_ID must be restored after the agent call so "
            "a re-used executor thread doesn't leak the id into the next "
            "session's tools"
        )

    @pytest.mark.asyncio
    async def test_prompt_restores_prior_xiaoban_session_id(self, agent, monkeypatch):
        """If the env already had XIAOBAN_SESSION_ID set (e.g. nested
        agent loops), the prior value must be restored after the inner
        prompt completes — not popped, not left at the inner id."""
        monkeypatch.setenv("XIAOBAN_SESSION_ID", "outer-sess")
        ...
        def mock_run(*args, **kwargs):
            captured["inner"] = os.environ.get("XIAOBAN_SESSION_ID")
            return {"final_response": "ok", "messages": []}
        ...
        assert os.environ.get("XIAOBAN_SESSION_ID") == "outer-sess"
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert XIAOBAN_VERSION in result
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["xiaoban-acp"]
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["xiaoban-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["xiaoban-acp", "mcp-srv"]
