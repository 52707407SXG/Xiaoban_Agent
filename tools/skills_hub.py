#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Xiaoban Skills Hub.
...
Used by xiaoban_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            xiaoban_meta = metadata.get("xiaoban", {})
            if isinstance(xiaoban_meta, dict):
                raw_tags = xiaoban_meta.get("tags", [])
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                xiaoban_meta = meta_block.get("xiaoban", {})
                if isinstance(xiaoban_meta, dict):
                    tags = xiaoban_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Xiaoban centralized index source
# ---------------------------------------------------------------------------

XIAOBAN_INDEX_URL = "https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/api/skills-index.json"
XIAOBAN_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "xiaoban-index.json"
XIAOBAN_INDEX_TTL = 6 * 3600  # 6 hours


def _load_xiaoban_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for XIAOBAN_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if XIAOBAN_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - XIAOBAN_INDEX_CACHE_FILE.stat().st_mtime
            if age < XIAOBAN_INDEX_TTL:
                return json.loads(XIAOBAN_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(XIAOBAN_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Xiaoban index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Xiaoban index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        XIAOBAN_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        XIAOBAN_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class XiaobanIndexSource(SkillSource):
    """Skill source backed by the centralized Xiaoban Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_xiaoban_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "xiaoban-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "xiaoban-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        XiaobanIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if source_filter == "all":
        for src in sources:
            if (src.source_id() == "xiaoban-index"
