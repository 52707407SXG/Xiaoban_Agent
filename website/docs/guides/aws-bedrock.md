...
sidebar_position: 14
title: "AWS Bedrock"
description: "Use Xiaoban-Agent with Amazon Bedrock — native Converse API, IAM authentication, Guardrails, and cross-region inference"
---

# AWS Bedrock

Xiaoban-Agent supports Amazon Bedrock as a native provider using the **Converse API** — not the OpenAI-compatible endpoint. This gives you full access to the Bedrock ecosystem: IAM authentication, Guardrails, cross-region inference profiles, and all foundation models.
...
- **boto3** — install with `pip install xiaoban-agent[bedrock]`
...
Bedrock models use **inference profile IDs** for on-demand invocation. The `xiaoban model` picker shows these automatically, with recommended models at the top:
...
**[sample-xiaoban-agent-on-aws-with-bedrock](https://github.com/JiaDe-Wu/sample-xiaoban-agent-on-aws-with-bedrock)** — creates VPC, IAM role, EC2 instance, and configures Bedrock automatically. Deploy in any region with one click.
