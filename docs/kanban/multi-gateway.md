...
Xiaoban supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.xiaoban/config.yaml`:
...
Or set the env var: `XIAOBAN_KANBAN_DISPATCH_IN_GATEWAY=false`
