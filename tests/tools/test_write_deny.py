...
class TestWriteDenyExactPaths:
    ...
    def test_xiaoban_env(self):
        # ``.env`` under the active XIAOBAN_HOME (profile-aware, not just
        # ``~/.xiaoban``) must be write-denied. The hermetic test conftest
        # points XIAOBAN_HOME at a tempdir — resolve via get_xiaoban_home()
        # to match the denylist.
        from xiaoban_constants import get_xiaoban_home
        path = str(get_xiaoban_home() / ".env")
        assert _is_write_denied(path) is True

    def test_xiaoban_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.xiaoban/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "xiaoban_root"
        ...
        # Sanity check: XIAOBAN_HOME does point to the profile dir, not the root.
        from xiaoban_constants import get_xiaoban_home, get_default_xiaoban_root
        assert get_xiaoban_home() == profile_home
        assert get_default_xiaoban_root() == root
...
class TestWriteAllowed:
    ...
    def test_xiaoban_control_files_requested_writable(self):
        from xiaoban_constants import get_xiaoban_home
