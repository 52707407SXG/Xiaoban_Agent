...
These rows appear in `xiaoban tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Hermes runs the Portal login inline (without changing your inference provider or your other tools).
...
### `hermes portal info` shows "not logged in" after `xiaoban setup --portal`
...
If a model is genuinely unavailable, [open an issue](https://github.com/52707407SXG/Xiaoban-Agent/issues) — most gaps are routing config we can update.
