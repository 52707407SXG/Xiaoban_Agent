...
describe('usePromptActions file attachment sync', () => {
  ...
  it('uploads file bytes via file.attach on a remote gateway and submits the rewritten ref', async () => {
    ...
    Object.defineProperty(window, 'xiaobanDesktop', {
      configurable: true,
    ...
    const requestGateway = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      ...
      if (method === 'file.attach') {
        return {
          attached: true,
          path: '/remote/work/.xiaoban/desktop-attachments/report.txt',
          ref_text: '@file:.xiaoban/desktop-attachments/report.txt',
          uploaded: true
        } as never
      }
      return {} as never
    ...
    expect(calls[1]?.params).toEqual({
      session_id: RUNTIME_SESSION_ID,
      text: '@file:.xiaoban/desktop-attachments/report.txt\n\nconvert this to epub'
    })
  })
...
describe('usePromptActions eager-upload races', () => {
  ...
  it('joins an in-flight eager upload at submit instead of staging the file twice', async () => {
    ...
    // under .xiaoban/desktop-attachments/. Submit must await the in-flight upload
    ...
    const requestGateway = vi.fn(async (method: string) => {
      ...
      if (method === 'file.attach') {
        ...
        return { attached: true, ref_text: '@file:.xiaoban/desktop-attachments/doc.pdf', uploaded: true } as never
      }
      return {} as never
    })
...
describe('usePromptActions eager attachment upload (drop-time)', () => {
  ...
  it('uploads a dropped file the moment it lands (active session) and rewrites the chip with the gateway ref', async () => {
    ...
    $connection.set({ mode: 'remote' } as never)
    const readFileDataUrl = vi.fn(async () => 'data:application/pdf;base64,JVBERi0=')
    Object.defineProperty(window, 'xiaobanDesktop', { configurable: true, value: { readFileDataUrl } })

    const calls: string[] = []
    const requestGateway = vi.fn(async (method: string) => {
      calls.push(method)
      if (method === 'file.attach') {
        return { attached: true, ref_text: '@file:.xiaoban/desktop-attachments/DEVIS_signed.pdf', uploaded: true } as never
      }
      return {} as never
    })
    ...
    expect(chip.refText).toBe('@file:.xiaoban/desktop-attachments/DEVIS_signed.pdf')
