...
import { getProfiles, transcribeAudio } from '@/xiaoban'
...
async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.xiaobanDesktop?.readFileDataUrl
...
export function usePromptActions({
  ...
  // separate `xiaoban gateway` process performs the actual transfer, so we
