...
# Xiaoban-internal env vars that should NOT leak into terminal subprocesses.
_XIAOBAN_PROVIDER_ENV_FORCE_PREFIX = "_XIAOBAN_FORCE_"

# Xiaoban-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Xiaoban inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from xiaoban_cli.config import OPTIONAL_ENV_VARS
    ...
    blocked.update({
        ...
        "XIAOBAN_DASHBOARD_SESSION_TOKEN",
...
_XIAOBAN_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _inject_context_xiaoban_home(env: dict) -> None:
    """Bridge the context-local Xiaoban home override into subprocess env."""
    try:
        from xiaoban_constants import get_xiaoban_home_override

        value = get_xiaoban_home_override()
        if value:
            env["XIAOBAN_HOME"] = value
    except Exception:
        pass


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Xiaoban-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_XIAOBAN_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _XIAOBAN_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_XIAOBAN_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_XIAOBAN_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _XIAOBAN_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_xiaoban_home(sanitized)
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("XIAOBAN_GIT_BASH_PATH")
    ...
    #   PortableGit: %LOCALAPPDATA%\xiaoban\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\xiaoban\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _xiaoban_portable_git = os.path.join(_local_appdata, "xiaoban", "git") if _local_appdata else ""
    if _xiaoban_portable_git:
        for candidate in (
            os.path.join(_xiaoban_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_xiaoban_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        "Git Bash not found. Xiaoban-Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set XIAOBAN_GIT_BASH_PATH to your bash.exe location."
    )
...
# Cached directory containing the ``xiaoban`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_XIAOBAN_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_xiaoban_bin_dir() -> str | None:
    """Return the directory holding the ``xiaoban`` console-script, or None.
    ...
    xiaoban install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``xiaoban``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``xiaoban`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``xiaoban`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("xiaoban")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``xiaoban`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _XIAOBAN_BIN_DIR
    if _XIAOBAN_BIN_DIR is not _SENTINEL:
        return _XIAOBAN_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("xiaoban")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "xiaoban" or base.startswith("xiaoban."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "xiaoban.exe" if _IS_WINDOWS else "xiaoban"
    ...
    _XIAOBAN_BIN_DIR = candidate
    return candidate


def _prepend_xiaoban_bin_dir(existing_path: str) -> str:
    """Prepend the xiaoban install dir to ``existing_path`` if it's missing.
    ...
    """
    bin_dir = _resolve_xiaoban_bin_dir()
    if not bin_dir:
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_XIAOBAN_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_XIAOBAN_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _XIAOBAN_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    path_key = _path_env_key(run_env)
    if path_key is not None:
        new_path = _append_missing_sane_path_entries(run_env.get(path_key, ""))
        # Ensure the xiaoban install dir is reachable so plugins can shell out
        # to bare ``xiaoban`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_xiaoban_bin_dir(new_path)

    _inject_context_xiaoban_home(run_env)
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Xiaoban trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``XIAOBAN_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under XIAOBAN_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_xiaoban_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "xiaoban_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._xiaoban_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_xiaoban_pgid", None)
                    if pgid is None:
                        raise
