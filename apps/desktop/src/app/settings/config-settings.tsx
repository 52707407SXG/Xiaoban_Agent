...
import {
  getElevenLabsVoices,
  getXiaobanConfigDefaults,
  getXiaobanConfigRecord,
  getXiaobanConfigSchema,
  saveXiaobanConfig
} from '@/xiaoban'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, XiaobanConfigRecord } from '@/types/xiaoban'
...
export function ConfigSettings({
  ...
  const [_defaults, setDefaults] = useState<XiaobanConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getXiaobanConfigRecord(), getXiaobanConfigDefaults(), getXiaobanConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          await saveXiaobanConfig(config)
  ...
  const updateConfig = (next: XiaobanConfigRecord) => {
