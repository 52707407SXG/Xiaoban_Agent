"""disk_cleanup — ephemeral file cleanup for Xiaoban-Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under XIAOBAN_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly XIAOBAN_HOME and /tmp/xiaoban-*
Never touches: ~/.xiaoban/logs/ or any system directory.
"""
...
try:
    ...
    def get_xiaoban_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("XIAOBAN_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".xiaoban").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$XIAOBAN_HOME/logs/``."""
    return get_xiaoban_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$XIAOBAN_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under XIAOBAN_HOME or ``/tmp/xiaoban-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    xiaoban_home = get_xiaoban_home()
    try:
        path.resolve().relative_to(xiaoban_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/xiaoban-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("xiaoban-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "xiaoban-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $XIAOBAN_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so XIAOBAN_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        xiaoban_home = get_xiaoban_home()
        for parent in ("cron", "cronjobs"):
            base = xiaoban_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside XIAOBAN_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under XIAOBAN_HOME, but never recurse into known
    # durable state trees.  Some installs place the Xiaoban checkout, venv,
    # and desktop build under XIAOBAN_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    xiaoban_home = get_xiaoban_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in xiaoban_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    xiaoban_home = get_xiaoban_home()
    try:
        rel = path.resolve().relative_to(xiaoban_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "xiaoban-agent",
        }:
            return None
        ...
        # Path isn't under XIAOBAN_HOME (e.g. /tmp/xiaoban-*) — fall through.
        pass
