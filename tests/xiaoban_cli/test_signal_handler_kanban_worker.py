...
for interactive ``xiaoban chat -q`` invocations, but kanban workers spawned by
...
The fix: when the process is a dispatcher-spawned worker (``XIAOBAN_KANBAN_TASK``
...
def _synthetic_worker_script() -> str:
    ...
    shape without needing the full xiaoban_cli boot path (config, providers,
...
def _is_alive_like_dispatcher(pid: int) -> bool:
    """Mirrors xiaoban_cli/kanban_db.py:_pid_alive on Linux.
...
def test_sigterm_with_kanban_task_env_terminates_quickly():
    """With XIAOBAN_KANBAN_TASK set, SIGTERM should kill the process in <2s
    even when a non-daemon thread is still alive."""
    proc = _spawn_synthetic({"XIAOBAN_KANBAN_TASK": "t_test_28181"})
    try:
        t0 = time.time()
        ...
        while time.time() < deadline:
            ...
            time.sleep(0.02)
        pytest.fail(
            f"process still alive 2s after SIGTERM with XIAOBAN_KANBAN_TASK set "
...
def test_sigterm_without_kanban_task_env_uses_keyboard_interrupt_path():
    """Without XIAOBAN_KANBAN_TASK, the original KeyboardInterrupt path runs.

    This is the contrast case proving the fix is gated on the env var: in
    interactive ``xiaoban chat -q`` (no env var), behavior is unchanged. The
...
def test_real_handler_uses_os_exit_for_kanban_workers():
    ...
    os._exit(0) when XIAOBAN_KANBAN_TASK is set.
    ...
    assert "XIAOBAN_KANBAN_TASK" in body, (
        "_signal_handler_q must gate its kanban-worker exit path on "
        "XIAOBAN_KANBAN_TASK — see #28181"
    )
