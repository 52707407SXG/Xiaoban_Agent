"""ACP agent server — exposes Xiaoban-Agent via the Agent Client Protocol."""
...
try:
    from xiaoban_cli import __version__ as XIAOBAN_VERSION
except Exception:
    XIAOBAN_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Xiaoban running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Xiaoban cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Xiaoban/OpenAI-compatible user content payload."""
...
class XiaobanACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Xiaoban AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Xiaoban version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Xiaoban maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from xiaoban_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from xiaoban_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Xiaoban estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_xiaoban_session_id: str,
        previous_xiaoban_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.xiaoban.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_xiaoban_session_id,
                previous_xiaoban_session_id=previous_xiaoban_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_xiaoban_session_id: Optional[str] = None,
        previous_xiaoban_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Xiaoban changes it.

        When the internal Xiaoban head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_xiaoban_session_id`` so the
        attached ``_meta.xiaoban.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # xiaoban_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_xiaoban_session_id or session_id,
            previous_xiaoban_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["xiaoban-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="xiaoban-agent", version=XIAOBAN_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Xiaoban' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Xiaoban setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Xiaoban
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Xiaoban on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Xiaoban' local kawaii waiting/status
        ...
        # not the executor's. Also set XIAOBAN_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue XIAOBAN_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind XIAOBAN_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("XIAOBAN_INTERACTIVE")
            ...
            previous_session_id = os.environ.get("XIAOBAN_SESSION_ID")
            ...
            try:
                ...
                # Restore XIAOBAN_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("XIAOBAN_INTERACTIVE", None)
                else:
                    os.environ["XIAOBAN_INTERACTIVE"] = previous_interactive
                # Restore XIAOBAN_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("XIAOBAN_SESSION_ID", None)
                else:
                    os.environ["XIAOBAN_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Xiaoban DB session id before the turn so we
            ...
            pre_turn_xiaoban_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (XIAOBAN_SESSION_KEY in
        ...
        # _meta.xiaoban.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_xiaoban_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_xiaoban_id
            and pre_turn_xiaoban_id
            and post_turn_xiaoban_id != pre_turn_xiaoban_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_xiaoban_session_id=post_turn_xiaoban_id,
                    previous_xiaoban_session_id=pre_turn_xiaoban_id,
                )
            except Exception:
                logger.debug(
        ...
        # Xiaoban' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Xiaoban-Agent v{XIAOBAN_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Xiaoban has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
