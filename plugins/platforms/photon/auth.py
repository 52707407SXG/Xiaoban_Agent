...
Credential storage mirrors every other Xiaoban channel:

    * runtime SDK creds  -> ``~/.xiaoban/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.xiaoban/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a xiaoban dependency
...
# registers Xiaoban as its own client_id.
...
# Default name of the project Xiaoban provisions for the operator.
DEFAULT_PROJECT_NAME = "Xiaoban-Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of xiaoban-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.xiaoban/auth.json`` honouring the active Xiaoban profile."""
    try:
        from xiaoban_constants import get_xiaoban_home
        return Path(get_xiaoban_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.xiaoban")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.xiaoban/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.xiaoban/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.xiaoban/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: xiaoban_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `xiaoban photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `xiaoban photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `xiaoban photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `xiaoban photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `xiaoban photon setup`)"
