...
执行 `nix profile install` 后，`hermes`、`xiaoban-agent` 和 `hermes-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`xiaoban setup` 引导你完成提供商选择，`xiaoban gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.hermes/`。
...
- 路由是透明的：`xiaoban chat`、`hermes sessions list`、`xiaoban version` 等命令都会在底层 exec 进容器
- 所有 CLI 参数原样转发
...
| 给 Agent 设置个性 | `${services.xiaoban-agent.stateDir}/.hermes/SOUL.md` | 直接管理该文件 |
...
| 覆盖 hermes 包 | `package` | `inputs.xiaoban-agent.packages.${system}.default.override { ... }` |
...
`environment`（非密钥变量）和 `environmentFiles`（密钥文件）在激活时（`nixos-rebuild switch`）都会合并到 `$HERMES_HOME/.env` 中。Hermes 在每次启动时读取此文件，因此更改在 `systemctl restart xiaoban-agent` 后生效——无需重建容器。
...
Agent 身份文件是独立的：Hermes 从 `$HERMES_HOME/SOUL.md` 加载其主要 `SOUL.md`，在 NixOS 模块中对应 `${services.xiaoban-agent.stateDir}/.hermes/SOUL.md`。将 `SOUL.md` 放入 `documents` 只会创建一个工作区文件，不会替换主角色文件。
...
2. **`.managed` 标记文件**，位于 `HERMES_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it xiaoban-agent hermes config set ...` 也会被屏蔽）
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `hermes` 和 `xiaoban-agent` 二进制文件存在且 `xiaoban version` 可运行 |
