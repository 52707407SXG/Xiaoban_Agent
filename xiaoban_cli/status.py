"""
Status command for xiaoban CLI.

Shows the status of all Xiaoban-Agent components.
"""
...
from xiaoban_cli.auth import AuthError, resolve_provider
from xiaoban_cli.colors import Colors, color
from xiaoban_cli.config import get_env_path, get_env_value, get_xiaoban_home, load_config
from xiaoban_cli.models import provider_label
from xiaoban_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from xiaoban_cli.nous_subscription import get_nous_subscription_features
from xiaoban_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``xiaoban config``'s
...
def show_status(args):
    """Show status of all Xiaoban-Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 M Xiaoban-Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: xiaoban portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: xiaoban model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: xiaoban auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: xiaoban auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: xiaoban model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from xiaoban_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from xiaoban_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   xiaoban gateway")
    ...
    jobs_file = get_xiaoban_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_xiaoban_home() / "sessions" / "sessions.json"
