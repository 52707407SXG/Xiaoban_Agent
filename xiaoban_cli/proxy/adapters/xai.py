...
# xAI's public API is OpenAI-compatible for the endpoints Xiaoban commonly
# uses. The Responses endpoint is included because Xiaoban' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Xiaoban-managed OAuth credentials."""

    auth_hint = "xiaoban auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`xiaoban auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`xiaoban auth reset xai-oauth` or re-authenticate with "
                    "`xiaoban auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `xiaoban auth add xai-oauth --type oauth`."
            )
