...
/**
 * Return true when Xiaoban should enable Termux-focused TUI defaults.
 *
 * Defaults to on in Termux, with an explicit opt-out for debugging:
 *   XIAOBAN_TUI_TERMUX_MODE=0
 */
export const isTermuxTuiMode = (env: NodeJS.ProcessEnv = process.env): boolean => {
  ...
  const override = String(env.XIAOBAN_TUI_TERMUX_MODE ?? '').trim().toLowerCase()
