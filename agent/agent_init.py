...
def _build_codex_gpt55_autoraise_notice(autoraise: Dict[str, float]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: xiaoban config set compression.codex_gpt55_autoraise false"
    )
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .xiaoban.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / XIAOBAN_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.xiaoban/SOUL.md as the primary
    ...
    # True only while the current conversation turn can still consume a
    # mid-turn supplement. It closes atomically with the final response.
    ...
    agent._cache_ttl = "5m"
    try:
        from xiaoban_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind XIAOBAN_DEV_CREDITS.
    ...
    # both live under ~/.xiaoban/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from xiaoban_logging import setup_logging, setup_verbose_logging
    setup_logging(xiaoban_home=_ra()._xiaoban_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that xiaoban_logging.setup_logging() does not
        ...
        # handler level inside xiaoban_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from xiaoban_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from xiaoban_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `xiaoban model` to "
                        "select a provider, or run `xiaoban setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff XIAOBAN_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["XIAOBAN_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.xiaoban/sessions/ alongside gateway sessions
    xiaoban_home = get_xiaoban_home()
    agent.logs_dir = xiaoban_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.xiaoban/sessions/session_{sid}.json)
    ...
    try:
        from xiaoban_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from xiaoban_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "xiaoban"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Xiaoban' built-in
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from xiaoban_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from xiaoban_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Xiaoban-Agent.  Choose a model with at least "
