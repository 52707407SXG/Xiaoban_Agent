...
def server():
    with patch.dict("sys.modules", {
        "xiaoban_constants": MagicMock(get_xiaoban_home=MagicMock(return_value="/tmp/xiaoban_test")),
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `XIAOBAN_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("XIAOBAN_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("XIAOBAN_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/xiaoban-agent-dev": {"name": "xiaoban-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "xiaoban-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "xiaoban_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "xiaoban-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "xiaoban-agent-dev"
