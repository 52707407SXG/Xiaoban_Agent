...
const STORAGE_KEY = 'xiaoban:native-notifications'
...
// "Backgrounded" = the user isn't on Xiaoban. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.xiaobanDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.xiaobanDesktop
