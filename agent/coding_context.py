"""Coding-context awareness — base Xiaoban, every interactive surface.

When the user runs Xiaoban inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Xiaoban shifts into a **coding posture**. This module is the
...
logger = logging.getLogger("xiaoban.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "xiaoban", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _XIAOBAN_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``XIAOBAN_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Xiaoban should operate in its coding posture right now."""
