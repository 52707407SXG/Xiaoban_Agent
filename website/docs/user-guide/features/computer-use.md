...
Xiaoban-Agent can drive your desktop — clicking, typing, scrolling,
...
```
xiaoban computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `xiaoban computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `xiaoban tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Xiaoban app). `xiaoban computer-use doctor` will tell you which permission is missing. |
...
```
xiaoban -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.xiaoban/config.yaml`.

## `xiaoban computer-use doctor` — your first triage stop

`xiaoban computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ xiaoban computer-use doctor
...
agent is acting here." Each Xiaoban run declares its own cua-driver
**session id** (something like `xiaoban-3a7b9c14d2e8`); the cursor's
...
Xiaoban intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Xiaoban-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Xiaoban skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Xiaoban
...
Xiaoban applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.xiaoban/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Xiaoban applies four layers of optimisation:
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Xiaoban warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Xiaoban from inside the RDP / console
...
```
XIAOBAN_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
XIAOBAN_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Xiaoban disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Xiaoban sets
...
When it's on, `xiaoban computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Xiaoban at a binary you built from source instead
of the published release. Xiaoban resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`XIAOBAN_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Xiaoban testing (see notes).
...
### Option B — point Xiaoban straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`XIAOBAN_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
XIAOBAN_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
XIAOBAN_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Xiaoban is using your build

- `xiaoban computer-use status` prints the resolved binary path and
  version.
- `xiaoban computer-use doctor` confirms the binary is reachable and
...
- **Xiaoban spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Xiaoban
  change needed either way.
- **Local builds skip the version check.** Xiaoban warns when the
...
**First action when anything's off: run `xiaoban computer-use doctor`.**
...
Run `xiaoban computer-use install` to fetch the cua-driver binary, or
run `xiaoban tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `xiaoban computer-use doctor` will
...
- **Xiaoban-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Xiaoban `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Xiaoban (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Xiaoban among them)
