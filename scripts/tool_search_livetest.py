#!/usr/bin/env python3
"""Live test harness for Xiaoban-Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any xiaoban imports.
ORIGINAL_HOME = os.environ.get("XIAOBAN_HOME")
ORIGINAL_AUTH = Path.home() / ".xiaoban" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.xiaoban/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.xiaoban/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="xiaoban_ts_live_"))
    xiaoban_home = home_dir / ".xiaoban"
    xiaoban_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, xiaoban_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".xiaoban" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, xiaoban_home / ".env")
        ...
        load_xiaoban_dotenv(xiaoban_home=str(Path.home() / ".xiaoban"))
    ...
    (xiaoban_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return xiaoban_home
...
def reset_module_state():
    """Drop cached modules so the new XIAOBAN_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "xiaoban_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["XIAOBAN_HOME"] = str(home)
...
def main():
    ...
    # Restore original XIAOBAN_HOME
    if ORIGINAL_HOME is not None:
        os.environ["XIAOBAN_HOME"] = ORIGINAL_HOME
    else:
