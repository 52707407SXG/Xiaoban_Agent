...
## 5. Xiaoban Configuration

### Config location

`$XIAOBAN_HOME/config.yaml` (defaults to `~/.xiaoban/config.yaml` when `XIAOBAN_HOME` is unset)
...
Restart the Xiaoban session for changes to take effect. The MCP connection is
