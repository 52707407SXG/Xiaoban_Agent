...
description: "How Xiaoban stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `xiaoban_cli/cron.py` | CLI `xiaoban cron` subcommands |
...
Jobs are stored in `~/.xiaoban/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `xiaoban cron` commands are run or during active CLI sessions.
...
```python
# ~/.xiaoban/scripts/check_competitors.py
...
2. **Environment variable** — `XIAOBAN_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.xiaoban/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `xiaoban cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `xiaoban cron` CLI provides direct job management:

```bash
xiaoban cron list                    # Show all jobs
xiaoban cron create                  # Interactive job creation (alias: add)
xiaoban cron edit <job_id>           # Edit job configuration
xiaoban cron pause <job_id>          # Pause a running job
xiaoban cron resume <job_id>         # Resume a paused job
xiaoban cron run <job_id>            # Trigger immediate execution
xiaoban cron remove <job_id>         # Delete a job
```
