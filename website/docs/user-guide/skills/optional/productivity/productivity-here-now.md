...
| | |
|---|---|
| Source | Optional — install with `xiaoban skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${XIAOBAN_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${XIAOBAN_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${XIAOBAN_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client xiaoban
```
...
```bash
PUBLISH="${XIAOBAN_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client xiaoban
```
...
```bash
DRIVE="${XIAOBAN_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `xiaoban`)    |
