...
Storage
-------
Bundles live in ``~/.xiaoban/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``XIAOBAN_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``xiaoban bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under XIAOBAN_HOME.

    Honors ``XIAOBAN_BUNDLES_DIR`` for tests; falls back to
    ``<XIAOBAN_HOME>/skill-bundles``.
    """
    override = os.environ.get("XIAOBAN_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_xiaoban_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `xiaoban bundles` CLI subcommand.
# ---------------------------------------------------------------------------
