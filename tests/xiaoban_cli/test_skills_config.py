"""Tests for xiaoban_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("xiaoban_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from xiaoban_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("xiaoban_cli.config.load_config")
    ...
    @patch("xiaoban_cli.config.load_config")
    @patch.dict("os.environ", {"XIAOBAN_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("XIAOBAN_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """XIAOBAN_SESSION_PLATFORM should be used when XIAOBAN_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_xiaoban_platform_takes_precedence(self, tmp_path, monkeypatch):
        """XIAOBAN_PLATFORM should win over XIAOBAN_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("XIAOBAN_HOME", str(tmp_path))
        monkeypatch.setenv("XIAOBAN_PLATFORM", "telegram")
        monkeypatch.setenv("XIAOBAN_SESSION_PLATFORM", "discord")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from xiaoban_cli.skills_config import _get_categories
        skills = [
