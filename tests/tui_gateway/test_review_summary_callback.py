    ...
    with patch.dict(
        "sys.modules",
        {
            "xiaoban_constants": MagicMock(
                get_xiaoban_home=MagicMock(return_value="/tmp/xiaoban_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'xiaoban-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'xiaoban-release' patched"
    }
