...
如果你期望任务自动触发，需要运行一个 gateway（前台运行用 `xiaoban gateway`，安装为服务用 `xiaoban gateway start`）。如需单次调试，可手动触发一次 tick：`hermes cron tick`。
...
3. 在 [github.com/52707407SXG/Xiaoban-Agent](https://github.com/52707407SXG/Xiaoban-Agent) 提交 issue，并附上：
   - 任务 ID 和调度表达式
   - 投递目标
   - 预期行为与实际行为
   - 日志中的相关错误信息
