...
XIAOBAN_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env XIAOBAN_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_max_turns(tmp_path: Path, monkeypatch) -> None:
    ...
    (xiaoban_home / ".env").write_text(
        "XIAOBAN_MAX_ITERATIONS=90\nOPENROUTER_API_KEY=fresh-key\n",
        encoding="utf-8",
    )

    monkeypatch.setattr(gateway_run, "_xiaoban_home", xiaoban_home)
    monkeypatch.setenv("XIAOBAN_MAX_ITERATIONS", "9000")
    monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)

    gateway_run._reload_runtime_env_preserving_config_authority()

    assert os.environ["OPENROUTER_API_KEY"] == "fresh-key"
    assert os.environ["XIAOBAN_MAX_ITERATIONS"] == "9000"
...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    xiaoban_home = tmp_path / ".xiaoban"
    xiaoban_home.mkdir()
    (xiaoban_home / "config.yaml").write_text(yaml.safe_dump({"agent": {}}), encoding="utf-8")
    (xiaoban_home / ".env").write_text("XIAOBAN_MAX_ITERATIONS=123\n", encoding="utf-8")
    ...
    assert os.environ["XIAOBAN_MAX_ITERATIONS"] == "123"


def test_current_max_iterations_reloads_before_reading(monkeypatch) -> None:
    monkeypatch.setenv("XIAOBAN_MAX_ITERATIONS", "90")

    def _fake_reload() -> None:
        os.environ["XIAOBAN_MAX_ITERATIONS"] = "200"
