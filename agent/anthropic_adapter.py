"""Anthropic Messages API adapter for Xiaoban-Agent.

Translates between Xiaoban's internal OpenAI-style message format and
...
# Xiaoban effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Xiaoban historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``xiaoban models``) never mutates ``~/.xiaoban/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Xiaoban)
      2. CLAUDE_CODE_OAUTH_TOKEN env var
      3. Claude Code credentials (~/.claude.json or ~/.claude/.credentials.json)
         — with automatic refresh if expired and a refresh token is available
      4. Anthropic credential_pool OAuth entry (~/.xiaoban/auth.json)
    ...
    # 1. Xiaoban-managed OAuth/setup token env var
    ...
    # 4. Xiaoban credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration Xiaoban configs.
...
# ── Xiaoban-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.xiaoban/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_XIAOBAN_OAUTH_FILE = get_xiaoban_home() / ".anthropic_oauth.json"
...
def run_xiaoban_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Xiaoban-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Xiaoban with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from xiaoban_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_xiaoban_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Xiaoban-managed OAuth credentials from ~/.xiaoban/.anthropic_oauth.json."""
    if _XIAOBAN_OAUTH_FILE.exists():
        try:
            data = json.loads(_XIAOBAN_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Xiaoban OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See xiaoban-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for m in result:
        if m["role"] == "assistant" and isinstance(m["content"], list):
            ...
            # replaying it verbatim.  See xiaoban-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See xiaoban-agent#13848 (Kimi) and
    xiaoban-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Xiaoban-Agent", "Claude Code")
                text = text.replace("Xiaoban agent", "Claude Code")
                text = text.replace("xiaoban-agent", "claude-code")
        ...
        #      a) bare Xiaoban-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Xiaoban surfaces in its CLI. We
