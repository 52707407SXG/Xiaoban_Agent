...
//!
//! Driven when the installer is launched as `Xiaoban-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Xiaoban desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `xiaoban update`
//!      or repair bootstrap can race locked files),
//!   2. run `xiaoban update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in xiaoban_cli/main.py),
//!   3. run `xiaoban desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `xiaoban update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_xiaoban) and
...
/// `xiaoban update` exit code meaning "another xiaoban process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// xiaoban_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `xiaoban update`'s own guard decide.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating Xiaoban"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    let xiaoban_home = crate::paths::xiaoban_home();
    let install_root = xiaoban_home.join("xiaoban-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_xiaoban`
    ...
    let xiaoban = resolve_xiaoban(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the xiaoban CLI under {}. Is Xiaoban installed? \
    ...
    // async on Windows. If it still holds the venv shim, `xiaoban update`
    ...
    // ---- stage 1: xiaoban update -----------------------------------------
    // Pass --branch so `xiaoban update` targets the branch this installer was
    ...
    // without --branch, `xiaoban update` switches the checkout to `main` (a
    ...
    // --force skips `xiaoban update`'s Windows running-exe guard (which would
    ...
    // time `xiaoban update` runs there is no legitimate xiaoban.exe to protect,
    // and the guard would only produce a false "Xiaoban is still running" stop.
    ...
    // Retry-once for the update-boundary crash. `xiaoban update` lazily imports
    ...
    // second `xiaoban update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Xiaoban" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Xiaoban is still running. Close all Xiaoban windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "xiaoban update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::xiaoban_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 2: xiaoban desktop --build-only ----------------------------
    // `xiaoban update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `xiaoban update` already does above, and `xiaoban update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `xiaoban desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Xiaoban manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_xiaoban_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Xiaoban to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend xiaoban.exe (or the desktop Xiaoban.exe
            ...
            // force-kill every Xiaoban.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Xiaoban still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_xiaoban();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_xiaoban(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Xiaoban.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Xiaoban.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `xiaoban.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `xiaoban.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\xiaoban.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startXiaoban()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// xiaoban.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named xiaoban.exe.)
fn force_kill_other_xiaoban() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `xiaoban <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv xiaoban shim under an install root, regardless of existence.
fn venv_xiaoban(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("xiaoban.exe")
    } else {
        install_root.join("venv").join("bin").join("xiaoban")
    }
}

/// Resolve the xiaoban CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `xiaoban` on PATH.
fn resolve_xiaoban(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_xiaoban(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "xiaoban.exe" } else { "xiaoban" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let xiaoban_home = crate::paths::xiaoban_home();
    let mut envs = vec![(
        "XIAOBAN_HOME".to_string(),
        xiaoban_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        xiaoban_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_xiaoban_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Xiaoban.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.xiaoban-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.xiaoban-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::xiaoban_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_xiaoban_is_under_install_root() {
        let root = Path::new("/x/xiaoban-agent");
        let shim = venv_xiaoban(root);
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_xiaoban(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/xiaoban-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".xiaoban-update-in-progress");
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Xiaoban.app"]),
            Some(PathBuf::from("/Applications/Xiaoban.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Xiaoban.app");
        let tmp = base.join("Xiaoban.app.xiaoban-update-new");
        let old = base.join("Xiaoban.app.xiaoban-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Xiaoban.app");
        let tmp = base.join("Xiaoban.app.xiaoban-update-new"); // intentionally absent
        let old = base.join("Xiaoban.app.xiaoban-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Xiaoban.app");
        let tmp = base.join("Xiaoban.app.xiaoban-update-new"); // absent
        let old = base.join("Xiaoban.app.xiaoban-update-old");
        write_marker(&target, "OLD");
