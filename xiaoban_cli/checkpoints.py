"""`xiaoban checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.xiaoban/checkpoints/``.  Actions:

    xiaoban checkpoints               # same as `status`
    xiaoban checkpoints status        # total size, project count, breakdown
    xiaoban checkpoints list          # per-project checkpoint counts + workdir
    xiaoban checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    xiaoban checkpoints clear [-f]    # nuke the entire base (asks first)
    xiaoban checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    xiaoban checkpoints
    xiaoban checkpoints prune --retention-days 3 --max-size-mb 200
    xiaoban checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: xiaoban checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``xiaoban checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `xiaoban checkpoints` → status
