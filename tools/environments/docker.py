...
from tools.environments.local import _XIAOBAN_PROVIDER_ENV_BLOCKLIST
...
def _load_xiaoban_env_vars() -> dict[str, str]:
    """Load ~/.xiaoban/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Xiaoban profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale xiaoban-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=xiaoban-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Xiaoban process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=xiaoban-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a xiaoban process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Xiaoban
    process leaves its container behind permanently because there's no
    subsequent Xiaoban process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=xiaoban-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=xiaoban-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``XIAOBAN_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("XIAOBAN_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using XIAOBAN_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'xiaoban'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``xiaoban-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.xiaoban/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. xiaoban-agent:latest) already use /init as PID 1
        ...
        container_name = f"xiaoban-{uuid.uuid4().hex[:8]}"
        # Labels make xiaoban-created containers identifiable to:
        #   * the orphan reaper (`xiaoban-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`xiaoban-task-id`, `xiaoban-profile`)
        #   * operators running `docker ps --filter label=xiaoban-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Xiaoban profile is captured at
        ...
        label_args = [
            "--label", "xiaoban-agent=1",
            "--label", f"xiaoban-task-id={task_label}",
            "--label", f"xiaoban-profile={profile_name}",
        ]
        ...
        self._labels = {
            "xiaoban-agent": "1",
            "xiaoban-task-id": task_label,
            "xiaoban-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Xiaoban process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Xiaoban secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _XIAOBAN_PROVIDER_ENV_BLOCKLIST)
        xiaoban_env = _load_xiaoban_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = xiaoban_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("xiaoban-task-id", "")
        profile_label = self._labels.get("xiaoban-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"xiaoban-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``xiaoban-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=xiaoban-agent=1",
                    "--filter", f"label=xiaoban-task-id={task_label}",
                    "--filter", f"label=xiaoban-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Xiaoban process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Xiaoban exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Xiaoban process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Xiaoban startup. That covers the
        ...
        # outlives Xiaoban processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"xiaoban-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``xiaoban /quit``
