...
def _restore_tool_modules():
    original_xiaoban_home = os.environ.get("XIAOBAN_HOME")
    original_modules = {
        name: module
        ...
        or name == "xiaoban_cli"
        or name.startswith("xiaoban_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_xiaoban_home is None:
            os.environ.pop("XIAOBAN_HOME", None)
        else:
            os.environ["XIAOBAN_HOME"] = original_xiaoban_home
        _reset_modules(("tools", "xiaoban_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    xiaoban_cli = types.ModuleType("xiaoban_cli")
    xiaoban_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["xiaoban_cli"] = xiaoban_cli
    xiaoban_home = tmp_path / "xiaoban-home"
    os.environ["XIAOBAN_HOME"] = str(xiaoban_home)
    sys.modules["xiaoban_cli.config"] = types.SimpleNamespace(
        get_xiaoban_home=lambda: xiaoban_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="xiaoban-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": xiaoban_home / "modal_snapshots.json",
