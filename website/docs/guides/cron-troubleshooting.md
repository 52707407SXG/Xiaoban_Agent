...
description: "Diagnose and fix common Xiaoban cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`xiaoban gateway` for foreground, or `xiaoban gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `xiaoban cron tick`.
...
```bash
date
xiaoban cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.xiaoban/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.xiaoban/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.xiaoban/.env` |
...
| `local` | Write access to `~/.xiaoban/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `xiaoban cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `xiaoban skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `xiaoban skills list`.
...
2. `~/.xiaoban/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `xiaoban cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Xiaoban config directory). Verify:
```bash
ls ~/.xiaoban/scripts/your-script.py   # Must exist
xiaoban cron edit <job_id> --script ~/.xiaoban/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `xiaoban skills install <skill-name>`.
...
Jobs are stored in `~/.xiaoban/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.xiaoban/cron/jobs.json
chmod 600 ~/.xiaoban/cron/jobs.json   # Your user should own it
```
...
```bash
xiaoban cron list                    # Show all jobs, states, next_run times
xiaoban cron run <job_id>            # Schedule for next tick (for testing)
xiaoban cron edit <job_id>           # Fix configuration issues
xiaoban logs                         # View recent Xiaoban logs
xiaoban skills list                  # Verify installed skills
```
...
1. Run the job with `xiaoban cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.xiaoban/logs/agent.log` for scheduler messages and `~/.xiaoban/logs/errors.log` for warnings
3. Open an issue at [github.com/52707407SXG/Xiaoban_Agent](https://github.com/52707407SXG/Xiaoban_Agent) with:
