...
| | |
|---|---|
| Source | Optional — install with `xiaoban skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Xiaoban core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Xiaoban native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Xiaoban native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Xiaoban native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Xiaoban usage patterns
...
- Do not silently choose Parallel when Xiaoban native tools are already sufficient.
