...
| | |
|---|---|
| Source | Optional — install with `xiaoban skills install official/productivity/siyuan` |
...
3. Store it in `${XIAOBAN_HOME:-~/.xiaoban}/.env`:
   ```
...
```yaml
# In ~/.xiaoban/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
