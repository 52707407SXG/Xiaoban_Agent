# Xiaoban Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: xiaoban-achievements plugin
...
### `GET /api/plugins/xiaoban-achievements/achievements`
...
### `POST /api/plugins/xiaoban-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/xiaoban-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/xiaoban-achievements/overview`
...
- Persist plugin data under:
  - `~/.xiaoban/plugins/xiaoban-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.xiaoban/plugins/xiaoban-achievements/scan_checkpoint.json`
...
- `~/.xiaoban/plugins/xiaoban-achievements/`
