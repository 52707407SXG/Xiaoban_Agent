...
```bash
xiaoban teams-pipeline validate
```
...
```bash
xiaoban teams-pipeline token-health
xiaoban teams-pipeline token-health --force-refresh
```
...
```bash
xiaoban teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
xiaoban teams-pipeline maintain-subscriptions
xiaoban teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Xiaoban cron (recommended if you already run the Xiaoban gateway)

Xiaoban ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.xiaoban/scripts/`. First create the script:

```bash
mkdir -p ~/.xiaoban/scripts
cat > ~/.xiaoban/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec xiaoban teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.xiaoban/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
xiaoban cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/xiaoban-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Xiaoban Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=xiaoban
EnvironmentFile=/etc/xiaoban/env
ExecStart=/usr/local/bin/xiaoban teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/xiaoban-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Xiaoban Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now xiaoban-teams-pipeline-maintain.timer
systemctl list-timers xiaoban-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/xiaoban teams-pipeline maintain-subscriptions >> /var/log/xiaoban/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.xiaoban/.env` at the top of a wrapper script that crontab calls.
...
```bash
xiaoban teams-pipeline subscriptions   # should show expirationDateTime advanced
xiaoban teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
xiaoban teams-pipeline list
xiaoban teams-pipeline list --status failed
xiaoban teams-pipeline show <job-id>
```

### Replay a stored job

```bash
xiaoban teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
xiaoban teams-pipeline fetch --meeting-id <meeting-id>
xiaoban teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `xiaoban teams-pipeline maintain-subscriptions --dry-run`
- inspect `xiaoban teams-pipeline list --status failed`
...
- run `xiaoban teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `xiaoban teams-pipeline run`
...
- [ ] `xiaoban teams-pipeline validate` returns an OK snapshot
- [ ] `xiaoban teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Xiaoban cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
