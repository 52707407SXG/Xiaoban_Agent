...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'xiaoban:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.xiaobanDesktop?.saveImageFromUrl) {
        if (await window.xiaobanDesktop.saveImageFromUrl(src)) {
