...
Config file: `$XIAOBAN_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `xiaoban` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `xiaoban-{identity}` → `xiaoban-coder`). |
...
All Supermemory API calls send `x-sm-source: xiaoban`, and document writes stamp
`metadata.sm_source: xiaoban`. This is a **functional routing key, not telemetry**:
it groups Xiaoban-written memories into a dedicated "Xiaoban" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Xiaoban profile:

```json
{
  "container_tag": "xiaoban-{identity}"
}
```

For a profile named `coder`, this resolves to `xiaoban-coder`. The default profile resolves to `xiaoban-default`. Without `{identity}`, all profiles share the same container.
