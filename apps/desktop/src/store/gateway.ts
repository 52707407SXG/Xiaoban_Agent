import type { ConnectionState, GatewayEvent } from '@xiaoban/shared'
...
const isOpen = (gateway: XiaobanGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<XiaobanGateway | null>(null)
...
let primaryGateway: XiaobanGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: XiaobanGateway | null, profile = 'default'): void {
...
export function activeGateway(): XiaobanGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<XiaobanGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
