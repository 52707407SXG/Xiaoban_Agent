...
await build({
  ...
  // Skip the prebuilt @xiaoban/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@xiaoban/ink': resolve(root, 'packages/xiaoban-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The xiaoban_cli launcher
