...
Opt-in toolset (not loaded in the default `hermes-cli` set). Add via `--toolsets video_gen` or enable it in `xiaoban tools` → Video Generation, which also walks you through picking a backend.
