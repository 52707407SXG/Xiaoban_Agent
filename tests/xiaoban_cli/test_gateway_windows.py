"""Tests for xiaoban_cli.gateway_windows."""

from pathlib import Path

import pytest

import xiaoban_cli.gateway as gateway
import xiaoban_cli.gateway_windows as gateway_windows
import xiaoban_cli.setup as setup
...
def test_exec_schtasks_decodes_with_replace_errors(monkeypatch):
    ...
    code, out, err = gateway_windows._exec_schtasks(["/Query", "/TN", "Xiaoban_Gateway"])
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda xiaoban_home: "")
    monkeypatch.setattr("xiaoban_cli.config.get_xiaoban_home", lambda: str(xiaoban_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "xiaoban_cli.main"]
    assert cwd == str(xiaoban_home.resolve())
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_xiaoban_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".xiaoban"
        home.mkdir()
        monkeypatch.setattr("xiaoban_cli.config.get_xiaoban_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".xiaoban"
        project = tmp_path / "checkout"
        monkeypatch.setattr("xiaoban_cli.config.get_xiaoban_home", lambda: missing)
        assert gateway_windows._stable_gateway_working_dir(project) == str(project)


def test_write_task_script_anchors_cmd_cd_at_xiaoban_home(monkeypatch, tmp_path):
    project = tmp_path / "project"
    xiaoban_home = tmp_path / "xiaoban-home"
    xiaoban_home.mkdir()
    ...
    assert f"cd /d {gateway_windows._quote_cmd_script_arg(str(xiaoban_home.resolve()))}" in content
...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids):
    script_path = tmp_path / "Xiaoban_Gateway_alice.cmd"
    startup_entry = tmp_path / "Startup" / "Xiaoban_Gateway_alice.cmd"
    calls = []

    monkeypatch.setattr(gateway_windows, "_prompt_install_choices", lambda *args, **kwargs: (False, True))
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Xiaoban_Gateway_alice")
...
def test_gateway_cmd_script_uses_pythonw_without_replace_or_start_churn(monkeypatch):
    ...
    content = gateway_windows._build_gateway_cmd_script(
        r"C:\\Xiaoban\\xiaoban-agent\\venv\\Scripts\\python.exe",
        r"C:\\Xiaoban\\xiaoban-agent",
        r"C:\\XiaobanHome\\profiles\\alice",
        "--profile alice",
    )
...
def test_elevated_gateway_command_uses_pythonw_hidden_console(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows, "_derive_venv_pythonw", lambda exe: exe.replace("python.exe", "pythonw.exe"))
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Xiaoban\venv\Scripts\python.exe")
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    ok, detail = gateway_windows._install_scheduled_task("Xiaoban_Gateway_alice", script_path)

    assert ok is True
    assert "/Change" not in [arg for call in calls for arg in call]
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "Xiaoban_Gateway_alice")
...
def test_install_scheduled_task_success_start_now_uses_direct_spawn_not_task_run(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_install_scheduled_task",
        lambda task_name, script_path: (True, "Created Scheduled Task 'Xiaoban_Gateway_alice'"),
    )
...
def test_install_access_denied_launches_elevated_install_before_startup_fallback(monkeypatch, tmp_path, capsys):
    ...
    assert "Launched elevated Xiaoban gateway install prompt" in out
...
def test_install_startup_fallback_does_not_auto_spawn_when_gateway_stopped(monkeypatch, tmp_path, capsys):
    ...
    assert "xiaoban --profile alice gateway start" in out
...
def test_uninstall_access_denied_prompts_before_elevating(monkeypatch, tmp_path, capsys):
    ...
    assert "UAC is Windows' admin approval prompt" in out
    assert "Launched elevated Xiaoban gateway uninstall prompt" in out
...
# resume_pending=True) never fires when `xiaoban gateway stop` kills the
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("xiaoban_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("xiaoban_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    `xiaoban gateway start` fails with "another instance is running".
    """
    pid = 77777
    events = []
    ...
    monkeypatch.setattr("xiaoban_cli.gateway._get_restart_drain_timeout", lambda: 1.0)
