...
Plugins built on Xiaoban-Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Xiaoban
...
:class:`~xiaoban_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Xiaoban' main config
...
handles every provider, fallback chain, and per-task override Xiaoban
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`xiaoban_cli.plugins.PluginContext`
