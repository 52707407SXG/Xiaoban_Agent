...
def backup_env(monkeypatch, tmp_path):
    """Isolate XIAOBAN_HOME + reload modules so every test starts clean."""
    ...
    # Reload so get_xiaoban_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd xiaoban_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under XIAOBAN_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level XIAOBAN_DIR picks up the tmp HOME."""
    import xiaoban_constants
    importlib.reload(xiaoban_constants)
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.xiaoban/cron/jobs.json
