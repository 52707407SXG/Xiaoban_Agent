# Xiaoban-Agent — Web UI

Browser-based dashboard for managing Xiaoban-Agent configuration, API keys, and monitoring active sessions.
...
cd ../
python -m xiaoban_cli.main web --no-open
...
`xiaoban dashboard` on port 9119 serves the **built** bundle from `xiaoban_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
This outputs to `../xiaoban_cli/web_dist/`, which the FastAPI server serves as a static SPA. The built assets are included in the Python package via `pyproject.toml` package-data.
...
- **Do not introduce new `uppercase`** (the literal Tailwind class) in `xiaoban-agent/web/src`. Prefer `text-display` for new brand chrome. Legacy `uppercase` call sites (e.g. `components/ui/label.tsx`, `card.tsx`) remain until migrated.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Xiaoban-Agent” — mixed case, no Mondwest, no `text-display` |
