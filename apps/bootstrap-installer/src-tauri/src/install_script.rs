...
//!
//! Resolution order:
//!   1. Dev shortcut: a sibling repo checkout via $XIAOBAN_SETUP_DEV_REPO_ROOT
...
//! Electron app's APP_ROOT/../.. trick, because Xiaoban-Setup.exe is meant
...
///
/// `pin` is the commit-or-branch from either Xiaoban-Setup's build-time
...
pub async fn resolve(
    ...
    // 1. Dev shortcut.
    if let Ok(repo_root) = std::env::var("XIAOBAN_SETUP_DEV_REPO_ROOT") {
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    let url = format!(
        "https://raw.githubusercontent.com/52707407SXG/Xiaoban-Agent/{}/scripts/{}",
        commit_or_ref,
        kind.filename()
    );
    ...
    let response = reqwest::Client::new()
        .get(&url)
        .header("User-Agent", "xiaoban-setup/0.0.1")
        .send()
        .await
