...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("XIAOBAN_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "XIAOBAN_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "XIAOBAN_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Xiaoban-created private-chat topic lanes are marked with
    ...
    # older PTB/clients. Streaming edits stay on Xiaoban' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        Xiaoban must skip rich delivery up front and use the legacy MarkdownV2
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `xiaoban update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other Xiaoban or OpenClaw instance is running "
            "with this token, then restart the gateway with 'xiaoban gateway restart'."
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # readable by the Xiaoban process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("XIAOBAN_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("XIAOBAN_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("XIAOBAN_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("XIAOBAN_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("XIAOBAN_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }

            disable_fallback = (os.getenv("XIAOBAN_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
            ...
            try:
                ...
                from xiaoban_cli.commands import telegram_menu_commands
    ...
    def _build_provider_keyboard(self, providers: list):
        ...
        CLI ``xiaoban model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from xiaoban_cli.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from xiaoban_cli.models import PROVIDER_GROUPS
    ...
    # Scripts live in ~/.xiaoban/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".xiaoban" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@xiaoban_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Xiaoban bot profiles. A message like
...
# xiaoban_cli/{setup,gateway}.py, and the _send_telegram dispatch in
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("XIAOBAN_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
...
def interactive_setup() -> None:
    ...
    _PLATFORMS["telegram"] static dict dispatch in xiaoban_cli/gateway.py.
    """
    from xiaoban_cli import setup as _setup_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'xiaoban-agent[telegram]'",
