...
Small Node helper that bridges Xiaoban-Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Xiaoban is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Xiaoban-Sidecar-Token`)
...
The Xiaoban plugin's `xiaoban photon setup` command runs `npm install`
here automatically.
