...
@pytest.fixture
def bare_cli():
    """A XiaobanCLI with no __init__ — we only exercise the redraw helper."""
    cli = object.__new__(XiaobanCLI)
    return cli
...
class TestForceFullRedraw:
    ...
    def test_missing_app_attr_is_safe(self, bare_cli):
        # Simulate XiaobanCLI before the TUI has ever been constructed.
        bare_cli._force_full_redraw()  # must not raise
