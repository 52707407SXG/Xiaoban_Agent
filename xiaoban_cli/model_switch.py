...
- ``xiaoban_cli.providers``        -- canonical provider identity + overlays
- ``xiaoban_cli.model_normalize``  -- per-provider name formatting
...
_XIAOBAN_MODEL_WARNING = (
    "Nous Research Xiaoban 3 & 4 models are NOT agentic and are not designed "
    "for use with Xiaoban-Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Xiaoban 3 / Xiaoban 4 chat families.
# The previous substring check (`"xiaoban" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``xiaoban-brain:qwen3-14b-ctx16k`` that just
# happen to carry "xiaoban" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Xiaoban-3-Llama-3.1-70B, xiaoban-4-405b, openrouter/xiaoban3:70b
# Negative examples it must NOT match:
#   xiaoban-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_XIAOBAN_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])xiaoban[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_xiaoban_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Xiaoban 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_XIAOBAN_NON_AGENTIC_RE.search(model_name))


def _check_xiaoban_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Xiaoban 3/4 chat model."""
    if is_nous_xiaoban_non_agentic(model_name):
        return _XIAOBAN_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``xiaoban config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from xiaoban_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'xiaoban model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'xiaoban doctor' — config issues detected:"
        ...
        from xiaoban_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from xiaoban_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from xiaoban_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in xiaoban_cli.runtime_provider.resolve_runtime_provider;
    ...
    xiaoban_warn = _check_xiaoban_model_warning(new_model)
    if xiaoban_warn:
        warnings.append(xiaoban_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from xiaoban_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from xiaoban_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from xiaoban_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by xiaoban provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Xiaoban release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from xiaoban_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from xiaoban_cli.models import fetch_lmstudio_models
        from xiaoban_cli.auth import AuthError
    ...
    # --- 1. Check Xiaoban-mapped providers ---
    from xiaoban_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from xiaoban_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for xiaoban_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(xiaoban_id)
        if (
            _alias_target
            and _alias_target != xiaoban_id
        ...
        pconfig = PROVIDER_REGISTRY.get(xiaoban_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (XIAOBAN_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from xiaoban_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(xiaoban_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `xiaoban model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(xiaoban_id)
        if not model_ids:
            model_ids = curated.get(xiaoban_id, [])
            if xiaoban_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(xiaoban_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Xiaoban-only providers (nous, openai-codex, copilot, opencode-go) ---
    from xiaoban_cli.providers import XIAOBAN_OVERLAYS
    from xiaoban_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Xiaoban provider ID.
    # XIAOBAN_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Xiaoban IDs ("copilot").
    _mdev_to_xiaoban = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in XIAOBAN_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Xiaoban slug — e.g. "github-copilot" → "copilot"
        xiaoban_slug = _mdev_to_xiaoban.get(pid, pid)
        if xiaoban_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(xiaoban_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, xiaoban_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or xiaoban_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(xiaoban_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", xiaoban_slug, exc)
        ...
        # configured.
        if not has_creds and xiaoban_slug == "anthropic":
            try:
                ...
                if (xiaoban_creds and xiaoban_creds.get("accessToken")) or \
        ...
        if xiaoban_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(xiaoban_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(xiaoban_slug)
                model_ids = _ids if _ids else (curated.get(xiaoban_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(xiaoban_slug, []) or curated.get(pid, [])
        elif xiaoban_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `xiaoban model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from xiaoban_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(xiaoban_slug, []) or curated.get(pid, [])
                if xiaoban_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(xiaoban_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models] if max_models is not None else model_ids

        results.append({
            "slug": xiaoban_slug,
            "name": get_label(xiaoban_slug),
            "is_current": xiaoban_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "xiaoban",
        ...
        seen_slugs.add(xiaoban_slug.lower())
        _record_builtin_endpoint(xiaoban_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or XIAOBAN_OVERLAYS (keeps /model in sync
    # with `xiaoban model`).
    try:
        from xiaoban_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Xiaoban's canonical write key (matches
            ...
            # Xiaoban writes ``models:`` as a dict keyed by model id
            # (see xiaoban_cli/main.py::_save_custom_provider); older
            ...
            if should_probe:
                try:
                    from xiaoban_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Xiaoban's own writer uses; a hyphen variant is accepted
            ...
            # active model. Xiaoban's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, xiaoban_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`xiaoban_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from xiaoban_cli.models import fetch_openrouter_models
