...
subparser, placing the flag BEFORE the subcommand (e.g. 'xiaoban --yolo chat')
...
def _build_parser():
    """Build the xiaoban argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("xiaoban_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "xiaoban_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets XIAOBAN_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `xiaoban gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `xiaoban <argv>` must exit 0 (help) rather than
