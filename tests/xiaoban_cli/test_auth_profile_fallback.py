...
When ``XIAOBAN_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.xiaoban/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.xiaoban            (has its own auth.json fixture)
    * Profile     -> tmp_path/.xiaoban/profiles/coder   (active, XIAOBAN_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Xiaoban is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from xiaoban_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    xiaoban_home = tmp_path / "classic"
    xiaoban_home.mkdir()
    monkeypatch.setenv("XIAOBAN_HOME", str(xiaoban_home))

    _write(xiaoban_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (XIAOBAN_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.xiaoban as the "real home" — which is NOT equal
    # to the XIAOBAN_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    xiaoban_home = tmp_path / "classic"
    ...
    _write(xiaoban_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from xiaoban_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: XIAOBAN_HOME is set to a custom path that is NOT under
    # ~/.xiaoban/profiles/ — get_default_xiaoban_root() returns XIAOBAN_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from xiaoban_cli.auth import read_credential_pool, write_credential_pool
