...
[`COMMAND_REGISTRY`](https://github.com/52707407SXG/Xiaoban-Agent/blob/main/hermes_cli/commands.py)
...
When Hermes adds new commands (e.g. after `xiaoban update`), regenerate
...
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Xiaoban-Agent what time is it?`). In channels, Hermes replies in a thread attached to that message. |
