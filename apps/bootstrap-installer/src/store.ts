...
export const $xiaobanHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, xiaobanHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_xiaoban_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $xiaobanHome.set(xiaobanHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Xiaoban" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Xiaoban-Setup.exe --update);
...
export async function launchXiaobanDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_xiaoban_desktop', { installRoot })
}
