...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      xiaoban-agent = self'.packages.default;
      xiaobanVenv = xiaoban-agent.xiaobanVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "xiaoban-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${xiaobanVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "xiaoban-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "xiaoban-build-package" { } ''
          echo "PASS: package built at ${xiaoban-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "xiaoban-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "xiaoban-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${xiaoban-agent}/bin/xiaoban || (echo "FAIL: xiaoban binary missing"; exit 1)
          test -x ${xiaoban-agent}/bin/xiaoban-agent || (echo "FAIL: xiaoban-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${xiaoban-agent}/bin/xiaoban version 2>&1 | grep -qi "xiaoban" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "xiaoban-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in xiaoban xiaoban-agent xiaoban-acp; do
            test -x ${xiaoban-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "xiaoban-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking xiaoban --help ==="
          ${xiaoban-agent}/bin/xiaoban --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${xiaoban-agent}/bin/xiaoban --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "xiaoban-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${xiaoban-agent}/share/xiaoban-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${xiaoban-agent}/share/xiaoban-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "XIAOBAN_BUNDLED_SKILLS" ${xiaoban-agent}/bin/xiaoban || \
            (echo "FAIL: XIAOBAN_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: XIAOBAN_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "xiaoban-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${xiaoban-agent}/share/xiaoban-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${xiaoban-agent}/share/xiaoban-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "XIAOBAN_BUNDLED_PLUGINS" ${xiaoban-agent}/bin/xiaoban || \
            (echo "FAIL: XIAOBAN_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: XIAOBAN_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "xiaoban-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${xiaoban-agent}/share/xiaoban-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${xiaoban-agent}/share/xiaoban-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${xiaoban-agent}/share/xiaoban-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "XIAOBAN_BUNDLED_LOCALES" ${xiaoban-agent}/bin/xiaoban || \
            (echo "FAIL: XIAOBAN_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: XIAOBAN_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (XIAOBAN_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && XIAOBAN_BUNDLED_LOCALES=${xiaoban-agent}/share/xiaoban-agent/locales \
            ${xiaobanVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with XIAOBAN_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${xiaobanVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${xiaobanVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without XIAOBAN_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "xiaoban-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${xiaoban-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${xiaoban-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "XIAOBAN_TUI_DIR" ${xiaoban-agent}/bin/xiaoban || \
            (echo "FAIL: XIAOBAN_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: XIAOBAN_TUI_DIR set in wrapper"
        ...
        # Verify XIAOBAN_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        xiaoban-node = pkgs.runCommand "xiaoban-node-version" { } ''
          set -e
          echo "=== Checking XIAOBAN_NODE in wrapper ==="
          grep -q "XIAOBAN_NODE" ${xiaoban-agent}/bin/xiaoban || \
            (echo "FAIL: XIAOBAN_NODE not set in wrapper"; exit 1)
          echo "PASS: XIAOBAN_NODE present in wrapper"

          XIAOBAN_NODE=$(sed -n "s/^export XIAOBAN_NODE='\(.*\)'/\1/p" ${xiaoban-agent}/bin/xiaoban)
          test -x "$XIAOBAN_NODE" || (echo "FAIL: XIAOBAN_NODE=$XIAOBAN_NODE not executable"; exit 1)
          echo "PASS: XIAOBAN_NODE executable at $XIAOBAN_NODE"

          NODE_MAJOR=$("$XIAOBAN_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All XIAOBAN_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify XIAOBAN_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "xiaoban-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(XIAOBAN_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking XIAOBAN_MANAGED guards ==="
          check_blocked "config set" ${xiaoban-agent}/bin/xiaoban config set model foo
          check_blocked "config edit" ${xiaoban-agent}/bin/xiaoban config edit
        ...
        extra-python-packages = let
          ...
          xiaobanWithExtra = xiaoban-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "xiaoban-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${xiaobanWithExtra}/bin/xiaoban || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${xiaobanWithExtra}/bin/xiaoban || \
          ...
          if grep -q "PYTHONPATH" ${xiaoban-agent}/bin/xiaoban; then
        ...
        extra-dependency-groups = let
          xiaobanWithGroups = xiaoban-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "xiaoban-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${xiaobanWithGroups}"
          echo "venv: ${xiaobanWithGroups.xiaobanVenv}"
        ...
        messaging-variant = pkgs.runCommand "xiaoban-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.xiaobanVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "xiaoban-config-roundtrip" {
          ...
          merge_and_load() {
            local xiaoban_home="$1"
            export XIAOBAN_HOME="$xiaoban_home"
            ${configMergeScript} ${nixSettings} "$xiaoban_home/config.yaml"
            ${xiaobanVenv}/bin/python3 -c '
import json, sys
