...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `xiaoban-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.xiaoban/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.xiaoban/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because xiaoban uses an internal scheduler, not system crontab.
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("XIAOBAN_INTERACTIVE", raising=False)
        monkeypatch.delenv("XIAOBAN_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("XIAOBAN_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("XIAOBAN_INTERACTIVE", false_like_value)
        monkeypatch.delenv("XIAOBAN_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("XIAOBAN_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["XIAOBAN_INTERACTIVE", "XIAOBAN_GATEWAY_SESSION", "XIAOBAN_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("XIAOBAN_INTERACTIVE", "XIAOBAN_GATEWAY_SESSION", "XIAOBAN_EXEC_ASK"):
...
class TestResolveModelOverride:
    ...
    def test_keeps_bare_custom_when_a_named_entry_exists(self, monkeypatch):
        import xiaoban_cli.runtime_provider as rp_mod
