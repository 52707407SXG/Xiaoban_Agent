...
class TestTelegramYamlConfigLoading:
    ...
    def test_top_level_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        xiaoban_home = self._write_config(tmp_path, "telegram:\n  reply_to_mode: off\n")
    ...
    def test_extra_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        xiaoban_home = self._write_config(
            tmp_path, "telegram:\n  extra:\n    reply_to_mode: \"off\"\n"
        )
        monkeypatch.setenv("XIAOBAN_HOME", str(xiaoban_home))
        monkeypatch.delenv("TELEGRAM_REPLY_TO_MODE", raising=False)
...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/52707407SXG/Xiaoban_Agent/issues/23994:
    reply_to_mode 'off' was ignored when sending via Xiaoban-created DM topic
