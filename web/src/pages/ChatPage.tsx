/**
 * ChatPage — embeds `xiaoban --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (xiaoban_cli/web_server.py)                          .
 *          ▼                                                              .
...
import { XIAOBAN_BASE_PATH, buildWsAuthParam } from "@/lib/api";
...
function buildWsUrl(
  ...
  // Profile-scoped chat: the PTY child gets XIAOBAN_HOME pointed at the
  // selected profile, so the conversation runs with that profile's model,
  // skills, memory, and sessions (see web_server._resolve_chat_argv).
  if (profile) qs.set("profile", profile);
  return `${proto}//${window.location.host}${XIAOBAN_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__XIAOBAN_SESSION_TOKEN__ &&
    !window.__XIAOBAN_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `xiaoban dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__XIAOBAN_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Xiaoban TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[xiaoban-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__XIAOBAN_AUTH_REQUIRED__``) this
    ...
    // forwarding them into Xiaoban. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="xiaoban-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
