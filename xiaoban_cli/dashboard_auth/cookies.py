...
Three cookies in play:
  - xiaoban_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - xiaoban_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         Nous Portal issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / xiaoban
  ...
  - xiaoban_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/xiaoban) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/xiaoban`` covers
...
setter wrote ``__Secure-xiaoban_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "xiaoban_session_at"
SESSION_RT_COOKIE = "xiaoban_session_rt"
PKCE_COOKIE = "xiaoban_session_pkce"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /xiaoban`` we want ``Path=/xiaoban`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (xiaoban #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/xiaoban``)
