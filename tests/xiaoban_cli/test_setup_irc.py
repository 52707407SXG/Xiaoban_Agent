"""Tests for IRC gateway configuration via `xiaoban setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Xiaoban install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Xiaoban install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#xiaoban")
            monkeypatch.setenv("IRC_NICKNAME", "xiaoban-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `xiaoban setup gateway` experience with IRC present."""
