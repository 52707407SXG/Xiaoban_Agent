...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Xiaoban-Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Xiaoban instantly when PRs are opened or updated.
:::
...
- **Xiaoban-Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  xiaoban gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.xiaoban/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Xiaoban can access GitHub. Start a chat:
...
```
Run: gh pr list --repo 52707407SXG/Xiaoban-Agent --state open --limit 3
```
...
Still in the chat, ask Xiaoban to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo 52707407SXG/Xiaoban-Agent
```
...
A skill gives Xiaoban consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.xiaoban/skills/code-review
```

Create `~/.xiaoban/skills/code-review/SKILL.md`:
...
Verify it loaded — start `xiaoban` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Xiaoban your team's standards:
...
2. Teach Xiaoban your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
xiaoban gateway status    # Is the gateway running?
xiaoban cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/guides/build-a-xiaoban-plugin)** — wrap the review logic into a shareable plugin
