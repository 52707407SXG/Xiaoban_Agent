...
on Japanese locales, etc.).  Xiaoban's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``xiaoban_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Xiaoban is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``XIAOBAN_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("XIAOBAN_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Xiaoban-managed Git install directories so
    ...
    # already-running shells don't see the change — which means xiaoban
    ...
    # install without Xiaoban-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Xiaoban's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Xiaoban's
    ``xiaoban config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Xiaoban (User env var in Windows Settings, or
    export in a PowerShell profile) and Xiaoban picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Xiaoban-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\xiaoban\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``xiaoban`` from right after install) retain their old PATH.

    Any subprocess Xiaoban spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Xiaoban-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "xiaoban", "git", "cmd"),
        os.path.join(local_appdata, "xiaoban", "git", "bin"),
        os.path.join(local_appdata, "xiaoban", "git", "usr", "bin"),
        # Xiaoban venv Scripts directory — host of the xiaoban.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes xiaoban, but harmless to include.
        os.path.join(local_appdata, "xiaoban", "xiaoban-agent", "venv", "Scripts"),
