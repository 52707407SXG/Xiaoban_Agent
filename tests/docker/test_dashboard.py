"""Harness: dashboard opt-in via XIAOBAN_DASHBOARD.

Today (tini): dashboard starts once when XIAOBAN_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``xiaoban`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without XIAOBAN_DASHBOARD, no dashboard process should be running."""
    subprocess.run(
    ...
    time.sleep(5)
    r = docker_exec(container_name, "pgrep", "-f", "xiaoban dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without XIAOBAN_DASHBOARD"
    )
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without XIAOBAN_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `xiaoban doctor` and any other health check).
    ...
    XIAOBAN_DASHBOARD is unset, so the slot reflects reality.
    """
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name, built_image,
         "sleep", "60"],
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without XIAOBAN_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with XIAOBAN_DASHBOARD=1, s6-svstat reports the slot as up."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         ...
         # XIAOBAN_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
         ...
         "-e", "XIAOBAN_DASHBOARD_BASIC_AUTH_USERNAME=admin",
         "-e", "XIAOBAN_DASHBOARD_BASIC_AUTH_PASSWORD=test-dashboard-pw",
...
def test_dashboard_opt_in_starts(
    ...
    """With XIAOBAN_DASHBOARD=1, a dashboard process should be visible."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "XIAOBAN_DASHBOARD=1",
    ...
    ok, _ = _poll(
        container_name, "pgrep -f 'xiaoban dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with XIAOBAN_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """XIAOBAN_DASHBOARD_PORT changes the dashboard's listen port."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "XIAOBAN_DASHBOARD=1", "-e", "XIAOBAN_DASHBOARD_PORT=9120",
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "xiaoban dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as xiaoban, so the
    # xiaoban user can kill it (same UID).
...
# whenever `XIAOBAN_DASHBOARD_HOST` was non-loopback, silently disabling
...
"""
    ...
    probe = (
        "/opt/xiaoban/.venv/bin/python - <<'PY'\n"
        f"{py_program}"
        "PY"
    )
    end = time.monotonic() + deadline_s
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``XIAOBAN_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "XIAOBAN_DASHBOARD=1",
         "-e", "XIAOBAN_DASHBOARD_HOST=0.0.0.0",
         "-e", "XIAOBAN_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
         built_image, "sleep", "120"],
        check=True, capture_output=True, timeout=30,
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"XIAOBAN_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and XIAOBAN_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``XIAOBAN_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "XIAOBAN_DASHBOARD=1",
         "-e", "XIAOBAN_DASHBOARD_HOST=0.0.0.0",
         "-e", "XIAOBAN_DASHBOARD_INSECURE=1",
         built_image, "sleep", "120"],
        check=True, capture_output=True, timeout=30,
    )
