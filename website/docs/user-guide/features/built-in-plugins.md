...
description: "Plugins shipped with Xiaoban-Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Xiaoban ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.xiaoban/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Xiaoban Plugin](/guides/build-a-xiaoban-plugin) to write your own.
...
2. **User** — `~/.xiaoban/plugins/<name>/`
3. **Project** — `./.xiaoban/plugins/<name>/` (requires `XIAOBAN_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `xiaoban_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `xiaoban memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `xiaoban plugins list` and the interactive `xiaoban plugins` UI), but none load until you explicitly enable them:

```bash
xiaoban plugins enable disk-cleanup
```

Or via `~/.xiaoban/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Xiaoban. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
xiaoban plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `xiaoban plugins enable <name>`.
...
| `xiaoban-achievements` | dashboard tab | Steam-style collectible badges generated from your real Xiaoban session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `xiaoban memory` and `xiaoban plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `XIAOBAN_HOME` or `/tmp/xiaoban-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under XIAOBAN_HOME | always | Never |
...
**State** — everything lives at `$XIAOBAN_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `XIAOBAN_HOME` or `/tmp/xiaoban-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `xiaoban plugins enable disk-cleanup` (or check the box in `xiaoban plugins`).

**Disabling again:** `xiaoban plugins disable disk-cleanup`.
...
**Enabling:** `xiaoban plugins enable security-guidance` (or check the box in `xiaoban plugins`).

**Disabling again:** `xiaoban plugins disable security-guidance`.
...
Traces Xiaoban turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Xiaoban' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `xiaoban logs`.
...
```bash
xiaoban tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Xiaoban and the next turn ships a trace.
...
Then put the credentials in `~/.xiaoban/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Xiaoban turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `XIAOBAN_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Xiaoban session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `xiaoban chat` session lives under one Langfuse session.

**Verify:**

```bash
xiaoban plugins list                 # observability/langfuse should show "enabled"
xiaoban chat -q "hello"              # check the Langfuse UI for a "Xiaoban turn" trace
```
...
| `XIAOBAN_LANGFUSE_RELEASE` | — | Release/version tag |
| `XIAOBAN_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `XIAOBAN_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `XIAOBAN_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Xiaoban-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Xiaoban-prefixed wins when both are set.
...
**Disabling:** `xiaoban plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.xiaoban/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
xiaoban plugins enable google_meet
...
**Disabling:** `xiaoban plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.xiaoban/cache/google_meet/` until you remove them.

### xiaoban-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Xiaoban session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Xiaoban feature changes.

**How it works:**

- Scans your entire `~/.xiaoban/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$XIAOBAN_HOME/plugins/xiaoban-achievements/state.json`
...
| Secret | Hidden until Xiaoban detects the first related signal in your history |

**API** — routes mount under `/api/plugins/xiaoban-achievements/`:
...
**State files** — live under `$XIAOBAN_HOME/plugins/xiaoban-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Xiaoban updates. |
...
**Enabling:** Nothing to enable — `xiaoban-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `xiaoban dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/xiaoban-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.xiaoban/plugins/xiaoban-achievements/` that ships no dashboard. The plugin's state files under `$XIAOBAN_HOME/plugins/xiaoban-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Xiaoban plugin — see [Build a Xiaoban Plugin](/guides/build-a-xiaoban-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.xiaoban/plugins/<name>/`
- Manifest source is reported as `bundled` in `xiaoban plugins list`
