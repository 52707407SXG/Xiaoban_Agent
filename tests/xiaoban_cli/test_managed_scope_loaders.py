...
xiaoban_cli.config.load_config, so the managed overlay has to be wired into each.
...
def homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XIAOBAN_MANAGED_DIR", str(managed))
...
def test_gateway_run_loader_honors_managed(homes, monkeypatch):
    ...
    monkeypatch.setattr(gr, "_xiaoban_home", home, raising=False)
...
def test_gateway_config_loader_honors_managed(homes, monkeypatch):
    ...
    # load_gateway_config resolves home via get_xiaoban_home() (XIAOBAN_HOME env).
...
def test_tui_loader_honors_managed(homes, monkeypatch):
    ...
    monkeypatch.setattr(ts, "_xiaoban_home", home, raising=False)
    monkeypatch.setattr(ts, "_cfg_cache", None, raising=False)
    monkeypatch.setattr(ts, "_cfg_mtime", None, raising=False)
    monkeypatch.setattr(ts, "get_xiaoban_home_override", lambda: None, raising=False)
    cfg = ts._load_cfg()
...
def test_logging_config_honors_managed(homes, monkeypatch):
    ...
    level, _max, _bk = xiaoban_logging._read_logging_config()
    assert level == "DEBUG"


def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # xiaoban_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("XIAOBAN_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import xiaoban_time

    assert xiaoban_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (XIAOBAN_TIMEZONE, XIAOBAN_REDACT_SECRETS, XIAOBAN_MAX_ITERATIONS,
    ...
    # writing XIAOBAN_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
