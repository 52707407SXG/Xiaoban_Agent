...
After `nix profile install`, `hermes`, `xiaoban-agent`, and `hermes-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `xiaoban setup` walks you through provider selection, `xiaoban gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.hermes/`.
...
```nix
services.xiaoban-agent.environmentFiles = [ "/var/lib/hermes/env" ];
```
:::
...
- The routing is transparent: `xiaoban chat`, `hermes sessions list`, `xiaoban version`, etc. all exec into the container under the hood
...
```nix
services.xiaoban-agent.configFile = /etc/hermes/config.yaml;
```
...
| Give the agent a personality | `${services.xiaoban-agent.stateDir}/.hermes/SOUL.md` | manage the file directly |
...
| Override the hermes package | `package` | `inputs.xiaoban-agent.packages.${system}.default.override { ... }` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$HERMES_HOME/.env` at activation time (`nixos-rebuild switch`). Hermes reads this file on every startup, so changes take effect with a `systemctl restart xiaoban-agent` — no container recreation needed.
...
The agent identity file is separate: Hermes loads its primary `SOUL.md` from `$HERMES_HOME/SOUL.md`, which in the NixOS module is `${services.xiaoban-agent.stateDir}/.hermes/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
2. **`.managed` marker file** in `HERMES_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it xiaoban-agent hermes config set ...` is also blocked)
...
| Check | What it tests |
|---|---|
| `package-contents` | `hermes` and `xiaoban-agent` binaries exist and `xiaoban version` runs |
...
# Container mode
docker exec xiaoban-agent cat /data/.hermes/.env
```
