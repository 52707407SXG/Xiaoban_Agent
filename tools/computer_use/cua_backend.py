...
etc.) surface as specific blocked checks via `xiaoban computer-use doctor`
...
# fetches the latest release, so a `XIAOBAN_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `XIAOBAN_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD = os.environ.get("XIAOBAN_CUA_DRIVER_CMD", "cua-driver")
...
def _cua_telemetry_disabled() -> bool:
    """True when Xiaoban should disable cua-driver telemetry for this user.
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of 52707407SXG/Xiaoban-Agent#47072: instead of hardcoding
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or XIAOBAN_CUA_DRIVER_CMD resolves."""
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `xiaoban computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        f"{installer}\n"
        "Or run `xiaoban tools` and enable the Computer Use toolset to install it automatically."
    )
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of 52707407SXG/Xiaoban-Agent#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(self, bridge: _AsyncBridge) -> None:
        ...
        self._started = False
        # Surface 4 of 52707407SXG/Xiaoban-Agent#47072: per-tool
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Xiaoban-managed secrets out of the child env.
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    52707407SXG/Xiaoban-Agent#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self) -> None:
        ...
        # Surface 6 of 52707407SXG/Xiaoban-Agent#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per Xiaoban run, with
        ...
        #     concurrent Xiaoban runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one Xiaoban run = one backend = one session — and pass it as
        ...
        self._session_id: str = f"xiaoban-{uuid.uuid4().hex[:12]}"

    # ── Lifecycle ──────────────────────────────────────────────────
    def start(self) -> None:
        _maybe_nudge_update()
        # The MCP client SDK (`mcp`) is an optional dependency (the
        # `computer-use` / `mcp` extras), not part of Xiaoban' minimal core.
    ...
    def capture(self, mode: str = "som", app: Optional[str] = None) -> CaptureResult:
        ...
        Maps xiaoban `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of 52707407SXG/Xiaoban-Agent#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of 52707407SXG/Xiaoban-Agent#47072: prefer the
    ...
    def click(
        ...
        *,
        ...
        # 52707407SXG/Xiaoban-Agent#47072). cua-driver-rs gained an explicit
