"""
xiaoban fallback — manage the fallback provider chain.
...
Subcommands:
  xiaoban fallback [list]   Show the current fallback chain (default when no subcommand)
  xiaoban fallback add      Pick provider + model via the same picker as `xiaoban model`,
                           then append the selection to the chain
  xiaoban fallback remove   Pick an entry to delete from the chain
  xiaoban fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.xiaoban/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from xiaoban_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `xiaoban model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  xiaoban fallback add")
        print()
        return
    ...
    print("  Docs: https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `xiaoban model`, then append the selection to the chain."""
    from xiaoban_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `xiaoban model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `xiaoban fallback list` to view, or `xiaoban fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``xiaoban fallback [subcommand]``."""
