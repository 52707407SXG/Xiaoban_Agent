...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      ...
      mirroring the ``xiaoban model`` CLI picker. Adds network calls
    ...
    try:
        from xiaoban_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def _append_unconfigured_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    from xiaoban_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            else f"run `xiaoban model` to configure ({auth_type})"
        )
