...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated XIAOBAN_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="xiaoban_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".xiaoban"))
    prev_home = os.environ.get("XIAOBAN_HOME")
    os.environ["XIAOBAN_HOME"] = os.path.join(test_home, ".xiaoban")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("xiaoban_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["XIAOBAN_HOME"] = prev_home
