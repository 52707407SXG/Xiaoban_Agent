...
base deps) during ``xiaoban update``, leaving the venv in a broken state
...
def temp_pyproject(tmp_path, monkeypatch):
    """Point xiaoban_cli.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    def test_no_action_when_all_deps_present(self, temp_pyproject, fake_venv_python):
        ...
        with patch("xiaoban_cli.main._resolve_install_target_python", return_value=py), \
             patch("xiaoban_cli.main.subprocess.run") as mock_run, \
             patch("xiaoban_cli.main._run_install_with_heartbeat") as mock_install:
            mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")

            from xiaoban_cli.main import _verify_core_dependencies_installed
    ...
    def test_triggers_reinstall_when_dep_missing(self, temp_pyproject, fake_venv_python):
        ...
        with patch("xiaoban_cli.main._resolve_install_target_python", return_value=py), \
             patch("xiaoban_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("xiaoban_cli.main._run_install_with_heartbeat") as mock_install:
    ...
    def test_skips_deps_excluded_by_environment_markers(self, temp_pyproject, fake_venv_python):
        ...
        with patch("xiaoban_cli.main._resolve_install_target_python", return_value=py), \
             patch("xiaoban_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("xiaoban_cli.main._run_install_with_heartbeat"), \
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("xiaoban_cli.main._resolve_install_target_python") as mock_resolve, \
             patch("xiaoban_cli.main._run_install_with_heartbeat") as mock_install:
            from xiaoban_cli.main import _verify_core_dependencies_installed
    ...
    def test_repair_reinstall_quarantines_running_shim_on_windows(
        ...
        quarantine the running ``xiaoban.exe`` on Windows before installing.
        ...
        is left missing and ``xiaoban`` drops off PATH. Previously this path
        ...
        with patch("xiaoban_cli.main._resolve_install_target_python", return_value=py), \
             patch("xiaoban_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("xiaoban_cli.main._is_windows", return_value=True), \
             patch("xiaoban_cli.main._venv_scripts_dir", return_value=fake_scripts), \
             patch("xiaoban_cli.main._run_install_with_heartbeat"), \
             patch("xiaoban_cli.main._quarantine_running_xiaoban_exe", return_value=[]) as mock_quar:
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        venv's interpreter — not the outer Python that drove `xiaoban update`.
        ...
        with patch("xiaoban_cli.main._is_windows", return_value=True):
            from xiaoban_cli.main import _resolve_install_target_python
