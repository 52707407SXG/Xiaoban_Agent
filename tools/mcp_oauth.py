...
This module provides the glue:
    - ``XiaobanTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Xiaoban-Agent"
"""
...
from xiaoban_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__xiaoban_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses XIAOBAN_HOME so each profile gets its own OAuth tokens.
    Layout: ``XIAOBAN_HOME/mcp-tokens/``
    """
    try:
        from xiaoban_constants import get_xiaoban_home
        base = Path(get_xiaoban_home())
    except ImportError:
        base = Path(os.environ.get("XIAOBAN_HOME", str(Path.home() / ".xiaoban")))
...
# ---------------------------------------------------------------------------
# XiaobanTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class XiaobanTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        XIAOBAN_HOME/mcp-tokens/<server_name>.json         -- tokens
        XIAOBAN_HOME/mcp-tokens/<server_name>.client.json   -- client info
        XIAOBAN_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Xiaoban records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Xiaoban.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `xiaoban mcp login <server>` later to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = XiaobanTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Xiaoban-Agent")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`xiaoban mcp login {server_name}` interactively first to complete "
