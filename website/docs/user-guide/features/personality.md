...
description: "Customize Xiaoban-Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Xiaoban-Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `XIAOBAN_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Xiaoban is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Xiaoban now seeds a default `SOUL.md` automatically in:

```text
~/.xiaoban/SOUL.md
```

More precisely, it uses the current instance's `XIAOBAN_HOME`, so if you run Xiaoban with a custom home directory, it will use:

```text
$XIAOBAN_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Xiaoban creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Xiaoban loads `SOUL.md` only from `XIAOBAN_HOME`
- Xiaoban does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Xiaoban falls back to a built-in default identity
...
If Xiaoban loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `XIAOBAN_HOME`, the personality belongs to the Xiaoban instance itself.

That also makes it easier to teach users:
- "Edit `~/.xiaoban/SOUL.md` to change Xiaoban' default personality."
...
- how Xiaoban should handle uncertainty, disagreement, or ambiguity
...
## What Xiaoban injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Xiaoban falls back to a built-in default identity ("You are Xiaoban-Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Xiaoban ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Xiaoban, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Xiaoban its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.xiaoban/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.xiaoban/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-xiaoban)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Xiaoban speaks
- `display.skin` and `/skin` affect how Xiaoban looks in the terminal
