 ...
 * null to keep it enabled. `XIAOBAN_DESKTOP_DISABLE_GPU` overrides detection
...
function detectRemoteDisplay(options = {}) {
  ...
  const override = String(env.XIAOBAN_DESKTOP_DISABLE_GPU || '')
    .trim()
    .toLowerCase()
  if (GPU_OVERRIDE_ON.has(override)) return 'override (XIAOBAN_DESKTOP_DISABLE_GPU)'
