...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Xiaoban with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.xiaoban/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Xiaoban
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``xiaoban tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``xiaoban setup`` for
