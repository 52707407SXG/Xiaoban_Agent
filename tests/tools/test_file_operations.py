...
class TestIsWriteDenied:
    ...
    def test_oauth_mcp_tokens_and_pairing_denied(self, path):
        ...
        full_path = str(xiaoban_home / path)
    ...
    def test_xiaoban_control_files_requested_writable(self, path):
        from xiaoban_constants import get_xiaoban_home

        assert _is_write_denied(str(get_xiaoban_home() / path)) is False
    ...
    def test_pairing_dir_denied(self, tmp_path, monkeypatch):
        ...
        PR #30383 introduced ~/.xiaoban/pairing/{platform}-approved.json as the
...
class TestShellFileOpsHelpers:
    ...
    def test_read_file_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "'\x07__XIAOBAN_FENCE_a9f7b3__\x1b]0;cat "
            "'/tmp/test/a.py' 2> /dev/null\x07\n"
            "print('ok')\n"
            "__XIAOBAN_FENCE_a9f7b3__\x07'\n"
        )
        ...
        assert "XIAOBAN_FENCE" not in result.content
    ...
    def test_read_file_raw_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "__XIAOBAN_FENCE_a9f7b3__\x07'\n"
            "alpha\n"
            "\x1b]0;cat '/tmp/test/a.txt'\x07__XIAOBAN_FENCE_a9f7b3__\n"
        )
