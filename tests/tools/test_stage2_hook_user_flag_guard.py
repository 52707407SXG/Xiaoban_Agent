...
guidance, while still allowing:
  ...
  - root start (id -u == 0)
  - `--user <xiaoban-uid>` (the supported non-root start, #34648 / #34837)
...
root, and the baked image dirs are owned by the xiaoban build UID, so an
arbitrary pinned UID can't write them (EACCES on a bind mount, hard crash on a
named volume). The supported path is root start + XIAOBAN_UID/XIAOBAN_GID (or the
PUID/PGID aliases), which remaps the xiaoban user and chowns the volume.

The guard fires only when the current UID is neither root NOR the xiaoban UID,
so the #34648 `--user 10000:10000` case (pinning to the xiaoban UID itself) is
unaffected.
...
def _guard_block(text: str) -> str:
    ...
    assert m, "expected the --user guard block (cur_uid + non-root/non-xiaoban check)"
    return m.group(1)
...
def test_guard_present_and_mentions_remediation(path: Path) -> None:
    ...
    # Must check non-root AND non-xiaoban-uid (so --user 10000:10000 is allowed).
    assert '"$cur_uid" != 0' in block
    assert '"$cur_uid" != "$(id -u xiaoban)"' in block
    assert "exit 1" in block
    # Must point users at the supported env vars.
    assert "XIAOBAN_UID" in block and "XIAOBAN_GID" in block
    assert "PUID" in block and "PGID" in block


def _run_guard(text: str, *, cur_uid: int, xiaoban_uid: int = 10000) -> subprocess.CompletedProcess:
    ...
    with tempfile.TemporaryDirectory() as d:
        script = (
            "set -e\n"
            # Stub `id`: `id -u` -> cur_uid; `id -u xiaoban` -> xiaoban_uid.
            f'id() {{ if [ "$2" = xiaoban ]; then echo {xiaoban_uid}; else echo {cur_uid}; fi; }}\n'
            + block
...
def test_arbitrary_user_uid_is_rejected() -> None:
    """An arbitrary host UID (1000), neither root nor xiaoban, is rejected."""
    for text in (_read(STAGE2_HOOK), _read(MAIN_WRAPPER)):
        proc = _run_guard(text, cur_uid=1000, xiaoban_uid=10000)
...
def test_root_start_passes() -> None:
    ...
    for text in (_read(STAGE2_HOOK), _read(MAIN_WRAPPER)):
        proc = _run_guard(text, cur_uid=0, xiaoban_uid=10000)
        assert proc.returncode == 0, proc.stderr
        assert "GUARD_PASSED" in proc.stdout


def test_user_pinned_to_xiaoban_uid_passes() -> None:
    """`--user 10000:10000` (the xiaoban UID itself) is the supported non-root
    start from #34648 / #34837 and must NOT be blocked."""
    for text in (_read(STAGE2_HOOK), _read(MAIN_WRAPPER)):
        proc = _run_guard(text, cur_uid=10000, xiaoban_uid=10000)
        assert proc.returncode == 0, proc.stderr
        assert "GUARD_PASSED" in proc.stdout


def test_user_pinned_to_remapped_xiaoban_uid_passes() -> None:
    """After a XIAOBAN_UID remap the xiaoban UID is e.g. 4242; a container pinned
    to that same UID must still pass (cur_uid == xiaoban_uid)."""
    for text in (_read(STAGE2_HOOK), _read(MAIN_WRAPPER)):
        proc = _run_guard(text, cur_uid=4242, xiaoban_uid=4242)
        assert proc.returncode == 0, proc.stderr
        assert "GUARD_PASSED" in proc.stdout
