"""Windows UTF-8 bootstrap for Xiaoban entry points.
...
should be imported at the very top of every Xiaoban entry point
(``xiaoban``, ``xiaoban-agent``, ``xiaoban-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import xiaoban_bootstrap``
# (or ``from xiaoban_bootstrap import apply_windows_utf8_bootstrap``) at
