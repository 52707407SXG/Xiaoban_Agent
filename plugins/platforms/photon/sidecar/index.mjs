// Xiaoban-Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Xiaoban): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Xiaoban -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Xiaoban-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `xiaoban photon telemetry`)
...
// instead of a cryptic module-resolution error during import. Apply Xiaoban'
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-xiaoban-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Xiaoban supplied them so a known-good
