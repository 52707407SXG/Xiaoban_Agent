...
Backend is selected during ``xiaoban tools`` setup (web.backend in config.yaml).
When available, Xiaoban can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Xiaoban config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Xiaoban' config/.env layer (``xiaoban config set``, ``xiaoban tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.xiaoban/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``xiaoban tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `xiaoban tools` repaint.
        try:
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `xiaoban tools` to set one up."
                ),
            }
        else:
            logger.info(
...
WEB_SEARCH_SCHEMA = {
    "name": "web_search",
    "description": "Search the web for information. Returns up to 5 results by default with titles, URLs, and descriptions that should be cited in answers to current/latest/live questions. The query is passed through to the configured backend, so operators such as site:domain, filetype:pdf, intitle:word, -term, and \"exact phrase\" may work when the backend supports them.",
    "parameters": {
        "type": "object",
        "properties": {
            "query": {
