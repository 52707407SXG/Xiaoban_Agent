"""Tests for the Xiaoban console entrypoint bridge."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`xiaoban.cli` should use `xiaoban_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "xiaoban" / "cli.py"
    called = []

    fake_main_module = types.ModuleType("xiaoban_cli.main")

    def fake_main():
        called.append("xiaoban_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "xiaoban_cli.main", fake_main_module)
    ...
    assert called == ["xiaoban_cli.main"]
