 ...
 *
 * Drives apps/desktop's first-launch install of Xiaoban-Agent by spawning
...
function bootstrapCacheDir(xiaobanHome) {
  return path.join(xiaobanHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.xiaoban/xiaoban-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(xiaobanHome) {
  if (!xiaobanHome) return null
  const candidate = path.join(xiaobanHome, 'xiaoban-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(xiaobanHome, commit) {
  return path.join(bootstrapCacheDir(xiaobanHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/52707407SXG/Xiaoban-Agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, xiaobanHome, emit, _download = downloadInstallScript }) {
  ...
  const cached = cachedScriptPath(xiaobanHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(xiaobanHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, xiaobanHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, hiddenWindowsChildOptions({
      ...
      env: {
        ...process.env,
        // Pass XIAOBAN_HOME through so install.ps1 respects the caller's
        // choice rather than re-computing the default.
        XIAOBAN_HOME: xiaobanHome || process.env.XIAOBAN_HOME || ''
      }
    }))
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, xiaobanHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, xiaobanHome }) {
  const args = ['--dir', activeRoot, '--xiaoban-home', xiaobanHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, xiaobanHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, xiaobanHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, xiaobanHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, xiaobanHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(xiaobanHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, xiaobanHome, emit })
