"""Contract test: the s6-overlay stage2 hook seeds $XIAOBAN_HOME/logs/gateways
as the xiaoban user.
...
xiaoban user and s6-log crash-loops on `mkdir: Permission denied`.

Seeding `logs/gateways` in stage2 (cont-init runs before any service starts)
guarantees the parent already exists xiaoban-owned by the time the first
...
def _seed_mkdir_block(text: str) -> str:
    """Extract the `as_xiaoban mkdir -p \\ ...` seed block."""
    m = re.search(r"as_xiaoban mkdir -p \\\n(?:[^\n]*\\\n)*[^\n]*\n", text)
    assert m, "stage2-hook.sh must contain the as_xiaoban mkdir -p seed block"
    return m.group(0)


def test_logs_gateways_is_seeded(stage2_text: str) -> None:
    block = _seed_mkdir_block(stage2_text)
    assert '"$XIAOBAN_HOME/logs/gateways"' in block, (
        "logs/gateways must be seeded xiaoban-owned in stage2 so profiles "
    ...
    assert '"$XIAOBAN_HOME/logs"' in block
