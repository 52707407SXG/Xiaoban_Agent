...
transport, discovers their tools, and registers them into the xiaoban-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.xiaoban/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.xiaoban/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            xiaoban_home = os.path.expanduser(
                os.getenv(
                    "XIAOBAN_HOME", os.path.join(os.path.expanduser("~"), ".xiaoban")
                )
            )
            candidates = [
                os.path.join(xiaoban_home, "node", "bin", resolved_command),
                ...
                # image (which the Xiaoban Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Xiaoban MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Xiaoban gateways know how to render.
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Xiaoban' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (XIAOBAN_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit XIAOBAN_SESSION_PLATFORM, so the
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Xiaoban' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Xiaoban is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `xiaoban mcp login {server_name}` (or delete the tokens "
            f"file under ~/.xiaoban/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local XIAOBAN_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_xiaoban_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_xiaoban_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local XIAOBAN_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_xiaoban_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Xiaoban config file.
    ...
    ``os.environ`` (which includes ``~/.xiaoban/.env`` loaded at startup).
    """
    try:
        ...
        if _env_enabled("XIAOBAN_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Xiaoban' MEDIA: tag convention and out to messaging
            ...
            # Xiaoban' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Xiaoban's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Xiaoban registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``xiaoban tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Xiaoban registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple xiaoban
