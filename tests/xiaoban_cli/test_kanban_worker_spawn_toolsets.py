...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    XIAOBAN_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_xiaoban_argv", lambda: ["xiaoban"])
    ...
    assert pid == 4242
    assert captured["env"]["XIAOBAN_HOME"] == str(profile)
    assert captured["env"]["XIAOBAN_KANBAN_TASK"] == "t_spawn_tools"
