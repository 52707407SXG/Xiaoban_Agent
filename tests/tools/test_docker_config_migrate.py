...
def _run_migration(xiaoban_home: Path, **env_overrides: str) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env.update(
        {
            "XIAOBAN_HOME": str(xiaoban_home),
            "XIAOBAN_SKIP_CHMOD": "1",
...
def test_docker_config_migrate_does_not_rewrite_invalid_yaml(tmp_path: Path) -> None:
    ...
    assert "xiaoban config:" in proc.stderr
...
def test_docker_config_migrate_skip_env_leaves_config_unchanged(tmp_path: Path) -> None:
    ...
    proc = _run_migration(tmp_path, XIAOBAN_SKIP_CONFIG_MIGRATION="1")
