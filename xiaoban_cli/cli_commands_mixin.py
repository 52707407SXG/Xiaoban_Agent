...
``cli.py``'s ``XiaobanCLI`` class. ``XiaobanCLI`` inherits ``CLICommandsMixin`` so
...
from xiaoban_constants import display_xiaoban_home, is_termux as _is_termux_environment
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: xiaoban --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Xiaoban config/state.
        ...
        """
        from xiaoban_cli.backup import (
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_xiaoban_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from xiaoban_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run xiaoban chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in xiaoban_cli/colors.py
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `xiaoban gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `xiaoban sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from xiaoban_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `xiaoban sessions list` to see available sessions.")
            return
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to xiaoban_cli.curator so the CLI and the `xiaoban curator`
        ...
        try:
            from xiaoban_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to xiaoban_cli.skills_hub."""
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``xiaoban bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: xiaoban bundles create "
        ...
        _cprint(
            ...
            f"Manage with `xiaoban bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Xiaoban-managed isolated debug "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Xiaoban keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from xiaoban_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_xiaoban_home()}/skins/\n")
            return
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="xiaoban_prompt_")
        try:
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Xiaoban is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Xiaoban is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Xiaoban is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from xiaoban_cli.models import _is_anthropic_fast_model
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Xiaoban-Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``xiaoban update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Xiaoban-Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Xiaoban-Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="M  Update Xiaoban-Agent",
            detail="This will exit the current session and run `xiaoban update`.",
            choices=choices,
        )
        if raw is None:
