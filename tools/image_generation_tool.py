...
selectable via ``xiaoban tools`` → Image Generation; the active model is
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"  • Pick a different model via `xiaoban tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.xiaoban"
    ...
    # Xiaoban cache roots can be translated without side effects. SSH can still
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `xiaoban tools` → Image Generation."
            )
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`xiaoban setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `xiaoban tools` "
        "→ Image Generation (run `xiaoban plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping xiaoban with a FAL key configured
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `xiaoban tools` → Image Generation."
                ),
...
# model/provider via `xiaoban tools` or `/skills`.
