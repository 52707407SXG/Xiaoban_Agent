...
class TestToolsetConsistency:
    ...
    def test_xiaoban_platforms_share_core_tools(self):
        """All xiaoban-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on xiaoban-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["xiaoban-cli", "xiaoban-telegram", "xiaoban-discord", "xiaoban-whatsapp", "xiaoban-slack", "xiaoban-signal", "xiaoban-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_xiaoban_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("xiaoban-whatsapp")

    def test_xiaoban_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("xiaoban-api-server")
