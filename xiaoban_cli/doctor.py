"""
Doctor command for xiaoban CLI.

Diagnoses issues with Xiaoban-Agent setup.
"""
...
from xiaoban_cli.config import get_project_root, get_xiaoban_home, get_env_path
...
_DHH = display_xiaoban_home()  # user-facing display path (e.g. ~/.xiaoban or ~/.xiaoban/profiles/coder)

# Load environment variables from ~/.xiaoban/.env so API key checks work
_env_path = get_env_path()
load_xiaoban_dotenv(xiaoban_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from xiaoban_cli.colors import Colors, color
from xiaoban_cli.models import _XIAOBAN_USER_AGENT
from xiaoban_constants import OPENROUTER_MODELS_URL
...
from xiaoban_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.xiaoban/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("XIAOBAN_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from xiaoban_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches xiaoban_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``xiaoban --version`` reporting a stale version while
    ...
    """
    try:
        from xiaoban_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != xiaoban_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'xiaoban update', or set "
            "xiaoban_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-xiaoban and dashboard static services are up
    ...
    for static in ("main-xiaoban", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `xiaoban profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from xiaoban_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the XIAOBAN_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("XIAOBAN_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via XIAOBAN_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `xiaoban`.
    os.environ.setdefault("XIAOBAN_INTERACTIVE", "1")

    # Handle `xiaoban doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.xiaoban/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Xiaoban Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`xiaoban doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and xiaoban_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.xiaoban/.env (primary location for user config)
    env_path = XIAOBAN_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'xiaoban setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'xiaoban setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'xiaoban setup' to create one")
                issues.append("Run 'xiaoban setup' to create .env")
    
    # Check ~/.xiaoban/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from xiaoban_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from xiaoban_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'xiaoban config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from xiaoban_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.xiaoban/.env or run 'xiaoban setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'xiaoban setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'xiaoban config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from xiaoban_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from xiaoban_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'xiaoban setup' to migrate config")
                else:
                    issues.append("Run 'xiaoban doctor --fix' or 'xiaoban setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'xiaoban doctor --fix'")
        except Exception:
            pass

        # Detect stale XIAOBAN_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives XIAOBAN_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            import yaml
            from xiaoban_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            env_ghost = load_env().get("XIAOBAN_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"XIAOBAN_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `xiaoban setup` run)",
                )
                if should_fix:
                    if remove_env_value("XIAOBAN_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale XIAOBAN_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove XIAOBAN_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the XIAOBAN_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale XIAOBAN_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'xiaoban doctor --fix'"
                    )
        except Exception:
            pass
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Xiaoban' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = xiaoban_home / subdir_name
    ...
    soul_path = xiaoban_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Xiaoban a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Xiaoban-Agent Persona\n\n"
                "<!-- Edit this file to customize how Xiaoban communicates. -->\n\n"
                "You are Xiaoban, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = xiaoban_home / "memories"
    ...
    state_db_path = xiaoban_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from xiaoban_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'xiaoban doctor --fix' "
                        "(or 'xiaoban sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = xiaoban_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'xiaoban doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "xiaoban"
        ...
        _cmd_link = _cmd_link_dir / "xiaoban"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(xiaoban not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/xiaoban → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/xiaoban points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/xiaoban → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/xiaoban — run 'xiaoban doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/xiaoban exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/xiaoban not found",
                    "(xiaoban command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/xiaoban → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/xiaoban symlink — run 'xiaoban doctor --fix'")
    ...
    try:
        from xiaoban_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `xiaoban doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable XIAOBAN_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Xiaoban one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'xiaoban config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": _XIAOBAN_USER_AGENT,
            }
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``xiaoban doctor``.
    ...
    # never the legitimate source for `xiaoban doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'xiaoban setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = XIAOBAN_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: xiaoban skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = XIAOBAN_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: xiaoban memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: xiaoban memory setup",
                    "No Honcho API key — run 'xiaoban memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run xiaoban memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: xiaoban memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: xiaoban memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from xiaoban_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "xiaoban -p" in content:
                            _m = _re.search(r"xiaoban -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'xiaoban doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
