#!/usr/bin/env python3
"""Google Workspace API CLI for Xiaoban-Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Xiaoban-facing JSON contract and falls back to the Python client
...
from _xiaoban_home import get_xiaoban_home
...
def _gws_binary() -> str | None:
    override = os.getenv("XIAOBAN_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Xiaoban-Agent")
