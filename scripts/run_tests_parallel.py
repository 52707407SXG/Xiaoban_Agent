...
Environment:
    XIAOBAN_TEST_WORKERS  Override worker count (default: os.cpu_count())
    XIAOBAN_TEST_PATHS    Override discovery roots (colon-sep, default: 'tests')
...
#                        52707407SXG/xiaoban-agent:test image, via
#                        ``XIAOBAN_TEST_IMAGE`` so the fixture skips
...
_DEFAULT_FILE_TIMEOUT_SECONDS = 300.0  # Keeps headroom above test_run_agent.py (~229s on a 2-CPU runner).
...
def _format_file(file: Path, repo_root: Path) -> str:
    ...
    ``/home/runner/work/xiaoban-agent/xiaoban-agent/tests/acp/test_auth.py``.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "--jobs",
        type=int,
        default=int(os.environ.get("XIAOBAN_TEST_WORKERS") or (os.cpu_count() or 4) * 2),
        help="Parallel worker count (default: $XIAOBAN_TEST_WORKERS or cpu_count*2)",
    )
    parser.add_argument(
        "--paths",
        default=os.environ.get("XIAOBAN_TEST_PATHS", ":".join(_DEFAULT_ROOTS)),
    ...
    parser.add_argument(
        ...
        type=float,
        default=float(
            os.environ.get("XIAOBAN_TEST_FILE_TIMEOUT", _DEFAULT_FILE_TIMEOUT_SECONDS)
        ),
        help=(
            "Per-file wall-clock cap in seconds. On timeout, the pytest "
            "subprocess and its full process tree are SIGKILL'd. "
            f"Default: {_DEFAULT_FILE_TIMEOUT_SECONDS}s ({round(_DEFAULT_FILE_TIMEOUT_SECONDS/60)} min), env: XIAOBAN_TEST_FILE_TIMEOUT."
        ),
    )
    parser.add_argument(
        "--slice",
        ...
        help=(
            ...
            "Env: XIAOBAN_TEST_SLICE (format: I/N)."
        ),
    )
    parser.add_argument(
        "paths_positional",
    ...
    # Parse --slice (or XIAOBAN_TEST_SLICE) early so we can exit on bad input
    # before doing any expensive discovery.
    slice_raw = args.slice or os.environ.get("XIAOBAN_TEST_SLICE")
