"""
Xiaoban-Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.xiaoban/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Xiaoban PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing xiaoban-agent or xiaoban PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Xiaoban-Agent" comment and following line
                if '# Xiaoban-Agent' in line or '# xiaoban-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('xiaoban' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing xiaoban
                if 'xiaoban' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the xiaoban wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "xiaoban",
        Path("/usr/local/bin/xiaoban"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains xiaoban_cli reference)
                content = wrapper.read_text()
                if 'xiaoban_cli' in content or 'xiaoban-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(xiaoban_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``xiaoban`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Xiaoban home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (xiaoban_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``xiaoban gateway run`` processes
    ...
    try:
        from xiaoban_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from xiaoban_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``XIAOBAN_HOME`` and ``XIAOBAN_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\xiaoban\git\cmd``, ``%LOCALAPPDATA%\xiaoban\git\bin``,
#      ``%LOCALAPPDATA%\xiaoban\git\usr\bin``, ``%LOCALAPPDATA%\xiaoban\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\xiaoban\git\`` and Node to
#      ``%LOCALAPPDATA%\xiaoban\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``xiaoban dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\xiaoban\gateway-service\`` and sometimes
...
def _xiaoban_path_markers(xiaoban_home: Path) -> list[str]:
    """Path-entry substrings that identify Xiaoban-owned User-PATH entries."""
    root = str(xiaoban_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare xiaoban-agent install dir.
    markers = [root + "\\xiaoban-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if XIAOBAN_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "xiaoban".  We don't want to catch
    # unrelated entries like "cxiaoban-foo" or "ephermeral", so we look for
    # backslash-xiaoban as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(xiaoban_home: Path) -> list[str]:
    """Strip Xiaoban-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _xiaoban_path_markers(xiaoban_home)
            kept: list[str] = []
...
def remove_xiaoban_env_vars_windows() -> list[str]:
    """Delete XIAOBAN_HOME and XIAOBAN_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("XIAOBAN_HOME", "XIAOBAN_GIT_BASH_PATH"):
                try:
...
def _is_default_xiaoban_home(xiaoban_home: Path) -> bool:
    """Return True when ``xiaoban_home`` points at the default (non-profile) root."""
    try:
        from xiaoban_constants import get_default_xiaoban_root
        return xiaoban_home.resolve() == get_default_xiaoban_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its XIAOBAN_HOME directory.

    We shell out to ``xiaoban -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current XIAOBAN_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m xiaoban_cli.main` so we don't depend on a `xiaoban`
    #    wrapper that may be half-removed mid-uninstall.
    xiaoban_invocation = [_sys.executable, "-m", "xiaoban_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                xiaoban_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's XIAOBAN_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``xiaoban uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$XIAOBAN_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from xiaoban_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(xiaoban_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         M Xiaoban Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Xiaoban Chat GUI installation was found.")
        print(f"  Checked: {xiaoban_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Xiaoban agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(xiaoban_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Xiaoban agent at {xiaoban_home / 'xiaoban-agent'}")
        print(f"  • Your config, sessions, and secrets under {xiaoban_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Xiaoban agent is still installed. Run 'xiaoban' to use the CLI,")
    print("or 'xiaoban uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.xiaoban/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.xiaoban/ for future reinstall
    """
    project_root = get_project_root()
    xiaoban_home = get_xiaoban_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie XIAOBAN_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_xiaoban_home(xiaoban_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.xiaoban data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            M Xiaoban-Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {xiaoban_home / 'config.yaml'}")
    print(f"  Secrets: {xiaoban_home / '.env'}")
    print(f"  Data:    {xiaoban_home / 'cron/'}, {xiaoban_home / 'sessions/'}, {xiaoban_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their XIAOBAN_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Xiaoban data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Xiaoban code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``xiaoban`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$XIAOBAN_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in xiaoban_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\xiaoban\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(xiaoban_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Xiaoban-owned PATH entries in User environment")

        log_info("Removing XIAOBAN_HOME / XIAOBAN_GIT_BASH_PATH User env vars...")
        removed_env = remove_xiaoban_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Xiaoban-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing xiaoban command...")
    ...
    #     (only when they still point into this Xiaoban home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Xiaoban-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(xiaoban_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Xiaoban-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(xiaoban_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE XIAOBAN_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from xiaoban_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(xiaoban_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.xiaoban/, just remove the xiaoban-agent subdir
            if xiaoban_home in project_root.parents or project_root.parent == xiaoban_home:
    ...
    #     under XIAOBAN_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(xiaoban_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(xiaoban_home)
    ...
    # 5. Optionally remove ~/.xiaoban/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile XIAOBAN_HOME dirs live under
        ...
        try:
            if xiaoban_home.exists():
                shutil.rmtree(xiaoban_home)
                log_success(f"Removed {xiaoban_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {xiaoban_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {xiaoban_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {xiaoban_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/52707407SXG/Xiaoban_Agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/52707407SXG/Xiaoban_Agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Xiaoban-Agent! M")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m xiaoban_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import xiaoban_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``xiaoban_constants`` + ``xiaoban_cli.colors``
    (and lazily ``xiaoban_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m xiaoban_cli.uninstall")
    parser.add_argument(
        "--mode",
