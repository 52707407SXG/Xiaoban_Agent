...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    token = _jwt(
        {
            ...
            "org_id": "org_123",
            "client_id": "xiaoban-cli",
            "product_id": "nous-xiaoban-agent",
            "nous_client": "xiaoban-agent",
            "exp": int(time.time()) + 900,
            "paid_access": True,
            "subscription_tier": 2,
        }
    )
    monkeypatch.setattr("xiaoban_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    ...
    assert info.product_id == "nous-xiaoban-agent"
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("xiaoban_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("xiaoban_cli.auth.resolve_nous_access_token", lambda: "fresh-token")
    monkeypatch.setattr("xiaoban_cli.nous_account._fetch_nous_account_info", lambda *a, **kw: payload)
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("xiaoban_cli.auth.get_provider_auth_state", lambda provider: {})
...
def test_entitlement_message_for_inference_key_without_portal_login():
    ...
    assert "Log in with `xiaoban model`" in message
...
def test_entitlement_message_for_unknown_entitlement_is_explicit():
    ...
    assert "Run `xiaoban model`" in message
