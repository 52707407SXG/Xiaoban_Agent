...
import { XIAOBAN_BASE_PATH, getWsTicket } from "@/lib/api";
...
export class GatewayClient {
  ...
  async connect(token?: string): Promise<void> {
    ...
    } else if (window.__XIAOBAN_AUTH_REQUIRED__) {
      const { ticket } = await getWsTicket();
      authParamName = "ticket";
      authParamValue = ticket;
    } else {
      authParamName = "token";
      authParamValue = window.__XIAOBAN_SESSION_TOKEN__ ?? "";
      if (!authParamValue) {
        this.setState("error");
        throw new Error(
          "Session token not available — page must be served by the Xiaoban dashboard",
        );
      }
    }
    ...
    const ws = new WebSocket(
      `${scheme}//${location.host}${XIAOBAN_BASE_PATH}/api/ws?${authParamName}=${encodeURIComponent(authParamValue)}`,
    );
    this.ws = ws;
