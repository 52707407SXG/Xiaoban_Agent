    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `xiaoban setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Xiaoban "
...
def compress_context(
    ...
    *,
    ...
    # long-lived ``xiaoban_state.SessionDB`` class still bound to the
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "process (or `xiaoban update`) to resync.",
    ...
    if agent._session_db:
        try:
            ...
            if in_place:
                ...
                # from ``xiaoban_logging._session_context`` (set once per turn in
                ...
                try:
                    agent._session_db.create_session(
                        ...
                        source=agent.platform or os.environ.get("XIAOBAN_SESSION_SOURCE", "cli"),
                        model=agent.model,
                ...
                try:
                    from xiaoban_cli.goals import migrate_goal_to_session
    ...
    # engines (e.g. xiaoban-lcm) use boundary_reason="compression" to preserve
    # DAG lineage / checkpoint per-session state across the boundary instead of
    # re-initializing fresh. See xiaoban-lcm#68. Built-in ContextCompressor
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="xiaoban_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
