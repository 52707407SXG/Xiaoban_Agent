...
def test_root_session_no_compression(db):
    ...
    assert prov["currentXiaobanSessionId"] == "root1"
    assert prov["rootXiaobanSessionId"] == "root1"
    assert prov["parentXiaobanSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_xiaoban_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentXiaobanSessionId"] == "old"
    assert prov["rootXiaobanSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousXiaobanSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootXiaobanSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootXiaobanSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_xiaoban_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousXiaobanSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"xiaoban"}
    assert "sessionProvenance" in meta["xiaoban"]
    assert meta["xiaoban"]["sessionProvenance"]["currentXiaobanSessionId"] == "root1"
