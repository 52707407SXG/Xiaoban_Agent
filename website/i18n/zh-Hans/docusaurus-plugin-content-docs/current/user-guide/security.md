...
1. **CLI 标志**：使用 `hermes --yolo` 或 `xiaoban chat --yolo` 启动会话
...
许多功能（Mistral TTS、ElevenLabs、Honcho 记忆、Bedrock、Slack、Matrix 等）依赖并非每个用户都需要的 Python 包。Hermes 在首次使用时**懒加载**安装这些包，而非在 `xiaoban-agent[all]` 下急切安装。实现位于 `tools/lazy_deps.py`。
