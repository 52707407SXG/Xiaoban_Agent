...
description: "How to update Xiaoban-Agent to the latest version or uninstall it"
---
...
```bash
xiaoban update --check    # see if a newer release is on PyPI
xiaoban update            # runs pip install --upgrade xiaoban-agent
```

Or manually:

```bash
pip install --upgrade xiaoban-agent    # or: uv pip install --upgrade xiaoban-agent
```
...
By default `xiaoban update` tracks `origin/main`. Pass `--branch <name>` to update against a different branch — useful for QA channels, feature branches, or release-candidate testing:

```bash
xiaoban update --branch release-candidate
...
### Preview-only: `xiaoban update --check`

Want to know if an update is available before pulling? Run `xiaoban update --check` — for git installs it fetches and compares commits against `origin/main`; for pip installs it queries PyPI for the latest release. No files are modified, no gateway is restarted. Useful in scripts and cron jobs that gate on "is there an update".
...
🔄 Restarting gateways...
✅ Gateway restarted
✅ Xiaoban-Agent updated successfully!
```
...
If `git status --short` shows unexpected changes after `xiaoban update`, stop and inspect them before continuing. This usually means local modifications were reapplied on top of the updated code, or a dependency step refreshed lockfiles.
:::
...
You no longer need to wrap `xiaoban update` in `screen` or `tmux` to survive a terminal drop.
...
nix flake update xiaoban-agent

# Or rebuild with the latest
nix profile upgrade xiaoban-agent
```
