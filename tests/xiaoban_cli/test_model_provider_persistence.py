"""Tests that provider selection via `xiaoban model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated XIAOBAN_HOME with a minimal string-format config."""
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from xiaoban_cli.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_update_config_for_provider_uses_atomic_yaml_write(self, config_home):
        ...
        from xiaoban_cli.auth import _update_config_for_provider
    ...
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from xiaoban_cli.main import _model_flow_api_key_provider
        ...
        with patch("xiaoban_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("xiaoban_cli.auth.deactivate_provider"), \
    ...
    def test_copilot_provider_saved_when_selected(self, config_home):
        ...
        from xiaoban_cli.main import _model_flow_copilot
        from xiaoban_cli.config import load_config
        ...
        with patch(
            ...
            "xiaoban_cli.main._prompt_reasoning_effort_selection",
            return_value="high",
        ), patch(
            "xiaoban_cli.auth.deactivate_provider",
        ):
    ...
    def test_named_custom_provider_preserves_explicit_api_mode(self, config_home):
        ...
        from xiaoban_cli.main import _model_flow_named_custom
        ...
        with patch("xiaoban_cli.auth._save_model_choice"), \
             patch("xiaoban_cli.auth.deactivate_provider"), \
             patch("xiaoban_cli.models.fetch_api_models", return_value=["gpt-5.4"]), \
             patch("xiaoban_cli.curses_ui.curses_radiolist", side_effect=OSError("no tty in test")), \
    ...
    def test_copilot_acp_provider_saved_when_selected(self, config_home):
        ...
        from xiaoban_cli.main import _model_flow_copilot_acp
        from xiaoban_cli.config import load_config

        with patch(
            "xiaoban_cli.auth.get_external_process_provider_status",
            return_value={
            ...
            "xiaoban_cli.auth.resolve_external_process_provider_credentials",
            return_value={
    ...
    def test_opencode_go_models_are_selectable_and_persist_normalized(self, config_home, monkeypatch):
        ...
        with patch("xiaoban_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.7"]), \
             patch("xiaoban_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("xiaoban_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
    ...
    def test_opencode_go_same_provider_switch_recomputes_api_mode(self, config_home, monkeypatch):
        ...
        with patch("xiaoban_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.5"]), \
             patch("xiaoban_cli.auth._prompt_model_selection", return_value="minimax-m2.5"), \
             patch("xiaoban_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
...
class TestBaseUrlValidation:
    ...
    def test_invalid_base_url_rejected(self, config_home, monkeypatch, capsys):
        ...
        with patch("xiaoban_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("xiaoban_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value="nano ~/.xiaoban/.env"):
