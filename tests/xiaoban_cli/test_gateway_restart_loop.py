...
Covers:
- Defense 1: gateway stop/restart refuse when _XIAOBAN_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "xiaoban  gateway  restart",         # double spaces
        "XIAOBAN GATEWAY RESTART",           # uppercase
    ])
    def test_xiaoban_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.xiaoban.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.xiaoban.gateway.plist",
        "launchctl stop ai.xiaoban.gateway",
        "systemctl restart xiaoban-gateway",
        "systemctl stop xiaoban-gateway.service",
        "systemctl start xiaoban-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill xiaoban gateway process",
        "pkill -f xiaoban.*gateway",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "xiaoban cron list",
        "xiaoban update",
        "xiaoban config set model claude",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_xiaoban_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade xiaoban then run xiaoban gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.xiaoban.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify xiaoban gateway stop/restart refuse when _XIAOBAN_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_XIAOBAN_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_XIAOBAN_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _XIAOBAN_GATEWAY=1.

    Issue #37453: systemctl --user restart xiaoban-gateway runs as a child of the
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart xiaoban-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-xiaoban systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _XIAOBAN_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart xiaoban-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart xiaoban-gateway"]
