...
from xiaoban_cli.psutil_android import (
    MARKER,
    REPLACEMENT,
    PSUTIL_URL,
...
def test_install_psutil_android_compat_uses_patched_tree(tmp_path):
    ...
    with patch("urllib.request.urlretrieve", side_effect=fake_urlretrieve), \
         patch.object(xiaoban_main, "_run_install_with_heartbeat", side_effect=fake_run_install):
        xiaoban_main._install_psutil_android_compat(
            ["uv", "pip"],
            env={"XIAOBAN_TEST": "1"},
        )

    assert captured["cmd"][:4] == ["uv", "pip", "install", "--no-build-isolation"]
    assert captured["env"] == {"XIAOBAN_TEST": "1"}
