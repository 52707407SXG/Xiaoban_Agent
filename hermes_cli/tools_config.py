    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_hermes_home()}/xiaoban-agent && npm install --workspaces=false")
                if result.stderr:
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/xiaoban-agent:latest"
            )
            return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Xiaoban Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://xiaoban-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
