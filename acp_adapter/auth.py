"""ACP auth helpers — detect and advertise Xiaoban authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "xiaoban-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Xiaoban runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Xiaoban can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Xiaoban.
    ...
    not have Xiaoban provider credentials configured yet, so Xiaoban always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Xiaoban using the currently configured "
