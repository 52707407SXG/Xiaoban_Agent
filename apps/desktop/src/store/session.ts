...
import type { XiaobanConnection } from '@/global'
import type { ChatMessage } from '@/lib/chat-messages'
import { persistBoolean, persistString, storedBoolean, storedString } from '@/lib/storage'
import type { SessionInfo, UsageStats } from '@/types/xiaoban'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'xiaoban.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'xiaoban.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'xiaoban.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'xiaoban.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'xiaoban.desktop.composer.fast'

let configuredDefaultProjectDir = ''

function workspaceCwdKey(connection: XiaobanConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.xiaobanDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.xiaobanDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<XiaobanConnection | null>(null)
...
export const setConnection = (next: Updater<XiaobanConnection | null>) => updateAtom($connection, next)
