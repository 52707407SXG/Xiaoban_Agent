"""
Lazy dependency installer for opt-in Xiaoban-Agent backends.

Many Xiaoban features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``xiaoban-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``xiaoban tools`` or the manual pip command.
...
def _allow_lazy_installs() -> bool:
    ...
    """
    if os.environ.get("XIAOBAN_DISABLE_LAZY_INSTALLS") == "1":
        return False
    try:
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``xiaoban update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    Mirrors the strategy in ``xiaoban_cli.tools_config._pip_install`` but
...
def active_features() -> list[str]:
    ...
    Used by ``xiaoban update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``xiaoban update``. Never raises; lazy-install failures
