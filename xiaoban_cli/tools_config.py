"""
Unified tool configuration for Xiaoban-Agent.

`xiaoban tools` and `xiaoban setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.xiaoban/config.yaml under
...
from xiaoban_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from xiaoban_cli.nous_account import format_nous_portal_entitlement_message
...
from xiaoban_cli.cli_output import (  # noqa: E402 — late import block
...
CONFIGURABLE_TOOLSETS = [
    ...
    ("mystand_parser",  "🏠 My Stand Parser",           "mystand_parse"),
...
# They're still in _XIAOBAN_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `xiaoban tools` → Video Generation, which walks
...
# `xiaoban tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from xiaoban_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `xiaoban tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``xiaoban tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from xiaoban_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``xiaoban tools`` checklist actually offers
    for ``platform``.
    ...
    so ``xiaoban tools`` never claims to add or remove a toolset the user was
...
from xiaoban_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Xiaoban routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set XIAOBAN_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "XIAOBAN_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "XIAOBAN_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "XIAOBAN_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "XIAOBAN_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "XIAOBAN_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("XIAOBAN_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_driver_env() -> dict:
    """cua-driver child env with the Xiaoban telemetry policy applied.
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``xiaoban update`` and
      by ``xiaoban computer-use install --upgrade``.
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `xiaoban update` calls this
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Xiaoban runtime process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_xiaoban_home()}/xiaoban-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in xiaoban-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/52707407SXG/xiaoban-agent:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.xiaoban/config.yaml")
        ...
        # Run the full `xiaoban auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.xiaoban/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from xiaoban_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: xiaoban auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `xiaoban auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: xiaoban auth spotify")
        except Exception as exc:
        ...
        try:
            from xiaoban_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: xiaoban plugins enable observability/langfuse")
        _print_info("    Restart Xiaoban for tracing to take effect.")
        _print_info("    Verify: xiaoban plugins list")
        ...
        try:
            ...
            _print_info("    Run later: xiaoban auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `xiaoban auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: xiaoban auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: xiaoban auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``xiaoban tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``xiaoban tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: xiaoban tools post-setup <key>")
        return 2
...
def _get_platform_tools(
    ...
    *,
    ...
    def _declared_toolset_tools(ts_name: str, seen: set[str] | None = None) -> set[str]:
        """Resolve tools from static toolset declarations only.

        Runtime plugin registration can add check_fn-gated tools to an existing
        toolset. That should affect the final schema, not whether the base
        toolset is recognized from a composite default.
        """
        if seen is None:
            seen = set()
        if ts_name in seen:
        ...
        for included in ts_def.get("includes") or []:
            tools.update(_declared_toolset_tools(str(included), seen))
        return tools
    ...
    # "xiaoban-cli" (which include all _XIAOBAN_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[xiaoban-cli, spotify]`` after enabling Spotify via ``xiaoban
        ...
        if composite_tools:
            ...
            for ts_key, _, _ in CONFIGURABLE_TOOLSETS:
                if not _toolset_allowed_for_platform(ts_key, platform):
                    continue
                ts_tools = _declared_toolset_tools(ts_key)
        ...
        # (e.g. "xiaoban-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
            all_tool_names.update(_declared_toolset_tools(ts_name))
        ...
        # through ``xiaoban tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `xiaoban tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"xiaoban-{platform}"
    platform_tool_universe = set(resolve_toolset(_default_ts))
    configurable_tool_universe = set()
    for ck in configurable_keys:
        configurable_tool_universe.update(_declared_toolset_tools(ck))
    claimed = set()
    for ts_key in enabled_toolsets:
        claimed.update(resolve_toolset(ts_key))
    skip = configurable_keys | plugin_ts_keys | platform_default_keys
    skip |= {k for k in TOOLSETS if k.startswith("xiaoban-")}
    ...
    # shipped as a bundled plugin but user must opt in via `xiaoban tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `xiaoban tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by xiaoban tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (xiaoban-cli, xiaoban-telegram, etc.)
    ...
    # Opening `xiaoban tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `xiaoban tools` gets a silent no-op
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `xiaoban model`).
    if managed_feature:
        from xiaoban_cli.nous_subscription import (
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `xiaoban tools` and `xiaoban setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print(color("M Tool Summary", Colors.CYAN, Colors.BOLD))
        print()
        ...
        print()
        return
    print(color("M Xiaoban Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from xiaoban_constants import display_xiaoban_home
    print(color(f"  Tool configuration saved to {display_xiaoban_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'xiaoban' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, xiaoban mcp
