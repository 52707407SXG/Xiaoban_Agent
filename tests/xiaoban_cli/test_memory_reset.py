"""Tests for the `xiaoban memory reset` CLI command.
...
- Profile-scoped reset (uses XIAOBAN_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake XIAOBAN_HOME with memory files."""
    xiaoban_home = tmp_path / ".xiaoban"
    memories = xiaoban_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("XIAOBAN_HOME", str(xiaoban_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nXiaoban repo is at ~/.xiaoban/xiaoban-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `xiaoban memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        xiaoban_home = tmp_path / ".xiaoban"
        (xiaoban_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("XIAOBAN_HOME", str(xiaoban_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's XIAOBAN_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_xiaoban_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
