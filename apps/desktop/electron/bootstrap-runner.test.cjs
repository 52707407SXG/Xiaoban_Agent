...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'xiaoban-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/xiaoban-runner-test',
    sourceRepoRoot: null,
    xiaobanHome: '/tmp/xiaoban-runner-test',
    logRoot: '/tmp/xiaoban-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'xiaoban-agent', 'scripts')
