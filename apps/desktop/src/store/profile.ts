...
import { getProfiles, setApiRequestProfile } from '@/xiaoban'
...
import type { ProfileInfo } from '@/types/xiaoban'
...
// /api/profiles/active `current`). "default" is the root ~/.xiaoban. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'xiaoban.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'xiaoban.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.xiaobanDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new XIAOBAN_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.xiaobanDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.xiaobanDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'xiaoban.desktop.showAllProfiles'
...
// Switch to the default (root ~/.xiaoban) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.xiaobanDesktop?.touchBackend?.(target).catch(() => undefined)
}
