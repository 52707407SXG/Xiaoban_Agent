"""Slash command definitions and autocomplete for the Xiaoban CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Xiaoban config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Xiaoban works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Xiaoban is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.xiaoban/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Xiaoban to the latest version", "Info"),
    CommandDef("version", "Show Xiaoban version", "Info", aliases=("v",)),
...
COMMAND_DESCRIPTION_ZH: dict[str, str] = {
    "start": "处理平台启动探测消息，不主动回复",
    "new": "开启一个全新会话，清空当前上下文",
    "topic": "启用或查看 Telegram 私聊主题会话",
    "clear": "清屏，并开启一个新会话",
    "redraw": "强制重绘界面，终端显示错乱时用",
    "history": "查看当前会话历史",
    "save": "保存当前对话",
    "retry": "重试上一条用户消息",
    "prompt": "用系统编辑器编写下一条消息，写完后发送",
    "undo": "撤回最近几轮对话并重新提问",
    "title": "设置或查看当前会话标题",
    "handoff": "把当前会话交接到 Telegram、Discord 等消息平台",
    "branch": "从当前会话分出一个新分支，方便换思路探索",
    "compress": "压缩当前对话上下文，给后续聊天腾空间",
    "rollback": "列出或恢复文件检查点",
    "snapshot": "创建或恢复小伴配置和状态快照",
    "stop": "停止正在运行的后台任务",
    "approve": "批准一个等待确认的高风险命令",
    "deny": "拒绝一个等待确认的高风险命令",
    "background": "把一条提示词放到后台执行",
    "agents": "查看正在运行的子 Agent 和任务",
    "queue": "把提示词排队到下一轮，不打断当前任务",
    "steer": "在下一次工具调用后插入一条消息，不中断当前任务",
    "goal": "设置一个持续目标，让小伴跨多轮推进直到完成",
    "subgoal": "给当前目标添加或管理补充条件",
    "status": "查看当前会话、模型、Token 和上下文信息",
    "whoami": "查看你当前斜杠命令权限",
    "profile": "查看当前配置档案名称和主目录",
    "sethome": "把当前聊天设为默认接收频道",
    "resume": "恢复一个以前命名过的会话",
    "sessions": "浏览并恢复历史会话",
    "config": "查看当前配置",
    "model": "切换或查看当前模型",
    "codex-runtime": "切换 OpenAI/Codex 模型使用的 Codex 运行时",
    "personality": "切换当前会话人格",
    "statusbar": "显示或隐藏底部状态栏",
    "timestamps": "显示或隐藏消息时间戳",
    "verbose": "切换工具执行过程的详细显示程度",
    "footer": "显示或隐藏回复末尾的运行时信息",
    "yolo": "切换 YOLO 模式，跳过高风险命令确认",
    "reasoning": "查看或调整模型推理强度与显示方式",
    "fast": "切换快速模式或普通模式",
    "skin": "查看或切换界面皮肤主题",
    "indicator": "切换小伴忙碌时的等待动画样式",
    "voice": "开启、关闭或查看语音模式",
    "busy": "设置小伴忙碌时按 Enter 的行为",
    "tools": "管理工具：查看、启用或禁用工具",
    "toolsets": "列出可用工具集",
    "skills": "搜索、安装、查看或管理技能",
    "learn": "把当前流程或指定资料整理成可复用技能",
    "memory": "查看待写入记忆，或切换记忆写入审批",
    "bundles": "列出技能包，一个命令可同时启用多个技能",
    "cron": "管理定时任务",
    "suggestions": "查看建议自动化，并接受或忽略",
    "blueprint": "用蓝图模板创建自动化任务",
    "curator": "管理后台技能维护任务",
    "kanban": "管理多 Agent 协作看板",
    "reload": "重新加载 .env 环境变量到当前会话",
    "reload-mcp": "从配置重新加载 MCP 服务",
    "reload-skills": "重新扫描已安装或已删除的技能",
    "browser": "连接浏览器工具到本机 Chromium 浏览器",
    "plugins": "查看已安装插件和启用状态",
    "commands": "分页浏览所有命令和技能",
    "help": "查看可用命令说明",
    "restart": "等待当前任务结束后，平滑重启网关",
    "usage": "查看当前会话 Token 用量和限额",
    "credits": "查看 Nous 余额并充值",
    "billing": "管理终端计费、充值、自动续费和限额",
    "insights": "查看用量分析",
    "platforms": "查看网关和消息平台状态",
    "platform": "暂停、恢复或列出异常消息平台",
    "copy": "复制上一条小伴回复到剪贴板",
    "paste": "从剪贴板附加图片",
    "image": "给下一条消息附加本地图片",
    "update": "更新小伴到最新版本",
    "version": "查看小伴版本",
    "debug": "上传调试报告并生成可分享链接",
    "quit": "退出 CLI，可选择同时删除会话历史",
}


def _display_description(cmd: CommandDef) -> str:
    """Return the Chinese CLI/TUI description for a command."""
    return COMMAND_DESCRIPTION_ZH.get(cmd.name, cmd.description)
...
def _build_description(cmd: CommandDef) -> str:
    ...
    description = _display_description(cmd)
    if cmd.args_hint:
        return f"{description}（用法：/{cmd.name} {cmd.args_hint}）"
    return description
...
for _cmd in COMMAND_REGISTRY:
    if not _cmd.gateway_only:
        ...
        for _alias in _cmd.aliases:
            COMMANDS[f"/{_alias}"] = f"{_display_description(_cmd)}（等同 /{_cmd.name}）"
...
def gateway_help_lines() -> list[str]:
    ...
    for cmd in COMMAND_REGISTRY:
        ...
        lines.append(f"`/{cmd.name}{args}` -- {_display_description(cmd)}{alias_note}")
    return lines
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`xiaoban_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/xiaoban`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Xiaoban
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``xiaoban skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``xiaoban skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``xiaoban skills list`` and
...
# unrelated command landed. These claim their slots right after /xiaoban,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /xiaoban <command>).
...
# native slot, the alias spelling stays reachable via /xiaoban reset).
...
# ``/xiaoban <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/xiaoban" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /xiaoban credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /xiaoban billing.
#   - debug: the log/report upload surface; reached via /xiaoban debug on Slack.
_SLACK_VIA_XIAOBAN_ONLY = frozenset({"credits", "billing", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/xiaoban <verb>`` subcommand.
    ...
    can still reach them via ``/xiaoban <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/xiaoban`` is always reserved as the first entry so the
    legacy ``/xiaoban <subcommand>`` form keeps working for anything that
    ...
    # Reserve /xiaoban as the catch-all top-level command.
    entries.append(("xiaoban", "Talk to Xiaoban or run a subcommand", "[subcommand] [args]"))
    seen.add("xiaoban")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_XIAOBAN_ONLY:
            # Intentionally Slack-via-/xiaoban only (see _SLACK_VIA_XIAOBAN_ONLY).
            return
    ...
    # everything except /xiaoban, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://xiaoban-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /xiaoban handler.

    Maps both canonical names and aliases so /xiaoban bg do stuff works
    the same as /xiaoban background do stuff.

    Plugin-registered slash commands are included so ``/xiaoban <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from xiaoban_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
