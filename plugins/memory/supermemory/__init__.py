...
_DEFAULT_CONTAINER_TAG = "xiaoban"
...
def _load_supermemory_config(xiaoban_home: str) -> dict:
    config = _default_config()
    config_path = Path(xiaoban_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, xiaoban_home: str) -> None:
    config_path = Path(xiaoban_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "xiaoban"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "xiaoban", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `xiaoban memory setup`.
        # All other options are documented for $XIAOBAN_HOME/supermemory.json
    ...
    def save_config(self, values, xiaoban_home):
        ...
        _save_supermemory_config(sanitized, xiaoban_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from xiaoban_constants import get_xiaoban_home
        self._xiaoban_home = kwargs.get("xiaoban_home") or str(get_xiaoban_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._xiaoban_home)
