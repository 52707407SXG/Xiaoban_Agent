...
""",
        ...
        from xiaoban_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        xiaoban_home = tmp_path / ".xiaoban"
        xiaoban_home.mkdir()
        (xiaoban_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("XIAOBAN_HOME", str(xiaoban_home))
        from xiaoban_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from xiaoban_cli.plugins import PluginContext, PluginManifest, PluginManager
