...
The ACP adapter wraps Xiaoban' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
xiaoban acp / xiaoban-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.xiaoban/.env
  -> configure stderr logging
  -> construct XiaobanACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'xiaoban-agent[acp]==<version>' xiaoban-acp`, pointed at the `xiaoban-agent` PyPI release.
...
- `allow_once` -> Xiaoban `once`
- `allow_always` -> Xiaoban `always`
- reject options -> Xiaoban `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Xiaoban tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["xiaoban-acp"])
...
Instead it reuses Xiaoban' runtime resolver:

- `acp_adapter/auth.py`
- `xiaoban_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Xiaoban provider/credentials. It also always advertises a terminal setup auth method (`xiaoban-setup`, args `--setup`) so first-run registry clients can open Xiaoban' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.xiaoban/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `xiaoban-acp` toolset definition
- `xiaoban_cli/main.py` — `xiaoban acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `xiaoban-acp` script
