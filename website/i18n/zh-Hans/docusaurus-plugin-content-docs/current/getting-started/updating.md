...
sidebar_position: 3
title: "更新与卸载"
description: "如何将 Xiaoban-Agent 更新至最新版本或将其卸载"
---
...
```bash
xiaoban update --check    # 查看 PyPI 上是否有更新的版本
xiaoban update            # 执行 pip install --upgrade xiaoban-agent
```

或手动执行：

```bash
pip install --upgrade xiaoban-agent    # 或：uv pip install --upgrade xiaoban-agent
```
...
想在拉取前确认是否有更新？运行 `xiaoban update --check` — 对于 Git 安装方式，它会获取并与 `origin/main` 比较提交；对于 pip 安装方式，它会查询 PyPI 上的最新版本。不修改任何文件，不重启 gateway。适合在以"是否有更新"为条件的脚本和 cron 任务中使用。
...
:::warning 更新后工作树出现脏状态
如果 `xiaoban update` 后 `git status --short` 显示意外变更，请在继续前停下来检查。这通常意味着本地修改被重新应用到了更新后的代码之上，或依赖步骤刷新了锁文件。
:::
...
你不再需要将 `xiaoban update` 包裹在 `screen` 或 `tmux` 中来应对终端断开。
