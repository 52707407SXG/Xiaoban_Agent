...
export function mediaStreamUrl(path: string): string {
  return `xiaoban-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// expose GET /api/media (xiaoban_cli/web_server.py).
export async function gatewayMediaDataUrl(path: string): Promise<string> {
  const file = filePathFromMediaPath(path)

  const result = await window.xiaobanDesktop!.api<{ data_url: string }>({
