# nix/nixosModules.nix — NixOS module for xiaoban-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: xiaoban runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $XIAOBAN_HOME/.env
# and read by xiaoban at startup — no container recreation needed for env changes.
#
# Tool resolution: the xiaoban wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.xiaoban-agent;
    effectivePackage =
    ...
    xiaoban-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-xiaoban-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "xiaoban-config-attrs";
      description = "Xiaoban YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "xiaoban-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # XIAOBAN_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "xiaoban-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'XIAOBAN_DOC_EOF'\n${value}\nXIAOBAN_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "xiaoban-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/xiaoban";
    ...
    # xiaoban user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "xiaoban-container-entrypoint" ''
      set -eu

      XIAOBAN_UID="''${XIAOBAN_UID:?XIAOBAN_UID must be set}"
      XIAOBAN_GID="''${XIAOBAN_GID:?XIAOBAN_GID must be set}"

      # ── Group: ensure a group with GID=$XIAOBAN_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$XIAOBAN_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="xiaoban"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$XIAOBAN_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$XIAOBAN_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$XIAOBAN_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$XIAOBAN_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="xiaoban"
        TARGET_HOME="/home/xiaoban"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$XIAOBAN_UID" -g "$XIAOBAN_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$XIAOBAN_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$XIAOBAN_UID:$XIAOBAN_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure XIAOBAN_HOME is owned by the target user.
      ...
      if [ -n "''${XIAOBAN_HOME:-}" ] && [ -d "$XIAOBAN_HOME" ]; then
        find "$XIAOBAN_HOME" \! -user "$XIAOBAN_UID" -exec chown "$XIAOBAN_UID:$XIAOBAN_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/xiaoban-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/xiaoban-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/xiaoban ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/xiaoban
        chmod 0440 /etc/sudoers.d/xiaoban
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$XIAOBAN_UID" --regid="$XIAOBAN_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $XIAOBAN_HOME/.env.
    ...
    # Default: /var/lib/xiaoban/workspace → /data/workspace.
  ...
  in {
    options.services.xiaoban-agent = with lib; {
      enable = mkEnableOption "Xiaoban-Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = xiaoban-agent;
        description = "The xiaoban-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/xiaoban";
        description = "State directory. Contains .xiaoban/ subdir (XIAOBAN_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Xiaoban config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $XIAOBAN_HOME/.env at activation time.
          Xiaoban reads this file on every startup via load_xiaoban_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $XIAOBAN_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $XIAOBAN_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `xiaoban gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the xiaoban user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the xiaoban plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Xiaoban discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          xiaoban_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as xiaoban (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in xiaoban-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the xiaoban CLI to environment.systemPackages and export
          XIAOBAN_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.xiaoban symlink to the service
            stateDir. These users are automatically added to the xiaoban group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.xiaoban-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the xiaoban CLI to system PATH and export XIAOBAN_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.xiaoban/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.XIAOBAN_HOME = "${cfg.stateDir}/.xiaoban";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.xiaoban-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the xiaoban user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.xiaoban-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed xiaoban
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure xiaoban is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.xiaoban        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.xiaoban/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.xiaoban/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.xiaoban/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.xiaoban/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.xiaoban/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."xiaoban-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.xiaoban
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.xiaoban ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.xiaoban ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.xiaoban -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.xiaoban/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.xiaoban/$_subdir"
            chmod 2770 "${cfg.stateDir}/.xiaoban/$_subdir"
            find "${cfg.stateDir}/.xiaoban/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # xiaoban-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.xiaoban/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.xiaoban/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.xiaoban/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.xiaoban/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.xiaoban/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.xiaoban/.managed
          chmod 0644 ${cfg.stateDir}/.xiaoban/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.xiaoban/.container-mode <<'XIAOBAN_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    xiaoban_bin=${containerDataDir}/current-package/bin/xiaoban
    XIAOBAN_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.xiaoban/.container-mode
            chmod 0644 ${cfg.stateDir}/.xiaoban/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.xiaoban/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.xiaoban";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.xiaoban" ]; then
                  rm -f "${symlinkPath}"
                  echo "xiaoban-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.xiaoban -> stateDir/.xiaoban for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.xiaoban";
                target = "${cfg.stateDir}/.xiaoban";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "xiaoban-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.xiaoban/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.xiaoban/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.xiaoban/auth.json
              fi
            ''}
          ''}
          ...
          # Xiaoban reads $XIAOBAN_HOME/.env at startup via load_xiaoban_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.xiaoban/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'XIAOBAN_NIX_ENV_EOF'
    ${envFileContent}
    XIAOBAN_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.xiaoban/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.xiaoban/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.xiaoban/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.xiaoban-agent = {
          description = "Xiaoban-Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            XIAOBAN_HOME = "${cfg.stateDir}/.xiaoban";
            XIAOBAN_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $XIAOBAN_HOME/.env by the activation script. load_xiaoban_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/xiaoban"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the xiaoban group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.xiaoban-agent = {
          description = "Xiaoban-Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              XIAOBAN_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              XIAOBAN_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env XIAOBAN_UID="$XIAOBAN_UID" \
                --env XIAOBAN_GID="$XIAOBAN_GID" \
                --env XIAOBAN_HOME=${containerDataDir}/.xiaoban \
                --env XIAOBAN_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/xiaoban gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
