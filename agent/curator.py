...
def _state_file() -> Path:
    return get_xiaoban_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.xiaoban/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `xiaoban update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``xiaoban curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``xiaoban update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``xiaoban curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `xiaoban curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Xiaoban' background skill CURATOR. This is an "
    ...
    "into ~/.xiaoban/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: xiaoban-config-*, xiaoban-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.xiaoban/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.xiaoban/logs/curator/``)
    ...
    authored skill data in ``~/.xiaoban/skills/``.

    ``ensure_xiaoban_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_xiaoban_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every xiaoban install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: xiaoban curator status
        keep an umbrella stable: xiaoban curator pin document-tools
    ...
    lines.append("full report: xiaoban curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: xiaoban curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.xiaoban/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.xiaoban/skills/.archive/` for "
            "safety and can be restored via `xiaoban curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.xiaoban/skills/.archive/`. "
            "Restore any via `xiaoban curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `xiaoban curator restore <name>`")
    lines.append("- All archives live under `~/.xiaoban/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``xiaoban curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `xiaoban curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `xiaoban update` only if "
        ...
        # recorded in state so `xiaoban curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``xiaoban model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `xiaoban model` → auxiliary
