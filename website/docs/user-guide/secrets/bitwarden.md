...
Pull API keys from [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) at process startup instead of storing them in plaintext inside `~/.xiaoban/.env`. One bootstrap secret (a machine-account access token) replaces N per-provider keys, and rotating a credential becomes a single change in the Bitwarden web app.
...
2. Xiaoban stores that single token in `~/.xiaoban/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `xiaoban` (or the gateway, or a cron job) starts, after `~/.xiaoban/.env` has loaded, Xiaoban calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
4. By default Xiaoban **overrides** values already in your environment, so Bitwarden is the source of truth — rotate a key once in the web app and every Xiaoban process picks it up on next start. Flip `override_existing: false` in config if you want `.env` to win instead.

The `bws` binary is auto-downloaded into `~/.xiaoban/bin/` on first use — no `apt`, no `brew`, no `sudo`.
...
2. Create or pick a **Project** (e.g. "Xiaoban keys").
...
4. **Machine accounts → New machine account → My Xiaoban machine** → **Projects** tab → grant Read access to your project.
...
1. Download and verify `bws v2.0.0` into `~/.xiaoban/bin/bws`.
2. Prompt you for the access token (input is hidden). Stored in `~/.xiaoban/.env` as `BWS_ACCESS_TOKEN`.
...
```bash
xiaoban secrets bitwarden status
```

From now on, every `xiaoban` invocation pulls fresh secrets at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `xiaoban secrets bitwarden setup` | Interactive wizard (install binary, prompt for token, pick project, test fetch) |
| `xiaoban secrets bitwarden status` | Show config + binary version + token presence |
| `xiaoban secrets bitwarden sync` | Dry-run: pull secrets now and show what would be applied |
| `xiaoban secrets bitwarden sync --apply` | Pull and export into the current shell's environment |
| `xiaoban secrets bitwarden install` | Just download the pinned `bws` binary (no auth required) |
| `xiaoban secrets bitwarden disable` | Flip `enabled: false`; leaves token + project id in place |

## Configuration

Defaults in `~/.xiaoban/config.yaml`:
...
| `cache_ttl_seconds` | `300` | How long an in-process fetch result is reused. Set to `0` to disable caching. Cache is per-process; new `xiaoban` invocations start fresh. |
...
| `auto_install` | `true` | When true, `bws` is auto-downloaded into `~/.xiaoban/bin/` on first use. |

## Failure modes

Bitwarden never blocks Xiaoban startup. If anything goes wrong, you'll see a one-line warning in stderr and Xiaoban continues with whatever credentials `.env` already had:

| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `xiaoban secrets bitwarden setup` |
...
- Xiaoban will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — Xiaoban does not auto-upgrade `bws` to "latest" because upstream release shapes can change.

## When NOT to use this

- **Single-machine personal setups** where `~/.xiaoban/.env` is fine. You're trading one credential for another and adding a network dependency at startup.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple Xiaoban installations.
