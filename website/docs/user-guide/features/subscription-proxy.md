...
chat completions — use your Xiaoban-managed provider subscription as their
...
| Use case | "Use Xiaoban as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Xiaoban stores
the refresh token in `~/.xiaoban/auth.json` — the same place all Xiaoban
...
Model:      Xiaoban-4-70B    # or Xiaoban-4.3-36B, Xiaoban-4-405B
```
...
interface in `xiaoban_cli/proxy/adapters/`.
...
If you see `not logged in`, run `xiaoban portal`. If you see
...
`xiaoban_cli/proxy/adapters/<provider>.py` and registering it in
