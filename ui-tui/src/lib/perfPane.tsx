...
//
// Both gate on XIAOBAN_DEV_PERF=1 and dump JSON-lines (default ~/.xiaoban/perf.log,
// override XIAOBAN_DEV_PERF_LOG). Tagged { src: 'react' | 'frame' } for jq.
// XIAOBAN_DEV_PERF_MS (default 2) skips sub-ms idle frames; set 0 to capture all.
//
...
import { scrollFastPathStats } from '@xiaoban/ink'
import { Profiler, type ProfilerOnRenderCallback, type ReactNode } from 'react'

const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.XIAOBAN_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.XIAOBAN_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.XIAOBAN_DEV_PERF_LOG?.trim() || join(homedir(), '.xiaoban', 'perf.log')
