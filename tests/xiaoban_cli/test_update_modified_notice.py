"""Guard: every `xiaoban update` path that reports user-modified skills must
also tell the user how to find them.

`xiaoban update` keeps (does not overwrite) bundled skills the user edited and
prints a ``~ N user-modified (kept)`` count. There are two independent update
code paths in ``xiaoban_cli/main.py`` that print this notice (the git-pull path
in ``_cmd_update_impl`` and the unpack/install path). Both must point the user
at ``xiaoban skills list-modified`` so the count is actionable — otherwise,
...
_HINT_RE = re.compile(r"xiaoban skills list-modified")
...
def test_every_user_modified_notice_points_at_list_modified():
    ...
    for idx in count_sites:
        ...
        assert _HINT_RE.search(window), (
            ...
            "`xiaoban skills list-modified` within the following lines — the "
