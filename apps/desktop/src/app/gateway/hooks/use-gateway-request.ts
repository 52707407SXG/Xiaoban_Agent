...
import type { XiaobanGateway } from '@/xiaoban'
...
export function useGatewayRequest() {
  ...
  const gatewayRef = useRef<XiaobanGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.xiaobanDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<XiaobanGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as XiaobanGateway | null
      }),
    []
  )
  ...
  const requestGateway = useCallback(
    async <T>(method: string, params: Record<string, unknown> = {}) => {
      ...
      if (!gateway) {
        throw new Error('Xiaoban gateway unavailable')
      }
