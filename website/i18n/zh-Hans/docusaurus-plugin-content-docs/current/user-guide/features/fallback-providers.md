...
`hermes fallback` 复用 `xiaoban model` 的提供商选择器——相同的提供商列表、相同的凭据提示、相同的验证流程。使用子命令 `add`、`list`（别名 `ls`）、`remove`（别名 `rm`）和 `clear` 来管理备用链。更改会持久化到 `config.yaml` 顶层的 `fallback_providers:` 列表中。
...
| Nous Portal | `nous` | `xiaoban setup --portal`（全新安装）或 `hermes auth add nous`（OAuth） |
...
| Qwen Portal（OAuth） | `qwen-oauth` | `xiaoban model`（Qwen Portal OAuth；可选：`HERMES_QWEN_BASE_URL`） |
