...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.xiaoban/media/'
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("XIAOBAN_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("XIAOBAN_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_operator_configured_extra_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XIAOBAN_MEDIA_ALLOW_DIRS", str(extra_root))
    ...
    def test_recency_trust_allows_freshly_produced_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("XIAOBAN_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("XIAOBAN_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("XIAOBAN_MEDIA_TRUST_RECENT_SECONDS", "600")
    ...
    def test_recency_trust_rejects_old_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("XIAOBAN_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("XIAOBAN_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("XIAOBAN_MEDIA_TRUST_RECENT_SECONDS", "60")
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.xiaoban/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.delenv("XIAOBAN_MEDIA_DELIVERY_STRICT", raising=False)
        monkeypatch.delenv("XIAOBAN_MEDIA_ALLOW_DIRS", raising=False)
    ...
    def test_denylist_blocks_xiaoban_credentials(self, tmp_path, monkeypatch):
        """~/.xiaoban/.env and ~/.xiaoban/auth.json stay blocked even in
        ...
        fake_home = tmp_path / "home"
        xiaoban_dir = fake_home / ".xiaoban"
        xiaoban_dir.mkdir(parents=True)
        env_file = xiaoban_dir / ".env"
        env_file.write_text("OPENAI_API_KEY=sk-...")
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._XIAOBAN_HOME",
            xiaoban_dir,
        )

        assert BasePlatformAdapter.validate_media_delivery_path(str(env_file)) is None

    def test_denylist_blocks_xiaoban_config_in_active_profile(self, tmp_path, monkeypatch):
        ...
        xiaoban_dir = fake_home / ".xiaoban"
        xiaoban_dir.mkdir(parents=True)
        config_file = xiaoban_dir / "config.yaml"
    ...
    def test_denylist_blocks_shared_xiaoban_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Xiaoban root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".xiaoban" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        xiaoban_root = fake_home / ".xiaoban"
        config_file = xiaoban_root / "config.yaml"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._XIAOBAN_ROOT",
            xiaoban_root,
        )
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the XIAOBAN_HOME root (google_token.json)
        ...
        xiaoban_dir = fake_home / ".xiaoban"
        xiaoban_dir.mkdir(parents=True)
        token = xiaoban_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._XIAOBAN_HOME", xiaoban_dir)
        monkeypatch.setattr("gateway.platforms.base._XIAOBAN_ROOT", xiaoban_dir)
    ...
    def test_denylist_blocks_pairing_directory_contents(self, tmp_path, monkeypatch):
        """Files under ~/.xiaoban/pairing/ (platform pairing tokens) are
    ...
    def test_xiaoban_cache_still_delivers_under_denied_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "home"
        xiaoban_dir = fake_home / ".xiaoban"
        cache_dir = xiaoban_dir / "cache" / "documents"
    ...
    def test_denylist_blocks_non_cache_file_under_xiaoban_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.xiaoban
        ...
        xiaoban_dir = fake_home / ".xiaoban"
        xiaoban_dir.mkdir(parents=True)
        artifact = xiaoban_dir / "adhoc_report.pdf"
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting XIAOBAN_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_strict_mode_truthy_aliases(self, monkeypatch, tmp_path):
        """``XIAOBAN_MEDIA_DELIVERY_STRICT=true|yes|on|1`` all enable strict mode."""
        self._patch_roots(monkeypatch)
        from gateway.platforms.base import _media_delivery_strict_mode

        for raw in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("XIAOBAN_MEDIA_DELIVERY_STRICT", raw)
    ...
    def test_root_home_xiaoban_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.xiaoban/.env`` stays blocked under the $HOME exception — it is a
...
class TestGetHumanDelay:
    def test_off_mode(self):
        with patch.dict(os.environ, {"XIAOBAN_HUMAN_DELAY_MODE": "off"}):
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_default_is_off(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("XIAOBAN_HUMAN_DELAY_MODE", None)
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_natural_mode_range(self):
        with patch.dict(os.environ, {"XIAOBAN_HUMAN_DELAY_MODE": "natural"}):
            delay = BasePlatformAdapter._get_human_delay()
            assert 0.8 <= delay <= 2.5

    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "XIAOBAN_HUMAN_DELAY_MODE": "natural",
            "XIAOBAN_HUMAN_DELAY_MIN_MS": "oops",
            "XIAOBAN_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_uses_env_vars(self):
        env = {
            "XIAOBAN_HUMAN_DELAY_MODE": "custom",
            "XIAOBAN_HUMAN_DELAY_MIN_MS": "100",
            "XIAOBAN_HUMAN_DELAY_MAX_MS": "200",
        }
        with patch.dict(os.environ, env):
            delay = BasePlatformAdapter._get_human_delay()
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"XIAOBAN_MEDIA_DELIVERY_STRICT": "1",
                                     "XIAOBAN_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XIAOBAN_MEDIA_DELIVERY_STRICT", "0")
