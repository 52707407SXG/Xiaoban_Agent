"""
Dump command for xiaoban CLI.

Outputs a compact, plain-text summary of the user's Xiaoban setup
...
from xiaoban_cli.config import get_xiaoban_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.xiaoban_build_sha`` by the Dockerfile's
    ``XIAOBAN_GIT_SHA`` build-arg (see ``xiaoban_cli/build_info.py``).
    ...
    try:
        from xiaoban_cli.build_info import get_build_sha
...
def _redact(value: str) -> str:
    ...
    ``xiaoban dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from xiaoban_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(xiaoban_home: Path) -> int:
...
def _cron_summary(xiaoban_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = xiaoban_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"xiaoban_home:      {display_xiaoban_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `xiaoban auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `xiaoban auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["xiaoban-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(xiaoban_home)}")
    lines.append(f"  skills:             {_count_skills(xiaoban_home)}")
