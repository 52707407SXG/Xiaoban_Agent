...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves xiaoban_cli.kanban_db on its own
  ...
  - worker logs are captured to XIAOBAN_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "XIAOBAN_KANBAN_TASK": task.id,
            "XIAOBAN_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="xiaoban_e2e_")
    os.environ["XIAOBAN_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from xiaoban_cli import kanban_db as kb

    # Point the `xiaoban` CLI child processes will run at the worktree
    # xiaoban_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "xiaoban")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m xiaoban_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/xiaoban_e2e_*/worker_*.log
    import glob
