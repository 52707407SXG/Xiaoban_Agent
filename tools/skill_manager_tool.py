...
~/.xiaoban/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `xiaoban config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.xiaoban/skills/ (single source of truth)
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`xiaoban curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.xiaoban/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Xiaoban profiles.
    ...
    # Default profile (~/.xiaoban/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.xiaoban/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`xiaoban -p "
            ...
            base += (
                ...
                f"Switch profiles (`xiaoban -p <name>`) to edit there, or "
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_xiaoban_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "Create only when the user explicitly asks you to make, learn, or "
        "remember a reusable skill, or when a dedicated background "
        "self-improvement process has that mandate. Do not create skills merely "
        "because a task was complex, successful, or user-corrected.\n"
        "Update only when the user explicitly asks you to revise a skill, or "
        "when a dedicated background self-improvement process has that mandate. "
        "If ordinary work reveals stale instructions, OS-specific failures, "
        "missing steps, or pitfalls, offer to patch the skill instead of "
        ...
        "will refuse with a message pointing the user to `xiaoban curator unpin <name>`. "
