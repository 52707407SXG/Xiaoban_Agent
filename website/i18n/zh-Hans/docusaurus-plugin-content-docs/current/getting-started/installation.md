...
原生 Windows 无需 WSL 即可运行 Hermes——CLI、gateway、TUI 和工具均可原生运行。（原生安装与 WSL2 安装可干净共存；唯一仅限 WSL2 的功能见下方功能说明。）遇到 bug 请[提交 issue](https://github.com/52707407SXG/Xiaoban-Agent/issues)。
...
| 用户级（git 安装程序）                  | `~/.hermes/xiaoban-agent/`      | `~/.local/bin/hermes`（符号链接）        | `~/.hermes/`                          |
| Root 模式（`sudo curl … \| sudo bash`） | `/usr/local/lib/xiaoban-agent/` | `/usr/local/bin/hermes`                  | `/root/.hermes/`（或 `$HERMES_HOME`） |
...
4. **验证：** `hermes doctor` 现在应能正常运行。如果出现 `ModuleNotFoundError: No module named 'dotenv'`，说明你在用系统 Python 调用仓库源码中的 `hermes` 文件（`~/.hermes/xiaoban-agent/hermes`），而非 venv 启动器（`~/.hermes/xiaoban-agent/venv/bin/hermes`）——请修正步骤 3。
...
| `API key not set`           | 运行 `xiaoban model` 配置提供商，或 `hermes config set OPENROUTER_API_KEY your_key` |
...
Hermes 会自动检测安装方式（`pip`、git 安装程序、Homebrew 或 NixOS），`xiaoban update` 会打印对应路径的更新命令。无需设置任何环境变量——检测基于安装目录结构（Python site-packages、`~/.hermes/xiaoban-agent/`、Homebrew 前缀或 Nix store 路径）。`hermes doctor` 也会在其环境摘要中显示检测到的安装方式。
