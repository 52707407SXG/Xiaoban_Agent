...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Xiaoban passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("XIAOBAN_HOME", "/users/alice/.xiaoban/profiles/backend-worker")
        monkeypatch.setenv("XIAOBAN_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "XIAOBAN_KANBAN_DB",
            "/users/alice/.xiaoban/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.xiaoban/kanban/boards/smoke"]'
            in cmd
        )
