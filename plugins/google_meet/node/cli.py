"""`xiaoban meet node ...` subcommand tree.

Wired into the existing ``xiaoban meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``xiaoban meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="xiaoban-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``xiaoban meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             xiaoban meet node approve <name> ws://<host>:{args.port} {token}")
        try:
