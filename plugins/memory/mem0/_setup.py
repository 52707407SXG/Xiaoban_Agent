...
def _save_mem0_json(xiaoban_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(xiaoban_home) / "mem0.json"
    existing = {}
...
def _setup_platform(xiaoban_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "xiaoban-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "xiaoban"},
    ...
    provider.save_config(provider_config, xiaoban_home)

    if env_writes:
        _write_env(Path(xiaoban_home) / ".env", env_writes)
...
def _setup_oss(xiaoban_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(xiaoban_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "xiaoban-user")
    ...
    if env_writes:
        _write_env(Path(xiaoban_home) / ".env", env_writes)
    _save_mem0_json(xiaoban_home, {"mode": "oss", "user_id": user_id, "agent_id": "xiaoban", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, xiaoban_home: str) -> str:
...
_PGVECTOR_CONTAINER = "xiaoban-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "xiaoban"
...
def _setup_oss_interactive(xiaoban_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], xiaoban_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], xiaoban_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'xiaoban-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "xiaoban-user")

    agent_id = input("  Agent ID [xiaoban]: ").strip()
    agent_id = agent_id or "xiaoban"
    ...
    if env_writes:
        _write_env(Path(xiaoban_home) / ".env", env_writes)
    _save_mem0_json(xiaoban_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(xiaoban_home: str, config: dict) -> None:
    """Entry point called by xiaoban memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(xiaoban_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(xiaoban_home, config, flags)
        return
