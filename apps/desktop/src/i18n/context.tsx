...
import { getXiaobanConfigRecord, type XiaobanConfigRecord, saveXiaobanConfig } from '@/xiaoban'
...
export interface I18nConfigClient {
  getConfig: () => Promise<XiaobanConfigRecord>
  saveConfig: (config: XiaobanConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.xiaobanDesktop?.api) {
      return Promise.resolve({})
    }

    return getXiaobanConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.xiaobanDesktop?.api) {
    ...
    return saveXiaobanConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: XiaobanConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: XiaobanConfigRecord, locale: Locale): XiaobanConfigRecord {
