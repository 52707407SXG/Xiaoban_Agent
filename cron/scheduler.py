...
Uses a file-based lock (~/.xiaoban/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `xiaoban update` reloads
# the module) fail with ModuleNotFoundError for xiaoban_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from xiaoban_constants import get_xiaoban_home
from xiaoban_cli._subprocess_compat import windows_hide_flags
from xiaoban_cli.config import load_config, _expand_env_vars
from xiaoban_time import now as _xiaoban_now
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy xiaoban_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from xiaoban_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``xiaoban tools`` config for the ``cron`` platform.
    ...
    try:
        from xiaoban_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_xiaoban_home: Path | None = None


def _get_xiaoban_home() -> Path:
    """Resolve Xiaoban home dynamically while preserving test monkeypatch hooks."""
    return _xiaoban_home or get_xiaoban_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    xiaoban_home = _get_xiaoban_home()
    lock_dir = xiaoban_home / "cron"
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("XIAOBAN_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid XIAOBAN_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within XIAOBAN_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Xiaoban-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against XIAOBAN_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_xiaoban_home() / "scripts"
    ...
    # escape — scripts MUST reside within XIAOBAN_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _xiaoban_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_xiaoban_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_xiaoban_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["XIAOBAN_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed XIAOBAN_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads XIAOBAN_SESSION_PLATFORM
    #     and XIAOBAN_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via XIAOBAN_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     XIAOBAN_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read XIAOBAN_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the XIAOBAN_CRON_AUTO_DELIVER_* vars set
    # below, so clearing XIAOBAN_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "XIAOBAN_CRON_AUTO_DELIVER_PLATFORM",
        "XIAOBAN_CRON_AUTO_DELIVER_CHAT_ID",
        "XIAOBAN_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_xiaoban_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_xiaoban_home() / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["XIAOBAN_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["XIAOBAN_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["XIAOBAN_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        # Model resolution precedence: per-job override > XIAOBAN_MODEL env >
        ...
        model = job.get("model") or os.getenv("XIAOBAN_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_xiaoban_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"XIAOBAN_MODEL={os.getenv('XIAOBAN_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`cronjob action=update job_id={job_id} model=<name>` or set a "
                "default with `xiaoban model <name>`."
            )
        ...
        prefill_file = (
            os.getenv("XIAOBAN_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_xiaoban_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject XIAOBAN_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # XIAOBAN_HOME. When a workdir is configured, also inject project
        ...
        # override via XIAOBAN_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("XIAOBAN_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid XIAOBAN_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_xiaoban_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_xiaoban_now().strftime('%b %d %H:%M')}"
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _xiaoban_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _xiaoban_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set XIAOBAN_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("XIAOBAN_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid XIAOBAN_CRON_MAX_PARALLEL value; defaulting to unbounded")
