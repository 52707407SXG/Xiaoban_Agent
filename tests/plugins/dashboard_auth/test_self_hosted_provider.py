...
_ISSUER = "https://auth.example.com/application/o/xiaoban"
_CLIENT_ID = "xiaoban-dashboard"
...
class TestStartLogin:
    ...
    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://xiaoban.example/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://xiaoban.example/auth/callback"
    ...
    def test_two_calls_differ(self, provider):
        a = provider.start_login(redirect_uri="https://xiaoban.example/auth/callback")
        b = provider.start_login(redirect_uri="https://xiaoban.example/auth/callback")
        assert (
            a.cookie_payload["xiaoban_session_pkce"]
            != b.cookie_payload["xiaoban_session_pkce"]
        )
...
class TestPluginRegister:
    ...
    def clear_env(self, monkeypatch):
        for var in (
            "XIAOBAN_DASHBOARD_OIDC_ISSUER",
            "XIAOBAN_DASHBOARD_OIDC_CLIENT_ID",
            "XIAOBAN_DASHBOARD_OIDC_SCOPES",
        ):
    ...
    def patch_config(self, monkeypatch):
        def _set(oauth_block):
            ...
            monkeypatch.setattr("xiaoban_cli.config.load_config", lambda: cfg)
    ...
    def test_skips_when_unconfigured(self, patch_config):
        ...
        assert "XIAOBAN_DASHBOARD_OIDC_ISSUER" in oidc_plugin.LAST_SKIP_REASON
        assert "self_hosted" in oidc_plugin.LAST_SKIP_REASON

    def test_skips_when_only_issuer_set(self, patch_config, monkeypatch):
        patch_config(None)
        monkeypatch.setenv("XIAOBAN_DASHBOARD_OIDC_ISSUER", _ISSUER)
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
    ...
    def test_registers_from_env(self, patch_config, monkeypatch):
        patch_config(None)
        monkeypatch.setenv("XIAOBAN_DASHBOARD_OIDC_ISSUER", _ISSUER)
        monkeypatch.setenv("XIAOBAN_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        ctx = MagicMock()
        oidc_plugin.register(ctx)
    ...
    def test_empty_env_does_not_shadow_config(self, patch_config, monkeypatch):
        ...
        monkeypatch.setenv("XIAOBAN_DASHBOARD_OIDC_ISSUER", "")
        monkeypatch.setenv("XIAOBAN_DASHBOARD_OIDC_CLIENT_ID", "")
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_config_load_failure_falls_through(self, monkeypatch):
        ...
        monkeypatch.setattr("xiaoban_cli.config.load_config", _broken)
        ctx = MagicMock()
    ...
    def test_non_https_issuer_skips_with_reason(self, patch_config, monkeypatch):
        patch_config(None)
        monkeypatch.setenv(
            "XIAOBAN_DASHBOARD_OIDC_ISSUER", "http://insecure.example"
        )
        monkeypatch.setenv("XIAOBAN_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        ctx = MagicMock()
        oidc_plugin.register(ctx)
