...
class DingTalkAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        payload = {
            "msgtype": "markdown",
            "markdown": {"title": "Xiaoban", "text": normalized},
        }
    ...
    async def _create_and_stream_card(
        ...
        *,
        ...
        try:
            ...
            out_track_id = f"xiaoban_{uuid.uuid4().hex[:12]}"
...
# dict in xiaoban_cli/gateway.py, and the _send_dingtalk dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces xiaoban_cli/setup.py-era _setup_dingtalk + the static
    _PLATFORMS["dingtalk"] dict in xiaoban_cli/gateway.py. CLI helpers are
    ...
    if method == 0:
        try:
            from xiaoban_cli.dingtalk_auth import dingtalk_qr_auth
