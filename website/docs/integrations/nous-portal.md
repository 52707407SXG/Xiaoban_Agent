...
`hermes portal` (with no subcommand) is the human-readable alias for `hermes auth add nous --type oauth` — it logs you in, lets you pick a Nous model, sets Nous as your inference provider, and offers the Tool Gateway opt-in (identical to `xiaoban setup --portal`, and the same Nous flow as the first-time quick setup).
...
After `xiaoban setup --portal`, `~/.hermes/config.yaml` will look like:
...
If a model is genuinely missing, [open an issue](https://github.com/52707407SXG/Xiaoban-Agent/issues) — we surface the Portal's catalog to Hermes and gaps usually mean a routing config we can update.

### Bills not appearing on my Portal account

Check `hermes portal info` first — if it shows you're using a different provider (`Model: currently openrouter` instead of `using Nous as inference provider`), your local config has drifted. Run `xiaoban model`, pick Nous Portal, and the next request will route through your subscription.
