...
the SAME provider universe as ``xiaoban model`` (the CLI/TUI picker).
...
def test_every_xiaoban_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `xiaoban model` universe."""
    ...
    assert not missing, (
        "providers shown in `xiaoban model` but not configurable in the desktop "
