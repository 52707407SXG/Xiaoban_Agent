"""Persistent slash-command worker — one XiaobanCLI per TUI session.
...
def _env_float(name: str, default: float) -> float:
    ...
    import time on a typo (e.g. ``XIAOBAN_SLASH_WATCHDOG_POLL_S=2s``) and kill
...
_WATCHDOG_POLL_S = max(0.05, _env_float("XIAOBAN_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, _env_float("XIAOBAN_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _run(cli: XiaobanCLI, command: str) -> str:
...
def main():
    ...
    os.environ["XIAOBAN_SESSION_KEY"] = args.session_key
    os.environ["XIAOBAN_INTERACTIVE"] = "1"

    # Start before the (hundreds-of-ms) XiaobanCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = XiaobanCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
