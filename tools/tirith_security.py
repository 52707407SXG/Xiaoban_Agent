...
it is automatically downloaded from GitHub releases to $XIAOBAN_HOME/bin/tirith.
...
def _get_xiaoban_home() -> str:
    """Return the Xiaoban home directory, respecting XIAOBAN_HOME env var."""
    return str(get_xiaoban_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_xiaoban_home(), ".tirith-install-failed")
...
def _xiaoban_bin_dir() -> str:
    """Return $XIAOBAN_HOME/bin, creating it if needed."""
    d = os.path.join(_get_xiaoban_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $XIAOBAN_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_xiaoban_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $XIAOBAN_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $XIAOBAN_HOME/bin/tirith
    ...
    xiaoban_bin = os.path.join(_xiaoban_bin_dir(), "tirith")
    if os.path.isfile(xiaoban_bin) and os.access(xiaoban_bin, os.X_OK):
        _resolved_path = xiaoban_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return xiaoban_bin
