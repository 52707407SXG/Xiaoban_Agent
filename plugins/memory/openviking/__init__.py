...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Xiaoban peer ID in OpenViking (default: xiaoban)
...
_DEFAULT_AGENT = "xiaoban"
_AGENT_PROMPT_LABEL = "Xiaoban peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "XIAOBAN_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Xiaoban slash-skill scaffolding before sending content to OpenViking.
...
def _load_xiaoban_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("XIAOBAN_HOME", "")).expanduser() if os.environ.get("XIAOBAN_HOME") else Path.home() / ".xiaoban"
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Xiaoban session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Xiaoban only", "write values only to Xiaoban .env"),
    ...
    _save_xiaoban_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Xiaoban .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See xiaoban-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_xiaoban_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Xiaoban peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, xiaoban_home: str, config: dict) -> None:
        ...
        from xiaoban_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        xiaoban_home_path = Path(xiaoban_home)
        env_path = xiaoban_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_xiaoban_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Xiaoban run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Xiaoban' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Xiaoban canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See xiaoban-agent#28296.
    ...
    def _tool_read(self, args: dict) -> str:
        ...
        # (e.g. viking://user/xiaoban/.overview.md).
