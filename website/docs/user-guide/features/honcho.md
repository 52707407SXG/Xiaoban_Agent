...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Xiaoban's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Xiaoban instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
xiaoban memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `xiaoban` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Xiaoban syncs them back at session init.
...
The `xiaoban honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `xiaoban memory setup honcho` (or run `xiaoban memory setup` and pick it from the list); the `xiaoban honcho` subcommand then appears on the next invocation.

```bash
xiaoban memory setup honcho    # Configure Honcho directly (works before activation)
xiaoban honcho status          # Connection status, config, and key settings
xiaoban honcho setup           # Redirects to `xiaoban memory setup` (post-activation alias)
xiaoban honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
xiaoban honcho peer            # Show or update peer names + dialectic reasoning level
xiaoban honcho mode            # Show or set recall mode (hybrid/context/tools)
xiaoban honcho tokens          # Show or set token budget for context and dialectic
xiaoban honcho identity        # Seed or show the AI peer's Honcho identity
xiaoban honcho sync            # Sync Honcho config to all existing profiles
xiaoban honcho peers           # Show peer identities across all profiles
xiaoban honcho sessions        # List known Honcho session mappings
xiaoban honcho map             # Map current directory to a Honcho session name
xiaoban honcho enable          # Enable Honcho for the active profile
xiaoban honcho disable         # Disable Honcho for the active profile
xiaoban honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `xiaoban honcho`

If you previously used the standalone `xiaoban honcho setup`:
...
No re-login or re-setup needed. Run `xiaoban memory setup` and select "honcho" — the wizard detects your existing config.
