...
XIAOBAN_HOME_DIR="${XIAOBAN_HOME:-$HOME/.xiaoban}"
XIAOBAN_CFG="${XIAOBAN_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Xiaoban config has twozero_td MCP entry ──
if [[ ! -f "$XIAOBAN_CFG" ]]; then
    echo -e " ${FAIL} Xiaoban config not found at ${XIAOBAN_CFG}"
    manual_steps+=("Create ${XIAOBAN_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$XIAOBAN_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Xiaoban config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Xiaoban config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$XIAOBAN_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${XIAOBAN_CFG} manually"); }
    manual_steps+=("Restart Xiaoban session to pick up config change")
fi
