"""Tests for the Xiaoban plugin system (xiaoban_cli.plugins)."""
...
from xiaoban_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<xiaoban_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<xiaoban_home>/plugins/``; we derive
    ``<xiaoban_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <XIAOBAN_HOME>/config.yaml.
        # Config is always read from XIAOBAN_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        xiaoban_home_str = os.environ.get("XIAOBAN_HOME")
        if xiaoban_home_str:
            xiaoban_home = Path(xiaoban_home_str)
        else:
            xiaoban_home = base.parent
        xiaoban_home.mkdir(parents=True, exist_ok=True)
        cfg_path = xiaoban_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.xiaoban/plugins/ are discovered."""
        plugins_dir = tmp_path / "xiaoban_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("XIAOBAN_HOME", str(tmp_path / "xiaoban_test"))
    ...
    def test_execution_middleware_does_not_retry_downstream_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("xiaoban_cli.plugins.get_plugin_manager", lambda: manager)
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XIAOBAN_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".xiaoban" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        xiaoban_home = tmp_path / "xiaoban_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under xiaoban_plugins.<name>."""
        plugins_dir = tmp_path / "xiaoban_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("XIAOBAN_HOME", str(tmp_path / "xiaoban_test"))

        # Clean up any prior namespace module
        sys.modules.pop("xiaoban_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "xiaoban_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="xiaoban_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import xiaoban_cli.plugins as plugins_mod
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `xiaoban plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("xiaoban_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        xiaoban_home = tmp_path / "xiaoban_test"
        plugins_dir = xiaoban_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "xiaoban_test" / "plugins"
        # Set XIAOBAN_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("xiaoban_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("xiaoban_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("xiaoban_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("xiaoban_cli.plugins.PluginContext.dispatch_tool.__module__", "xiaoban_cli.plugins"):
...
class TestPluginDebugLogging:
    """XIAOBAN_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("XIAOBAN_PLUGINS_DEBUG", raising=False)
        from xiaoban_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With XIAOBAN_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("XIAOBAN_PLUGINS_DEBUG", "1")
        from xiaoban_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from XIAOBAN_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """XIAOBAN_HOME at the root resolves to 'default'."""
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """XIAOBAN_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".xiaoban" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XIAOBAN_HOME", str(prof))
    ...
    def test_works_without_cli_ref(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".xiaoban" / "profiles" / "worker1"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XIAOBAN_HOME", str(prof))
        ctx = self._ctx()
