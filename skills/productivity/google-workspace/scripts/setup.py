#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Xiaoban-Agent.
...
# Ensure sibling modules (_xiaoban_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _xiaoban_home import display_xiaoban_home, get_xiaoban_home

XIAOBAN_HOME = get_xiaoban_home()
TOKEN_PATH = XIAOBAN_HOME / "google_token.json"
CLIENT_SECRET_PATH = XIAOBAN_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = XIAOBAN_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Xiaoban profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Xiaoban Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Xiaoban Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'xiaoban-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Xiaoban home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_xiaoban_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Xiaoban")
