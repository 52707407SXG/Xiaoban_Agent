...
posts are converted into Xiaoban gateway events instead of being silently
ignored.
"""
...
def _make_channel_message(text="channel id test @xiaoban_bot"):
...
async def test_text_handler_uses_effective_message_for_channel_post(telegram_adapter_cls):
    ...
    assert event.text == "channel id test @xiaoban_bot"
