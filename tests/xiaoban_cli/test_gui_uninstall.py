"""Tests for xiaoban_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import xiaoban_cli.gui_uninstall as gu


def _make_agent(xiaoban_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = xiaoban_home / "xiaoban-agent"
    (agent_root / "xiaoban_cli").mkdir(parents=True)
    (agent_root / "xiaoban_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(xiaoban_home: Path) -> None:
    """Create the source-built GUI artifacts a `xiaoban desktop` run produces."""
    desktop = xiaoban_home / "xiaoban-agent" / "apps" / "desktop"
    ...
    (xiaoban_home / "xiaoban-agent" / "node_modules").mkdir(parents=True)
    (xiaoban_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(xiaoban_home: Path) -> None:
    (xiaoban_home / "config.yaml").write_text("x: 1\n")
    (xiaoban_home / ".env").write_text("KEY=secret\n")
    (xiaoban_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    xiaoban_home = tmp_path / ".xiaoban"
    xiaoban_home.mkdir()
    assert gu.agent_is_installed(xiaoban_home) is False
    _make_agent(xiaoban_home)
    assert gu.agent_is_installed(xiaoban_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    xiaoban_home = tmp_path / ".xiaoban"
    (xiaoban_home / "xiaoban-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(xiaoban_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    xiaoban_home = tmp_path / ".xiaoban"
    _make_gui_build(xiaoban_home)
    artifacts = gu.source_built_gui_artifacts(xiaoban_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(xiaoban_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(xiaoban_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    xiaoban_home = tmp_path / ".xiaoban"
    agent_root = _make_agent(xiaoban_home)
    _make_gui_build(xiaoban_home)
    _make_user_data(xiaoban_home)
    ...
    removed = gu.uninstall_gui(xiaoban_home)
    ...
    assert not (xiaoban_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "xiaoban_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (xiaoban_home / "config.yaml").exists()
    assert (xiaoban_home / ".env").exists()
    assert (xiaoban_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    xiaoban_home = tmp_path / ".xiaoban"
    _make_agent(xiaoban_home)
    userdata = tmp_path / "Xiaoban-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(xiaoban_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(xiaoban_home)
    ...
    assert summary["xiaoban_home"] == str(xiaoban_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Xiaoban"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Xiaoban"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Xiaoban"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Xiaoban"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    XIAOBAN_HOME with all the destructive externals stubbed out.
    """
    import xiaoban_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_xiaoban_home", lambda: xiaoban_home)
    ...
    from xiaoban_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (xiaoban_home / "config.yaml").exists()
    assert xiaoban_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole XIAOBAN_HOME non-interactively."""
    import xiaoban_cli.uninstall as uninstall

    xiaoban_home = tmp_path / ".xiaoban"
    (xiaoban_home / "xiaoban-agent" / "xiaoban_cli").mkdir(parents=True)
    (xiaoban_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not xiaoban_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m xiaoban_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_xiaoban_home", lambda: xiaoban_home)
    ...
    assert not (desktop / "release").exists()
    assert not (xiaoban_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "xiaoban_cli").exists()
    assert (xiaoban_home / "config.yaml").exists()
