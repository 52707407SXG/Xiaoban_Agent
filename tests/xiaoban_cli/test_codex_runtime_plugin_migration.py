...
from xiaoban_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
    MIGRATION_END_MARKER,
    _build_xiaoban_tools_mcp_entry,
    _format_toml_value,
...
class TestMigrate:
    ...
    def test_no_servers_still_writes_permissions_default(self, tmp_path):
        ...
        report = migrate({}, codex_home=tmp_path, discover_plugins=False, expose_xiaoban_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from xiaoban_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from xiaoban_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_xiaoban_tools=False)
        assert report.written
    ...
    def test_discover_plugins_false_skips_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, discover_plugins=False, expose_xiaoban_tools=False)
    ...
    def test_dry_run_skips_plugin_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, dry_run=True, discover_plugins=True, expose_xiaoban_tools=False)
        assert called["yes"] is False
    ...
    def test_expose_xiaoban_tools_writes_callback_mcp_entry(self, tmp_path):
        """When expose_xiaoban_tools=True (production default), an
        [mcp_servers.xiaoban-tools] entry is written so codex calls back
        into Xiaoban for browser/web/delegate_task/vision/memory tools.

        This is the fix for 'all other tools that codex doesn't provide
        should be useable by xiaoban' — quirk #7."""
        report = migrate({}, codex_home=tmp_path,
                         discover_plugins=False,
                         default_permission_profile=None,
                         expose_xiaoban_tools=True)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.xiaoban-tools]" in text
        assert "xiaoban_tools_mcp_server" in text
        ...
        assert "xiaoban-tools" in report.migrated

    def test_expose_xiaoban_tools_disabled_skips_entry(self, tmp_path):
        """expose_xiaoban_tools=False suppresses the callback registration."""
        migrate({}, codex_home=tmp_path,
                discover_plugins=False,
                default_permission_profile=None,
                expose_xiaoban_tools=False)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.xiaoban-tools]" not in text
        assert "xiaoban_tools_mcp_server" not in text

    def test_dry_run_doesnt_write(self, tmp_path):
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, dry_run=True, expose_xiaoban_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(xiaoban_cfg, codex_home=tmp_path, expose_xiaoban_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_idempotent_re_run_replaces_managed_block(self, tmp_path):
        # First migration
        migrate({"mcp_servers": {"a": {"command": "x"}}}, codex_home=tmp_path, expose_xiaoban_tools=False)
        first_text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.a]" in first_text
        # Second migration with different servers
        migrate({"mcp_servers": {"b": {"command": "y"}}}, codex_home=tmp_path, expose_xiaoban_tools=False)
    ...
    def test_managed_root_keys_stay_top_level_when_config_ends_in_table(self, tmp_path):
        """TOML has no explicit 'leave current table' syntax. If Xiaoban appends
        ...
        migrate({}, codex_home=tmp_path, discover_plugins=False, expose_xiaoban_tools=False)
        new_text = target.read_text()
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Xiaoban' managed block, re-running
        ...
        migrate({"mcp_servers": {"xiaoban-mcp": {"command": "npx"}}},
        ...
        assert "[mcp_servers.xiaoban-mcp]" in final
    ...
    def test_skipped_keys_reported(self, tmp_path):
        ...
        }, codex_home=tmp_path, expose_xiaoban_tools=False)
    ...
    def test_invalid_mcp_servers_value(self, tmp_path):
        report = migrate({"mcp_servers": "notadict"}, codex_home=tmp_path, expose_xiaoban_tools=False)
...
class TestStripUnmanagedPluginTables:
    ...
    ``xiaoban codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "xiaoban_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
        migrate({}, codex_home=tmp_path, discover_plugins=True, expose_xiaoban_tools=False)
        new_text = target.read_text()
...
# ---- Bug C: XIAOBAN_HOME tempdir leak into ~/.codex/config.toml ----


class TestXiaobanHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_xiaoban_tools_mcp_entry()`` read ``XIAOBAN_HOME``
    ...
    once codex spawned the xiaoban-tools MCP subprocess.
    """

    def test_tempdir_detector_recognizes_pytest_paths(self):
        assert _looks_like_test_tempdir(
            "/private/var/folders/abc/pytest-of-kshitij/pytest-137/popen-gw2/test_X/xiaoban_test"
        )
        assert _looks_like_test_tempdir(
            "/tmp/pytest-of-user/pytest-12/test_X/xiaoban"
        )
        assert _looks_like_test_tempdir(
            "/private/var/folders/zz/T/pytest-of-bob/pytest-1"
        )

    def test_tempdir_detector_accepts_real_xiaoban_home(self):
        assert not _looks_like_test_tempdir("/Users/alice/.xiaoban")
        assert not _looks_like_test_tempdir("/home/bob/.xiaoban")
        assert not _looks_like_test_tempdir("/opt/xiaoban")
        assert not _looks_like_test_tempdir("")

    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when XIAOBAN_HOME points at a pytest
        tempdir, _build_xiaoban_tools_mcp_entry() must NOT propagate it."""
        monkeypatch.setenv(
            "XIAOBAN_HOME",
            "/private/var/folders/xx/pytest-of-user/pytest-99/test_x/xiaoban_test",
        )
        entry = _build_xiaoban_tools_mcp_entry()
        env = entry.get("env", {})
        assert "XIAOBAN_HOME" not in env, (
            f"pytest-tempdir XIAOBAN_HOME leaked into codex MCP entry: "
            f"{env.get('XIAOBAN_HOME')!r}"
        )

    def test_real_xiaoban_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate XIAOBAN_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.xiaoban"
        monkeypatch.setenv("XIAOBAN_HOME", real_path)
        entry = _build_xiaoban_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("XIAOBAN_HOME") == real_path

    def test_unset_xiaoban_home_omits_env_key(self, monkeypatch):
        """When XIAOBAN_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever XIAOBAN_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "XIAOBAN_HOME" not in env, (
            f"XIAOBAN_HOME should not be set when env var is unset, got: "
            f"{env.get('XIAOBAN_HOME')!r}"
        )
