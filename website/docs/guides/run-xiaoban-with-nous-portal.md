...
3. Sets `model.provider: nous` in `~/.xiaoban/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Xiaoban is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
xiaoban setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: manual paste (for Cloud Shell, Codespaces, EC2 Instance Connect)
xiaoban auth add nous --type oauth --manual-paste
# Then re-run `xiaoban setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Xiaoban-Agent release notes" and summarize the top 3 hits.
```

You should see Xiaoban call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`xiaoban setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
xiaoban config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Xiaoban-4 for agent work

Xiaoban-4-70B and Xiaoban-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them via [Nous Chat](https://chat.nousresearch.com) for conversation/research work, or through the [subscription proxy](/user-guide/features/subscription-proxy) from non-agent tools. For Xiaoban-Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https://portal.nousresearch.com/info) carries this warning too — it's the official Nous guidance, not just a Xiaoban-side opinion.
...
These rows appear in `xiaoban tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Xiaoban runs the Portal login inline (without changing your inference provider or your other tools).

Verify your mix with:

```bash
xiaoban portal tools
```
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Xiaoban will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
xiaoban cron create "every day at 9am" \
...
If you use [Xiaoban profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.xiaoban/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `xiaoban portal info` shows "not logged in" after `xiaoban setup --portal`
...
```bash
xiaoban config set model.provider nous
```
...
Re-verify with `xiaoban portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Xiaoban doesn't replay it endlessly. Just log in again:

```bash
xiaoban auth add nous
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/52707407SXG/Xiaoban_Agent/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`xiaoban portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Xiaoban profiles where you're using the wrong one (check `xiaoban profile list`)

### Want to revoke and start clean

```bash
xiaoban auth logout nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Xiaoban tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Xiaoban configurations
