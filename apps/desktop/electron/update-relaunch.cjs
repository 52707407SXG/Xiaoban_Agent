 ...
 * Background
 * ----------
 * After `xiaoban update` + `xiaoban desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything XIAOBAN_DESKTOP_* is preserved
// plus XIAOBAN_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['XIAOBAN_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['XIAOBAN_DESKTOP_']
