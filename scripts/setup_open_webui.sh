...
# Bootstrap Open WebUI against Xiaoban-Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.xiaoban/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-xiaoban.sh
...
#   OPEN_WEBUI_NAME='Johnny Xiaoban'
...
#   XIAOBAN_API_PORT=8642
#   XIAOBAN_API_HOST=127.0.0.1
#   XIAOBAN_API_MODEL_NAME='Xiaoban-Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Xiaoban-Agent WebUI}"
...
XIAOBAN_ENV_FILE="${XIAOBAN_ENV_FILE:-$HOME/.xiaoban/.env}"
XIAOBAN_API_PORT="${XIAOBAN_API_PORT:-8642}"
XIAOBAN_API_HOST="${XIAOBAN_API_HOST:-127.0.0.1}"
XIAOBAN_API_CONNECT_HOST="${XIAOBAN_API_CONNECT_HOST:-127.0.0.1}"
XIAOBAN_API_MODEL_NAME="${XIAOBAN_API_MODEL_NAME:-Xiaoban-Agent}"
XIAOBAN_API_BASE_URL="http://${XIAOBAN_API_CONNECT_HOST}:${XIAOBAN_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-xiaoban.sh"
LOG_DIR="$HOME/.xiaoban/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$XIAOBAN_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$XIAOBAN_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.xiaoban.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.xiaoban</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.xiaoban"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.xiaoban"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-xiaoban.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Xiaoban-Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-xiaoban.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.xiaoban/logs/openwebui.log
StandardError=append:%h/.xiaoban/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-xiaoban.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$XIAOBAN_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Xiaoban API server is configured...'
  upsert_env API_SERVER_ENABLED true "$XIAOBAN_ENV_FILE"
  upsert_env API_SERVER_HOST "$XIAOBAN_API_HOST" "$XIAOBAN_ENV_FILE"
  upsert_env API_SERVER_PORT "$XIAOBAN_API_PORT" "$XIAOBAN_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$XIAOBAN_API_MODEL_NAME" "$XIAOBAN_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$XIAOBAN_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Xiaoban gateway so API server settings take effect...'
  xiaoban gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${XIAOBAN_API_CONNECT_HOST}:${XIAOBAN_API_PORT}/health" >/dev/null; then
    log 'Xiaoban API server did not answer on the first check. Trying to start gateway in the background...'
    nohup xiaoban gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${XIAOBAN_API_CONNECT_HOST}:${XIAOBAN_API_PORT}/health" >/dev/null
  ...
  log "Xiaoban API endpoint: ${XIAOBAN_API_BASE_URL}"
