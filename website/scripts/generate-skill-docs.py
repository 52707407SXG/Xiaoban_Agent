...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/52707407SXG/Xiaoban-Agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "Hermes also syncs bundled skills on `xiaoban update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.hermes/skills/` tree, it is still shipped with Hermes; restore it with `xiaoban skills reset <name> --restore`.",
        "",
