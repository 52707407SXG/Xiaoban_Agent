"""
Interactive setup wizard for Xiaoban-Agent.
...
_DOCS_BASE = "https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs"
...
# display_xiaoban_home imported lazily at call sites (stale-module safety during xiaoban update)
...
from xiaoban_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from xiaoban_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("M Xiaoban Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Xiaoban using environment variables or config commands:")
    print_info("  xiaoban config set model.provider custom")
    print_info("  xiaoban config set model.base_url http://localhost:8080/v1")
    print_info("  xiaoban config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'xiaoban setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from xiaoban_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'xiaoban setup')")


def _print_setup_summary(config: dict, xiaoban_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'xiaoban setup' to configure"))
    ...
    # Video generation — opt-in via `xiaoban tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from xiaoban_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'xiaoban setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'xiaoban setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'xiaoban setup terminal'"))
    ...
    # Spotify (OAuth via xiaoban auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'xiaoban setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {xiaoban_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('xiaoban setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('xiaoban setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('xiaoban setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('xiaoban setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('xiaoban setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('xiaoban config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('xiaoban config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('xiaoban config set <key> <value>', Colors.GREEN)}")
...
# between `xiaoban tools` and `xiaoban setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``xiaoban model``)
    ...
    provider added to ``xiaoban model`` is automatically available here.
    ...
    # Delegate to the shared xiaoban model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from xiaoban_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: xiaoban model")
    ...
    # on demand via `xiaoban auth add`, `xiaoban setup` vision, and
    # `xiaoban setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``xiaoban model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from xiaoban_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.xiaoban/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Xiaoban auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via xiaoban setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'xiaoban setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Xiaoban runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `xiaoban setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `xiaoban setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into XIAOBAN_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("XIAOBAN_MAX_ITERATIONS")
    ...
    print_info("  Run `xiaoban setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # XIAOBAN_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from xiaoban_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_xiaoban_home(Path(get_xiaoban_home()))
    except Exception:
        pass
...
def _profile_name_from_xiaoban_home(xiaoban_home) -> str | None:
    """Return the active profile name when XIAOBAN_HOME is a profile dir."""
    if xiaoban_home.parent.name == "profiles":
        return xiaoban_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Xiaoban delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Xiaoban to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from xiaoban_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  xiaoban config edit")
    print_info("   Open config in your editor:  xiaoban config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from xiaoban_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Xiaoban from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'xiaoban setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     xiaoban config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_xiaoban_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo xiaoban gateway install --system")
                print_info("  Or run in foreground:  xiaoban gateway")
        else:
            from xiaoban_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   xiaoban gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   xiaoban gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `xiaoban setup tools` and `xiaoban tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``xiaoban_cli.auth`` — lists every supported
    ...
    """
    try:
        from xiaoban_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from xiaoban_cli.config
    ...
    if section_key == "model":
        ...
        from xiaoban_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_xiaoban.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_xiaoban migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_xiaoban", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Xiaoban.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Xiaoban to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Xiaoban at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Xiaoban at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Xiaoban at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Xiaoban at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Xiaoban equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Xiaoban config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Xiaoban.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Xiaoban's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(xiaoban_home: Path) -> bool:
    ...
    print_info("Xiaoban can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: xiaoban claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=xiaoban_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: xiaoban claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Xiaoban configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Xiaoban config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Xiaoban (use xiaoban claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``xiaoban setup --portal`` and ``xiaoban portal``. This is the
    ...
    Xiaoban session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``xiaoban setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``xiaoban model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``xiaoban portal`` always offers a model picker,
    ...
    print(color("│     M Xiaoban Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `xiaoban portal` == quick setup's Nous step.
    try:
        from xiaoban_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `xiaoban portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `xiaoban portal`: %s", exc)
        print()
    ...
    print_info("  Run `xiaoban portal info` to inspect routing.")
    print_info("  Run `xiaoban` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      xiaoban setup           — full or quick (auto-detected)
      xiaoban setup model     — just model/provider
      xiaoban setup tts       — just text-to-speech
      xiaoban setup terminal  — just terminal backend
      xiaoban setup gateway   — just messaging platforms
      xiaoban setup tools     — just tool configuration
      xiaoban setup agent     — just agent settings
    """
    from xiaoban_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     M Xiaoban Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             M Xiaoban-Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Xiaoban-Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, xiaoban_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Xiaoban configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'xiaoban setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(xiaoban_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Xiaoban?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, xiaoban_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, xiaoban_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {xiaoban_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'xiaoban config edit'")
    ...
    # Tune later with `xiaoban setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, xiaoban_home)


def _run_first_time_quick_setup(config: dict, xiaoban_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``xiaoban setup <section>``
    or switch providers with ``xiaoban model``.
    """
    from xiaoban_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'xiaoban setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    xiaoban setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  xiaoban setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``xiaoban tools`` (which
    ...
    try:
        ...
        from xiaoban_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``xiaoban setup agent`` /
    ``xiaoban config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, xiaoban_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `xiaoban update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        xiaoban tools")
        print_info("  Seed skills:         xiaoban skills opt-in --sync")
        print_info("  Add MCP servers:     xiaoban mcp add")
        print_info("  Enable plugins:      xiaoban plugins")
        print_info("  Tune agent settings: xiaoban setup agent")
        print()
        _print_setup_summary(config, xiaoban_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, xiaoban_home)


def _blank_slate_walkthrough(config: dict, xiaoban_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`xiaoban update` runs from re-injecting them. Opt back in any")
            print_info("time with `xiaoban skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `xiaoban tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `xiaoban plugins list` / `xiaoban plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `xiaoban plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `xiaoban mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `xiaoban mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   xiaoban tools")
    print_info("  Seed skills:         xiaoban skills opt-in --sync")
    print_info("  Add MCP servers:     xiaoban mcp add")
    print_info("  Tune agent settings: xiaoban setup agent")
    print()
...
def _run_quick_setup(config: dict, xiaoban_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'xiaoban setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Xiaoban to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'xiaoban setup gateway'.")
