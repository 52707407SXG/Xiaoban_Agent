"""Tests for the unified provider catalog (xiaoban_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``xiaoban model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from xiaoban_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
...
def test_catalog_covers_every_xiaoban_model_provider():
    """PARITY CONTRACT: the catalog == the `xiaoban model` universe."""
    slugs = {d.slug for d in provider_catalog()}
    for entry in CANONICAL_PROVIDERS:
        assert entry.slug in slugs, (
            f"{entry.slug} is shown in `xiaoban model` but missing from provider_catalog()"
        )


def test_catalog_has_no_providers_outside_xiaoban_model():
    """The catalog must not invent providers `xiaoban model` doesn't show."""
