...
def test_xiaoban_provider_subclass_exists():
    """XiaobanMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _XIAOBAN_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _XIAOBAN_PROVIDER_CLS is not None
    assert issubclass(_XIAOBAN_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_xiaoban_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns XiaobanMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _XIAOBAN_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("XIAOBAN_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _XIAOBAN_PROVIDER_CLS is not None
    assert isinstance(provider, _XIAOBAN_PROVIDER_CLS)
    assert provider._xiaoban_server_name == "srv"
