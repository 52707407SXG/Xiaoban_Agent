...
def test_main_wrapper_preserves_docker_workdir() -> None:
    ...
    assert "_xiaoban_orig_cwd" in text, (
        "main-wrapper.sh must save the original cwd before cd /opt/data"
    )
    assert 'XIAOBAN_ORIG_CWD:-$PWD' in text, (
    ...
    # first exec / if-block.
    activate_idx = text.index("/opt/xiaoban/.venv/bin/activate")
    restore_idx = text.index('cd "$_xiaoban_orig_cwd"')
    exec_idx = text.index("if [ $# -eq 0 ]")
    assert activate_idx < restore_idx < exec_idx, (
        "cd $_xiaoban_orig_cwd must appear after venv activation and "
...
def test_dashboard_run_resets_home_before_dropping_privileges() -> None:
    ...
    assert "exec s6-setuidgid xiaoban xiaoban dashboard" in text
...
def test_dashboard_run_does_not_derive_insecure_from_bind_host() -> None:
    ...
    ``XIAOBAN_DASHBOARD_HOST``. Doing so disables the OAuth auth gate on
    ...
    The opt-in is now explicit: ``XIAOBAN_DASHBOARD_INSECURE=1`` (truthy).
    ...
    assert '127.0.0.1|localhost' not in text, (
        ...
        "is the authority now — opt in via XIAOBAN_DASHBOARD_INSECURE instead."
    )
    ...
    assert "XIAOBAN_DASHBOARD_INSECURE" in text, (
        "Explicit XIAOBAN_DASHBOARD_INSECURE opt-in is missing."
    )
    # Truthy values aligned with the rest of the s6 scripts
    # (e.g. XIAOBAN_DASHBOARD).
    for truthy in ("1", "true", "TRUE", "True", "yes", "YES", "Yes"):
        assert truthy in text, (
            f"XIAOBAN_DASHBOARD_INSECURE should accept truthy value {truthy!r}"
        )
...
def test_stage2_hook_repairs_profiles_and_cron_ownership_on_every_boot() -> None:
    ...
    assert 'if [ -d "$XIAOBAN_HOME/profiles" ]; then' in text
    assert 'chown -R xiaoban:xiaoban "$XIAOBAN_HOME/profiles" 2>/dev/null || true' in text

    assert 'if [ -d "$XIAOBAN_HOME/cron" ]; then' in text
    assert 'chown -R xiaoban:xiaoban "$XIAOBAN_HOME/cron" 2>/dev/null || true' in text
