...
const XIAOBAN_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type XiaobanRefType = (typeof XIAOBAN_REF_TYPES)[number]
...
const ICON_PATHS: Record<XiaobanRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as XiaobanRefType] ?? ICON_FALLBACK
...
const XIAOBAN_DIRECTIVE_RE = new RegExp(
...
export const xiaobanDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(XIAOBAN_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as XiaobanRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: XiaobanRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...xiaobanDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Xiaoban directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.xiaobanDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.xiaobanDesktop?.readFileDataUrl(id)
