...
///
/// `xiaoban_home_override` propagates to the child as $XIAOBAN_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, xiaoban_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = xiaoban_home_override {
        cmd.env("XIAOBAN_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, xiaoban_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = xiaoban_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_xiaoban_home() {
