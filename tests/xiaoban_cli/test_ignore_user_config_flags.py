"""Tests for --ignore-user-config and --ignore-rules flags on `xiaoban chat`.
...
files. In Xiaoban the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.xiaoban/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → XiaobanCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("XIAOBAN_IGNORE_USER_CONFIG", "XIAOBAN_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``XIAOBAN_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<xiaoban_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._xiaoban_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_xiaoban_home", tmp_path)
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("XIAOBAN_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``XiaobanCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting XIAOBAN_IGNORE_RULES=1 flips XiaobanCLI.ignore_rules True."""
        monkeypatch.setenv("XIAOBAN_IGNORE_RULES", "1")

        # Import XiaobanCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of XiaobanCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.XiaobanCLI)
        # Replicate the exact logic from cli.py XiaobanCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("XIAOBAN_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True

    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("XIAOBAN_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.XiaobanCLI)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``xiaoban_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("XIAOBAN_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("XIAOBAN_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("XIAOBAN_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("XIAOBAN_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("XIAOBAN_IGNORE_USER_CONFIG") == "1"
        assert "XIAOBAN_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "XIAOBAN_IGNORE_USER_CONFIG" not in os.environ
        assert "XIAOBAN_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real xiaoban parser accepts both flags."""
        ...
        import inspect
        import xiaoban_cli.main as hm
        src = inspect.getsource(hm)
        assert "XIAOBAN_IGNORE_USER_CONFIG" in src
        assert "XIAOBAN_IGNORE_RULES" in src
