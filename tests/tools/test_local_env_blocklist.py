...
Verifies that Xiaoban-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed Xiaoban secrets.

See: https://github.com/52707407SXG/Xiaoban_Agent/issues/1002
See: https://github.com/52707407SXG/Xiaoban_Agent/issues/1264
"""
...
from tools.environments.local import (
    LocalEnvironment,
    _XIAOBAN_PROVIDER_ENV_BLOCKLIST,
    _XIAOBAN_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.xiaoban/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a Xiaoban inference secret
        ...
        models`` run inside a Xiaoban terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _XIAOBAN_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        Xiaoban-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "XIAOBAN_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _XIAOBAN_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_XIAOBAN_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_XIAOBAN_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_XIAOBAN_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_XIAOBAN_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_XIAOBAN_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                assert var in _XIAOBAN_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _XIAOBAN_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Xiaoban-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _XIAOBAN_PROVIDER_ENV_BLOCKLIST
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to Xiaoban, and blocklisting
        ...
        leaked_block = general_chain & _XIAOBAN_PROVIDER_ENV_BLOCKLIST
    ...
    def test_extra_auth_vars_covered(self):
        ...
        assert extras.issubset(_XIAOBAN_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        for name, metadata in OPTIONAL_ENV_VARS.items():
            ...
            if category in {"tool", "messaging"}:
                assert name in _XIAOBAN_PROVIDER_ENV_BLOCKLIST, (
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_xiaoban_bin_injection(self):
        """These tests assert the sane-path merge in isolation. Disable the
        xiaoban-install-dir prepend (a separate concern, covered by
        TestXiaobanBinDirOnPath) so a real ``xiaoban`` on the test runner's PATH
        doesn't shift the asserted PATH layout."""
        from tools.environments import local as local_mod
        saved = local_mod._XIAOBAN_BIN_DIR
        local_mod._XIAOBAN_BIN_DIR = None  # resolved -> no dir to inject
        yield
        local_mod._XIAOBAN_BIN_DIR = saved
...
class TestXiaobanBinDirOnPath:
    """The xiaoban install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``xiaoban`` via the terminal tool must work
    even when the gateway was launched without the xiaoban install dir on
    PATH (systemd, service managers, cron). See the discussion that motivated
    _resolve_xiaoban_bin_dir / _prepend_xiaoban_bin_dir.
    """

    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._XIAOBAN_BIN_DIR = local_mod._SENTINEL

    def test_resolves_via_which(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/xiaoban/bin/xiaoban" if name == "xiaoban" else None)
        monkeypatch.setattr(local_mod.os.path, "isdir", lambda p: p == "/opt/xiaoban/bin")
        assert local_mod._resolve_xiaoban_bin_dir() == "/opt/xiaoban/bin"
    ...
    def test_resolves_via_sys_executable_dir(self, monkeypatch, tmp_path):
        ...
        (venv_bin / "xiaoban").write_text("#!/bin/sh\n")
        ...
        assert local_mod._resolve_xiaoban_bin_dir() == str(venv_bin)
    ...
    def test_returns_none_when_unresolvable(self, monkeypatch):
        ...
        assert local_mod._resolve_xiaoban_bin_dir() is None

    def test_prepend_adds_missing_dir_at_front(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._XIAOBAN_BIN_DIR = "/opt/xiaoban/bin"
        out = local_mod._prepend_xiaoban_bin_dir("/usr/bin:/bin")
        assert out.split(os.pathsep)[0] == "/opt/xiaoban/bin"
    ...
    def test_prepend_is_idempotent(self, monkeypatch):
        ...
        self._reset_cache()
        local_mod._XIAOBAN_BIN_DIR = "/opt/xiaoban/bin"
        once = local_mod._prepend_xiaoban_bin_dir("/usr/bin:/bin")
        twice = local_mod._prepend_xiaoban_bin_dir(once)
        assert twice == once
        assert once.split(os.pathsep).count("/opt/xiaoban/bin") == 1

    def test_prepend_noop_when_unresolved(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._XIAOBAN_BIN_DIR = None
        assert local_mod._prepend_xiaoban_bin_dir("/usr/bin:/bin") == "/usr/bin:/bin"

    def test_make_run_env_injects_xiaoban_bin_dir(self, monkeypatch):
        """A gateway env missing the xiaoban dir gets it back in the subshell PATH."""
        from tools.environments import local as local_mod
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._XIAOBAN_BIN_DIR = "/opt/xiaoban/bin"
        ...
        assert entries[0] == "/opt/xiaoban/bin"
