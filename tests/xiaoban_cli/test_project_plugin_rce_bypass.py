...
Execution via the ``XIAOBAN_ENABLE_PROJECT_PLUGINS`` bypass in the web
server's dashboard plugin loader.

Two primitives combined into the original advisory chain:

1. ``xiaoban_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.xiaoban/plugins/`` source via
   ``os.environ.get("XIAOBAN_ENABLE_PROJECT_PLUGINS")`` — truthy for
...
2. ``xiaoban_cli.web_server._mount_plugin_api_routes`` then imported
...
# ---------------------------------------------------------------------------
# Layer 1 — XIAOBAN_ENABLE_PROJECT_PLUGINS env gate uses truthy semantics.
# ---------------------------------------------------------------------------
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.xiaoban/plugins``
        and isolate the user-plugins dir to an empty tmp tree."""
        monkeypatch.setenv("XIAOBAN_HOME", str(tmp_path / "home"))
        (tmp_path / "home").mkdir()
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            monkeypatch.delenv("XIAOBAN_ENABLE_PROJECT_PLUGINS", raising=False)
        else:
            monkeypatch.setenv("XIAOBAN_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"XIAOBAN_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
...
class TestDiscoveryScrubsApiField:
    ...
    def test_project_safe_api_path_is_scrubbed(self, tmp_path, monkeypatch):
        monkeypatch.setenv("XIAOBAN_HOME", str(tmp_path / "home"))
        (tmp_path / "home").mkdir()
        monkeypatch.setenv("XIAOBAN_ENABLE_PROJECT_PLUGINS", "1")
    ...
    def test_bundled_safe_api_path_survives(self, tmp_path, monkeypatch):
        ...
        xiaoban_home.mkdir()
        monkeypatch.setenv("XIAOBAN_BUNDLED_PLUGINS", str(tmp_path / "bundled"))
        dashboard = _write_plugin_manifest(
            tmp_path / "bundled",
            "safe-bundled",
            {
...
class TestEndToEndPocBlocked:
    ...
    ``XIAOBAN_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XIAOBAN_ENABLE_PROJECT_PLUGINS", "0")
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "xiaoban_dashboard_plugin_evil"
            assert target != payload_py
            assert "evil-repo" not in target.parts
        assert "xiaoban_dashboard_plugin_evil" not in sys.modules
