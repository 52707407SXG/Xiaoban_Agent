"""``xiaoban setup`` subcommand parser.
...
def build_setup_parser(subparsers, *, cmd_setup: Callable) -> None:
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Xiaoban with an interactive wizard. "
        "Run a specific section: xiaoban setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'xiaoban setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
