...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``xiaoban tools`` repaint, tool-registration scans,
    ...
    2. ``~/.xiaoban/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_xiaoban_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.xiaoban/.env`` first, then ``os.environ``.

    Wraps :func:`xiaoban_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from xiaoban_cli.config import get_env_value as _xiaoban_get_env_value

        value = _xiaoban_get_env_value(name)
...
def xiaoban_xai_user_agent() -> str:
    """Return a stable Xiaoban-specific User-Agent for xAI HTTP calls."""
    try:
        from xiaoban_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Xiaoban-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Xiaoban-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``xiaoban_cli.config.get_env_value`` so keys
    stored in ``~/.xiaoban/.env`` (the standard Xiaoban location) are honored —
