...
Covers:
 - Normal path (tokens already in Xiaoban auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Xiaoban
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `xiaoban auth openai-codex`. The old
...
@pytest.fixture()
def xiaoban_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Xiaoban auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(xiaoban_auth_only_env):
    """openai-codex appears when tokens are already in Xiaoban auth store."""
...
def test_codex_picker_uses_live_codex_catalog(xiaoban_auth_only_env, tmp_path, monkeypatch):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Xiaoban
    auth store."""
    xiaoban_home = tmp_path / ".xiaoban"
    xiaoban_home.mkdir()
