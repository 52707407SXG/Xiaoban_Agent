...
The provider list shown by ``xiaoban model`` (CLI/TUI) and the desktop Settings
→ Providers tabs (Accounts + API keys) **must be the same set**.  Historically
they were not: the CLI picker read :data:`xiaoban_cli.models.CANONICAL_PROVIDERS`
...
the same universe ``xiaoban model`` renders (``CANONICAL_PROVIDERS``), joining:

* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`xiaoban_cli.auth.PROVIDER_REGISTRY` (credential truth), and
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``xiaoban model`` shows.
"""
...
# xiaoban_cli.auth.PROVIDER_REGISTRY and providers.base.ProviderProfile.
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `xiaoban model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``xiaoban model`` universe.
