...
class TestXAIProviderIsAvailable:
    ...
    auth-store lock. It runs on every ``xiaoban tools`` repaint and at
    ...
    def test_available_via_auth_store(self, monkeypatch, tmp_path):
        """Cheap probe should detect xai-oauth tokens in ~/.xiaoban/auth.json
...
class TestXAIBackendWiring:
    ...
    def test_is_backend_available_does_not_call_resolver(self, monkeypatch):
        ...
        dispatch and every `xiaoban tools` repaint. It must not invoke the
    ...
    def test_xai_not_in_legacy_backend_candidate_chain(self, monkeypatch):
        ...
        chain is for users who set env vars but never ran ``xiaoban tools``,
...
class TestXAIProviderOAuthPath:
    ...
    runtime resolver (``xiaoban auth`` sign-in) rather than an env-var key.
    Patches at the ``xiaoban_cli.runtime_provider.resolve_runtime_provider``
