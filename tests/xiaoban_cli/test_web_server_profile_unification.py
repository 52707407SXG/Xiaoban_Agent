...
profile switcher can target any profile's XIAOBAN_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via XIAOBAN_HOME.
"""
import pytest
import yaml
...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_xiaoban_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_xiaoban_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_probe_runs_inside_profile_scope(
        ...
        import xiaoban_cli.mcp_config as mcp_config
        from xiaoban_constants import get_xiaoban_home
        ...
        def fake_probe(name, config, connect_timeout=30):
            seen["home"] = str(get_xiaoban_home())
            return [("tool-a", "desc")]
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same XIAOBAN_HOME the rest
        ...
        monkeypatch.setattr(
            web_server,
            "_spawn_xiaoban_action",
            lambda subcommand, name: calls.append(list(subcommand)) or _FakeProc(),
        )
        monkeypatch.setattr(
            "xiaoban_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid():
            seen_homes.append(str(get_xiaoban_home()))
            return None
        ...
        assert resp.json()["xiaoban_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_xiaoban_home(self, isolated_profiles, monkeypatch):
        import xiaoban_cli.web_server as web_server

        monkeypatch.setattr(
            "xiaoban_cli.main._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = web_server._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["XIAOBAN_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "XIAOBAN_TUI_GATEWAY_URL" not in env
    ...
    def test_chat_argv_unscoped_keeps_legacy_env(self, isolated_profiles, monkeypatch):
        ...
        assert env is not None
        assert env.get("XIAOBAN_HOME") != str(isolated_profiles["worker_beta"])
