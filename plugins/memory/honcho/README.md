...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/xiaoban>
...
```bash
xiaoban memory setup honcho   # configure Honcho directly (works on a fresh install)
xiaoban memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
xiaoban config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.xiaoban/.env
```

> `xiaoban honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `xiaoban memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$XIAOBAN_HOME/honcho.json` | Profile-local (isolated Xiaoban instances) |
| 2 | `~/.xiaoban/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Xiaoban profile: `xiaoban` (default) or `xiaoban_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `xiaoban honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Xiaoban session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`xiaoban-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`xiaoban`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-xiaoban-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening xiaoban in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Xiaoban session ID (timestamp + hex). Every `xiaoban` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Xiaoban profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`xiaoban`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Xiaoban profile: `xiaoban` (default) or `xiaoban_<profile>` (e.g. `xiaoban -p coder` -> host key `xiaoban_coder`). Older `xiaoban.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `XIAOBAN_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `xiaoban-agent`) |
...
|---------|-------------|
| `xiaoban memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `xiaoban honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `xiaoban memory setup`) |
| `xiaoban honcho status` | Show resolved config for active profile |
| `xiaoban honcho enable` / `disable` | Toggle Honcho for active profile |
| `xiaoban honcho mode <mode>` | Change recall or observation mode |
| `xiaoban honcho peer --user <name>` | Update user peer name |
| `xiaoban honcho peer --ai <name>` | Update AI peer name |
| `xiaoban honcho tokens --context <N>` | Set context token budget |
| `xiaoban honcho tokens --dialectic <N>` | Set dialectic max chars |
| `xiaoban honcho map <name>` | Map current directory to a session name |
| `xiaoban honcho sync` | Create host blocks for all Xiaoban profiles |
