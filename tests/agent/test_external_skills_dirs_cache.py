...
@pytest.fixture
def xiaoban_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.xiaoban/`` with a config.yaml referencing one external dir."""
...
def test_returns_configured_external_dir(xiaoban_home_with_config):
    _home, external, _cfg = xiaoban_home_with_config
    result = get_external_skills_dirs()
    assert result == [external.resolve()]


def test_cache_reuses_result_without_reparsing(xiaoban_home_with_config):
    """Subsequent calls hit the cache and skip YAML parsing entirely."""
    _home, _external, _cfg = xiaoban_home_with_config
...
def test_cache_invalidates_on_mtime_change(xiaoban_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = xiaoban_home_with_config
...
def test_returned_list_is_a_copy(xiaoban_home_with_config):
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    ...
    home_b = tmp_path / "home_b" / ".xiaoban"
