...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_xiaoban_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Xiaoban does not currently need to route them into the agent loop.
            ...
            # /model work directly without /xiaoban prefix). A single regex
            ...
            # manifest (see `xiaoban slack manifest`). In Socket Mode, Slack
            ...
            from xiaoban_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/xiaoban$")

            @self._app.command(_slash_pattern)
            async def handle_xiaoban_command(ack, command):
            ...
            for _action_id in (
                "xiaoban_approve_once",
                "xiaoban_approve_session",
                "xiaoban_approve_always",
                "xiaoban_deny",
            ):
            ...
            for _action_id in (
                "xiaoban_confirm_once",
                "xiaoban_confirm_always",
                "xiaoban_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Xiaoban handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Xiaoban session — matching the per-thread behavior channels already
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                from xiaoban_cli.commands import is_gateway_known_command

                first_token = original_text[1:].split(maxsplit=1)[0]
                # Strip "@suffix" the same way get_command() does, so
                # forms like ``!stop@xiaoban`` still resolve.
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "xiaoban_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "xiaoban_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "xiaoban_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "xiaoban_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "xiaoban_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "xiaoban_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "xiaoban_confirm_once": "once",
            "xiaoban_confirm_always": "always",
            "xiaoban_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "xiaoban_approve_once": "once",
            "xiaoban_approve_session": "session",
            "xiaoban_approve_always": "always",
            "xiaoban_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/xiaoban <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/xiaoban
        ...
        if slash_name in {"xiaoban", ""}:
            # Legacy /xiaoban <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from xiaoban_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/xiaoban   ``).
        ...
        # "Running /cmd…" ack shown by handle_xiaoban_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/xiaoban <question>" must produce public replies so
...
# ``xiaoban_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``xiaoban_cli/setup.py::_setup_slack``.
    """
    from pathlib import Path
    from xiaoban_cli.config import get_env_value, save_env_value
    from xiaoban_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under XIAOBAN_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from xiaoban_cli.slack_cli import _build_full_manifest
            from xiaoban_constants import get_xiaoban_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Xiaoban",
                bot_description="Your Xiaoban agent on Slack",
            )
            target = Path(get_xiaoban_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `xiaoban slack manifest --write` anytime to refresh after "
                "Xiaoban adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `xiaoban update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://github.com/52707407SXG/Xiaoban_Agent/tree/main/website/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``xiaoban_cli.gateway.get_env_value`` at call time (not via the
    ...
    """
    import xiaoban_cli.gateway as gateway_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'xiaoban-agent[slack]'",
        # Interactive setup wizard — replaces xiaoban_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in xiaoban_cli/gateway.py.
