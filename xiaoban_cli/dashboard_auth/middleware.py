...
from xiaoban_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from xiaoban_cli.dashboard_auth.cookies import read_session_cookies
from xiaoban_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /xiaoban``, the
    ``login_url`` is prefixed (``/xiaoban/login?next=...``) so the
    ...
    """
    from xiaoban_cli.dashboard_auth.prefix import prefix_from_request
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["xiaoban.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``xiaoban_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from xiaoban_cli.dashboard_auth.cookies import clear_session_cookies
        from xiaoban_cli.dashboard_auth.prefix import prefix_from_request
