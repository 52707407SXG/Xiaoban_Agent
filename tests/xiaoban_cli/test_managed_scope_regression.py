...
def xiaoban_home(tmp_path, monkeypatch):
    ...
    # /etc/xiaoban on the dev/CI box can't influence the test.
    monkeypatch.setenv("XIAOBAN_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(xiaoban_home, monkeypatch):
    from xiaoban_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(xiaoban_home, monkeypatch):
    from xiaoban_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(xiaoban_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_xiaoban_dotenv(xiaoban_home=str(home))
