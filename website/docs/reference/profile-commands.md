...
This page covers all commands related to [Xiaoban profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `xiaoban profile` without a subcommand shows help.
...
```bash
xiaoban profile use <name>
```

Sets `<name>` as the active profile. All subsequent `xiaoban` commands (without `-p`) will use this profile.
...
xiaoban profile use default
```

## `xiaoban profile create`

```bash
xiaoban profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `xiaoban profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `xiaoban update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.xiaoban`), use `xiaoban skills opt-out` / `xiaoban skills opt-in`. |
...
xiaoban profile create work2 --clone-from work

# Clone everything from a specific profile
xiaoban profile create work2-backup --clone-from work --clone-all
```

## `xiaoban profile describe`

```bash
xiaoban profile describe [<name>] [options]
```
...
xiaoban profile describe researcher

# Set it explicitly
xiaoban profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
xiaoban profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
xiaoban profile describe --all --auto
```

## `xiaoban profile delete`

```bash
xiaoban profile delete <name> [options]
```
...
```bash
xiaoban profile delete mybot
xiaoban profile delete mybot --yes
```
...
## `xiaoban profile show`

```bash
xiaoban profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Xiaoban home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ xiaoban profile show work
Profile: work
Path:    ~/.xiaoban/profiles/work
...
## `xiaoban profile alias`

```bash
xiaoban profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Xiaoban installation.
...
```bash
xiaoban profile alias work
# Creates/updates ~/.local/bin/work

xiaoban profile alias work --name mywork
# Creates ~/.local/bin/mywork

xiaoban profile alias work --remove
# Removes the wrapper script
```

## `xiaoban profile rename`

```bash
xiaoban profile rename <old-name> <new-name>
```
...
```bash
xiaoban profile rename mybot assistant
# ~/.xiaoban/profiles/mybot → ~/.xiaoban/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `xiaoban profile export`

```bash
xiaoban profile export <name> [options]
```
...
```bash
xiaoban profile export work
# Creates work.tar.gz in the current directory

xiaoban profile export work -o ./work-2026-03-29.tar.gz
```

## `xiaoban profile import`

```bash
xiaoban profile import <archive> [options]
```
...
```bash
xiaoban profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

xiaoban profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`xiaoban profile export` / `import` are still the right commands for
...
### `xiaoban profile install`

```bash
xiaoban profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `xiaoban -p telemetry`). |
...
xiaoban profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
xiaoban profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
xiaoban profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
xiaoban profile install ./telemetry/
```

### `xiaoban profile update`

```bash
xiaoban profile update <name> [--force-config] [--yes]
```
...
### `xiaoban profile info`

```bash
xiaoban profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Xiaoban version, author, env var requirements, the source URL/path, and
...
`xiaoban profile list` also shows the distribution name and version in a
`Distribution` column, and `xiaoban profile show <name>` / `delete <name>`
...
xiaoban profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
xiaoban profile install https://github.com/your-org/internal-assistant.git
```
...
`xiaoban_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Xiaoban version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Xiaoban can clone from.
3. Tell recipients to run `xiaoban profile install <your-repo-url>`.
...
## `xiaoban -p` / `xiaoban --profile`

```bash
xiaoban -p <name> <command> [options]
xiaoban --profile <name> <command> [options]
```

Global flag to run any Xiaoban command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
xiaoban -p work chat -q "Check the server status"
xiaoban --profile dev gateway start
xiaoban -p personal skills list
xiaoban -p work config edit
```

## `xiaoban completion`

```bash
xiaoban completion <shell>
```
...
- `xiaoban profile <TAB>` — subcommands (list, use, create, etc.)
- `xiaoban profile use <TAB>` — profile names
- `xiaoban -p <TAB>` — profile names
