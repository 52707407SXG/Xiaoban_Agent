...
description: "Chat with Xiaoban from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Xiaoban from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Xiaoban](/guides/use-voice-mode-with-xiaoban).
...
```mermaid
flowchart TB
    subgraph Gateway["Xiaoban Gateway"]
...
For group chats, hooks, and automation flows, Xiaoban supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Xiaoban keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Xiaoban does not hide failures just because the text resembles a silence token.
...
```bash
xiaoban gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
xiaoban gateway              # Run in foreground
xiaoban gateway setup        # Configure messaging platforms interactively
xiaoban gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo xiaoban gateway install --system   # Linux only: install a boot-time system service
xiaoban gateway start        # Start the default service
xiaoban gateway stop         # Stop the default service
xiaoban gateway status       # Check default service status
xiaoban gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Xiaoban-Agent to the latest version |
...
Configure per-platform overrides in `~/.xiaoban/gateway.json`:
...
xiaoban pairing list          # View pending + approved users
xiaoban pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Xiaoban appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.xiaoban/config.yaml`:
...
Off by default. When enabled, Xiaoban prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.xiaoban/config.yaml`:
...
```bash
XIAOBAN_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
xiaoban gateway install               # Install as user service
xiaoban gateway start                 # Start the service
xiaoban gateway stop                  # Stop the service
xiaoban gateway status                # Check status
journalctl --user -u xiaoban-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo xiaoban gateway install --system
...
A system service needs root for every restart — including the automatic gateway restart at the end of `xiaoban update`. When `xiaoban update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart xiaoban-gateway` command (it never blocks on an interactive password prompt).
...
```bash
xiaoban gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `xiaoban update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo xiaoban update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/xiaoban-gateway`:

```
xiaoban ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed xiaoban-gateway*, /usr/bin/systemctl --no-ask-password start xiaoban-gateway*, /usr/bin/systemctl --no-ask-password restart xiaoban-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Xiaoban will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Xiaoban installations on the same machine (with different `XIAOBAN_HOME` directories), each gets its own systemd service name. The default `~/.xiaoban` uses `xiaoban-gateway`; other installations use `xiaoban-gateway-<hash>`. The `xiaoban gateway` commands automatically target the correct service for your current `XIAOBAN_HOME`.
:::

### macOS (launchd)

```bash
xiaoban gateway install               # Install as launchd agent
xiaoban gateway start                 # Start the service
xiaoban gateway stop                  # Stop the service
xiaoban gateway status                # Check status
tail -f ~/.xiaoban/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.xiaoban.gateway.plist`. It includes three environment variables:
...
- **XIAOBAN_HOME** — scopes the gateway to your Xiaoban installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `xiaoban gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `XIAOBAN_HOME` directory gets its own launchd label. The default `~/.xiaoban` uses `ai.xiaoban.gateway`; other installations use `ai.xiaoban.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `xiaoban-cli` | Full access |
| Telegram | `xiaoban-telegram` | Full tools including terminal |
| Discord | `xiaoban-discord` | Full tools including terminal |
| WhatsApp | `xiaoban-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `xiaoban-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `xiaoban-slack` | Full tools including terminal |
| Google Chat | `xiaoban-google_chat` | Full tools including terminal |
| Signal | `xiaoban-signal` | Full tools including terminal |
| SMS | `xiaoban-sms` | Full tools including terminal |
| Email | `xiaoban-email` | Full tools including terminal |
| Home Assistant | `xiaoban-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `xiaoban-mattermost` | Full tools including terminal |
| Matrix | `xiaoban-matrix` | Full tools including terminal |
| DingTalk | `xiaoban-dingtalk` | Full tools including terminal |
| Feishu/Lark | `xiaoban-feishu` | Full tools including terminal |
| WeCom | `xiaoban-wecom` | Full tools including terminal |
| WeCom Callback | `xiaoban-wecom-callback` | Full tools including terminal |
| Weixin | `xiaoban-weixin` | Full tools including terminal |
| BlueBubbles | `xiaoban-bluebubbles` | Full tools including terminal |
| QQBot | `xiaoban-qqbot` | Full tools including terminal |
| Yuanbao | `xiaoban-yuanbao` | Full tools including terminal |
| Microsoft Teams | `xiaoban-teams` | Full tools including terminal |
| API Server | `xiaoban-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `xiaoban-webhook` | Full tools including terminal |
| Raft | `xiaoban-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.xiaoban/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
