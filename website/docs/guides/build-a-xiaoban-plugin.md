...
title: "Build a Xiaoban Plugin"
description: "Step-by-step guide to building a complete Xiaoban plugin with tools, hooks, data files, and skills"
---

# Build a Xiaoban Plugin

This guide walks through building a complete Xiaoban plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Xiaoban has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `XIAOBAN_LOCAL_STT_COMMAND` to a shell template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.xiaoban/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `xiaoban skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
mkdir -p ~/.xiaoban/plugins/calculator
cd ~/.xiaoban/plugins/calculator
```
...
This tells Xiaoban: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Xiaoban may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `xiaoban my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Xiaoban continues fine
...
If your plugin doesn't show up — or shows up but isn't loading — set `XIAOBAN_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
XIAOBAN_PLUGINS_DEBUG=1 xiaoban plugins list
```
...
The same logs are always written to `~/.xiaoban/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
xiaoban logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `xiaoban plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.xiaoban/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.xiaoban/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.xiaoban/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.xiaoban/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `xiaoban plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Xiaoban into installing arbitrary packages — only specs Xiaoban itself ships are eligible |
...
auditable in `~/.xiaoban/logs/agent.log`. Plugins load after built-in
...
- **The system prompt is Xiaoban's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `xiaoban <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for xiaoban my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: xiaoban my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for xiaoban my-plugin."""
...
After registration, users can run `xiaoban my-plugin status`, `xiaoban my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `xiaoban name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `xiaoban chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `XIAOBAN_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `xiaoban <subcommand>` (e.g. `xiaoban kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "xiaoban kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Xiaoban from a hook.
...
Xiaoban has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.xiaoban/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `xiaoban gateway status` without SDK instantiation.
...
Xiaoban also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Xiaoban without any Python plugin. Declare them in `~/.xiaoban/config.yaml`:
...
Xiaoban connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.xiaoban/hooks/<name>/`:

```yaml
# ~/.xiaoban/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.xiaoban/hooks/long-task-alert/handler.py
...
xiaoban skills search my-workflow --source myorg/skills-repo
xiaoban skills install myorg/skills-repo/my-workflow
```
...
For STT, point `XIAOBAN_LOCAL_STT_COMMAND` at a shell template. Supported placeholders: `{input_path}`, `{output_path}`, `{format}`, `{voice}`, `{model}`, `{speed}` (TTS); `{input_path}`, `{output_dir}`, `{language}`, `{model}` (STT). Any path-interacting CLI is automatically a plugin.
...
```bash
pip install xiaoban-plugin-calculator
# Plugin auto-discovered on next xiaoban startup
```
...
```python
# Wrong — will break if Xiaoban passes extra context
def handler(args):
    ...
