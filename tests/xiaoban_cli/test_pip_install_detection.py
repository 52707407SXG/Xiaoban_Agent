...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("xiaoban_cli.config.get_managed_system", return_value=None), \
         patch("xiaoban_cli.config.get_xiaoban_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    """When XIAOBAN_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("xiaoban_cli.config.get_managed_system", return_value="NixOS"), \
         patch("xiaoban_cli.config.get_xiaoban_home", return_value=tmp_path):
        from xiaoban_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    """The stamp next to the running code is authoritative over $XIAOBAN_HOME.

    Models a host git install whose $XIAOBAN_HOME is shared with (and stamped
    'docker' by) a co-located container. The code-scoped stamp must win so the
    host install is correctly identified as 'git' and 'xiaoban update' works.
    """
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    ...
    with patch("xiaoban_cli.config.get_managed_system", return_value=None), \
         patch("xiaoban_cli.config.get_xiaoban_home", return_value=home):
        from xiaoban_cli.config import detect_install_method
...
def test_home_docker_stamp_ignored_when_not_containerized(tmp_path):
    ...
    'docker' into the shared $XIAOBAN_HOME. With no code-scoped stamp, a host
    ...
    with patch("xiaoban_cli.config.get_managed_system", return_value=None), \
         patch("xiaoban_cli.config.get_xiaoban_home", return_value=home), \
         patch("xiaoban_cli.config._running_in_container", return_value=False):
        from xiaoban_cli.config import detect_install_method
        assert detect_install_method(project_root=code) == "git"
...
def test_home_docker_stamp_honored_inside_container(tmp_path):
    ...
    with patch("xiaoban_cli.config.get_managed_system", return_value=None), \
         patch("xiaoban_cli.config.get_xiaoban_home", return_value=home), \
         patch("xiaoban_cli.config._running_in_container", return_value=True):
        from xiaoban_cli.config import detect_install_method
...
def test_home_non_docker_stamp_still_honored_for_backcompat(tmp_path):
    ...
    install that historically stamped 'git'/'pip' into $XIAOBAN_HOME keeps
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    """stamp_install_method writes next to the code, not into $XIAOBAN_HOME."""
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    home.mkdir()
    with patch("xiaoban_cli.config.get_xiaoban_home", return_value=home):
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``xiaoban update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("xiaoban_cli.config.get_managed_system", return_value=None), \
         patch("xiaoban_cli.config.get_xiaoban_home", return_value=tmp_path), \
         patch("xiaoban_constants.is_container", return_value=True):
        from xiaoban_cli.config import detect_install_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    from xiaoban_cli import banner

    hh = tmp_path / ".xiaoban"
    hh.mkdir()
    (hh / ".install_method").write_text("pip\n")

    with patch("xiaoban_cli.config.get_xiaoban_home", return_value=hh), \
         patch("xiaoban_constants.get_xiaoban_home", return_value=hh):
        buf = io.StringIO()
