...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    xiaoban_state = ModuleType("xiaoban_state")
    xiaoban_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "xiaoban_state", xiaoban_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Xiaoban"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Xiaoban"
