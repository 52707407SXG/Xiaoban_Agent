...
Regression coverage for [#26923](https://github.com/52707407SXG/Xiaoban_Agent/issues/26923):
...
def test_xai_loopback_login_manual_paste_skips_http_server(monkeypatch):
    ...
    def _fake_prompt(_redirect_uri):
        # Xiaoban generates state internally; we won't know it ahead of
        # time, so capture the state Xiaoban baked into the authorize
...
def test_xai_wait_for_callback_accepts_ready_stdin_code(monkeypatch):
    """Users can paste the Grok Build code while Xiaoban is still waiting."""
    class _StubServer:
