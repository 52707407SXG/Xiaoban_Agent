...
Config: Uses the existing Honcho config chain:
  1. $XIAOBAN_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, xiaoban_home):
        """Write config to $XIAOBAN_HOME/honcho.json (Honcho SDK native format)."""
        import json
        import os
        from pathlib import Path
        config_path = Path(xiaoban_home) / "honcho.json"
        existing = {}
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            try:
                ...
                # behave as they did before. New setups via `xiaoban honcho setup`
    ...
    def _start_session_init_background(self, *, wait_timeout: float = 0.0) -> None:
        ...
        This keeps Xiaoban CLI/gateway startup responsive when Honcho is down,
        ...
        with self._init_lock:
            ...
            init_session_id = self._lazy_init_session_id or "xiaoban-default"
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        # Skip under per-session strategy: every Xiaoban run creates a fresh
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                mem_dir = str(get_xiaoban_home() / "memories")
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "xiaoban-default",
