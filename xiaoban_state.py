#!/usr/bin/env python3
"""
SQLite State Store for Xiaoban-Agent.
...
DEFAULT_DB_PATH = get_xiaoban_home() / "state.db"
...
# xiaoban_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``xiaoban_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see xiaoban_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple xiaoban processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``xiaoban_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `xiaoban update` to rebuild the venv with a "
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._xiaoban_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._xiaoban_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Xiaoban surface and jump
    ...
    def delete_session_if_empty(
        ...
        and ``xiaoban sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Xiaoban, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Xiaoban session.

        A Xiaoban session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Xiaoban session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``XIAOBAN_DISABLE_FTS_TRIGRAM`` or not yet created), so
