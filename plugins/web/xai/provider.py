...
``{title, url, description, position}`` rows every other Xiaoban web
provider produces.
...
prefers Xiaoban-managed xAI Grok OAuth (via ``xiaoban auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.xiaoban/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``xiaoban tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `xiaoban auth` to sign in with "
