...
## 5. GODMODE FAST — Xiaoban 4 405B (Zero Refusal)

**Model:** `nousresearch/xiaoban-4-405b`
**Strategy:** Xiaoban models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Xiaoban-Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.xiaoban/config.yaml`:
