...
description: "Durable SQLite-backed task board for coordinating multiple Xiaoban profiles"
---
...
Xiaoban Kanban is a durable task board, shared across all your Xiaoban profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.xiaoban/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.xiaoban/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `xiaoban kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `xiaoban kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/xiaoban-kanban-v1-spec.pdf` in the repository.
...
- **Workspace** — the directory a worker operates in. Three kinds:
  - `scratch` (default) — fresh tmp dir under `~/.xiaoban/kanban/workspaces/<id>/` (or `~/.xiaoban/kanban/boards/<slug>/workspaces/<id>/` on non-default boards). **Deleted when the task completes** — scratch is ephemeral by design, so the dir is wiped the moment the worker (or `xiaoban kanban complete <id>`) marks the task done. If you want to keep the worker's output, use `worktree:` or `dir:<path>` instead. The first time a scratch workspace is created on an install, the dispatcher logs a warning and emits a `tip_scratch_workspace` event on the task (visible via `xiaoban kanban show <id>`).
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `XIAOBAN_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
xiaoban kanban boards list

# Create a new board.
xiaoban kanban boards create atm10-server \
    --name "ATM10 Server" \
...
xiaoban kanban --board atm10-server list
xiaoban kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
xiaoban kanban boards switch atm10-server
xiaoban kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
xiaoban kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `XIAOBAN_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.xiaoban/kanban/current` — the slug persisted by `xiaoban kanban
   boards switch`.
4. `default`.
...
`xiaoban dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<xiaoban-home>/kanban/attachments/<task_id>/` for the default board, or
  `<xiaoban-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `XIAOBAN_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
xiaoban kanban create "research AI funding landscape" --assignee researcher

# 4. Watch activity live (you)
xiaoban kanban watch

# 5. See the board (you)
xiaoban kanban list
xiaoban kanban stats
```

When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `xiaoban kanban show t_abcd`.
...
Override the config flag at runtime via `XIAOBAN_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `xiaoban gateway
...
until one comes up — `xiaoban kanban create` warns about this at creation
time.

Running `xiaoban kanban daemon` as a separate process is **deprecated**;
...
xiaoban kanban create "nightly ops review" \
    --assignee ops \
...
```bash
xiaoban kanban complete t_abc t_def t_hij --result "batch wrap"
xiaoban kanban archive  t_abc t_def t_hij
xiaoban kanban unblock  t_abc t_def
xiaoban kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `xiaoban kanban`.** When the dispatcher spawns a worker it sets `XIAOBAN_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `xiaoban kanban` CLI.
...
kanban_show()                                     # no args — uses XIAOBAN_KANBAN_TASK
...
### Why tools instead of shelling to `xiaoban kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `xiaoban kanban complete` *inside* the container, where `xiaoban` isn't installed and `~/.xiaoban/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.xiaoban/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `xiaoban chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `XIAOBAN_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/xiaoban_cli/test_kanban_db.py -q"],
...
2. `cd $XIAOBAN_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
```bash
xiaoban kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

xiaoban kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `xiaoban skills list` to see what's available); there's no runtime install.
...
```bash
xiaoban kanban create "Translate the docs site to French" \
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `xiaoban kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `xiaoban kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `xiaoban profile create --description "..."`, `xiaoban profile describe <name> --text "..."`, `xiaoban profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
Config knobs (all under `kanban:` in `~/.xiaoban/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `xiaoban profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.xiaoban/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
The plugin uses the standard Xiaoban dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
xiaoban kanban init                                     # create kanban.db + print daemon hint
xiaoban kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
xiaoban kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
xiaoban kanban show <id> [--json]
xiaoban kanban assign <id> <profile>                    # or 'none' to unassign
xiaoban kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
xiaoban kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
xiaoban kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
xiaoban kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
xiaoban kanban diagnostics [--json]                     # board health snapshot (alias: diag)
xiaoban kanban link <parent_id> <child_id>
xiaoban kanban unlink <parent_id> <child_id>
xiaoban kanban claim <id> [--ttl SECONDS]
xiaoban kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
xiaoban kanban complete <id>... [--result "..."]
xiaoban kanban block <id> "<reason>" [--ids <id>...]
xiaoban kanban unblock <id>...
xiaoban kanban archive <id>...

xiaoban kanban tail <id>                                # follow a single task's event stream
xiaoban kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
xiaoban kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
xiaoban kanban runs <id> [--json]                       # attempt history (one row per run)
xiaoban kanban assignees [--json]                       # profiles on disk + per-assignee task counts
xiaoban kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
xiaoban kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `xiaoban gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
xiaoban kanban stats [--json]                           # per-status + per-assignee counts
xiaoban kanban log <id> [--tail BYTES]                  # worker log from ~/.xiaoban/kanban/logs/
xiaoban kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
xiaoban kanban notify-list [<id>] [--json]
xiaoban kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
xiaoban kanban context <id>                             # what a worker sees
xiaoban kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
xiaoban kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
xiaoban kanban create "nightly backup audit" \
...
Every `xiaoban kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `xiaoban chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `xiaoban_cli.kanban.run_slash()` entry point that reuses the `xiaoban kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `xiaoban kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.xiaoban/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`xiaoban kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `xiaoban kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/xiaoban-kanban-v1-spec.pdf`.
...
```bash
xiaoban kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$XIAOBAN_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
xiaoban kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
xiaoban kanban notify-list
xiaoban kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
xiaoban kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
xiaoban kanban runs t_abcd
...
**Bulk close caveat.** `xiaoban kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`xiaoban kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `xiaoban kanban heartbeat $TASK` to signal liveness during long operations. |
...
`xiaoban kanban tail <id>` shows these for a single task. `xiaoban kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.xiaoban/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/xiaoban-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
