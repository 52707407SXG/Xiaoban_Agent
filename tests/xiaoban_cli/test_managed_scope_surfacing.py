"""Surfacing tests — managed scope shown in `config show` and `xiaoban doctor`."""
import pytest
...
def test_config_show_flags_managed(homes, capsys):
    from xiaoban_cli.config import show_config
...
def test_doctor_reports_managed_scope(homes, capsys):
    ...
    from xiaoban_cli import doctor
...
def test_doctor_silent_with_no_managed_scope(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("XIAOBAN_MANAGED_DIR", str(tmp_path / "nope"))
    from xiaoban_cli import managed_scope, doctor
