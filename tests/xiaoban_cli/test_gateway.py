...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /xiaoban-gateway.service"`` and runs ``systemctl --user daemon-reload``
    # — both target the *real* user environment because the conftest only
    # sandboxes ``XIAOBAN_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``XIAOBAN_HOME`` value, which
...
def test_run_gateway_refuses_root_in_official_docker(monkeypatch, tmp_path, capsys):
    project_root = tmp_path / "opt" / "xiaoban"
    ...
    monkeypatch.delenv("XIAOBAN_ALLOW_ROOT_GATEWAY", raising=False)
    ...
    assert exc_info.value.code == 1
    out = capsys.readouterr().out
    assert "Refusing to run the Xiaoban gateway as root" in out
    assert "/opt/xiaoban/docker/entrypoint.sh" in out
...
def test_run_gateway_root_guard_has_escape_hatch(monkeypatch):
    ...
    monkeypatch.setattr(gateway.os, "geteuid", lambda: 0)
    monkeypatch.setattr(gateway, "_is_official_docker_checkout", lambda: True)
    monkeypatch.setenv("XIAOBAN_ALLOW_ROOT_GATEWAY", "1")
...
def _clear_supervisor_markers(monkeypatch):
    ...
    monkeypatch.delenv("XIAOBAN_S6_SUPERVISED_CHILD", raising=False)
...
def test_run_gateway_refuses_when_service_supervising(monkeypatch, capsys):
    ...
    assert "xiaoban gateway restart" in out
    assert "--force" in out
...
def test_run_gateway_refuses_existing_process_before_importing_gateway_run(monkeypatch, capsys):
    ...
    assert "xiaoban gateway run --replace" in out
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("xiaoban_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("xiaoban_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_running_under_gateway_supervisor_markers(monkeypatch):
    ...
    monkeypatch.setenv("XPC_SERVICE_NAME", "org.nousresearch.xiaoban.gateway")
...
def test_run_gateway_windows_foreground_keeps_ctrl_c_enabled(monkeypatch):
    ...
    monkeypatch.delenv("XIAOBAN_GATEWAY_DETACHED", raising=False)
...
def test_run_gateway_windows_detached_absorbs_console_controls(monkeypatch):
    ...
    monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
    monkeypatch.setattr(gateway.sys, "stdin", _TTY())
    monkeypatch.setenv("XIAOBAN_GATEWAY_DETACHED", "1")
    monkeypatch.setattr(gateway.signal, "signal", fake_signal)
    monkeypatch.setattr(gateway.asyncio, "run", lambda coro: True)
...
def test_gateway_restart_on_windows_without_service_uses_detached_backend(monkeypatch):
    ...
    A Telegram-hosted agent may run `xiaoban gateway restart` via the terminal
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "xiaoban-gateway.service"

    monkeypatch.setattr(gateway, "get_systemd_unit_path", lambda system=False: unit_path)
    # Synthetic unit with a non-temp home: the real generator bakes the
    # hermetic test XIAOBAN_HOME (a tmp dir), which the temp-home write
...
def test_systemd_install_can_skip_enable_on_startup(monkeypatch, tmp_path, capsys):
    ...
    # hermetic test XIAOBAN_HOME) stays out of the way.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
...
def test_systemd_install_system_scope_skips_linger_and_uses_systemctl(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "etc" / "systemd" / "system" / "xiaoban-gateway.service"
...
def test_conflicting_systemd_units_warning(monkeypatch, tmp_path, capsys):
    user_unit = tmp_path / "user" / "xiaoban-gateway.service"
    system_unit = tmp_path / "system" / "xiaoban-gateway.service"
    ...
    assert "xiaoban gateway uninstall" in out
...
def test_install_linux_gateway_from_setup_system_choice_without_root_prints_followup(monkeypatch, capsys):
    ...
    assert "sudo xiaoban gateway install --system --run-as-user alice" in out
    assert "sudo xiaoban gateway start --system" in out
...
def test_scan_gateway_pids_detects_windows_xiaoban_exe_case_variants(monkeypatch):
    monkeypatch.setattr(gateway, "is_windows", lambda: True)
    ...
    def fake_run(cmd, **kwargs):
        if cmd[:4] == ["wmic.exe", "process", "get", "ProcessId,CommandLine"]:
            return SimpleNamespace(
                returncode=0,
                stdout=(
                    "CommandLine=C:\\Program Files\\Xiaoban\\Xiaoban.EXE gateway run --replace\n"
                    "ProcessId=2468\n\n"
                ),
                stderr="",
            )
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "xiaoban_cli.gateway"
