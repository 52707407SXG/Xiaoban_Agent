...
| | |
|---|---|
| Source | Optional — install with `xiaoban skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Xiaoban-Agent |
| License | MIT |
...
import upstream classes into Xiaoban itself.
...
```bash
mkdir -p ~/.xiaoban/cache/darwinian-evolver && cd ~/.xiaoban/cache/darwinian-evolver
...
```bash
cd ~/.xiaoban/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.xiaoban/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.xiaoban/skills/research/darwinian-evolver
DE_DIR=~/.xiaoban/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Xiaoban core.
   Custom driver scripts under `~/.xiaoban/skills/...` are user-side and fine.
