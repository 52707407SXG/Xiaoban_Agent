...
2. **已配置 LLM 提供商** — 运行 `xiaoban model` 或在 `~/.hermes/.env` 中设置首选提供商的凭据
...
语音模式在**经典 CLI**（`xiaoban chat`）和 **TUI**（`hermes --tui`）中均可使用。两者行为完全一致 — 相同的斜杠命令、相同的 VAD（语音活动检测）静音检测、相同的流式 TTS、相同的幻觉过滤器。TUI 额外将崩溃诊断日志转发至 `~/.hermes/logs/`，以便在异常音频后端出现按键录音失败时提供完整堆栈跟踪，而非静默消失。
