...
| | |
|---|---|
| Source | Optional — install with `xiaoban skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Xiaoban or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Xiaoban. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.xiaoban/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.xiaoban/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Xiaoban integration, either:

- configure the server in `~/.xiaoban/config.yaml` using the `native-mcp` skill, or
...
### Xiaoban cannot see the deployed server

The server-building part may be correct while the Xiaoban config is not. Load the `native-mcp` skill and configure the server in `~/.xiaoban/config.yaml`, then restart Xiaoban.
