...
When running Xiaoban inside Docker, the default `network_mode: host` gives the
...
The Xiaoban [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ xiaoban-agent │   │ xiaoban-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   XIAOBAN_UID=$(id -u) XIAOBAN_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=xiaoban,xiaoban-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://xiaoban-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Xiaoban trust model and vulnerability reporting
