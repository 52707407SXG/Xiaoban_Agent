...
description: "How to build an image-generation backend plugin for Xiaoban-Agent"
---
...
The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `xiaoban tools`.
...
| `display_name` | — | `name.title()` | Label shown in `xiaoban tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `xiaoban tools` model picker |
