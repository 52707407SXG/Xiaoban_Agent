"""Tests for xiaoban_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from xiaoban_cli import relaunch as relaunch_mod


class TestResolveXiaobanBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/xiaoban"
        ...
        assert relaunch_mod.resolve_xiaoban_bin() == fake

    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        fake = tmp_path / "xiaoban"
        fake.write_text("#!/bin/sh\n")
        fake.chmod(0o755)
        monkeypatch.setattr(sys, "argv", [str(fake.name)])
        monkeypatch.chdir(tmp_path)
        # Ensure we don't accidentally match a real 'xiaoban' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_xiaoban_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/xiaoban" if name == "xiaoban" else None
        )
        assert relaunch_mod.resolve_xiaoban_bin() == "/usr/bin/xiaoban"

    def test_returns_none_when_unresolvable(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_xiaoban_bin() is None
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_xiaoban_bin", lambda: "/usr/bin/xiaoban")
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv[0] == "/usr/bin/xiaoban"

    def test_falls_back_to_python_module(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_xiaoban_bin", lambda: None)
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv == [sys.executable, "-m", "xiaoban_cli.main", "--resume", "abc"]
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/xiaoban", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/xiaoban", ["/usr/bin/xiaoban", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        xiaoban).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead."""
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_xiaoban_bin", lambda: r"C:\Users\test\xiaoban.exe")
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\xiaoban.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_xiaoban_bin", lambda: r"C:\xiaoban.exe")
    ...
    def test_windows_surfaces_oserror_with_help(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_xiaoban_bin", lambda: r"C:\missing.exe")
...
class TestResolveXiaobanBinWindowsPyGuard:
    """On Windows, resolve_xiaoban_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a xiaoban.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\xiaoban.exe" if name == "xiaoban" else None,
        )

        bin_path = relaunch_mod.resolve_xiaoban_bin()
        # Must NOT be the .py — must be the xiaoban.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\xiaoban.exe"
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script = tmp_path / "xiaoban"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_xiaoban_bin() == str(script)

    def test_windows_py_argv0_with_no_xiaoban_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND xiaoban.exe
        isn't on PATH, return None so the caller falls back to
        python -m xiaoban_cli.main."""
        script = tmp_path / "main.py"
        script.write_text("# stub")
