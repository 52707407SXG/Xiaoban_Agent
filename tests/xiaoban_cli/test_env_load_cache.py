...
The cache exists to keep `xiaoban tools` → "All Platforms" fast: every
...
def test_load_env_caches_on_repeat_calls():
    ...
    from xiaoban_cli.config import invalidate_env_cache, load_env
    ...
    try:
        with patch("xiaoban_cli.config.get_env_path", return_value=env_path):
            first = load_env()
...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from xiaoban_cli import config as config_mod
    from xiaoban_cli.config import invalidate_env_cache, load_env, save_env_value
    ...
    monkeypatch.setattr(config_mod, "ensure_xiaoban_home", lambda: None)
...
def test_load_env_handles_missing_file():
    ...
    nonexistent = Path(tempfile.gettempdir()) / "xiaoban-test-no-such-env-xyz123.env"
    nonexistent.unlink(missing_ok=True)

    try:
        with patch("xiaoban_cli.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}
