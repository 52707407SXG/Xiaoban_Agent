...
const remoteConn = (over: Partial<XiaobanConnection> = {}): XiaobanConnection =>
  ({ baseUrl: 'https://xiaoban-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as XiaobanConnection

const localConn = (over: Partial<XiaobanConnection> = {}): XiaobanConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as XiaobanConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<XiaobanConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { xiaobanDesktop: { getConnection } })
})
