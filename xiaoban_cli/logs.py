"""``xiaoban logs`` — view and filter Xiaoban log files.
...
Usage examples::
    ...
    xiaoban logs                    # last 50 lines of agent.log
    xiaoban logs -f                 # follow agent.log in real time
    xiaoban logs errors             # last 50 lines of errors.log
    xiaoban logs gateway -n 100    # last 100 lines of gateway.log
    xiaoban logs gui -f            # follow gui.log (dashboard/pty/ws)
    xiaoban logs desktop -f        # follow desktop.log (Electron app boot/backend)
    xiaoban logs --level WARNING    # only WARNING+ lines
    xiaoban logs --session abc123   # filter by session ID substring
    xiaoban logs --component tools  # only tool-related lines
    xiaoban logs --since 1h         # lines from the last hour
    xiaoban logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_xiaoban_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Xiaoban runs — try 'xiaoban chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_xiaoban_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_xiaoban_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_xiaoban_home()}/logs/")
        return

    print(f"Log files in {display_xiaoban_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'xiaoban chat' to generate logs)")
