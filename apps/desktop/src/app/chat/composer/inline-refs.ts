...
export const XIAOBAN_SESSION_MIME = 'application/x-xiaoban-session'
...
export function writeSessionDrag(transfer: DataTransfer, payload: SessionDragPayload) {
  transfer.setData(XIAOBAN_SESSION_MIME, JSON.stringify(payload))
  transfer.effectAllowed = 'copy'
}

export function dragHasSession(transfer: DataTransfer | null) {
  return Boolean(transfer) && Array.from(transfer!.types || []).includes(XIAOBAN_SESSION_MIME)
}

export function readSessionDrag(transfer: DataTransfer | null): null | SessionDragPayload {
  const raw = transfer?.getData(XIAOBAN_SESSION_MIME)
