...
# 5. Symlinks the 'xiaoban' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}小伴 Xiaoban-Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/xiaoban-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink xiaoban into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up xiaoban command..."

XIAOBAN_BIN="$SCRIPT_DIR/venv/bin/xiaoban"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$XIAOBAN_BIN" "$COMMAND_LINK_DIR/xiaoban"
echo -e "${GREEN}✓${NC} Symlinked xiaoban → $COMMAND_LINK_DISPLAY_DIR/xiaoban"
...
echo "Other commands:"
echo "  xiaoban status        # Check configuration"
if is_termux; then
    echo "  xiaoban gateway       # Run gateway in foreground"
else
    echo "  xiaoban gateway install # Install gateway service (messaging + cron)"
fi
echo "  xiaoban cron list     # View scheduled jobs"
echo "  xiaoban doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m xiaoban.cli setup
fi
