...
def isolated_home(monkeypatch):
    """Temp XIAOBAN_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="xiaoban_test_31179_")
    xiaoban_home = os.path.join(test_home, ".xiaoban")
    os.makedirs(xiaoban_home)
...
def _fresh_modules():
    """Drop cached xiaoban modules so each test reloads against current env."""
