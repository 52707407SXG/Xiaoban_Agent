...
(/opt/xiaoban/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable XIAOBAN_HOME location instead.
"""
import importlib
...
def test_writable_install_returns_install_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "xiaoban_constants.get_xiaoban_home", lambda: xiaoban_home
    )

    resolved = whatsapp_common.resolve_whatsapp_bridge_dir()
    assert resolved == install_bridge
    # Nothing mirrored into XIAOBAN_HOME.
    assert not (xiaoban_home / "scripts" / "whatsapp-bridge").exists()


def test_readonly_install_mirrors_to_xiaoban_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable XIAOBAN_HOME."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)
    ...
    expected = xiaoban_home / "scripts" / "whatsapp-bridge"
...
def test_readonly_install_reuses_existing_mirror(tmp_path, monkeypatch):
    """If the XIAOBAN_HOME mirror already exists, return it without re-copying."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)

    xiaoban_home = tmp_path / "xiaoban_home"
    mirror = xiaoban_home / "scripts" / "whatsapp-bridge"
