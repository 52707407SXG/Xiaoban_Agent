...
Lets the LLM write a Python script that calls Xiaoban tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `xiaoban_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `xiaoban_tools.py` with file-based RPC stubs
...
# available on every platform Xiaoban itself runs on.
...
# match a safe prefix, the operational XIAOBAN_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "XIAOBAN_" prefix was deliberately removed (#27303) — it leaked
# XIAOBAN_*-named config that lacks a secret substring (e.g. XIAOBAN_BASE_URL,
# XIAOBAN_KANBAN_DB, XIAOBAN_*_WEBHOOK).  The child only needs the few
# location/profile vars in _XIAOBAN_CHILD_ALLOWED below; XIAOBAN_RPC_SOCKET /
# XIAOBAN_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational XIAOBAN_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set xiaoban_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_XIAOBAN_CHILD_ALLOWED = frozenset({
    "XIAOBAN_HOME",
    "XIAOBAN_PROFILE",
    "XIAOBAN_CONFIG",
    "XIAOBAN_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational XIAOBAN_* vars (_XIAOBAN_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret XIAOBAN_* vars dropped by the tightened allowlist (#27303). The
    # broad "XIAOBAN_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like XIAOBAN_KANBAN_DB / XIAOBAN_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _XIAOBAN_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("XIAOBAN_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped XIAOBAN_* var.
            _dropped_xiaoban.append(k)
    if _dropped_xiaoban:
        logger.debug(
            "execute_code: dropped %d non-allowlisted XIAOBAN_* var(s) from the "
            ...
            len(_dropped_xiaoban),
            ", ".join(sorted(_dropped_xiaoban)),
        )
    return scrubbed
...
# ---------------------------------------------------------------------------
# xiaoban_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "web_extract": (
        "web_extract",
        "urls: list, use_llm_processing: bool = True",
        '"""Extract content from URLs. Returns dict with results list of {url, title, content, error}."""',
        '{"urls": urls, "use_llm_processing": use_llm_processing}',
    ),
    "read_file": (
        "read_file",
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Xiaoban-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Xiaoban-profile soft guard."""',
...
def generate_xiaoban_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the xiaoban_tools.py stub module.
...
"""Auto-generated Xiaoban tools RPC stubs."""
...
def _connect():
    ...
    XIAOBAN_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["XIAOBAN_RPC_SOCKET"]
...
"""Auto-generated Xiaoban tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("XIAOBAN_RPC_DIR") or os.path.join(tempfile.gettempdir(), "xiaoban_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated xiaoban_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/xiaoban_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_xiaoban_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/xiaoban_tools.py", tools_src)
        ...
        env_prefix = (
            f"XIAOBAN_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("XIAOBAN_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with xiaoban_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="xiaoban_sandbox_")
    ...
    # selector from XIAOBAN_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"xiaoban_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated xiaoban_tools module.
        ...
        tools_src = generate_xiaoban_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "xiaoban_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  XIAOBAN_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["XIAOBAN_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the xiaoban-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from xiaoban_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _xiaoban_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _xiaoban_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # XIAOBAN_TIMEZONE is an internal Xiaoban setting and must not leak
        # into child processes.
        _tz_name = os.getenv("XIAOBAN_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("XIAOBAN_TIMEZONE", None)

        from xiaoban_constants import apply_subprocess_home_env
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.xiaoban/.env')).
...
def _load_config() -> dict:
    ...
    ``xiaoban --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (xiaoban-agent's python). Reproducible and the
...
_TOOL_DOC_LINES = [
    ...
    ("web_extract",
     "  web_extract(urls: list[str], use_llm_processing: bool = True) -> dict\n"
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``xiaoban tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and xiaoban-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.xiaoban/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Xiaoban tools programmatically. "
        ...
        f"Available via `from xiaoban_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into xiaoban_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from xiaoban_tools import {import_str}` "
