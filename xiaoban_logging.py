"""Centralized logging setup for Xiaoban-Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.xiaoban/logs/`` (profile-aware via ``get_xiaoban_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``xiaoban_cli.web_server``,
    ``xiaoban_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Xiaoban (TUI, gateway, ``hy_memory`` server, MCP
...
from xiaoban_constants import get_config_path, get_xiaoban_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_xiaoban_session_injector", False):
    ...
    _session_record_factory._xiaoban_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``xiaoban logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``xiaoban logs --component gateway``.
    "gateway": ("gateway", "xiaoban_plugins", "plugins.platforms"),
...
def setup_logging(
    *,
    xiaoban_home: Optional[Path] = None,
    ...
    """Configure the Xiaoban logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    xiaoban_home
        Override for the Xiaoban home directory.  Falls back to
        ``get_xiaoban_home()`` (profile-aware).
    log_level
    ...
    home = xiaoban_home or get_xiaoban_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_xiaoban_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._xiaoban_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the xiaoban group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from xiaoban_cli.config import is_managed
