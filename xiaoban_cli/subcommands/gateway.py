"""``xiaoban gateway`` and ``xiaoban proxy`` subcommand parsers.
...
from xiaoban_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if XIAOBAN_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy xiaoban.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Xiaoban gateway unit files "
            "(e.g. xiaoban.service) left over from older installs. Profile "
            "units (xiaoban-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.xiaoban/.env. "
            "Requires being logged in (xiaoban setup). Not available in managed installs."
        ),
    )
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `xiaoban proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
