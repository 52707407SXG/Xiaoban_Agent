"""
Xiaoban Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``xiaoban desktop`` / ``xiaoban gui``)
     Built inside the agent checkout under ``$XIAOBAN_HOME/xiaoban-agent/``:
       ...
       - ``$XIAOBAN_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Xiaoban.app`` or ``~/Applications/Xiaoban.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Xiaoban`` (NSIS per-user)
...
the app name ("Xiaoban"), separate from ``$XIAOBAN_HOME``:
  - macOS:   ``~/Library/Application Support/Xiaoban``
  - Windows: ``%APPDATA%\\Xiaoban``
  - Linux:   ``$XDG_CONFIG_HOME/Xiaoban`` (default ``~/.config/Xiaoban``)
...
``xiaoban uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(xiaoban_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return xiaoban_home / "xiaoban-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Xiaoban"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Xiaoban"
...
def source_built_gui_artifacts(xiaoban_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``xiaoban desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``xiaoban-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(xiaoban_home)
    ...
    return [
        ...
        xiaoban_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Xiaoban" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Xiaoban.app"),
            home / "Applications" / "Xiaoban.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Xiaoban).
            local_base / "Programs" / "Xiaoban",
            # Older / alternate layout some builds used.
            local_base / "xiaoban-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Xiaoban")
    else:
        ...
        paths += [
            data_base / "applications" / "xiaoban.desktop",
            data_base / "applications" / "Xiaoban.desktop",
        ]
    return paths


def agent_is_installed(xiaoban_home: Path) -> bool:
    """Return True when a usable Python agent install exists under XIAOBAN_HOME.
    ...
    if (agent_root / "xiaoban_cli").is_dir():
        return True
...
def gui_is_installed(xiaoban_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(xiaoban_home):
        if p.exists():
            return True
...
def gui_install_summary(xiaoban_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = xiaoban_home if xiaoban_home is not None else get_xiaoban_home()
...
def uninstall_gui(xiaoban_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``xiaoban-agent/xiaoban_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$XIAOBAN_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove xiaoban' or "
            "'sudo dnf remove xiaoban'). AppImage builds are a single file you "
