# Xiaoban Observer Hooks

Xiaoban observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Xiaoban' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Xiaoban catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Xiaoban only builds
