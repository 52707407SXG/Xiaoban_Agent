...
sidebar_position: 4
title: "Slack"
description: "Set up Xiaoban-Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Xiaoban-Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Xiaoban instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Xiaoban uses the modern
...
The fastest path is to paste a manifest Xiaoban generates for you. It
...
### Option A: From a Xiaoban-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   xiaoban slack manifest --write
   ```
   This writes `~/.xiaoban/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "Xiaoban-Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Xiaoban can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `xiaoban-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Xiaoban configuration issue.
:::
...
Xiaoban uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
xiaoban gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
xiaoban gateway              # Foreground
xiaoban gateway install      # Install as a user service
sudo xiaoban gateway install --system   # Linux only: boot-time system service
```
...
Every Xiaoban command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Xiaoban command with its description.

Under the hood: Xiaoban ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/52707407SXG/Xiaoban_Agent/blob/main/xiaoban_cli/commands.py)
...
When Xiaoban adds new commands (e.g. after `xiaoban update`), regenerate
...
3. Paste the new contents of `~/.xiaoban/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/xiaoban <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/xiaoban btw run the tests` — Xiaoban routes it the same way as `/btw
run the tests`. Free-form questions also work: `/xiaoban what's the
...
Slack never delivers them to Xiaoban.

As a workaround, Xiaoban recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Xiaoban treats it identically to the slash form and replies in the same
thread.
...
Xiaoban falls back to a text prompt, the prompt instructs you to reply
...
```bash
xiaoban slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Xiaoban behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Xiaoban-Agent what time is it?`). In channels, Xiaoban replies in a thread attached to that message. |
| **Threads** | If you @mention Xiaoban inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.xiaoban/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Xiaoban in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Xiaoban will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Xiaoban will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Xiaoban-Agent`).

---

## Multi-Workspace Support

Xiaoban can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Xiaoban also loads tokens from an **OAuth token file** at:

```
~/.xiaoban/slack_tokens.json
```
...
- When a message arrives, Xiaoban uses the correct workspace-specific client to respond.
...
Xiaoban supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Xiaoban-Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Xiaoban-Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Xiaoban now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Xiaoban-Agent`)
...
- Tokens should be stored in `~/.xiaoban/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Xiaoban config directory
