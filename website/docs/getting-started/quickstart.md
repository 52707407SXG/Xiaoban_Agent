...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Xiaoban-Agent — from install to chatting in under 5 minutes"
---
...
The single most important setup step. Use `xiaoban model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `xiaoban setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `xiaoban update`. Re-enable anything later with `xiaoban tools`, seed skills with `xiaoban skills opt-in --sync`, or tune settings with `xiaoban setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `xiaoban model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `xiaoban model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `xiaoban model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `xiaoban model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `xiaoban model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `xiaoban model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `xiaoban model` (requires `copilot` CLI + `copilot login`) |
...
Xiaoban-Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `xiaoban model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::
...
```bash
xiaoban gateway setup    # Interactive platform configuration
```
...
- `xiaoban tools` — tune tool access per platform
- `xiaoban skills` — browse and install reusable workflows
...
```bash
xiaoban skills browse                      # list everything available
xiaoban skills search kubernetes           # find skills by keyword
xiaoban skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `xiaoban skills browse` shows the exact slugs to use.
