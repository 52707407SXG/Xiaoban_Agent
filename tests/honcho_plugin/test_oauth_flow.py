    ...
    def do_POST(self):  # noqa: N802
        ...
        if grant_type == "authorization_code":
            body["config"] = {
                ...
                "hosts": {"xiaoban": {"saveMessages": True, "recallMode": "hybrid"}},
            }
...
def fake_as(monkeypatch):
    ...
    monkeypatch.setenv("HONCHO_OAUTH_CLIENT_ID", "xiaoban-desktop")
    try:
        yield base
    finally:
        server.shutdown()
...
def test_full_loopback_flow_then_refresh(tmp_path, fake_as):
    ...
    assert saved["hosts"]["xiaoban"]["apiKey"] == "hch-at-1"
    assert saved["hosts"]["xiaoban"]["oauth"]["refreshToken"] == "hch-rt-1"
    assert saved["hosts"]["xiaoban"]["recallMode"] == "hybrid"
    ...
    token, refreshed = oauth.ensure_fresh_token(
        config_path, "xiaoban", now=saved["hosts"]["xiaoban"]["oauth"]["expiresAt"] + 10
    )
    assert refreshed is True
    assert token == "hch-at-2"
    rotated = json.loads(config_path.read_text())["hosts"]["xiaoban"]["oauth"]
...
def test_state_mismatch_is_rejected(fake_as, tmp_path):
    ...
    with pytest.raises(ValueError, match="unknown or expired"):
        oauth_flow.complete_authorization(
            ...
            config_path=tmp_path / "honcho.json", host="xiaoban",
        )


def test_source_tags_the_authorize_link(fake_as):
    endpoints = oauth_flow.resolve_endpoints()
    url, _ = oauth_flow.begin_authorization(endpoints, source="xiaoban-cli")
    assert "source=xiaoban-cli" in url
    untagged, _ = oauth_flow.begin_authorization(endpoints)
    assert "source=" not in untagged


def test_client_id_defaults_to_xiaoban_agent(monkeypatch):
    ...
    assert oauth_flow.resolve_endpoints(**common).client_id == "xiaoban-agent"
...
def test_grant_persists_default_client_id(tmp_path, fake_as, monkeypatch):
    ...
    # store client_id=xiaoban-agent so refresh reuses the right client.
    monkeypatch.delenv("HONCHO_OAUTH_CLIENT_ID", raising=False)
    config_path = tmp_path / "honcho.json"
    ...
    saved = json.loads(config_path.read_text())
    assert saved["hosts"]["xiaoban"]["oauth"]["clientId"] == "xiaoban-agent"


def test_config_path_rides_the_authorize_link(fake_as):
    endpoints = oauth_flow.resolve_endpoints()
    url, _ = oauth_flow.begin_authorization(endpoints, config_path="~/.xiaoban/honcho.json")
    q = parse_qs(urlparse(url).query)
    assert q["config_path"][0] == "~/.xiaoban/honcho.json"
...
def test_display_config_path_never_leaks_absolute_path():
    ...
    under_home = Path.home() / ".xiaoban" / "profiles" / "work" / "honcho.json"
    assert oauth_flow._display_config_path(under_home) == "~/.xiaoban/profiles/work/honcho.json"
...
def test_cli_flow_stores_tokens_without_applying_config(tmp_path, fake_as):
    ...
    config_path = tmp_path / "honcho.json"
    config_path.write_text(json.dumps({"hosts": {"xiaoban": {"saveMessages": False}}}))
...
def test_launcher_runs_flow_in_background_and_reports_connected(monkeypatch, reset_flow):
    ...
    st = oauth_flow.start_loopback_flow_background(config_path=Path("/t/honcho.json"), host="xiaoban")
    assert st["state"] == "pending"  # returns immediately, before the flow finishes
    assert _wait_until(lambda: seen.get("source") == "xiaoban-desktop")  # default source tag
    assert seen["host"] == "xiaoban"
    gate.set()
...
def test_launcher_reports_error_on_flow_failure(monkeypatch, reset_flow):
    ...
    oauth_flow.start_loopback_flow_background(config_path=Path("/t/honcho.json"), host="xiaoban")
...
def test_launcher_is_idempotent_while_pending(monkeypatch, reset_flow):
    ...
    s1 = oauth_flow.start_loopback_flow_background(config_path=Path("/t/h.json"), host="xiaoban")
    assert _wait_until(lambda: len(calls) == 1)  # first flow is running
    s2 = oauth_flow.start_loopback_flow_background(config_path=Path("/t/h.json"), host="xiaoban")
    block.set()
...
def test_get_flow_status_reports_stored_connection(tmp_path, monkeypatch, reset_flow):
    ...
    monkeypatch.setattr(honcho_client, "resolve_active_host", lambda: "xiaoban")
    monkeypatch.delenv("HONCHO_API_KEY", raising=False)

    cfgfile.write_text(json.dumps({"hosts": {"xiaoban": {}}}))
    assert oauth_flow.get_flow_status()["connected"] is False

    cfgfile.write_text(json.dumps({"hosts": {"xiaoban": {"apiKey": "hch-v3-static"}}}))
    ...
    cfgfile.write_text(json.dumps({"hosts": {"xiaoban": {
        ...
        "oauth": {"refreshToken": "hch-rt-x", "expiresAt": 9_999_999_999,
                  "clientId": "xiaoban-desktop", "tokenEndpoint": "http://x/oauth/token"},
    }}}))
    s = oauth_flow.get_flow_status()
...
def test_memory_oauth_router_dispatches_by_provider_convention():
    ...
    from xiaoban_cli.memory_oauth import _resolve_flow
