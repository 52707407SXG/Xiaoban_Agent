...
Jobs are stored in ~/.xiaoban/cron/jobs.json
Output is saved to ~/.xiaoban/cron/output/{job_id}/{timestamp}.md
"""
...
XIAOBAN_DIR = get_xiaoban_home().resolve()
CRON_DIR = XIAOBAN_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``xiaoban cron status`` process share it
...
# threshold in `xiaoban cron status` (xiaoban_cli/cron.py), so the two never
# drift apart.
...
def _jobs_lock():
    ...
    and standalone ``xiaoban`` CLI invocations, which previously shared no lock
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    try:
        ...
        run_at = _xiaoban_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Xiaoban configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Xiaoban timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _xiaoban_now().tzinfo
    if dt.tzinfo is None:
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Xiaoban/system local time
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall xiaoban-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `xiaoban cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    concurrent reader in another process (``xiaoban cron status``) never sees a
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `xiaoban cron status` can tell a thread that is merely *alive and looping*
...
def _save_jobs_unlocked(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _xiaoban_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.xiaoban/scripts/; ``.sh`` / ``.bash`` files run via bash,
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _xiaoban_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _xiaoban_now().isoformat(),
        },
    )
...
def _machine_id() -> str:
    ...
    Uses ``XIAOBAN_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("XIAOBAN_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _xiaoban_now().strftime("%Y-%m-%d_%H-%M-%S")
