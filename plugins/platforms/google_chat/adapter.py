...
# ``model_tools`` import time, so ``xiaoban status``, ``xiaoban chat``, etc.
...
# ``xiaoban_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Xiaoban is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a xiaoban MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${XIAOBAN_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _xiaoban_home = _get_xiaoban_home()
        except (ModuleNotFoundError, ImportError):
            _xiaoban_home = _Path.home() / ".xiaoban"
        self._thread_count_store = _ThreadCountStore(
            _xiaoban_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Xiaoban is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("XIAOBAN_HOME", str(_Path.home() / ".xiaoban"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${XIAOBAN_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a xiaoban MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Xiaoban is thinking…' marker message.
        ...
        "Xiaoban is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Xiaoban is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Xiaoban is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Xiaoban is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Xiaoban is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``xiaoban setup``.

    The setup wizard at ``xiaoban_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.xiaoban/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. xiaoban-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.xiaoban/.env")
    print_info("Restart the gateway: xiaoban gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``xiaoban cron`` running as a
    separate process from ``xiaoban gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Xiaoban plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'xiaoban-agent[google_chat]'",
        ...
        # the "Xiaoban is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Xiaoban is thinking…' marker message "
