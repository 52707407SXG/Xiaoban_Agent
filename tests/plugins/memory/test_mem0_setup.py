...
def _inject_fake_xiaoban_cli(monkeypatch):
    """Inject fake xiaoban_cli modules so yaml/curses aren't required."""
    fake_config_mod = types.ModuleType("xiaoban_cli.config")
    fake_config_mod.save_config = lambda c: None

    fake_setup_mod = types.ModuleType("xiaoban_cli.memory_setup")
    fake_setup_mod._curses_select = lambda *a, **kw: 0
    fake_setup_mod._prompt = lambda label, default=None, secret=False: default or ""

    fake_xiaoban_cli = types.ModuleType("xiaoban_cli")
    fake_xiaoban_cli.config = fake_config_mod
    fake_xiaoban_cli.memory_setup = fake_setup_mod

    monkeypatch.setitem(sys.modules, "xiaoban_cli", fake_xiaoban_cli)
    monkeypatch.setitem(sys.modules, "xiaoban_cli.config", fake_config_mod)
    monkeypatch.setitem(sys.modules, "xiaoban_cli.memory_setup", fake_setup_mod)
...
class TestPostSetup:
    ...
    def test_platform_flag_mode(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", ["xiaoban", "--mode", "platform", "--api-key", "sk-test"])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_xiaoban_home", lambda: tmp_path)
        _inject_fake_xiaoban_cli(monkeypatch)
        config = {"memory": {}}
    ...
    def test_oss_flag_mode(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", [
            "xiaoban", "--mode", "oss", "--oss-llm-key", "sk-oai",
        ])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_xiaoban_home", lambda: tmp_path)
        _inject_fake_xiaoban_cli(monkeypatch)
...
class TestDryRun:
    ...
    def test_dry_run_platform_no_files(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", ["xiaoban", "--mode", "platform", "--api-key", "sk-test", "--dry-run"])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_xiaoban_home", lambda: tmp_path)
        _inject_fake_xiaoban_cli(monkeypatch)
        config = {"memory": {}}
        post_setup(str(tmp_path), config)
    ...
    def test_dry_run_oss_no_files(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", [
            "xiaoban", "--mode", "oss", "--oss-llm-key", "sk-oai", "--dry-run",
        ])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_xiaoban_home", lambda: tmp_path)
        _inject_fake_xiaoban_cli(monkeypatch)
        monkeypatch.setattr("plugins.memory.mem0._setup._install_provider_deps", lambda l, e, v: None)
