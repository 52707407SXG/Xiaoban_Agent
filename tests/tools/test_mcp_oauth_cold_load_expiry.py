...
1. ``XiaobanTokenStorage.set_tokens`` persisted only relative ``expires_in``,
...
- ``XiaobanMCPOAuthProvider._initialize`` seeds ``context.token_expiry_time``
...
# ---------------------------------------------------------------------------
# XiaobanTokenStorage — absolute expiry persistence
# ---------------------------------------------------------------------------
...
class TestSetTokensAbsoluteExpiry:
    def test_set_tokens_persists_absolute_expires_at(self, tmp_path, monkeypatch):
        ...
        storage = XiaobanTokenStorage("srv")
        before = time.time()
        asyncio.run(
            storage.set_tokens(
                OAuthToken(
...
class TestGetTokensReconstructsExpiresIn:
    ...
    def test_get_tokens_returns_zero_ttl_for_expired_token(
        ...
        monkeypatch.setenv("XIAOBAN_HOME", str(tmp_path))
        from tools.mcp_oauth import XiaobanTokenStorage, _get_token_dir
...
# ---------------------------------------------------------------------------
# XiaobanMCPOAuthProvider._initialize — seed token_expiry_time
# ---------------------------------------------------------------------------
...
async def test_initialize_seeds_token_expiry_time_from_stored_tokens(
    ...
    from tools.mcp_oauth import XiaobanTokenStorage
    from tools.mcp_oauth_manager import _XIAOBAN_PROVIDER_CLS, reset_manager_for_tests

    assert _XIAOBAN_PROVIDER_CLS is not None
    ...
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Xiaoban-Agent",
    )
    provider = _XIAOBAN_PROVIDER_CLS(
        server_name="srv",
...
async def test_initialize_skips_prefetch_when_no_tokens(tmp_path, monkeypatch):
    ...
    storage = XiaobanTokenStorage("srv")  # empty — no tokens on disk
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Xiaoban-Agent",
    )
