...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._xiaoban_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(xiaoban_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(xiaoban_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_xiaoban_bin(self):
        """After _INSTALL_FAILED, manual install in XIAOBAN_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        xiaoban_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(xiaoban_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(xiaoban_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._xiaoban_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == xiaoban_bin
            assert _tirith_mod._resolved_path == xiaoban_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# XIAOBAN_HOME isolation
# ---------------------------------------------------------------------------

class TestXiaobanHomeIsolation:
    def test_xiaoban_bin_dir_respects_xiaoban_home(self):
        """_xiaoban_bin_dir must use XIAOBAN_HOME, not hardcoded ~/.xiaoban."""
        from tools.tirith_security import _xiaoban_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"XIAOBAN_HOME": tmpdir}):
            result = _xiaoban_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_xiaoban_home(self):
        """_failure_marker_path must use XIAOBAN_HOME, not hardcoded ~/.xiaoban."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"XIAOBAN_HOME": "/custom/xiaoban"}):
            result = _failure_marker_path()
        assert result == "/custom/xiaoban/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets XIAOBAN_HOME; verify it's active."""
        xiaoban_home = os.getenv("XIAOBAN_HOME")
        assert xiaoban_home is not None, "XIAOBAN_HOME should be set by conftest"
        assert "xiaoban_test" in xiaoban_home, "Should point to test temp dir"

    def test_get_xiaoban_home_fallback(self):
        """Without XIAOBAN_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_xiaoban_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove XIAOBAN_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".xiaoban")
            result = _get_xiaoban_home()
