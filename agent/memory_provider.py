...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `xiaoban backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - xiaoban_home (str): The active XIAOBAN_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.xiaoban``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "xiaoban").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'xiaoban memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], xiaoban_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'xiaoban memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``xiaoban_home`` is the active XIAOBAN_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE XIAOBAN_HOME.

        ``xiaoban backup`` only walks XIAOBAN_HOME, so any provider state kept
        ...
        of the archive, and ``xiaoban import`` restores them to their original
