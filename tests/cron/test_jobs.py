...
class TestComputeNextRun:
    ...
    def test_once_recent_past_within_grace_returns_time(self, monkeypatch):
        ...
        monkeypatch.setattr("cron.jobs._xiaoban_now", lambda: now)
...
class TestAdvanceNextRun:
    ...
    def test_advances_interval_job(self, tmp_cron_dir):
        ...
        updated = get_job(job["id"])
        from cron.jobs import _ensure_aware, _xiaoban_now
        new_next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert new_next_dt > _xiaoban_now(), "next_run_at should be in the future after advance"
    ...
    def test_already_future_stays_future(self, tmp_cron_dir):
        ...
        from cron.jobs import _ensure_aware, _xiaoban_now
        new_next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert new_next_dt > _xiaoban_now(), "next_run_at should remain in the future"
...
class TestGetDueJobs:
    ...
    def test_long_execution_does_not_perpetually_defer(self, tmp_cron_dir, monkeypatch):
        ...
        stale = (_xiaoban_now() - timedelta(minutes=11)).isoformat()
        jobs[0]["next_run_at"] = stale
        jobs[0]["last_run_at"] = (_xiaoban_now() - timedelta(minutes=1)).isoformat()
        save_jobs(jobs)
    ...
    def test_stale_repeat_limited_job_consumes_one_run_on_catchup(self, tmp_cron_dir, monkeypatch):
        ...
        from cron.jobs import _xiaoban_now
        job = create_job(prompt="Limited", schedule="every 5m", repeat=3)
        jobs = load_jobs()
        jobs[0]["next_run_at"] = (_xiaoban_now() - timedelta(minutes=11)).isoformat()
        jobs[0]["last_run_at"] = (_xiaoban_now() - timedelta(minutes=11)).isoformat()
        save_jobs(jobs)
    ...
    def test_cron_next_run_offset_migration_is_rescheduled_not_fired(self, tmp_cron_dir, monkeypatch):
        ...
        # A 21:00 cron was stored while Xiaoban/system local time was UTC+10.
