"""
Email platform adapter for the Xiaoban gateway.

Allows users to interact with Xiaoban by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "xiaoban-agent" "version" "{_xiaoban_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `xiaoban config show` already read. Without the
    ...
    async def connect(self) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `xiaoban gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Xiaoban-Agent")
        ...
        msg_id = f"<xiaoban-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
...
# xiaoban_cli/gateway.py, and the _send_email dispatch in
