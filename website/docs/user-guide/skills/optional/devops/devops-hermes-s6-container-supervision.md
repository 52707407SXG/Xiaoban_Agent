...
| | |
|---|---|
| Source | Optional — install with `xiaoban skills install official/devops/hermes-s6-container-supervision` |
...
│   └── dashboard/run                  ← if HERMES_DASHBOARD=1, runs `xiaoban dashboard`
│
...
| `hermes_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `xiaoban gateway start/stop/restart` and routes to s6 when running in a container. |
...
HERMES_TEST_IMAGE=xiaoban-agent-harness:latest scripts/run_tests.sh tests/docker/ -v
