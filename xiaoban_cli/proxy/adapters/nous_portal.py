...
Reads the user's Nous OAuth state from ``~/.xiaoban/auth.json`` through the
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `xiaoban auth add nous` first."
                )
            ...
            if not runtime_key:
                raise RuntimeError(
                    ...
                    "Try `xiaoban auth add nous` to re-authenticate."
                )
    ...
    # to xiaoban_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
