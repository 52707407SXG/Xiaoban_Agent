...
const { buildDesktopBackendEnv, normalizeXiaobanHomeRoot } = require('./backend-env.cjs')
...
const USER_DATA_OVERRIDE = process.env.XIAOBAN_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.XIAOBAN_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with XIAOBAN_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[xiaoban] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}

ipcMain.handle('xiaoban:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[xiaoban] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[xiaoban] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[xiaoban] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// XIAOBAN_HOME — the user-facing root for everything Xiaoban-related. Mirrors
// scripts/install.ps1's $XiaobanHome and scripts/install.sh's $XIAOBAN_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\xiaoban (matches install.ps1)
//   macOS / Linux: ~/.xiaoban (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.xiaoban directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\xiaoban yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// XIAOBAN_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// XIAOBAN_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.xiaoban / %LOCALAPPDATA%\xiaoban.
function resolveXiaobanHome() {
  if (process.env.XIAOBAN_HOME) return normalizeXiaobanHomeRoot(process.env.XIAOBAN_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'xiaoban-home')
  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a XIAOBAN_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\xiaoban and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('XIAOBAN_HOME')
    if (fromRegistry) return normalizeXiaobanHomeRoot(fromRegistry)
  }
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'xiaoban')
    const legacy = path.join(app.getPath('home'), '.xiaoban')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.xiaoban setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.xiaoban')
}

const XIAOBAN_HOME = resolveXiaobanHome()

function xiaobanManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_xiaoban_node_dirs() in
  // xiaoban_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(XIAOBAN_HOME, 'node')
  const bin = path.join(root, 'bin')
  const entries = IS_WINDOWS ? [root, bin] : [bin, root]
  return entries.filter(directoryExists)
}

function pathWithXiaobanManagedNode(...entries) {
  return [...xiaobanManagedNodePathEntries(), ...entries, process.env.PATH]
    .filter(Boolean)
    .join(path.delimiter)
}

// ACTIVE_XIAOBAN_ROOT — the canonical mutable Xiaoban install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_XIAOBAN_ROOT = path.join(XIAOBAN_HOME, 'xiaoban-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_XIAOBAN_ROOT, 'venv')
...
// boots, going straight to `resolveXiaobanBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_XIAOBAN_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_XIAOBAN_ROOT, '.xiaoban-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
// active-profile.json records which Xiaoban profile the desktop launches its
// local backend as. When set, startXiaoban() passes `xiaoban --profile <name>
// dashboard …`, which deterministically pins XIAOBAN_HOME (see
// _apply_profile_override in xiaoban_cli/main.py) and bypasses the sticky
// ~/.xiaoban/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors xiaoban_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// xiaobanDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under XIAOBAN_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by xiaoban_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(XIAOBAN_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (xiaoban_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.XIAOBAN_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.XIAOBAN_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
  if (!Number.isFinite(raw) || raw <= 0) return 650
  return Math.max(120, raw)
})()
const APP_NAME = 'Xiaoban'
...
// 'xiaoban:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.xiaoban) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.xiaoban')
}
// Seed the native About panel with the live Xiaoban version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `xiaoban update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveXiaobanVersion(),
...
const MEDIA_PROTOCOL = 'xiaoban-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(xiaoban-media://…)`.
...
let mainWindow = null
let xiaobanProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by xiaobanProcess +
// connectionPromise + startXiaoban(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.XIAOBAN_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.XIAOBAN_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startXiaoban() calls (e.g. the renderer's
...
let bootProgressState = {
  ...
  message: 'Waiting to start Xiaoban backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (!text) return
  const lines = text.split(/\r?\n/).map(line => `[xiaoban] ${line}`)
  xiaobanLog.push(...lines)
  if (xiaobanLog.length > 300) {
    xiaobanLog.splice(0, xiaobanLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-xiaoban-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('xiaoban:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the xiaoban:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('xiaoban:bootstrap:event', ev)
}
...
// The Tauri updater writes XIAOBAN_HOME/.xiaoban-update-in-progress for the whole
...
// (`force_kill_other_xiaoban`, taskkill /IM xiaoban.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(XIAOBAN_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Xiaoban will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(XIAOBAN_HOME)
  }
  if (marker) {
...
function isXiaobanSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'xiaoban_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.XIAOBAN_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Xiaoban deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Xiaoban' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\xiaoban\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'xiaoban', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'xiaoban', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// install only has PortableGit under %LOCALAPPDATA%\xiaoban\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'xiaoban', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'xiaoban', 'git', 'bin', 'git.exe'))
  }
...
function recentXiaobanLog() {
  return xiaobanLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's xiaoban root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_XIAOBAN_ROOT.
// XIAOBAN_DESKTOP_XIAOBAN_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.XIAOBAN_DESKTOP_XIAOBAN_ROOT && path.resolve(process.env.XIAOBAN_DESKTOP_XIAOBAN_ROOT),
    !IS_PACKAGED && isXiaobanSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isXiaobanSourceRoot(ACTIVE_XIAOBAN_ROOT) ? ACTIVE_XIAOBAN_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_XIAOBAN_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('xiaoban:updates:progress', merged)
  }
}
...
// XIAOBAN_HOME/xiaoban-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_xiaoban_home). That binary owns
// ALL repo mutation — running `xiaoban update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'xiaoban-setup.exe' : 'xiaoban-setup'
  const candidate = path.join(XIAOBAN_HOME, name)
...
// Path to the venv shim whose lock decides whether `xiaoban update` can write
// fresh entry points. On Windows this is the file the running backend
// `xiaoban.exe` holds open; on POSIX it's never mandatory-locked.
function venvXiaobanShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'xiaoban.exe')
    : path.join(updateRoot, 'venv', 'bin', 'xiaoban')
}
...
// only signals the direct child, so on Windows a backend `xiaoban.exe` that
// spawned its own grandchildren (a `xiaoban` REPL, a pty terminal session, the
...
// `xiaobanProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `xiaoban.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a xiaoban
// REPL, a pty terminal, the gateway) keep `xiaoban.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (xiaobanProcess && Number.isInteger(xiaobanProcess.pid)) pids.push(xiaobanProcess.pid)
  ...
  if (xiaobanProcess && !xiaobanProcess.killed) {
    try {
      xiaobanProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvXiaobanShimPath(updateRoot)
...
// `xiaoban update`). Instead we spawn the staged Xiaoban-Setup binary with
// --update and quit, so it can run `xiaoban update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri xiaoban-setup. Unlike Windows
      ...
      // whole update itself: `xiaoban update` (backend) + `xiaoban desktop
    ...
    if (!updater) {
      ...
      // `xiaoban desktop`, never the Tauri installer that self-copies
      // xiaoban-setup.exe into XIAOBAN_HOME). They DO have a working `xiaoban`
      ...
      // checkout they're on — bare `xiaoban update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'xiaoban update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `xiaoban update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `xiaoban update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, xiaobanRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message: 'Updating Xiaoban — this window will close and the updater will open. Don’t reopen Xiaoban yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // xiaoban.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `xiaoban update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: XIAOBAN_HOME,
      env: {
        ...process.env,
        XIAOBAN_HOME,
        PATH: pathWithXiaobanManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvXiaoban = path.join(venvBin, IS_WINDOWS ? 'xiaoban.exe' : 'xiaoban')
  const updaterArgs = fileExists(venvXiaoban) ? ['--update', '--branch', branch] : ['--repair', '--branch', branch]
...
// Resolve the xiaoban CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `xiaoban` on PATH.
function resolveXiaobanCliBinary(updateRoot) {
  const venvXiaoban = path.join(updateRoot, 'venv', 'bin', 'xiaoban')
  if (fileExists(venvXiaoban)) return venvXiaoban
  return findOnPath('xiaoban') || null
}
...
// macOS/Linux in-app update: backend (`xiaoban update`) + OS-aware GUI rebuild
// (`xiaoban desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const xiaoban = resolveXiaobanCliBinary(updateRoot)
  if (!xiaoban) {
    emitUpdateProgress({ stage: 'manual', message: 'xiaoban update', percent: null })
    return { ok: true, manual: true, command: 'xiaoban update', xiaobanRoot: updateRoot }
  }

  // Put the Xiaoban-managed Node and the venv on PATH so `xiaoban desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\xiaoban\node, not node\bin.
  const env = {
    XIAOBAN_HOME,
    PATH: pathWithXiaobanManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `xiaoban update` reaps stale `xiaoban dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads XIAOBAN_DESKTOP_CHILD_PID and excludes
  ...
  if (xiaobanProcess && Number.isInteger(xiaobanProcess.pid)) {
    desktopChildPids.push(xiaobanProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.XIAOBAN_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Xiaoban (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(xiaoban, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'xiaoban update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'xiaoban update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(xiaoban, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Xiaoban to retry.',
      error: rebuilt.error || 'rebuild-failed'
    })
    return { ok: false, backendUpdated: true, error: 'desktop rebuild failed' }
  }

  // Linux in-app update terminal state (#45205). `xiaoban desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Xiaoban…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `xiaoban-desktop-update-${Date.now()}.sh`)
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Xiaoban to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Xiaoban desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Xiaoban to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Xiaoban.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Xiaoban.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Xiaoban to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.xiaoban-update-new"; then
    rm -rf "$DST.xiaoban-update-old" 2>/dev/null || true
    mv "$DST" "$DST.xiaoban-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.xiaoban-update-new" "$DST"
    rm -rf "$DST.xiaoban-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Xiaoban to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `xiaoban
  ...
  return isXiaobanSourceRoot(ACTIVE_XIAOBAN_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveWebDist() {
  const override = process.env.XIAOBAN_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set XIAOBAN_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: xiaoban desktop --force-build`
  )
  return candidates[0]
}
...
function resolveXiaobanCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Xiaoban.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.XIAOBAN_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveXiaobanCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'xiaoban_cli.main', ...dashboardArgs],
    env: buildDesktopBackendEnv({
      xiaobanHome: XIAOBAN_HOME,
...
// createActiveBackend — build a backend pointing at ACTIVE_XIAOBAN_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `Xiaoban at ${ACTIVE_XIAOBAN_ROOT}`,
    command: fileExists(venvPython) ? venvPython : findSystemPython(),
    args: ['-m', 'xiaoban_cli.main', ...dashboardArgs],
    env: buildDesktopBackendEnv({
      xiaobanHome: XIAOBAN_HOME,
      pythonPathEntries: [ACTIVE_XIAOBAN_ROOT],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_XIAOBAN_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveXiaobanBackend(dashboardArgs) {
  // 1. Explicit override -- XIAOBAN_DESKTOP_XIAOBAN_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.XIAOBAN_DESKTOP_XIAOBAN_ROOT && path.resolve(process.env.XIAOBAN_DESKTOP_XIAOBAN_ROOT)
  if (overrideRoot && isXiaobanSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Xiaoban source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `xiaoban` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isXiaobanSourceRoot.)
  if (!IS_PACKAGED && isXiaobanSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Xiaoban source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_XIAOBAN_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\xiaoban\xiaoban-agent (Windows) or ~/.xiaoban/xiaoban-agent.
  ...
  //    to spawning xiaoban. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `xiaoban update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `xiaoban` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    XIAOBAN_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.XIAOBAN_DESKTOP_IGNORE_EXISTING !== '1') {
    let xiaobanCommand = null
    const xiaobanOverride = process.env.XIAOBAN_DESKTOP_XIAOBAN

    if (xiaobanOverride) {
      const resolvedOverride = findOnPath(xiaobanOverride)
      if (resolvedOverride) {
        xiaobanCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(xiaobanOverride, { isWsl: IS_WSL })) {
        xiaobanCommand = xiaobanOverride
      } else {
        rememberLog(`Ignoring Windows Xiaoban override under WSL: ${xiaobanOverride}`)
      }
    } else {
      xiaobanCommand = findOnPath('xiaoban')
    }

    if (xiaobanCommand) {
      if (looksLikeDesktopAppBinary(xiaobanCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Xiaoban CLI: ${xiaobanCommand}`)
        xiaobanCommand = null
      }
    }

    if (xiaobanCommand) {
      // Smoke-test the candidate before trusting it. A `xiaoban` shim
      ...
      const shellForProbe = isCommandScript(xiaobanCommand)
      if (verifyXiaobanCli(xiaobanCommand, { shell: shellForProbe })) {
        return {
          label: `existing Xiaoban CLI at ${xiaobanCommand}`,
          command: xiaobanCommand,
          args: dashboardArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Xiaoban CLI at ${xiaobanCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed xiaoban_cli module via system Python.
  ...
  if (python) {
    ...
    // having xiaoban_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\xiaoban\xiaoban-agent\venv.
    if (canImportXiaobanCli(python)) {
      return {
        kind: 'python',
        label: `installed xiaoban_cli module via ${python}`,
        command: python,
        args: ['-m', 'xiaoban_cli.main', ...dashboardArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: xiaoban_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveXiaobanBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Xiaoban-Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_XIAOBAN_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveXiaobanBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Xiaoban install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error('Xiaoban recovery was handed off to Xiaoban Setup. The desktop will restart when recovery completes.')
    ...
    const bootstrapResult = await runBootstrap({
      ...
      xiaobanHome: XIAOBAN_HOME,
      logRoot: path.join(XIAOBAN_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Xiaoban install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Xiaoban bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(XIAOBAN_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startXiaoban() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // xiaoban:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveXiaobanBackend(backend.args))
  }
  ...
  if (!isXiaobanSourceRoot(ACTIVE_XIAOBAN_ROOT)) {
    throw new Error(
      `Xiaoban install at ${ACTIVE_XIAOBAN_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Xiaoban' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\xiaoban\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `xiaoban` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Xiaoban on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Xiaoban.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isXiaobanSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Xiaoban venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `Xiaoban at ${ACTIVE_XIAOBAN_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Xiaoban runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Xiaoban backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Xiaoban backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Xiaoban backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Xiaoban-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('xiaoban:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveXiaobanCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('xiaoban:preview-file-changed', payload)
}
...
async function waitForXiaoban(baseUrl, token) {
  ...
  throw new Error(`Xiaoban backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('xiaoban:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('xiaoban:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Xiaoban...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('xiaoban:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('xiaoban:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('xiaoban:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'xiaoban:desktop:zoomLevel'
...
//
// Hosted Xiaoban gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``xiaoban_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / xiaoban #37247).
//     Both are set as HttpOnly cookies (``xiaoban_session_at`` ~15 min,
//     ``xiaoban_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:xiaoban-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Xiaoban gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Xiaoban backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root XIAOBAN_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.XIAOBAN_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.XIAOBAN_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by XIAOBAN_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (xiaoban #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Xiaoban gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Xiaoban gateway is selected, but no session token is saved. ' +
...
//   2. env override (XIAOBAN_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.XIAOBAN_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.XIAOBAN_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'XIAOBAN_DESKTOP_REMOTE_URL is set but XIAOBAN_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Xiaoban backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.XIAOBAN_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Xiaoban
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startXiaoban())
  ...
  // connect to Xiaoban gateway". Mirror the renderer's connect here so the test
...
function resetXiaobanConnection() {
...
// startXiaoban() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetXiaobanConnection() nulls xiaobanProcess.
  const dying = xiaobanProcess && !xiaobanProcess.killed ? xiaobanProcess : null
  resetXiaobanConnection()
...
// profile to startXiaoban() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startXiaoban() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForXiaoban(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: xiaobanLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited XIAOBAN_HOME env (see _apply_profile_override
  // step 3 in xiaoban_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveXiaobanBackend(dashboardArgs))
  const xiaobanCwd = resolveXiaobanCwd()
  const webDist = resolveWebDist()

  rememberLog(`Starting Xiaoban backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: xiaobanCwd,
        XIAOBAN_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        XIAOBAN_DESKTOP: '1',
        XIAOBAN_WEB_DIST: webDist
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Xiaoban backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Xiaoban backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Xiaoban backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForXiaoban(baseUrl, token), startFailed])
  ready = true
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Xiaoban backend for profile "${profile}"`,
    rememberLog
  })
...
async function startXiaoban() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startXiaoban() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Xiaoban backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Xiaoban backend at ${remote.baseUrl}`, 24)
      await waitForXiaoban(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Xiaoban backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.xiaoban/active_profile file) and
    // resolves XIAOBAN_HOME the same way `xiaoban -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Xiaoban runtime', 28)
    const backend = await ensureRuntime(resolveXiaobanBackend(dashboardArgs))
    const xiaobanCwd = resolveXiaobanCwd()
    const webDist = resolveWebDist()

    await advanceBootProgress('backend.spawn', `Starting Xiaoban backend via ${backend.label}`, 84)
    rememberLog(`Starting Xiaoban backend via ${backend.label}`)

    xiaobanProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: xiaobanCwd,
        env: {
          ...process.env,
          // Explicitly pin XIAOBAN_HOME for the child so Python's get_xiaoban_home()
          // resolves to the SAME location our resolveXiaobanHome() picked. Without
          // this pin, Python falls back to ~/.xiaoban on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\xiaoban, which differs from C:\Users\<u>\.xiaoban.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets XIAOBAN_HOME via setx; the desktop
    ...
    xiaobanProcess.stdout.on('data', rememberLog)
    xiaobanProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    xiaobanProcess.once('error', error => {
      rememberLog(`Xiaoban backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Xiaoban backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    xiaobanProcess.once('exit', (code, signal) => {
      rememberLog(`Xiaoban backend exited (${signal || code})`)
      xiaobanProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Xiaoban backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Xiaoban backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentXiaobanLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Xiaoban backend to launch', 86)
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([waitForDashboardPort(xiaobanProcess), backendStartFailed])

    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Xiaoban backend to become ready', 90)
    await Promise.race([waitForXiaoban(baseUrl, token), backendStartFailed])
    backendReady = true
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null xiaobanProcess when the child dies.
      childAlive: () => xiaobanProcess !== null && xiaobanProcess.exitCode === null && !xiaobanProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Xiaoban backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startXiaoban().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('xiaoban:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Xiaoban…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('xiaoban:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetXiaobanConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Xiaoban backend failed liveness probe; dropping stale connection.')
    resetXiaobanConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('xiaoban:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('xiaoban:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('xiaoban:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('xiaoban:window:openNewSession', async () => {
  createNewSessionWindow()

  return { ok: true }
})
ipcMain.handle('xiaoban:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startXiaoban() call restarts the
...
ipcMain.handle('xiaoban:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startXiaoban() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('xiaoban:bootstrap:cancel', async () => {
...
ipcMain.handle('xiaoban:boot-progress:get', async () => bootProgressState)
ipcMain.handle('xiaoban:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('xiaoban:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('xiaoban:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('xiaoban:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('xiaoban:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('xiaoban:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('xiaoban:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('xiaoban:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('xiaoban:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('xiaoban:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new XIAOBAN_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('xiaoban:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('xiaoban:requestMicrophoneAccess', async () => {
...
ipcMain.handle('xiaoban:api', async (_event, request) => {
...
ipcMain.handle('xiaoban:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('xiaoban:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('xiaoban:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('xiaoban:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('xiaoban:readFileText', async (_event, filePath) => {
...
ipcMain.handle('xiaoban:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('xiaoban:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('xiaoban:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('xiaoban:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('xiaoban:saveClipboardImage', async () => {
...
ipcMain.handle('xiaoban:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('xiaoban:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('xiaoban:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('xiaoban:titlebar-theme', (_event, payload) => {
...
ipcMain.on('xiaoban:native-theme', (_event, mode) => {
...
ipcMain.on('xiaoban:translucency', (_event, payload) => {
...
ipcMain.handle('xiaoban:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('xiaoban:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveXiaobanCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('xiaoban:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveXiaobanCwd()
}))

ipcMain.handle('xiaoban:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('xiaoban:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('xiaoban:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('xiaoban:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('xiaoban:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('xiaoban:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: xiaobanLog.slice(-200) }))
...
function terminalShellCommand() {
  // XIAOBAN_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.XIAOBAN_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Xiaoban' TUI into
  ...
  env.TERM_PROGRAM = 'Xiaoban'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a xiaoban/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from XIAOBAN_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.XIAOBAN_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `xiaoban:terminal:${id}:${suffix}`
}
...
ipcMain.handle('xiaoban:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('xiaoban:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

ipcMain.handle('xiaoban:fs:worktrees', async (_event, cwds) => worktreesForIpc(cwds))

ipcMain.handle('xiaoban:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Xiaoban.')
  }
...
ipcMain.handle('xiaoban:terminal:write', (_event, id, data) => {
...
ipcMain.handle('xiaoban:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('xiaoban:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('xiaoban:updates:check', async () =>
...
ipcMain.handle('xiaoban:updates:apply', async (_event, payload) =>
...
ipcMain.handle('xiaoban:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('xiaoban:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Xiaoban version (the one `release.py` bumps in
// xiaoban_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Xiaoban version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveXiaobanVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'xiaoban_cli', '__init__.py')
...
// Re-resolve the live Xiaoban version and push it into the native About panel
// just before showing it, so an in-place `xiaoban update` is reflected without
...
ipcMain.handle('xiaoban:version', async () => ({
  appVersion: resolveXiaobanVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  xiaobanRoot: resolveUpdateRoot()
}))
...
// the actual removal via `xiaoban uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (xiaoban_cli/uninstall.py + xiaoban_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_XIAOBAN_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isXiaobanSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'xiaoban_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, XIAOBAN_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Xiaoban agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import xiaoban_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_XIAOBAN_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_XIAOBAN_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `xiaoban-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `xiaoban-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('xiaoban:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('xiaoban:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('xiaoban:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('xiaoban:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// xiaoban:// deep links (e.g. xiaoban://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const XIAOBAN_PROTOCOL = 'xiaoban'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${XIAOBAN_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // xiaoban://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('xiaoban:deep-link', payload)
...
ipcMain.handle('xiaoban:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${XIAOBAN_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(XIAOBAN_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(XIAOBAN_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `xiaoban://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching xiaoban:// URL is in our own argv.
